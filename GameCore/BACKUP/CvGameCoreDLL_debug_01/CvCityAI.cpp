...
void CvCityAI::AI_reset()
{
	...
	m_iEmphasizeAvoidUnhappyCount = 0;
	m_iEmphasizeAvoidUnhealthCount = 0;
	m_iEmphasizeAvoidEpidemicsCount = 0;
...
int CvCityAI::AI_permanentSpecialistValue(SpecialistTypes eSpecialist) const
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		...
		if (getYieldPerSpecialist(eSpecialist, (YieldTypes)iI) != 0)
	...
	if (bHasYield)
	{
		...
		*/
		iValue += iFoodValue * getYieldPerSpecialist(eSpecialist, YIELD_FOOD) * AI_yieldMultiplier(YIELD_FOOD) * (100+iFoodX) / 200;
		iValue += iProdValue * getYieldPerSpecialist(eSpecialist, YIELD_PRODUCTION) * AI_yieldMultiplier(YIELD_PRODUCTION) * (100+iProdX) / 200;
		iValue += iCommerceValue * getYieldPerSpecialist(eSpecialist, YIELD_COMMERCE) * AI_yieldMultiplier(YIELD_COMMERCE) * (100+iComX) / 200;
		// MOD - START - Advanced Yield and Commerce Effects
	}
	...
	for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		...
		int iTemp = iCommerceValue * getCommercePerSpecialist(eSpecialist, (CommerceTypes)iI);
		// MOD - END - Advanced Yield and Commerce Effects
		if (iTemp != 0)
		{
...
// MOD - START - Advanced Yield and Commerce Effects
int CvCityAI::AI_getPotentialSpecialistEstimate(SpecialistTypes eSpecialist) const
{
	// Estimate 1/4 of the non-specialist population as potential specialists
	int iPotentialPopulation = std::max(getPopulation() - getSpecialistPopulation(), 0) / 4;

	if (GET_PLAYER(getOwnerINLINE()).isSpecialistValid(eSpecialist) || eSpecialist == GC.getDefineINT("DEFAULT_SPECIALIST"))
	{
		return iPotentialPopulation;
	}
	else
	{
		int iRemainingSpecialists = getMaxSpecialistCount(eSpecialist) - getSpecialistCount(eSpecialist);

		return std::min(iPotentialPopulation, iRemainingSpecialists);
	}
}
// MOD - END - Advanced Yield and Commerce Effects
...
void CvCityAI::AI_chooseProduction()
{
	...
	if (isProduction())
	{
		if (getProduction() > 0)
		{
			...
			// K-Mod: if we are building a wonder, do not cancel, keep building it (if no danger)
			/* original bts code
			...
			*/
			...
			{
				if (isLimitedWonderClass((BuildingClassTypes) GC.getBuildingInfo(eProductionBuilding).getBuildingClassType()) || GC.getBuildingInfo(eProductionBuilding).isBonusConverter())
				{
	...
	*/
	...
	int iNumWaterAreas;
	int iNumRelevantWaterAreas;
	CvArea* aWaterAreas[MAX_CITY_WATER_AREAS];
	CvArea* aRelevantWaterAreas[MAX_CITY_WATER_AREAS];
	waterAreas(aWaterAreas, iNumWaterAreas, true);
	waterAreas(aRelevantWaterAreas, iNumRelevantWaterAreas, true, getTeam());
	...
	int iExistingSeaWorkers = 0;
	if (iNumWaterAreas > 0)
	{
		iNeededSeaWorkers += AI_getSeaWorkersNeeded();

		iExistingSeaWorkers += plot()->plotCount(PUF_isUnitAIType, UNITAI_WORKER_SEA, -1, getOwnerINLINE());
		iExistingSeaWorkers += getNumTrainUnitAI(UNITAI_WORKER_SEA);

		for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
		{
			iNeededSeaWorkers += kPlayer.AI_neededSeaWorkers(aWaterAreas[iWater], plot(), this);
			iExistingSeaWorkers += kPlayer.AI_totalWaterAreaUnitAIs(aWaterAreas[iWater], UNITAI_WORKER_SEA, this);
		}
	}
	// MOD - END - Multiple City Water Areas
	// MOD - END - Sea Improvements
	...
	*/
	...
	if (iNumWaterAreas > 0)
	{
		for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
		{
			iNumWaterAreaCitySites += kPlayer.AI_getNumAdjacentAreaCitySites(aWaterAreas[iWater]->getID(), getArea(), iWaterAreaBestFoundValue);
		}
	}
	else
	{
		CvArea* pWaterArea = GC.getMap().findBiggestArea(true);
		if (pWaterArea != NULL)
		{
			if( kPlayer.AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_SETTLER_SEA) > 0 )
			{
				iNumWaterAreaCitySites += kPlayer.AI_getNumAdjacentAreaCitySites(pWaterArea->getID(), getArea(), iWaterAreaBestFoundValue);
			}
		}
	}
	// MOD - END - Multiple City Water Areas
	...
	if (plot()->getNumDefenders(getOwnerINLINE()) == 0) // XXX check for other team's units?
	{
		...
		if (AI_chooseUnit(UNITAI_FORTIFICATION))
		{
			return;
		}
...
***/
	...
	if (!bAlwaysPeace && !bUnitExempt && !bFinancialTrouble)
	{
		int iNukeWeight = kPlayer.AI_nukeWeight();
		if ((kPlayer.AI_isDoStrategy(AI_STRATEGY_OWABWNW) || GC.getGame().getSorenRandNum(1000, "AI consider Nuke") < std::min(600, iNukeWeight)))
		{
			...
			if ((iTotalNukes < iNukesWanted) && (GC.getGame().getSorenRandNum(100, "AI train nuke MWAHAHAH") < (90 - (80 * iTotalNukes) / iNukesWanted)))
			{
				...
				if ((iNumRelevantWaterAreas > 0) && kPlayer.AI_totalUnitAIs(UNITAI_MISSILE_CARRIER_SEA) < iTotalNukes / 2 && GC.getGame().getSorenRandNum(3, "AI train boat instead") == 0)
					// MOD - END - Multiple City Water Areas
				{
	...
	// MOD - END - Multiple City Water Areas
	{
		if( !(bLandWar && iWarSuccessRating < -30) && !bDanger && !bFinancialTrouble )
		{
			if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) < std::min(3,kPlayer.getNumCities()))
			{
				...
				bool bBuildWaterUnits = bWaterDanger;
				if (!bBuildWaterUnits && bPrimaryArea)
				{
					for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
					{
						if (kPlayer.AI_countNumAreaHostileUnits(aRelevantWaterAreas[iWater], true, false, false, false) > 0)
						{
							bBuildWaterUnits = true;
							break;
						}
					}
				}
			...
			// MOD - END - Multiple City Water Areas
			{
				...
				if( iOdds >= 0 )
				{
					...
					bool bNeedSeaExplorers = false;
					for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
					{
						if (kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_EXPLORE_SEA) == 0 && kPlayer.AI_neededExplorers(aRelevantWaterAreas[iWater]) > 0)
						{
							bNeedSeaExplorers = true;
							break;
						}
					}

					if (bNeedSeaExplorers)
					// MOD - END - Multiple City Water Areas
					{
					...
					// MOD - START - Multiple City Water Areas
					//if (kPlayer.AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_SETTLER_SEA) == 0)
					bool bNeedSeaSettlers = false;
					for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
					{
						if (kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_SETTLER_SEA) == 0)
						{
							bNeedSeaSettlers = true;
							break;
						}
					}
	...
	if (bLandWar && iWarSuccessRating < -10 && iEnemyPowerPerc - iWarSuccessRating > 150)
	{
		...
		if (bDanger)
		{
			defensiveTypes.push_back(std::make_pair(UNITAI_FORTIFICATION, 50));
		}
		// MOD - END - Fortification Units
	...
	// MOD - START - Fortification Units
	// Ensure construction of fortifications in border cities
	if (!bFinancialTrouble && iUnitSpending < iMaxUnitSpending + 8)
	{
		int iExistingFortifications = plot()->plotCount(PUF_isUnitAIType, UNITAI_FORTIFICATION, -1, getOwnerINLINE());
		if (iExistingFortifications < AI_neededFortifications())
		{
			if (AI_chooseUnit(UNITAI_FORTIFICATION, (bGetBetterUnits ? 40 : 60) - iWarSuccessRating/3))
			{
				if( gCityLogLevel >= 2 ) logBBAI("      City %S uses choose fortification def", getName().GetCString());
				return;
			}
		}
	}
...
***/
	...
	if( !(bDefenseWar && iWarSuccessRating < -50) )
	{
		if ((iAreaBestFoundValue > iMinFoundValue) || (iWaterAreaBestFoundValue > iMinFoundValue))
		{
			...
			// MOD - END - Multiple City Water Areas
			{
				...
				int iNumExistingSeaSettlers = plot()->plotCount(PUF_isUnitAIType, UNITAI_SETTLER_SEA, -1, getOwnerINLINE());
				iNumExistingSeaSettlers += getNumTrainUnitAI(UNITAI_SETTLER_SEA);
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iNumExistingSeaSettlers += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_SETTLER_SEA, this);
					if (iNumExistingSeaSettlers >= iSettlerSeaNeeded)
					{
						break;
					}
				}
    ...
    if (!bImportantCity || bLandWar || bAssault)
    {
		...
		if( bDefenseWar || (bLandWar && (iWarSuccessRating < -30)) )
		{
			...
			if (bDanger)
			{
				panicDefenderTypes.push_back(std::make_pair(UNITAI_FORTIFICATION, 80));
			}
			// MOD - END - Fortification Units
	...
	// K-Mod, short-circuit 2 - a strong chance to build some high value buildings.
	{
		...
		*/
		int iOdds = std::max(0, (bLandWar || (bAssault && iNumRelevantWaterAreas > 0) ? 90 - iBuildUnitProb/4 : 150 - iBuildUnitProb/2) * iBestBuildingValue / (iBestBuildingValue + 40 + iBuildUnitProb) - 20);
		// MOD - END - Multiple City Water Areas
		if (AI_chooseBuilding(0, INT_MAX, 0, iOdds))
		{
		...
		if( bBuildAssault )
		{
			...
			*/
			if (iNumRelevantWaterAreas > 0)
			{
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iTransportsAtSea = kPlayer.AI_totalAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_ASSAULT_SEA);
				}
			}
			// MOD - END - Multiple City Water Areas
			else
			...
			// MOD - END - Multiple City Water Areas
			{
				...
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iEscorts += kPlayer.AI_totalAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_ESCORT_SEA);
				}
				// MOD - END - Multiple City Water Areas
				...
				if (eBestAttackSeaUnit != NO_UNIT)
				{
					...
					for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
					{
						iAttackSea += kPlayer.AI_totalAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_ATTACK_SEA);
					}
					// MOD - END - Multiple City Water Areas
	...
	if ((iNumRelevantWaterAreas > 0) && !bDefenseWar && !bAssault)
	// MOD - END - Multiple City Water Areas
	{
		if( !bFinancialTrouble )
		{
	...
	if ((iNumRelevantWaterAreas > 0) && !bLandWar && !bAssault && !bFinancialTrouble && !bUnitExempt)
	// MOD - END - Multiple City Water Areas
	{
		// MOD - START - Multiple City Water Areas
		...
		int iPirateCount = plot()->plotCount(PUF_isUnitAIType, UNITAI_PIRATE_SEA, -1, getOwnerINLINE());
		iPirateCount += getNumTrainUnitAI(UNITAI_PIRATE_SEA);
		int iNeededPirates = 0;
		for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
		{
			iPirateCount += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_PIRATE_SEA, this);
			iNeededPirates += (1 + (aRelevantWaterAreas[iWater]->getNumTiles() / std::max(1, 200 - iBuildUnitProb)));
		}
		// MOD - END - Multiple City Water Areas
	...
	if (!bLandWar && !bFinancialTrouble)
	{
		...
		if ((iNumRelevantWaterAreas > 0) && (iWaterPercent > 40))
		// MOD - END - Multiple City Water Areas
		{
			if (kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_SPY) > 0)
			{
				...
				int iNumSeaSpies = plot()->plotCount(PUF_isUnitAIType, UNITAI_SPY_SEA, -1, getOwnerINLINE());
				iNumSeaSpies += getNumTrainUnitAI(UNITAI_SPY_SEA);
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iNumSeaSpies += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_SPY_SEA, this);
				}

				if (iNumSeaSpies < iNumRelevantWaterAreas)
				// MOD - END - Multiple City Water Areas
				{
	...
	// TODO: Change this to use the new AI_ChooseUnit that takes a chance parameter
	if (!bDanger)
	{
		...
		{
			if (GC.getGameINLINE().getSorenRandNum(100, "AI Build Inquisitor") < ((bLandWar) ? 5 : 20))
			{
				int iNeededInquisitors = kPlayer.AI_neededInquisitors(area(), eStateReligion);

				if (iNeededInquisitors > 0)
				{
					if (iNeededInquisitors > GET_PLAYER(getOwnerINLINE()).countRemoveNonStateReligionUnits(area(), eStateReligion))
					{
						if (AI_chooseUnit(UNITAI_INQUISITOR))
						{
							return;
						}
					}
	...
	if (!bLandWar)
	{
		...
		if (iNumRelevantWaterAreas > 0 && bFinancialTrouble)
		// MOD - END - Multiple City Water Areas
		{
			if (kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_MISSIONARY) > 0)
			{
				...
				int iNumSeaMissionaries = plot()->plotCount(PUF_isUnitAIType, UNITAI_MISSIONARY_SEA, -1, getOwnerINLINE());
				iNumSeaMissionaries += getNumTrainUnitAI(UNITAI_MISSIONARY_SEA);
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iNumSeaMissionaries += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_MISSIONARY_SEA, this);
				}

				if (iNumSeaMissionaries < iNumRelevantWaterAreas)
				// MOD - END - Multiple City Water Areas
				{
			...
			// MOD - START - Inquisition
			if (kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_INQUISITOR_SEA) > 0)
			{
				// MOD - START - Multiple City Water Areas
				//if (kPlayer.AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_INQUISITOR_SEA) == 0)
				int iNumSeaInquisitors = plot()->plotCount(PUF_isUnitAIType, UNITAI_INQUISITOR_SEA, -1, getOwnerINLINE());
				iNumSeaInquisitors += getNumTrainUnitAI(UNITAI_INQUISITOR_SEA);
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iNumSeaInquisitors += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_INQUISITOR_SEA);
				}

				if (iNumSeaInquisitors < iNumRelevantWaterAreas)
				// MOD - END - Multiple City Water Areas
				{
					if (AI_chooseUnit(UNITAI_INQUISITOR_SEA))
					{
						return;
					}
				}
	...
	if (!bLandWar)
	{
		...
		// MOD - END - Multiple City Water Areas
		{
			if (bPrimaryArea)
			{
				...
				int iNumSeaExplorers = 0;
				int iNeededSeaExplorers = 0;
				for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
				{
					iNumSeaExplorers += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_EXPLORE_SEA);
					iNeededSeaExplorers += kPlayer.AI_neededExplorers(aRelevantWaterAreas[iWater]);
				}

				if (iNumSeaExplorers < std::min(1, iNeededSeaExplorers))
				// MOD - END - Multiple City Water Areas
				{
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes* peBestUnitAI)
{
	...
	for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
	{
		iCoastalCities = kOwner.countNumCoastalCitiesByArea(aWaterAreas[iWater]);
	}
	// MOD - END - Multiple City Water Areas
	...
	if (bWarPossible)
	{
		...
		// MOD - START - Fortification Units
		aiUnitAIVal[UNITAI_FORTIFICATION] += ((iMilitaryWeight / ((bWarPlan || bLandWar) ? 13 : 22)) + ((bPrimaryArea) ? 1 : 0));
		// MOD - END - Fortification Units
	...
	else
	{
		...
		for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
		{
			CvArea* pWaterArea = aWaterAreas[iWater];
		...
		if ((iHasMetCount > 0) && bWarPossible)
		{
			if (bLandWar || bAssault || !bFinancialTrouble || (kOwner.calculateUnitCost() == 0))
			{
				...
				aiUnitAIVal[UNITAI_ATTACK_CITY] += ((iMilitaryWeight / ((bLandWar || bAssault) ? 6 : 14)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
	...
	// K-Mod end
	{
		for (int iI = 0; iI < NUM_UNITAI_TYPES; iI++)
		{
			if (kOwner.AI_unitAIDomainType((UnitAITypes)iI) == DOMAIN_SEA)
			{
				...
				aiUnitAIVal[iI] -= plot()->plotCount(PUF_isUnitAIType, UNITAI_WORKER_SEA, -1, getOwnerINLINE());
				aiUnitAIVal[iI] -= getNumTrainUnitAI(UNITAI_WORKER_SEA);

				for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
				{
					aiUnitAIVal[iI] -= kOwner.AI_totalWaterAreaUnitAIs(aWaterAreas[iWater], ((UnitAITypes)iI), this);
				}
				// MOD - END - Multiple City Water Areas
			}
	...
	aiUnitAIVal[UNITAI_SETTLE] *= ((bDanger) ? 8 : 12); // K-Mod, was ? 8 : 20
	...
	// MOD - START - Fortification Units
	aiUnitAIVal[UNITAI_FORTIFICATION] *= 2;
	// MOD - END - Fortification Units
...
UnitTypes CvCityAI::AI_bestUnitAI(UnitAITypes eUnitAI, bool bAsync, AdvisorTypes eIgnoreAdvisor, bool bIgnoreUnitFoodConsumption)
// MOD - END - Free Units May Consume Food
{
	...
	for (size_t i = 0; i < candidates.size(); i++)
	{
		...
		if ((eUnitAI != UNITAI_ICBM && iValue < iBestBaseValue * 2 / 3) || (eUnitAI == UNITAI_EXPLORE && iValue < iBestBaseValue))
			continue;
		...
		for (PromotionTypes p = (PromotionTypes)0; p < GC.getNumPromotionInfos(); p=(PromotionTypes)(p+1))
		{
			...
			if (!bFree)
			{
				// TODO: Make use of relation caching here to cut out some loops
				for (TraitTypes t = (TraitTypes)0; t < GC.getNumTraitInfos(); t=(TraitTypes)(t+1))
				{
					if (hasTrait(t) && GC.getTraitInfo(t).isFreePromotion(p))
					{
						...
						if ((kUnitInfo.getUnitCombatType() != NO_UNITCOMBAT && GC.getTraitInfo(t).isFreePromotionUnitCombat(kUnitInfo.getUnitCombatType())) || GC.getTraitInfo(t).isFreePromotionUnitClass(kUnitInfo.getUnitClassType()))
...
// MOD - START - Promotion AI
int CvCityAI::AI_promotionValue(PromotionTypes ePromotion) const
{
	const CvPromotionInfo& kPromotion = GC.getPromotionInfo(ePromotion);

	int iValue = 0;

	// TODO: Consider our current posture: whether our plans for our military are offensive or defensive

	// MOD - START - Special Promotions
	if (kPromotion.isSpecial())
	{
		iValue += 20;
	}
	...
	iValue += kPromotion.getAirRangeChange() * 20;
	iValue += kPromotion.getInterceptChange() * 3;
	iValue += kPromotion.getEvasionChange() * 3;
	iValue += (kPromotion.getFirstStrikesChange() * 2 + kPromotion.getChanceFirstStrikesChange()) * 8;
	iValue += (kPromotion.getWithdrawalChange() * 4) / 3;
	iValue += kPromotion.getCollateralDamageChange() * 2;
	iValue += kPromotion.getBombardRateChange() * 2;
	iValue += kPromotion.getEnemyHealChange() / 4;
	iValue += kPromotion.getNeutralHealChange() / 8;
	iValue += kPromotion.getFriendlyHealChange() / 4;
	iValue += kPromotion.getSameTileHealChange() * 2;
	iValue += kPromotion.getAdjacentTileHealChange() * 2;
	iValue += kPromotion.getCombatPercent() * 2;
	iValue += kPromotion.getCityAttackPercent();
	iValue += kPromotion.getCityDefensePercent();
	iValue += kPromotion.getHillsAttackPercent() / 4;
	iValue += kPromotion.getHillsDefensePercent() / 2;
	iValue += kPromotion.getRevoltProtection() / 2;
	iValue += kPromotion.getCollateralDamageProtection() / 3;
	iValue += kPromotion.getPillageChange() / 4;
	iValue += kPromotion.getUpgradeDiscount() / 16;
	iValue += kPromotion.getExperiencePercent();

	// MOD - START - Barbarian Threat Modifier
	// TODO: Move to centralized unit function?
	const CvPlayerAI& kOwner = GET_PLAYER(getOwnerINLINE());
	const bool bRebels = false; // TODO: Consider slavery and serfdom after rebel spawning is moved into the game core
	int iBarbarianThreatModifier = 0;

	if (bRebels || !GC.getGameINLINE().isOption(GAMEOPTION_NO_BARBARIANS))
	{
		EraTypes eEra = kOwner.getCurrentEra();
		const CvEraInfo& kEra = GC.getEraInfo(eEra);
		
		if (bRebels || !(kEra.isNoBarbCities() && kEra.isNoBarbUnits()))
		{
			CvPlot* pPlot = plot();

			// Scale threat by era
			iBarbarianThreatModifier = 100 / std::max(1, (int)eEra - 1);

			if (bRebels)
			{
				// We're under threat of local rebels
				iBarbarianThreatModifier = std::max(75, iBarbarianThreatModifier);
			}
			else
			{
				if (pPlot->area()->isBorderObstacle(kOwner.getTeam()))
				{
					// We're safe inside the great wall
					iBarbarianThreatModifier = std::min(20, iBarbarianThreatModifier);
				}

				if (kEra.isNoBarbUnits())
				{
					// We're past the era of barbarians spawning randomly
					iBarbarianThreatModifier *= 2;
					iBarbarianThreatModifier /= 3;
				}
			}
		}
	}
	// MOD - END - Barbarian Threat Modifier
	
	// MOD - START - Advanced Experience Limits
	iValue += (kPromotion.getExtraBarbarianMaxXPValue() * iBarbarianThreatModifier) / 100;
	// MOD - END - Advanced Experience Limits

	// MOD - START - Extra Barbarian Combat Modifier
	iValue += (kPromotion.getExtraBarbarianCombatModifier() * iBarbarianThreatModifier) / 100;
	// MOD - END - Extra Barbarian Combat Modifier

	iValue += kPromotion.getKamikazePercent() / 16;
	
	// MOD - START - Relation Caching
	for (int iRel = 0; iRel < kPromotion.getNumRelatedDomains(); iRel++)
	{
		DomainTypes eLoopDomain = (DomainTypes)kPromotion.getRelatedDomains(iRel);
		iValue += kPromotion.getDomainModifierPercent(eLoopDomain);
	}

	for (int iRel = 0; iRel < kPromotion.getNumRelatedTerrains(); iRel++)
	{
		TerrainTypes eLoopTerrain = (TerrainTypes)kPromotion.getRelatedTerrains(iRel);

		iValue += kPromotion.getTerrainAttackPercent(eLoopTerrain) / 4;
		if (kPromotion.getTerrainDefensePercent(eLoopTerrain) > 0) 
		{
			iValue += kPromotion.getTerrainDefensePercent(eLoopTerrain) / 4;
		}

		if (kPromotion.getTerrainDoubleMove(eLoopTerrain))
		{
			iValue += 20;
		}
	}

	for (int iRel = 0; iRel < kPromotion.getNumRelatedFeatures(); iRel++)
	{
		FeatureTypes eLoopFeature = (FeatureTypes)kPromotion.getRelatedFeatures(iRel);

		iValue += kPromotion.getFeatureAttackPercent(eLoopFeature) / 4;
		if (kPromotion.getFeatureDefensePercent(eLoopFeature) > 0) 
		{
			iValue += kPromotion.getFeatureDefensePercent(eLoopFeature) / 4;
		}
		
		if (kPromotion.getFeatureDoubleMove(eLoopFeature))
		{
			iValue += 20;
		}

		// MOD - START - Feature Damage Immunity
		if (kPromotion.getFeatureDamageImmunity(eLoopFeature))
		{
			iValue += 15;
		}
	...
	for (int iRel = 0; iRel < kPromotion.getNumRelatedUnitCombats(); iRel++)
	{
		UnitCombatTypes eLoopUnitCombat = (UnitCombatTypes)kPromotion.getRelatedUnitCombats(iRel);
		iValue += (kPromotion.getUnitCombatModifierPercent(eLoopUnitCombat) * kOwner.AI_getUnitCombatWeight(eLoopUnitCombat)) / 100;
	}

	for (int iRel = 0; iRel < kPromotion.getNumRelatedPromotions(); iRel++)
	{
		PromotionTypes eLoopPromotion = (PromotionTypes)kPromotion.getRelatedPromotions(iRel);
		const CvPromotionInfo& kLoopPromotion = GC.getPromotionInfo(eLoopPromotion);

		if (kLoopPromotion.getPrereqPromotion() == ePromotion)
		{
			iValue += (AI_promotionValue(eLoopPromotion) * 2) / 3;
		}
		else
		{
			if ((kLoopPromotion.getPrereqOrPromotion1() == ePromotion) ||
				(kLoopPromotion.getPrereqOrPromotion2() == ePromotion) ||
				(kLoopPromotion.getPrereqOrPromotion3() == ePromotion))
			{
				iValue += AI_promotionValue(eLoopPromotion) / 2;
			}
		}
	}
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync, AdvisorTypes eIgnoreAdvisor)
{
	...
	for (BuildingClassTypes eLoopClass = (BuildingClassTypes)0; eLoopClass < GC.getNumBuildingClassInfos(); eLoopClass=(BuildingClassTypes)(eLoopClass+1))
	{
		...
		*/
		if ((isLimitedWonderClass(eLoopClass) && (GC.getBuildingClassInfo(eLoopClass).getMaxPlayerInstances() < 2)) || kBuilding.isBonusConverter())
		// MOD - END - Bonus Converters - AI
		{
			if (isProductionAutomated())
				continue;
		...
		if (isProductionAutomated())
		{
			...
			*/
			...
			if (kBuilding.getNumPrereqGlobalBuildingClasses() > 0)
				continue;
		...
		if (iFocusFlags == 0 && iValue * 1250 / std::max(1, iTurnsLeft + 3) >= iBestValue)
		{
			...
			if (GC.getBuildingClassInfo(eLoopClass).getMaxPlayerInstances() > 1)
				iLimit = -1;

			// MOD - START - Bonus Converters - AI
			if (kBuilding.isBonusConverter())
				iLimit = kOwner.AI_getMaxBonusConverters(eLoopBuilding);
			// MOD - END - Bonus Converters - AI
			...
			if (iLimit == -1)
			{
				...
				// MOD - START - Advanced Building Prerequisites
				for (int iPre = 0; iPre < kBuilding.getNumPrereqGlobalBuildingClasses(); iPre++)
				{
					BuildingClassTypes ePrereqBuildingClass = (BuildingClassTypes)kBuilding.getPrereqGlobalBuildingClasses(iPre);
					...
					int iPrereqEach = kOwner.getBuildingClassPrereqBuilding(eLoopBuilding, ePrereqBuildingClass, -iBuilt);
					int iPrereqBuilt = kOwner.getBuildingClassCount(ePrereqBuildingClass);
			...
			if (iLimit != -1)
			{
				...
				if (isNationalWonderClass(eLoopClass) && (GC.getBuildingClassInfo(eLoopClass).getMaxPlayerInstances() < 2))
...
// MOD - START - Debugging
// TODO: Remove this
//static int iMinGUCValue = 0;
//static int iMaxGUCValue = 0;
// MOD - END - Debugging
...
int CvCityAI::AI_buildingValue(BuildingTypes eBuilding, int iFocusFlags, int iThreshold, bool bConstCache, bool bAllowRecursion, CivicTypes eCivic) const
// MOD - END - Advanced Yield and Commerce Effects
{
	PROFILE_FUNC();
	...
	if (GC.getBuildingClassInfo(eBuildingClass).getMaxPlayerInstances() > 1)
		iLimitedWonderLimit = -1;

	// MOD - START - Bonus Converters - AI
	if (GC.getBuildingInfo(eBuilding).isBonusConverter())
		iLimitedWonderLimit = kOwner.AI_getMaxBonusConverters(eBuilding);
	// MOD - END - Bonus Converters - AI	
	...
	// MOD - START - Fortification Units
	int iNeededFortifications = AI_neededFortifications();
	// MOD - END - Fortification Units

	bool bFinancialTrouble = GET_PLAYER(getOwnerINLINE()).AI_isFinancialTrouble();

	int iCultureValue = calculateCulturePercent(getOwnerINLINE());
	bool bCulturalTrouble = (iCultureValue < 50);
	
	int iRevoltRisk = getCitySeparatism() - getCitySeparatismTemp();
	bool bRevoltTrouble = (iRevoltRisk > 0);
	...
	for (int iPass = 0; iPass < 2; iPass++)
	{
				...
				if (GC.getGameINLINE().getBestLandUnit() == NO_UNIT || !GC.getUnitInfo(GC.getGameINLINE().getBestLandUnit()).isIgnoreBuildingDefense())
				{
					...
					*/
					...
					for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitCombats(); iRel++)
					{
						...
						defenseModifier += kBuilding.getUnitCombatDefenseModifier(eLoopUnitCombat) / 2;
					}
					...
					for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitClasses(); iRel++)
					{
						...
						defenseModifier += kBuilding.getUnitClassDefenseModifier(eLoopUnitClass) / 2;
					}
					// MOD - END - Relation Caching
					...
					int iTemp = 0;
					// bombard reduction
					int iOldBombardMod = getBuildingBombardDefense() - (bRemove ? bombardDefenseModifier : 0);
					iTemp += std::max((bRemove ? 0 : defenseModifier) + getBuildingDefense(), getNaturalDefense()) * std::min(bombardDefenseModifier, 100-iOldBombardMod) / std::max(80, 8 * (100 - iOldBombardMod));
					// defence bonus
					iTemp += std::max(0, std::min((bRemove ? 0 : defenseModifier) + getBuildingDefense() - getNaturalDefense() - 10, defenseModifier)) / 4;
					// MOD - END - Obsolete Defense
					
					iTemp += kBuilding.getNaturalDefenseModifier();
		...
		if (((iFocusFlags & BUILDINGFOCUS_HAPPY) || (iPass > 0)) && !isNoUnhappiness())
		{
			...
			for (int iRel = 0; iRel < kBuilding.getNumRelatedBuildingClasses(); iRel++)
			{
				...
				{
					iValue += (kBuilding.getBuildingHappinessChanges(eLoopBuildingClass) * kOwner.getBuildingClassCount(eLoopBuildingClass) * 8);
				}
			}
			// MOD - END - Relation Caching
		}
		...
		if (iFocusFlags & BUILDINGFOCUS_EXPERIENCE || iPass > 0)
		{
			...
			for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitCombats(); iRel++)
			{
				...
				if (kBuilding.getUnitCombatFreeExperience(eLoopUnitCombat) != 0)
				{
					...
					{
						iValue += kBuilding.getUnitCombatFreeExperience(eLoopUnitCombat) * iWeight / 2;
					}
				}
			}
			// MOD - END - Relation Caching
		...
		if ((iFocusFlags & BUILDINGFOCUS_DOMAINSEA))
		{
			...
			// MOD - START - Efficiency
			// MOD - START - Relation Caching
			bool bProvidesUnitCombatExperience = false;
			for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitCombats(); iRel++)
			{
				UnitCombatTypes eLoopUnitCombat = (UnitCombatTypes)kBuilding.getRelatedUnitCombats(iRel);
				...
				if (kBuilding.getUnitCombatFreeExperience(eLoopUnitCombat) != 0)
				{
					bProvidesUnitCombatExperience = true;
					break;
				}
			}
			// MOD - END - Relation Caching

			if (bProvidesUnitCombatExperience)
			{
		...
		if ((iFocusFlags & BUILDINGFOCUS_SPECIALIST) || (iPass > 0))
		{
			...
			{
				SpecialistTypes eLoopSpec = (SpecialistTypes)kBuilding.getRelatedSpecialists(iRel);
...
/************************************************************************************************/
			...
			if (kBuilding.isFreshWaterSource() && !plot()->isFreshWater())
			{
				iValue += 8;
				if (!plot()->isIrrigated())
				{
...
/************************************************************************************************/
			...
			// MOD - START - Bonus Converters - AI
			{
				...
				{
					iTempValue += (kOwner.AI_bonusVal((BonusTypes)(kBuilding.getFreeBonus()), 1) *
				...
				if (kBuilding.isBonusConverter())
				{
					for (int iProducedBonusIndex = 0; iProducedBonusIndex < kBuilding.getNumProducedBonuses(); iProducedBonusIndex++)
					{
						...
						iTempValue += (kOwner.AI_bonusVal(eProducedBonus, iProducedAmount)) *
							(kOwner.getNumTradeableBonuses(eProducedBonus) == 0 ? 2 : 1) *
							(iNumCities + iProducedAmount);
					}
				}

				// Limited resource-granting buildings are to be built in good cities only
			...
			if (kBuilding.getFreePromotion() != NO_PROMOTION)
			{
				...
				iValue += iTemp;
				*/
				int iPromoValue = AI_promotionValue((PromotionTypes)kBuilding.getFreePromotion()) / 2;

				// TODO: Scale the promo value by the likelihood of building units that can receive the promotion
				...
				int iProduction = getYieldRate(YIELD_PRODUCTION);
				iPromoValue *= 2*iProduction;
				iPromoValue /= 30 + iProduction;
				if (!kInfo.isSpecial())
				{
					iPromoValue *= getFreeExperience() + 1;
					iPromoValue /= getFreeExperience() + 2;
				}
				iValue += iPromoValue;
				// MOD - END - Promotion AI
				// MOD - START - Debugging
				// TODO: Remove this
				//CvWString szBuffer;
				//szBuffer.Format(L"%s city of %s free promotion value of %s for %s promotion: %d", kOwner.getReplayName(), getName().GetCString(), kBuilding.getText(), kInfo.getText(), iPromoValue);
				//gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
				// MOD - END - Debugging
			...
			if (kBuilding.getInsideCityCongestionThresholdChange() != 0 || kBuilding.getOutsideCityCongestionThresholdChange() != 0)
			{
				int iUrbanValue = kBuilding.getInsideCityCongestionThresholdChange();
				int iNumMilitaryUnits = GET_PLAYER(getOwnerINLINE()).getNumMilitaryUnits();
				int iCities = GET_PLAYER(getOwnerINLINE()).getNumCities();
				bool bWarPlan = (GET_TEAM(getTeam()).getAnyWarPlanCount(true) > 0);
				iUrbanValue *= iNumMilitaryUnits;
				iUrbanValue *= 30;
				iUrbanValue /= std::max(1, iCities);
				iUrbanValue /= (bWarPlan ? 2 : 3);3;


				int iExtraUnits = std::max(0, iNumMilitaryUnits - iCities);

				int iRuralValue = kBuilding.getOutsideCityCongestionThresholdChange();
				iRuralValue *= iExtraUnits * iExtraUnits;
				iRuralValue /= (bWarPlan ? 30 : 45);
			...
			// K-Mod. Still very rough, but a bit closer to true value.  (this is for the Statue of Liberty)
			{
				...
				if (iFreeSpecialists > 0)
				{
					...
					for (CommerceTypes i = (CommerceTypes)0; i < NUM_COMMERCE_TYPES; i = (CommerceTypes)(i+1))
					{
						...
						*/
						if (kOwner.getAllSpecialistCommerceChange(i))
							iSpecialistValue += kOwner.getAllSpecialistCommerceChange(i) * 4 * kOwner.AI_commerceWeight(i);
						// MOD - END - Advanced Yield and Commerce Effects
					}
			...
			// K-Mod... here's some code like the specialist value function. Kind of long, but much better.
			{
				...
				int iCityRate = getGreatPeopleRate() / 100;
				int iHighestRate = 0;
				int iLoop;
				for( CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop) )
				{
					int x = pLoopCity->getGreatPeopleRate() / 100;
					if (x > iHighestRate)
						iHighestRate = x;
				}
				if (iHighestRate > iCityRate)
				{
					iTempValue *= 100;
					iTempValue /= (2*100*(iHighestRate+3))/(std::max(3, iCityRate+3)) - 100;
				}
			...
			// NOTE: Much of this code is derived from CvPlayerAI::AI_civicValue
			if (kBuilding.isGlobalUnlimitedConscript())
			{
				const CvCity* pCapitalCity = kOwner.getCapitalCity();
				if (pCapitalCity != NULL)
				{
					UnitTypes eConscript = pCapitalCity->getConscriptUnit();
					if( eConscript != NO_UNIT )
					{
						int iCombatValue = GC.getGameINLINE().AI_combatValue(eConscript);
						if( iCombatValue > 33 )
						{
							int iTempValue = iNumCities + ((bWarPlan) ? 30 : 10);
							...
							iTempValue /= 100;
							iTempValue *= iNumCities;
							// reduce the value if unit spending is already high.
							int iUnitSpending = kOwner.AI_unitCostPerMil();
							int iMaxSpending = kOwner.AI_maxUnitCostPerMil();
							...
							// MOD - START - Debugging
							// TODO: Remove this
							//if (iMinGUCValue > iTempValue || iMaxGUCValue < iTempValue)
							//{
							//	iMinGUCValue = std::min(iMinGUCValue, iTempValue);
							//	iMaxGUCValue = std::max(iMaxGUCValue, iTempValue);
							//	CvWString szBuffer;
							//	szBuffer.Format(L"GlobalUnlimitedConscript Value Range: %d - %d", iMinGUCValue, iMaxGUCValue);
							//	gDLL->getInterfaceIFace()->addHumanMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NEWTURN", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
							//}
							// MOD - END - Debugging
			...
			const int iWarConscript = kBuilding.getWarConscript();
			const int iGlobalWarConscript = kBuilding.getGlobalWarConscript();
			if (iWarConscript != 0 || iGlobalWarConscript != 0)
			{
				// Local value depends on the this city's ability to produce a conscription unit
				UnitTypes eConscript = getConscriptUnit();
				if (eConscript != NO_UNIT)
				{
					iValue += iWarConscript * 3;
				}

				// Global value depends on the capital city's ability to produce a conscription unit
				if (eConscript == NO_UNIT)
				{
					const CvCity* pCapitalCity = kOwner.getCapitalCity();
					if (pCapitalCity != NULL)
					{
						eConscript = pCapitalCity->getConscriptUnit();
					}
				}

				if (eConscript != NO_UNIT)
				{
					iValue += iGlobalWarConscript * iNumCities * 6;
				}
			}
			...
			for (int iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
			{
				...
				iValue += (kBuilding.getDomainHealRateChanges(iI) / 2);
			...
			{
				iValue += (kBuilding.getUnitCombatHealRateChanges(iI) / 2);
			}
			// MOD - END - Building Heal Rate
			...
			for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitClasses(); iRel++)
			{
				...
				// MOD - START - Building Heal Rate
				iValue += (kBuilding.getUnitClassHealRateChanges(eLoopUnitClass) / 2);
				// MOD - END - Building Heal Rate
					
				eCivilization = kOwner.getCivilizationType();
				eLoopUnit = (UnitTypes)GC.getCivilizationInfo(eCivilization).getCivilizationUnits(eLoopUnitClass);
				if (eLoopUnit == NO_UNIT)
				{
					continue;
				}
				// ok, now we have civ specific unit, let's check building prereq
				bool bUnitNeedsBuilding = false;
				CvUnitInfo& kLoopUnit = GC.getUnitInfo(eLoopUnit);
					
				for (int iPre = 0; iPre < kLoopUnit.getNumPrereqAndBuildingClasses(); iPre++)
				{
					if (((BuildingClassTypes)kLoopUnit.getPrereqAndBuildingClasses(iPre)) == eBuildingClass)
					{
						bUnitNeedsBuilding = true;
						break;
					}
				}
					
				if (!bUnitNeedsBuilding)
				{
					bool bUnitCouldUseBuilding = false;
					bool bUnitHasOtherBuilding = false;
					BuildingClassTypes eUnitPrereqBuildingClass = NO_BUILDINGCLASS;
						
					for (int iPre = 0; iPre < kLoopUnit.getNumPrereqOrBuildingClasses(); iPre++)
					{
						eUnitPrereqBuildingClass = (BuildingClassTypes)kLoopUnit.getPrereqOrBuildingClasses(iPre);
						if (eUnitPrereqBuildingClass == NO_BUILDINGCLASS)
						{
							continue;
						}
							
						if (eUnitPrereqBuildingClass == eBuildingClass)
						{
							bUnitCouldUseBuilding = true;
						}
						else
						{
							BuildingTypes eUnitPrereqBuilding = ((BuildingTypes)(GC.getCivilizationInfo(eCivilization).getCivilizationBuildings(eUnitPrereqBuildingClass)));
							if (getNumActiveBuilding(eUnitPrereqBuilding) > 0)
							{
								bUnitHasOtherBuilding = true;
								break;
							}
						}
					}

					if (bUnitCouldUseBuilding && !bUnitHasOtherBuilding)
					{
						bUnitNeedsBuilding = true;
					}
				}

				// MOD - START - Building Resource Converter
				if (!bUnitNeedsBuilding)
				{
					// TODO: Consider whether this building converts a resource needed
					//       for the unit.  Be careful to make sure that we can actually
					//       train the unit.  Use canTrain?
				}
				// MOD - END - Building Resource Converter
				...
				if (bUnitNeedsBuilding)
				{
					...
					// MOD - START - Fortification Units
					// Avoid building fortification-enabling buildings in non-border cities
					if (kLoopUnit.getDefaultUnitAIType() == UNITAI_FORTIFICATION)
					{
						if (iNeededFortifications > 0)
						{
							iValue += iNeededFortifications * 3;
						}
						else
						{
							iValue -= 20;
					...
					// MOD - END - Fortification Units
					else if  (kLoopUnit.getDefaultUnitAIType() == UNITAI_ICBM)
					{
						iValue += kOwner.AI_nukeWeight();
					}					
					else if (kOwner.AI_totalAreaUnitAIs(area(), ((UnitAITypes)(kLoopUnit.getDefaultUnitAIType()))) == 0)
					{
						iValue += iNumCitiesInArea;
					}
					...
					ReligionTypes eReligion = (ReligionTypes)(kLoopUnit.getPrereqReligion());
					if (eReligion != NO_RELIGION)
					{
						...
						if (bCulturalVictory2 && kLoopUnit.getReligionSpreads(eReligion))
						{
			...
			if (bAllowRecursion)
			{
				...
				if (kBuilding.getFreeBuildingClass() != NO_BUILDINGCLASS)
				{
					...
					if (NO_BUILDING != eFreeBuilding)
					{
						// MOD - START - Advanced Free Buildings
						/*
						...
						*/
						int iFreeBuildingValue = 0;
						int iLoop;
						for (CvCity* pLoopCity = kOwner.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kOwner.nextCity(&iLoop))
						{
							if (pLoopCity->getNumBuilding(eFreeBuilding) > 0 || pLoopCity->getProductionBuilding() == eBuilding)
								continue;

							// MOD - START - Advanced Yield and Commerce Effects
							//int iLoopCityBuildingValue = pLoopCity->AI_buildingValue(eFreeBuilding, 0, 0, bConstCache, false);
							int iLoopCityBuildingValue = pLoopCity->AI_buildingValue(eFreeBuilding, 0, 0, bConstCache, false, eCivic); // Note that this includes flavor values right now, which probably isn't good
							// MOD - END - Advanced Yield and Commerce Effects

							iFreeBuildingValue += iLoopCityBuildingValue;
						}

						iFreeBuildingValue *= 2;
						iFreeBuildingValue /= 3;

						iValue += iFreeBuildingValue;
						// MOD - END - Advanced Free Buildings
					}
				}
				// MOD - START - Relation Caching
				for (int iRel = 0; iRel < kBuilding.getNumRelatedBuildingClasses(); iRel++)
				{
					BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getRelatedBuildingClasses(iRel);
					BuildingTypes eLoopBuilding = (BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass); // K-Mod
					...
					int iLimitForLoopBuilding = limitedWonderClassLimit(eLoopBuildingClass);
					...
					if ((kLoopBuilding.getPrereqNumOfBuildingClass(eBuildingClass) <= 0 && !kLoopBuilding.isBuildingClassNeededInCity(kBuilding.getBuildingClassType())) ||
						(iLimitForLoopBuilding > 0 && kOwner.getBuildingClassMaking(eLoopBuildingClass) >= iLimitForLoopBuilding) ||
					...
					if (kLoopBuilding.getPrereqNumOfBuildingClass(eBuildingClass) > 0)
					{
						...
						iPrereqBuildings = kOwner.getBuildingClassPrereqBuilding(eLoopBuilding, eBuildingClass, kOwner.getBuildingClassMaking(eLoopBuildingClass));
						...
						getNumBuilding(eBuilding) == 0 && iPrereqBuildings <= 0)
					{
						if (kBuilding.getProductionCost() > 0 && kLoopBuilding.getProductionCost() > 0)
						{
							...
							*/
							int iTempValue = AI_buildingValue(eLoopBuilding, 0, 0, bConstCache, false, eCivic);
							// MOD - END - Advanced Yield and Commerce Effects
							if (iTempValue > 0)
							{
							...
							if (iCanBuildPrereq >= iPrereqBuildings)
							{
								for (pLoopCity = kOwner.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kOwner.nextCity(&iLoop))
								{
									if (pLoopCity->canConstruct(eLoopBuilding, false, true) && pLoopCity->getProductionBuilding() != eLoopBuilding)
										...
										iHighestValue = std::max(pLoopCity->AI_buildingValue(eLoopBuilding, 0, 0, bConstCache, false, eCivic), iHighestValue);
										// MOD - END - Advanced Yield and Commerce Effects
								}
				...
				{
					// Consider the value of buildings that we could build with power
					for (int iMem = 0; iMem < GC.getNumBuildingClassesRequiringPower(); iMem++)
					{
						BuildingClassTypes eLoopBuildingClass = GC.getBuildingClassesRequiringPower(iMem);
						...
						if (eLoopBuilding != NO_BUILDING && getNumBuilding(eLoopBuilding) == 0)
						{
							const CvBuildingInfo& kLoopBuilding = GC.getBuildingInfo(eLoopBuilding);
							if (kLoopBuilding.isPrereqPower() && kOwner.canConstruct(eLoopBuilding))
							{
								int iTempValue = AI_buildingValue(eLoopBuilding, 0, 0, bConstCache, false, eCivic);
								if (iTempValue > 0)
								{
									// scale the bonus value by a rough approximation of how likely we are to build the thing
									// (note. the combined production cost is essentially the cost of completing kLoopBuilding given our current position.)
									iTempValue *= 2*kBuilding.getProductionCost();
									iTempValue /= 2*kBuilding.getProductionCost() + 3*kLoopBuilding.getProductionCost();
									iValue += iTempValue;
					...
					// Consider the value of constructed buildings that are presently inactive without power
					for (int iMem = 0; iMem < GC.getNumBuildingClassesInactiveWithoutPower(); iMem++)
					{
						...
						if (eLoopBuilding != NO_BUILDING)
						{
							if (getNumNonObsoleteBuilding(eLoopBuilding) > 0)
							{
								// We have this building and it's not obsolete but it doesn't have power
								if (GC.getBuildingInfo(eLoopBuilding).isInactiveWithoutPower())
								{
									int iTempValue = AI_buildingValue(eLoopBuilding, 0, 0, bConstCache, false, eCivic);
									iValue += iTempValue;
								}
							}
							else if (getNumBuilding(eLoopBuilding) == 0)
							{
								const CvBuildingInfo& kLoopBuilding = GC.getBuildingInfo(eLoopBuilding);
								if (!kLoopBuilding.isPrereqPower() && kOwner.canConstruct(eLoopBuilding))
								{
									// We don't have this building and it can be built without power but it's also inactive without power, so perform the potential building evaluation
									int iTempValue = AI_buildingValue(eLoopBuilding, 0, 0, bConstCache, false, eCivic);
									if (iTempValue > 0)
									{
										// scale the bonus value by a rough approximation of how likely we are to build the thing
...
/************************************************************************************************/
		...
		if (iPass > 0)
		{
			...
			for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
			{
				...
				*/
				...
				{
					if (kBuilding.getBonusYieldModifierYield(iMem) == iI)
					{
						if (hasBonus((BonusTypes)kBuilding.getBonusYieldModifierBonus(iMem)))
						{
							iTempValue += ((kBuilding.getBonusYieldModifierValue(iMem) * iBaseRate) / 27); // originally 12
						}
					}
				}
				// MOD - END - Advanced Yield and Commerce Effects
				...
				iRawYieldValue += (kBuilding.getYieldChange(iI) * 6); // was 6
				if (getBaseYieldRate((YieldTypes)iI) < 15)
				{
					iRawYieldValue += (kBuilding.getYieldChange(iI) * (15 - getBaseYieldRate((YieldTypes)iI)));
				}

				// MOD - START - Advanced Yield and Commerce Effects
				for (int iMem = 0; iMem < kBuilding.getNumSpecialistYieldChanges(); iMem++)
				{
					if (kBuilding.getSpecialistYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						iRawYieldValue += (kBuilding.getSpecialistYieldChangeValue(iMem) * getPopulation()) / 5;
					}
				}

				for (int iMem = 0; iMem < kBuilding.getNumBuildingYieldChanges(); iMem++)
				{
					if (kBuilding.getBuildingYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						BuildingTypes eCivBuilding = (BuildingTypes)GC.getCivilizationInfo(kOwner.getCivilizationType()).getCivilizationBuildings(kBuilding.getBuildingYieldChangeBuildingClass(iMem));
						if (eCivBuilding != NO_BUILDING)
						{
							iRawYieldValue += kBuilding.getBuildingYieldChangeValue(iMem) * getNumActiveBuilding(eCivBuilding) * 6;
						}
					}
				}
				...
				{
					if (kBuilding.getImprovementYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						...
						int iNumCurrentPlots = countNumImprovedPlots(eLoopImprovement, false);
						// MOD - START - Improvement Classes
						if (kOwner.canProduceImprovement(eLoopImprovement))
						{
							int iNumPotentialPlots = countNumImprovedPlots(eLoopImprovement, true) - iNumCurrentPlots;
							if (iNumPotentialPlots > 0 || iNumCurrentPlots > 0)
							{
								int iImprovementWeight = kOwner.AI_improvementWeight(eLoopImprovement, this, NO_TECH, eCivic);
								iRawYieldValue += (iChange * iNumCurrentPlots * 4) * iImprovementWeight / 100;
								iRawYieldValue += (iChange * iNumPotentialPlots * 2) * iImprovementWeight / 100;
							}
						}
						else
						{
							iRawYieldValue += iChange * iNumCurrentPlots * 4;
						}
				...
				*/
				...
				{
					if (kBuilding.getGlobalSpecialistYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						iTempValue += (kBuilding.getGlobalSpecialistYieldChangeValue(iMem) * kOwner.getTotalPopulation()) / 5;
					}
				}

				for (int iMem = 0; iMem < kBuilding.getNumGlobalBuildingYieldChanges(); iMem++)
				{
					if (kBuilding.getGlobalBuildingYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						iTempValue += kBuilding.getGlobalBuildingYieldChangeValue(iMem) * kOwner.getBuildingClassCountPlusMaking((BuildingClassTypes)kBuilding.getGlobalBuildingYieldChangeBuildingClass(iMem)) * 4;
					}
				}

				for (int iMem = 0; iMem < kBuilding.getNumGlobalImprovementYieldChanges(); iMem++)
				{
					if (kBuilding.getGlobalImprovementYieldChangeYield(iMem) == (YieldTypes)iI)
					{
						iTempValue += kBuilding.getGlobalImprovementYieldChangeValue(iMem) * kOwner.getImprovementCount((ImprovementTypes)kBuilding.getGlobalImprovementYieldChangeImprovement(iMem)) * 4;
					}
				}
				// MOD - END - Advanced Yield and Commerce Effects
		...
		if (iPass > 0)
		{
			for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
			{
				...
				iTempValue += (kBuilding.getCommerceChange(iI) * 6);
				if (getBaseCommerceRate((CommerceTypes) iI) < 15)
				{
					iTempValue += (kBuilding.getCommerceChange(iI) * (15 - getBaseCommerceRate((CommerceTypes) iI)) );
				}	
...
/************************************************************************************************/
				...
				{
					if (kBuilding.getSpecialistCommerceChangeCommerce(iMem) == (CommerceTypes)iI)
					{
						iTempValue += (kBuilding.getSpecialistCommerceChangeValue(iMem) * getPopulation()) / 5;
					}
				}

				for (int iMem = 0; iMem < kBuilding.getNumBuildingCommerceChanges(); iMem++)
				{
					if (kBuilding.getBuildingCommerceChangeCommerce(iMem) == (CommerceTypes)iI)
					{
						BuildingTypes eCivBuilding = (BuildingTypes)GC.getCivilizationInfo(kOwner.getCivilizationType()).getCivilizationBuildings(kBuilding.getBuildingCommerceChangeBuildingClass(iMem));
						if (eCivBuilding != NO_BUILDING)
						{
							iTempValue += kBuilding.getBuildingCommerceChangeValue(iMem) * getNumActiveBuilding(eCivBuilding) * 4;
						}
					}
				}
				// MOD - END - Advanced Yield and Commerce Effects
				...
				if ((CommerceTypes)iI == COMMERCE_CULTURE)
				{
					...
					if (bCulturalTrouble && bRevoltTrouble)
					{
						iTempValue *= 5;
					}
				...
				if ((CommerceTypes)iI == COMMERCE_CULTURE && bIsLimitedWonder)
					iTempValue += ((kBuilding.getGlobalCommerceModifier(iI) * iNumCities) * 2);
				if (findBaseYieldRateRank(YIELD_PRODUCTION) > 2 )
					iTempValue -= ((kBuilding.getGlobalCommerceModifier(iI) * iNumCities) * findBaseYieldRateRank(YIELD_PRODUCTION)); // We don't want less productive cities occupied with global modifier stuff when there are better cities for that
				...
				// MOD - START - Advanced Yield and Commerce Effects
				iTempValue += ((kBuilding.getAllSpecialistCommerce(iI) * kOwner.getTotalPopulation()) / 3); // Moved from above (I don't know what Karadoc was thinking, this shouldn't be scaled by the local city's modifier)

				for (int iMem = 0; iMem < kBuilding.getNumGlobalSpecialistCommerceChanges(); iMem++)
				{
					if (kBuilding.getGlobalSpecialistCommerceChangeCommerce(iMem) == (CommerceTypes)iI)
					{
						iTempValue += (kBuilding.getGlobalSpecialistCommerceChangeValue(iMem) * kOwner.getTotalPopulation()) / 5;
					}
				}

				for (int iMem = 0; iMem < kBuilding.getNumGlobalBuildingCommerceChanges(); iMem++)
				{
					if (kBuilding.getGlobalBuildingCommerceChangeCommerce(iMem) == (CommerceTypes)iI)
					{
						iTempValue += kBuilding.getGlobalBuildingCommerceChangeValue(iMem) * kOwner.getBuildingClassCountPlusMaking((BuildingClassTypes)kBuilding.getGlobalBuildingCommerceChangeBuildingClass(iMem)) * 4;
					}
				}
				// MOD - END - Advanced Yield and Commerce Effects
...
/************************************************************************************************/
	...
	if (!canConstruct(eBuilding))
	{
		...
		if (getPopulation() < 6)
		{
			// MOD - START - Small city special construction
			//iValue /= (8 - getPopulation());
			int iRank = findPopulationRank();
			if (iRank > 2)
			{
				iRank /= 2;
				iValue /= iRank;
			}
			// MOD - END - Small city special construction
		}
	}
	// The conditional above was originally disabled by K-Mod but was re-instated by Josh
	// Note from Karadoc: I don't understand the point of this, and it messes up the building value for things that are in-progress or already built.
	// Note from Josh (2012-07-26): I think it's to keep great generals (and maybe other specialists) from building important things in small cities
	//                              I changed the divisor to be based on city rank instead of absolute population count, this way doctrines get founded in the early game

	iValue = std::max(0, iValue);

	// MOD - START - Debugging
	//int iPreFlavorValue = iValue;
	//int iFlavorApplied = 0;
	// MOD - END - Debugging
	...
	if (iValue > 0)
	{
		...
		if (!isHuman())
		{
			...
			//if (iValue > 0)
			// MOD - START - Special Buildings Flavor
			//if (iValue > 0 && kBuilding.getProductionCost() > 0 && !bRemove) // K-Mod. Only use flavour adjustments for constructing ordinary buildings.
			if (iValue > 0 && !bRemove) // K-Mod. Only use flavour adjustments for constructing ordinary buildings.
			// MOD - END - Special Buildings Flavor
			{
				int iFlavour = 0;
	...
	{
		CvWString szBuffer;
		szBuffer.Format(L"%s value of %s is pre-flavor: %d, flavor: %d, diff: %d", kOwner.getReplayName(), kBuilding.getText(), iPreFlavorValue, iValue, iFlavorApplied);
		gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
	}
	*/
	// MOD - END - Debugging
				...
				for (PlayerTypes j = (PlayerTypes)0; j < MAX_CIV_PLAYERS; j = (PlayerTypes)(j+1))
				{
						...
						(kLoopPlayer.getTeam() == kOwner.getTeam() || kTeam.isHasMet(kLoopPlayer.getTeam())))
					{
						...
						*/
						...
						// Tech prereqs. Reduce the value for each missing tech.
						...
						// Resource prereq. Reduce the value for each missing resource.
						for (int k = 0; k < kLoopUnit.getNumPrereqAndBonuses(); k++)
						{
							BonusTypes ePrereqBonus = (BonusTypes)kLoopUnit.getPrereqAndBonuses(k);
							if (!kLoopPlayer.hasBonus(ePrereqBonus) && kLoopPlayer.countOwnedBonuses(ePrereqBonus) == 0)
							{
								iTemp /= 5;
							}
						}
						...
						{
							bool bBonusPresent = false;

							for (int k = 0; k < kLoopUnit.getNumPrereqOrBonuses(); k++)
							{
								BonusTypes ePrereqBonus = (BonusTypes)kLoopUnit.getPrereqAndBonuses(k);
								if (kLoopPlayer.hasBonus(ePrereqBonus) || kLoopPlayer.countOwnedBonuses(ePrereqBonus) > 0)
								{
...
*/
	...
	if (iNumWaterAreas > 0)
	{
		...
		for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
		{
			iNeededSeaWorkers += kOwner.AI_neededSeaWorkers(aWaterAreas[iWater], plot(), this);
		}
	}
// MOD - END - Sea Improvements
// MOD - END - Multiple City Water Areas
...
*/
	...
	if ((getCitySeparatism() - getCitySeparatismUnit()) > 0)
	{
		iDefenders += std::max(getCitySeparatism()/5, 3);
	}
...
// MOD - START - Fortification Units
int CvCityAI::AI_neededFortifications() const
{
	if (!(GET_TEAM(getTeam()).AI_isWarPossible()))
	{
		return 0;
	...
	const bool bAlert1 = kOwner.AI_isDoStrategy(AI_STRATEGY_ALERT1);
	const bool bAlert2 = kOwner.AI_isDoStrategy(AI_STRATEGY_ALERT2);
	const bool bTurtle = kOwner.AI_isDoStrategy(AI_STRATEGY_TURTLE);

	int iNeededFortifications = 0;

	if ((bAlert1 || bAlert2 || bTurtle) && area()->getAreaAIType(getTeam()) != AREAAI_NEUTRAL)
	{
		// TODO: Write a new function that determines the proximity of this city to the player's borders so that we can put fortifications in border cities
		// TODO: Write a function that determines the proximity of enemy territory to this city
		// TODO: Update the AI in this function to take enemy proximity and nearness to the player's exterior into account

		// Note: Karadoc deprecated and disabled the AI_calculateCulturePressure function that we relied upon, so this is temporarily disabled
		//int iCulturalPressure = AI_calculateCulturePressure();

		if (hasActiveWorldWonder() || isCapital() || isHolyCity())
			iNeededFortifications++;

		if (AI_isDanger())
			iNeededFortifications++;

		// Note: Temporarily disabled until a proper replacement for AI_calculateCulturePressure can be developed
		/*
		if (iCulturalPressure > 12)
			iNeededFortifications++;

		if (iCulturalPressure > 30)
			iNeededFortifications++;

		if (iCulturalPressure > 60)
			iNeededFortifications++;
		*/
	}

	return iNeededFortifications;
}
// MOD - END - Fortification Units
...
// MOD - START - City Growth Limits
int CvCityAI::AI_getEmphasizeAvoidUnhappyCount() const
{
	return m_iEmphasizeAvoidUnhappyCount;
}


bool CvCityAI::AI_isEmphasizeAvoidUnhappy() const
{
	return (AI_getEmphasizeAvoidUnhappyCount() > 0);
}


int CvCityAI::AI_getEmphasizeAvoidUnhealthCount() const
{
	return m_iEmphasizeAvoidUnhealthCount;
}


bool CvCityAI::AI_isEmphasizeAvoidUnhealth() const
{
	return (AI_getEmphasizeAvoidUnhealthCount() > 0);
}


int CvCityAI::AI_getEmphasizeAvoidEpidemicsCount() const
{
	return m_iEmphasizeAvoidEpidemicsCount;
}


bool CvCityAI::AI_isEmphasizeAvoidEpidemics() const
{
	return (AI_getEmphasizeAvoidEpidemicsCount() > 0);
}
// MOD - END - City Growth Limits
...
void CvCityAI::AI_setEmphasize(EmphasizeTypes eIndex, bool bNewValue)
{
	...
	{
		// MOD - START - City Growth Limits
		bool bWasGrowing = foodDifference() > 0 && isGrowthAllowed();
		// MOD - END - City Growth Limits

		m_pbEmphasize[eIndex] = bNewValue;

		const CvEmphasizeInfo& kEmphasize = GC.getEmphasizeInfo(eIndex);
		int iChange = ((AI_isEmphasize(eIndex)) ? 1 : -1);

		if (kEmphasize.isAvoidGrowth())
		{
			m_iEmphasizeAvoidGrowthCount += iChange;
			FAssert(AI_getEmphasizeAvoidGrowthCount() >= 0);
		}

		// MOD - START - City Growth Limits
		if (kEmphasize.isAvoidUnhappy())
		{
			m_iEmphasizeAvoidUnhappyCount += iChange;
			FAssert(AI_getEmphasizeAvoidUnhappyCount() >= 0);
		}

		if (kEmphasize.isAvoidUnhealth())
		{
			m_iEmphasizeAvoidUnhealthCount += iChange;
			FAssert(AI_getEmphasizeAvoidUnhealthCount() >= 0);
		}

		if (kEmphasize.isAvoidEpidemics())
		{
			m_iEmphasizeAvoidEpidemicsCount += iChange;
			FAssert(AI_getEmphasizeAvoidEpidemicsCount() >= 0);
		}
		// MOD - END - City Growth Limits

		if (kEmphasize.isGreatPeople())
		{
			m_iEmphasizeGreatPeopleCount += iChange;
			FAssert(AI_getEmphasizeGreatPeopleCount() >= 0);
		}

		for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			if (kEmphasize.getYieldChange(iI))
			{
				m_aiEmphasizeYieldCount[iI] += iChange;
		...
		for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
			if (kEmphasize.getCommerceChange(iI))
			{
				m_aiEmphasizeCommerceCount[iI] += iChange;
		...
		if ((getOwnerINLINE() == GC.getGameINLINE().getActivePlayer()) && isCitySelected())
		{
			...
			// MOD - START - City Growth Limits
			bool bIsGrowing = foodDifference() > 0 && isGrowthAllowed();
			if (bIsGrowing != bWasGrowing)
			{
...
int CvCityAI::AI_clearFeatureValue(int iIndex)
{
	...
	*/
	...
	// We always want to clear ice if that's an option
...
void CvCityAI::AI_getYieldMultipliers(int &iFoodMultiplier, int &iProductionMultiplier, int &iCommerceMultiplier, int &iDesiredFoodChange) const
{
	...
	// MOD - START - Division By Zero Bugfix
	/*
	...
	*/
	...
	int iProductionAdvantage = 100 * AI_yieldMultiplier(YIELD_PRODUCTION);
	iProductionAdvantage /= std::max(1, kPlayer.AI_averageYieldMultiplier(YIELD_PRODUCTION));
	iProductionAdvantage *= kPlayer.AI_averageYieldMultiplier(YIELD_COMMERCE);
	iProductionAdvantage /= std::max(1, AI_yieldMultiplier(YIELD_COMMERCE));
	// MOD - END - Division By Zero Bugfix
...
int CvCityAI::AI_getImprovementValue(CvPlot* pPlot, ImprovementTypes eImprovement, int iFoodPriority, int iProductionPriority, int iCommercePriority, int iDesiredFoodChange, int iClearFeatureValue, bool bEmphasizeIrrigation, BuildTypes* peBestBuild) const
{
	...
	else
	{
		...
		for (int iRel = 0; iRel < kImprovement.getNumBuilds(); iRel++)
		{
			BuildTypes eBuild = (BuildTypes)kImprovement.getBuilds(iRel);
	...
	else
	{
		for (int iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
		{
			...
			{
				// TODO: Take into consideration the actual probability and scale the value appropriately
				iValue++;
			}
		}
	}
	...
	//if (iValue >= 0) // condition disabled by K-Mod. (maybe the yield will be worth it!)
	{
		...
		*/
		...
		else
		{
			...
			const CvImprovementInfo& kImprovement = GC.getImprovementInfo(eImprovement);	
			const CvImprovementInfo& kPlotImprovement = GC.getImprovementInfo(pPlot->getImprovementType());			
			ImprovementClassTypes eImprovementClass = (ImprovementClassTypes)kImprovement.getImprovementClassType();
			ImprovementClassTypes ePlotImprovementClass = (ImprovementClassTypes)kPlotImprovement.getImprovementClassType();			
			if (eImprovementClass != NO_IMPROVEMENTCLASS && ePlotImprovementClass != NO_IMPROVEMENTCLASS)
			{
				int iYieldDiff = 0;
				int iYieldDiffOld = 0;
				for (YieldTypes i = (YieldTypes)0; i < NUM_YIELD_TYPES; i=(YieldTypes)(i+1))
				{		
					iYieldDiff += pPlot->calculateImprovementYieldChange(eImprovement, i, getOwnerINLINE(), false, false, this);
					iYieldDiffOld += pPlot->calculateImprovementYieldChange(pPlot->getImprovementType(), i, getOwnerINLINE(), false, false, this);
				}	
				if (iYieldDiffOld - iYieldDiff > 5)
				{
					iValue = -500; // if the yield difference between two farms is more than 5, don't even consider the inferior one
				}
			}
		}
...
bool CvCityAI::AI_chooseUnit(UnitAITypes eUnitAI, int iOdds)
{
	...
	if (eBestUnit != NO_UNIT)
	{
		...
		// K-Mod end
		{
			pushOrder(ORDER_TRAIN, eBestUnit, eUnitAI); // K-Mod
			return true;
		}
	}
...
bool CvCityAI::AI_chooseUnit(UnitTypes eUnit, UnitAITypes eUnitAI)
{
	if (eUnit != NO_UNIT)
	{
		pushOrder(ORDER_TRAIN, eUnit, eUnitAI); // K-Mod
		return true;
	}
	return false;	
}
...
bool CvCityAI::AI_chooseDefender()
{
	...
	// MOD - START - Fortification Units
	if (plot()->plotCheck(PUF_isUnitAIType, UNITAI_FORTIFICATION, -1, getOwnerINLINE()) == NULL)
	{
		if (AI_chooseUnit(UNITAI_FORTIFICATION))
		{
			return true;
							...
							if (eLoopUnit != NO_UNIT)
							{
								...
								if (kUnitInfo.getReligionSpreads(eReligion) > 0)
								{
									if (canTrain(eLoopUnit))
									{
										...
										iValue /= std::max(1, kUnitInfo.getProductionCost());
										// MOD - END - Division By Zero Bugfix
...
bool CvCityAI::AI_chooseBuilding(int iFocusFlags, int iMaxTurns, int iMinThreshold, int iOdds)
{
	...
	if (eBestBuilding != NO_BUILDING)
	{
		...
		// K-Mod end
		{
			pushOrder(ORDER_CONSTRUCT, eBestBuilding); // K-Mod
			return true;
		}
	}
...
bool CvCityAI::AI_chooseProject()
{
	...
	{
		pushOrder(ORDER_CREATE, eBestProject); // K-Mod
		return true;
	}
...
bool CvCityAI::AI_chooseProcess(CommerceTypes eCommerceType)
{
	...
	if (eBestProcess != NO_PROCESS)
	{
		pushOrder(ORDER_MAINTAIN, eBestProcess); // K-Mod
		return true;
	}
...
void CvCityAI::AI_juggleCitizens()
{
	...
	int iStarvingAllowance = 0;
	{
		...
		if (!isGrowthAllowed() || iHappinessLevel < (isHuman() ? 0 : 1))
		// MOD - END - City Growth Limits
		{
	...
	do
	{
			...
			&& unworked_it->first > worked_it->first)
		{
			...
			*/
			int iCurrentFood = worked_it->second.first
				? getYieldPerSpecialist((SpecialistTypes)worked_it->second.second, YIELD_FOOD)
				: getCityIndexPlot(worked_it->second.second)->getYield(YIELD_FOOD);

			int iNextFood = unworked_it->second.first
				? getYieldPerSpecialist((SpecialistTypes)unworked_it->second.second, YIELD_FOOD)
				: getCityIndexPlot(unworked_it->second.second)->getYield(YIELD_FOOD);
			// MOD - END - Advanced Yield and Commerce Effects
		...
		for (int i = 0; i < GC.getNumSpecialistInfos(); i++)
		{
			if (getSpecialistCount((SpecialistTypes)i) > 0)
			{
				...
				for (int j = 0; j < NUM_YIELD_TYPES; j++)
				{
					...
					int y = getYieldPerSpecialist((SpecialistTypes)i, (YieldTypes)j)*3/2;
					// MOD - START - Advanced Yield and Commerce Effects
					iYields[j] += y;
				...
				for (int j = 0 ; j < NUM_COMMERCE_TYPES; j++)
				{
					...
					int c = getCommercePerSpecialist((SpecialistTypes)i, (CommerceTypes)j)*3/2;
					// MOD - START - Advanced Yield and Commerce Effects
...
int CvCityAI::AI_yieldValue(short* piYields, short* piCommerceYields, bool bRemove, bool bIgnoreFood, bool bIgnoreStarvation, bool bWorkerOptimization, int iGrowthValue) const
{
		...
		if (bWorkerOptimization || !bFoodIsProduction)
		{
			...
			if (!AI_isEmphasizeAvoidGrowth())
			{
				...
				if (iGrowthValue > 0)
				{
					...
					// Anticipate changes from upcoming eras
					const CvTeamAI& kTeam = GET_TEAM(getTeam());
					EraTypes eCurrentEra = kOwner.getCurrentEra();
					EraTypes eResearchEra = kTeam.getCurrentResearchEra();
					if (eResearchEra != NO_ERA && eResearchEra > eCurrentEra)
					{
						const CvEraInfo& kResearchEra = GC.getEraInfo(eResearchEra);
						...
						int iTurnsUntilEra = kTeam.getTurnsUntilEra((EraTypes)(eCurrentEra+1));
						int iTurnsUntilEraLimit = (10 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getResearchPercent()) / 100;
						if (iTurnsUntilEra <= iApproxTurnsToGrow || iTurnsUntilEra <= iTurnsUntilEraLimit)
						{
							int iEraHappyChange = kResearchEra.getDiscoverHappiness();
							if (iEraHappyChange > 0)
							{
								iFutureHappy += iEraHappyChange;
							}
							else if (iEraHappyChange < 0)
							{
								iHappinessLevel += iEraHappyChange;
							}

							int iEraHealthChange = kResearchEra.getDiscoverHealth();
							if (iEraHealthChange < 0)
							{
								iHealthLevel += iEraHealthChange;
							}
						}
					}
					// MOD - END - Era Effects
					...
					if (iHappinessLevel < 5) // K-Mod. make it 5. We shouldn't really be worried about growing 10 pop all at once...
					{
						...
						if (iHappinessLevel + kMaxHappyIncrease > 0)
						{
							...
							// do we have temporary happiness?
							int iHappinessTimer = getHappinessTimer();
							if (iHappinessTimer > 0)
							{
								// no matter how soon it ends, it WILL end
								iFutureHappy--;
							}
					...
					// MOD - START - City Growth Limits
					// MOD - START - Epidemics
					int iMinEpidemicPop = GC.getDefineINT("EPIDEMIC_MIN_CITY_SIZE");
					bool bAcceptableEpidemicLevel = (getPopulation()+1 <= iMinEpidemicPop) || getEpidemicProbabilityPercent(1) <= kOwner.getAcceptableEpidemicPercent();
					if (!bAcceptableEpidemicLevel)
					{
					...
					if (iPopToGrow == 0 && iHappinessLevel >= 0 && iGoodTiles >= 0 && iHealthLevel >= 0 && bAcceptableEpidemicLevel)
					// MOD - END - Epidemics
					{
						if (!bBarFull)
						{
	...
	if( iProductionValue > 0 )
	{
		...
		else
		{
			...
			iProductionValue /= std::max(1, (iBaseProductionModifier + iExtraProductionModifier));
			// MOD - END - Division By Zero Bugfix
			...
			iProductionValue /= std::max(1, kOwner.AI_averageYieldMultiplier(YIELD_PRODUCTION));
			// MOD - END - Division By Zero Bugfix
		}
	...
	if( iCommerceValue > 0 )
	{
		...
		iCommerceValue /= std::max(1, kOwner.AI_averageYieldMultiplier(YIELD_COMMERCE));
		// MOD - END - Division By Zero Bugfix
...
//
	if( iFoodValue > 0 )
	{
		...
		iFoodValue /= std::max(1, kOwner.AI_averageYieldMultiplier(YIELD_FOOD));
		// MOD - END - Division By Zero Bugfix
...
int CvCityAI::AI_jobChangeValue(std::pair<bool, int> new_job, std::pair<bool, int> old_job, bool bIgnoreFood, bool bIgnoreStarvation, int iGrowthValue) const
{
	...
	for (YieldTypes i = (YieldTypes)0; i < NUM_YIELD_TYPES; i=(YieldTypes)(i+1))
	{
		...
		if (new_job.second >= 0)
		{
			...
			*/
			iYield += new_job.first
				? getYieldPerSpecialist((SpecialistTypes)new_job.second, i)
				: getCityIndexPlot(new_job.second)->getYield(i);
			// MOD - END - Advanced Yield and Commerce Effects
		}
		...
		if (old_job.second >= 0)
		{
			...
			*/
			iYield -= old_job.first
				? getYieldPerSpecialist((SpecialistTypes)old_job.second, i)
				: getCityIndexPlot(old_job.second)->getYield(i);
			// MOD - END - Advanced Yield and Commerce Effects
		}
	...
	if (new_job.first || old_job.first)
	{
		...
		for (CommerceTypes i = (CommerceTypes)0; i < NUM_COMMERCE_TYPES; i=(CommerceTypes)(i+1))
		{
			...
			*/
			if (new_job.second >= 0 && new_job.first)
				iCommerce += getCommercePerSpecialist((SpecialistTypes)new_job.second, i);

			if (old_job.second >= 0 && old_job.first)
				iCommerce -= getCommercePerSpecialist((SpecialistTypes)old_job.second, i);
			// MOD - END - Advanced Yield and Commerce Effects
		...
		if (iGPPGained || iGPPLost)
		{
			...
			{
				int iCityRate = (getGreatPeopleRate() / 100) + iGPPGained - iGPPLost;
				int iHighestRate = 0;
				int iLoop;
				for (CvCity* pLoopCity = kOwner.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kOwner.nextCity(&iLoop))
				{
					iHighestRate = std::max(iHighestRate, (int(pLoopCity->getGreatPeopleRate()/100)));
				}
				if (iHighestRate > iCityRate)
				{
					iGPPValue *= 100;
...
bool CvCityAI::AI_finalImprovementYieldDifference(CvPlot* pPlot, short* piYields) const
{
	...
	if (eFinalImprovement != NO_IMPROVEMENT && eFinalImprovement != eCurrentImprovement)
	{
		for (YieldTypes i = (YieldTypes)0; i < NUM_YIELD_TYPES; i=(YieldTypes)(i+1))
		{
			...
			int iYieldDiff = pPlot->calculateImprovementYieldChange(eFinalImprovement, i, getOwnerINLINE(), false, false, this) - pPlot->calculateImprovementYieldChange(eCurrentImprovement, i, getOwnerINLINE(), false, false, this);
			// MOD - END - Advanced Yield and Commerce Effects
...
bool CvCityAI::AI_timeWeightedImprovementYields(CvPlot const* pPlot, ImprovementTypes eImprovement, int time_scale, std::vector<float>& weighted_yields) const
{
	...
	while (eImprovement != NO_IMPROVEMENT && cited_improvements.insert(eImprovement).second)
	{
		...
		for (YieldTypes i = (YieldTypes)0; i < NUM_YIELD_TYPES; i=(YieldTypes)(i+1))
		{
			...
			int iYieldDiff = pPlot->calculateImprovementYieldChange(eImprovement, i, getOwnerINLINE(), false, false, this);
			// MOD - END - Advanced Yield and Commerce Effects
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		...
		if (pPlot->getImprovementType() != NO_IMPROVEMENT)
		{
			...
			if (eImprovementClass != NO_IMPROVEMENTCLASS && ePlotImprovementClass != NO_IMPROVEMENTCLASS)
			{
				...
				if (iYieldDiffOld - iYieldDiff > 3)
				{
					iValue = -1000; // if the yield difference between two farms is more than 5, don't even consider the inferior one
				}
			}
		}	
		if (iValue > iBestValue)
	...
	for (int iI = 0; iI < GC.getNumRouteInfos(); iI++)
	{
		...
		if (eRoute != eOldRoute)
		{
			...
			if (pPlot->getImprovementType() != NO_IMPROVEMENT)
			{
				...
				int iFood = kImprovement.getRouteYieldChanges(eRoute, YIELD_FOOD);
				int iProduction = kImprovement.getRouteYieldChanges(eRoute, YIELD_PRODUCTION);
				int iCommerce = kImprovement.getRouteYieldChanges(eRoute, YIELD_COMMERCE);

				if (eOldRoute != NO_ROUTE)
				{
					iFood -= kImprovement.getRouteYieldChanges(eOldRoute, YIELD_FOOD);
					iProduction -= kImprovement.getRouteYieldChanges(eOldRoute, YIELD_PRODUCTION);
					iCommerce -= kImprovement.getRouteYieldChanges(eOldRoute, YIELD_COMMERCE);
				}

				iTempValue += (iFood * 100);
				iTempValue += (iProduction * 80); // was 60
...
bool CvCityAI::AI_doPanic()
{
	...
	if (bLandWar)
	{
		int iOurDefense = GET_PLAYER(getOwnerINLINE()).AI_localDefenceStrength(plot(), getTeam()); // K-Mod
		int iEnemyOffense = GET_PLAYER(getOwnerINLINE()).AI_localAttackStrength(plot(), NO_TEAM); // K-Mod
		...
		if (pLoopPlot != NULL)
		{
		    ...
		    else
		    {
                ...
                else
                {
                    ...
                    if (eNonObsoleteBonus != NO_BONUS)
                    {
                        iTempValue += (GET_PLAYER(getOwnerINLINE()).AI_bonusVal(eNonObsoleteBonus, 0) * ((GET_PLAYER(getOwnerINLINE()).getNumTradeableBonuses(eNonObsoleteBonus) == 0) ? 4 : 2)); // K-Mod
                    }
...
void CvCityAI::AI_buildGovernorChooseProduction()
{
	...
	*/
	...
	if (iNumWaterAreas > 0)
	{
		int iNeededSeaWorkers = AI_getSeaWorkersNeeded();

		int iExistingSeaWorkers = plot()->plotCount(PUF_isUnitAIType, UNITAI_WORKER_SEA, -1, getOwnerINLINE());
		iExistingSeaWorkers += getNumTrainUnitAI(UNITAI_WORKER_SEA);

		for (int iWater = 0; iWater < iNumWaterAreas; iWater++)
		{
			iNeededSeaWorkers += kOwner.AI_neededSeaWorkers(aWaterAreas[iWater], plot(), this);
			iExistingSeaWorkers += kOwner.AI_totalWaterAreaUnitAIs(aWaterAreas[iWater], UNITAI_WORKER_SEA, this);
		}

		if (iExistingSeaWorkers < iNeededSeaWorkers)
		{
			bool bLocalResource = AI_countNumImprovableBonuses(true, NO_TECH, false, true) > 0;
			if (bLocalResource || iBestBuildingValue < 20)
			{
...
void CvCityAI::AI_barbChooseProduction()
{
	...
	*/
	...
	if (!bDanger && (iNumRelevantWaterAreas > 0) && (iWaterPercent > 30))
	// MOD - END - Multiple City Water Areas
	{
		...
		if (GC.getGameINLINE().getSorenRandNum(110, "AI arrrr!") < (iWaterPercent + 10))
		{
			...
			int iExistingSeaAttackers = plot()->plotCount(PUF_isUnitAIType, UNITAI_ATTACK_SEA, -1, getOwnerINLINE());
			iExistingSeaAttackers += getNumTrainUnitAI(UNITAI_ATTACK_SEA);
			for (int iWater = 0; iWater < iNumRelevantWaterAreas; iWater++)
			{
				iExistingSeaAttackers += kPlayer.AI_totalWaterAreaUnitAIs(aRelevantWaterAreas[iWater], UNITAI_ATTACK_SEA, this);
			}

			if (iExistingSeaAttackers < iNumCitiesInArea)
			// MOD - START - Multiple City Water Areas
			{
				if (AI_chooseUnit(UNITAI_ATTACK_SEA))
				{
...
int CvCityAI::AI_getYieldMagicValue(const int* piYieldsTimes100, bool bHealthy) const
{
	...
	int iValue = ((piYieldsTimes100[YIELD_FOOD] * 100 + piYieldsTimes100[YIELD_PRODUCTION]*70 + piYieldsTimes100[YIELD_COMMERCE]*40) - iPopEats * 102); // K-Mod, prod was *55
	iValue /= 100;
	return iValue;
}
	...
	if (eCurrentImprovement != NO_IMPROVEMENT && !bFinalBuildAssumed)
	{
		...
		if ((eFinalImprovement != NO_IMPROVEMENT) && (eFinalImprovement != eCurrentImprovement))
		{
			for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
			{
				...
				iYieldDiff = 100 * pPlot->calculateImprovementYieldChange(eFinalImprovement, ((YieldTypes)iI), getOwnerINLINE(), false, false, this);
				iYieldDiff -= 100 * pPlot->calculateImprovementYieldChange(eCurrentImprovement, ((YieldTypes)iI), getOwnerINLINE(), false, false, this);
				// MOD - END - Advanced Yield and Commerce Effects
...
int CvCityAI::AI_countGoodSpecialists(bool bHealthy) const
{
	...
	for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		...
		*/
		iValue += 100 * getYieldPerSpecialist(eSpecialist, YIELD_FOOD);
		iValue += 65 * getYieldPerSpecialist(eSpecialist, YIELD_PRODUCTION);
		iValue += 40 * getYieldPerSpecialist(eSpecialist, YIELD_COMMERCE);

		iValue += 40 * getCommercePerSpecialist(eSpecialist, COMMERCE_RESEARCH);
		iValue += 40 * getCommercePerSpecialist(eSpecialist, COMMERCE_GOLD);
		iValue += 20 * getCommercePerSpecialist(eSpecialist, COMMERCE_ESPIONAGE);
		iValue += 15 * getCommercePerSpecialist(eSpecialist, COMMERCE_CULTURE);
		// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Sea Improvements
int CvCityAI::AI_getSeaWorkersHave()
{
	return m_iSeaWorkersHave;
}

int CvCityAI::AI_getSeaWorkersNeeded()
{
	return m_iSeaWorkersNeeded;
}

void CvCityAI::AI_changeSeaWorkersHave(int iChange)
{
	m_iSeaWorkersHave += iChange;
	m_iSeaWorkersHave = std::max(0, m_iSeaWorkersHave);
}
// MOD - END - Sea Improvements
...
void CvCityAI::AI_updateWorkersNeededHere()
{
	...
	int iWorkersHave = 0;
	// MOD - START - Sea Improvements
	int iSeaWorkersNeeded = 0;
	int iSeaWorkersHave = 0;
	// MOD - END - Sea Improvements
	...
	if (getProductionUnit() != NO_UNIT)
	{
		...
		// MOD - START - Sea Improvements
		else if (getProductionUnitAI() == UNITAI_WORKER_SEA)
		{
			if (getProductionTurnsLeft() <= 2)
			{
				iSeaWorkersHave++;
	...
	for(CvSelectionGroup* pLoopSelectionGroup = kOwner.firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = kOwner.nextSelectionGroup(&iLoop))
	{
		if (pLoopSelectionGroup->AI_getMissionAIType() == MISSIONAI_BUILD)
		{
			...
			if (pMissionPlot != NULL)
			{
				...
				{
					// MOD - START - Sea Improvements
					if (pMissionPlot->isWater())
					{
						iSeaWorkersHave += pLoopSelectionGroup->getNumUnits();
					}
					else
					{
						iWorkersHave += pLoopSelectionGroup->getNumUnits();
					}
					// MOD - END - Sea Improvements
				}
			}
	...
	for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		...
		if (NULL != pLoopPlot && pLoopPlot->getWorkingCity() == this)
		{
			//if (pLoopPlot->getArea() == getArea())
			{
				...
				// MOD - START - Sea Improvements
				iSeaWorkersHave += pLoopPlot->plotCount(PUF_isUnitAIType, UNITAI_WORKER_SEA, -1, getOwner(), getTeam(), PUF_isNoMission, -1, -1);
				// MOD - END - Sea Improvements
				if (iI != CITY_HOME_PLOT)
				{
					if (pLoopPlot->getImprovementType() == NO_IMPROVEMENT)
	...
	// MOD - START - Sea Improvements
	m_iSeaWorkersNeeded = iSeaWorkersNeeded;
	m_iSeaWorkersHave = iSeaWorkersHave;
	// MOD - END - Sea Improvements
}
...
void CvCityAI::read(FDataStreamBase* pStream)
{
	...
	// MOD - START - City Growth Limits
	pStream->Read(&m_iEmphasizeAvoidUnhappyCount);
	pStream->Read(&m_iEmphasizeAvoidUnhealthCount);
	pStream->Read(&m_iEmphasizeAvoidEpidemicsCount);
	// MOD - END - City Growth Limits
	...
	// MOD - START - Sea Improvements
	pStream->Read(&m_iSeaWorkersNeeded);
	pStream->Read(&m_iSeaWorkersHave);
	// MOD - END - Sea Improvements
	// K-Mod
...
void CvCityAI::write(FDataStreamBase* pStream)
{
	...
	// MOD - START - City Growth Limits
	pStream->Write(m_iEmphasizeAvoidUnhappyCount);
	pStream->Write(m_iEmphasizeAvoidUnhealthCount);
	pStream->Write(m_iEmphasizeAvoidEpidemicsCount);
	// MOD - END - City Growth Limits
	...
	// MOD - START - Sea Improvements
	pStream->Write(m_iSeaWorkersNeeded);
	pStream->Write(m_iSeaWorkersHave);
	// MOD - END - Sea Improvements
