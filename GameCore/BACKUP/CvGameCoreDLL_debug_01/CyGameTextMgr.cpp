...
std::wstring CyGameTextMgr::getTechHelp(int iTech, bool bCivilopediaText, bool bPlayerContext, bool bStrategyText, bool bTreeInfo, int iFromTech)
{
	...
	if (bPlayerContext)
	{
		GAMETEXT.setTechHelp(szBuffer, (TechTypes)iTech, GC.getGameINLINE().getActiveCivilizationType(), GC.getGameINLINE().getActivePlayer(), bCivilopediaText, bStrategyText, bTreeInfo, (TechTypes)iFromTech);
	}
	else
	{
		GAMETEXT.setTechHelp(szBuffer, (TechTypes)iTech, NO_CIVILIZATION, NO_PLAYER, bCivilopediaText, bStrategyText, bTreeInfo, (TechTypes)iFromTech);
	}
...
// MOD - START - Aid
std::wstring CyGameTextMgr::getAidHelp(int iAid, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setAidHelp(szBuffer, (AidTypes)iAid, bCivilopediaText);
	return szBuffer.getCString();
}
// MOD - END - Aid

// MOD - START - Detriments
std::wstring CyGameTextMgr::getDetrimentHelp(int iDetriment, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setDetrimentHelp(szBuffer, (DetrimentTypes)iDetriment, bCivilopediaText);
	return szBuffer.getCString();
}
// MOD - END - Detriments
...
std::wstring CyGameTextMgr::getImprovementHelp(int iImprovement, bool bCivilopediaEffects, bool bCivilopediaImprovements)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setImprovementHelp(szBuffer, (ImprovementTypes)iImprovement, bCivilopediaEffects, bCivilopediaImprovements);
	return szBuffer.getCString();
}
...
std::wstring CyGameTextMgr::getWorldSizeHelp(int iWorld, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setWorldSizeHelp(szBuffer, (WorldSizeTypes)iWorld, bCivilopediaText);
	return szBuffer.getCString();
}

std::wstring CyGameTextMgr::getHandicapHelp(int iHandicap, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setHandicapHelp(szBuffer, (HandicapTypes)iHandicap, bCivilopediaText);
	return szBuffer.getCString();
}

std::wstring CyGameTextMgr::getConceptHelp(int iConcept, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setConceptHelp(szBuffer, (ConceptTypes)iConcept, bCivilopediaText);
	return szBuffer.getCString();
}

std::wstring CyGameTextMgr::getHotKeyClassHelp(int iHotKeyClass, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setHotKeyClassHelp(szBuffer, (HotKeyClassTypes)iHotKeyClass, bCivilopediaText);
	return szBuffer.getCString();
}
// MOD - END - Improved Civilopedia

// MOD - START - Era Effects
std::wstring CyGameTextMgr::getEraHelp(int iEra, bool bSplash, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setEraHelp(szBuffer, (EraTypes)iEra, bSplash, bCivilopediaText);
	return szBuffer.getCString();
}
// MOD - END - Era Effects
...
// MOD - START - Improved Civilopedia
std::wstring CyGameTextMgr::parseTraits(int /*LeaderHeadTypes*/ iTrait, int /*CivilizationTypes*/ iCivilization, bool bDawnOfMan, bool bCivilopediaText)
{
	CvWStringBuffer szBuffer;
	GAMETEXT.parseTraits(szBuffer, (TraitTypes)iTrait, (CivilizationTypes) iCivilization, bDawnOfMan, bCivilopediaText);
	return szBuffer.getCString();
}
// MOD - END - Improved Civilopedia
