...
void CvTeam::uninit()
{
	...
	// MOD - START - Building Discontinuation
	SAFE_DELETE_ARRAY(m_paiDiscontinuedBuildingCount);
	// MOD - END - Building Discontinuation
	SAFE_DELETE_ARRAY(m_paiResearchProgress);
	SAFE_DELETE_ARRAY(m_paiTechCount);
	// MOD - START - Tech Path Caching
	SAFE_DELETE_ARRAY(m_paiTechPathLength);
	SAFE_DELETE_ARRAY(m_paiTechPathCost);
	// MOD - END - Tech Path Caching
	...
	// MOD - START - Tech Transfer
	SAFE_DELETE_ARRAY(m_paiTechTransferCount);
	// MOD - END - Tech Transfer
	...
	// MOD - START - Advanced Yield and Commerce Effects
	if (m_ppaaiImprovementIrrigatedYieldChange != NULL)
	{
		for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
		{
			SAFE_DELETE_ARRAY(m_ppaaiImprovementIrrigatedYieldChange[iI]);
		}
		SAFE_DELETE_ARRAY(m_ppaaiImprovementIrrigatedYieldChange);
	}
	// MOD - END - Advanced Yield and Commerce Effects
}
...
void CvTeam::reset(TeamTypes eID, bool bConstructorCall)
{
	...
	if (!bConstructorCall)
	{
		...
		// MOD - START - Unit Roles
		FAssertMsg(m_paiUnitRoleCount == NULL, "about to leak memory, CvTeam::m_paiUnitRoleCount");
		...
		// MOD - START - Building Discontinuation
		FAssertMsg(m_paiDiscontinuedBuildingCount==NULL, "about to leak memory, CvPlayer::m_paiDiscontinuedBuildingCount");
		m_paiDiscontinuedBuildingCount = new int [GC.getNumBuildingInfos()];
		for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
		{
			m_paiDiscontinuedBuildingCount[iI] = 0;
		}
		// MOD - END - Building Discontinuation
		...
		// MOD - START - Tech Path Caching
		FAssertMsg(m_paiTechPathLength==NULL, "about to leak memory, CvPlayer::m_paiTechPathLength");
		m_paiTechPathLength = new int [GC.getNumTechInfos()];
		FAssertMsg(m_paiTechPathCost==NULL, "about to leak memory, CvPlayer::m_paiTechPathCost");
		m_paiTechPathCost = new int [GC.getNumTechInfos()];
		// MOD - END - Tech Path Caching
		for (iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
			...
			m_paiTechCount[iI] = 0;
			// MOD - START - Tech Path Caching
			m_paiTechPathLength[iI] = -1;
			m_paiTechPathCost[iI] = -1;
			// MOD - END - Tech Path Caching
		}
		...
		// MOD - START - Tech Transfer
		FAssertMsg(m_paiTechTransferCount==NULL, "about to leak memory, CvTeam::m_paiTechTransferCount");
		m_paiTechTransferCount = new int[GC.getNumTechInfos()];
		// MOD - END - Tech Transfer
		for (iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
		...
		// MOD - START - Advanced Yield and Commerce Effects
		FAssertMsg(m_ppaaiImprovementIrrigatedYieldChange==NULL, "about to leak memory, CvTeam::m_ppaaiImprovementYieldChange");
		m_ppaaiImprovementIrrigatedYieldChange = new int*[GC.getNumImprovementInfos()];
		for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
		{
			m_ppaaiImprovementIrrigatedYieldChange[iI] = new int[NUM_YIELD_TYPES];
			for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
			{
				m_ppaaiImprovementIrrigatedYieldChange[iI][iJ] = 0;
			}
		}
		// MOD - END - Advanced Yield and Commerce Effects
...
void CvTeam::addTeam(TeamTypes eTeam)
{
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		// MOD - START - Aid
		for (iJ = 0; iJ < GC.getNumAidInfos(); iJ++)
		{
			pLoopPlot->changeAidAmount(getID(), ((AidTypes)iJ), pLoopPlot->getAidAmount(eTeam, ((AidTypes)iJ)));
		}
		// MOD - END - Aid

		// MOD - START - Detriments
		for (iJ = 0; iJ < GC.getNumDetrimentInfos(); iJ++)
		{
			pLoopPlot->changeDetrimentAmount(getID(), ((DetrimentTypes)iJ), pLoopPlot->getDetrimentAmount(eTeam, ((DetrimentTypes)iJ)));
		}
		// MOD - END - Detriments
		...
		{
			DomainTypes eLoopDomain = (DomainTypes)iJ;
			int iChange = pLoopPlot->getCombatUnitDomainCount(eTeam, eLoopDomain);
			if (iChange != 0)
			{
				pLoopPlot->setCombatUnitDomainCount(getID(), eLoopDomain, pLoopPlot->getCombatUnitDomainCount(getID(), eLoopDomain) + iChange);
			}
		}
		// MOD - END - Congestion
	}
						...
						if (pLoopCity->getNumActiveBuilding((BuildingTypes)iJ) > 0)
						{
							if (GC.getBuildingInfo((BuildingTypes)iJ).isTeamShare())
							{
								for (iK = 0; iK < MAX_PLAYERS; iK++)
								{
									if (GET_PLAYER((PlayerTypes)iK).getTeam() == getID())
									{
										GET_PLAYER((PlayerTypes)iK).processBuilding(((BuildingTypes)iJ), pLoopCity->getNumActiveBuilding((BuildingTypes)iJ), pLoopCity->area());
									}
								}
							}

							processBuilding(((BuildingTypes)iJ), pLoopCity->getNumActiveBuilding((BuildingTypes)iJ));
						}
...
void CvTeam::shareCounters(TeamTypes eTeam)
{
	...
	// MOD - START - Unit Roles
	for (UnitRoleTypes eUnitRole = (UnitRoleTypes)0; eUnitRole < GC.getNumUnitRoleInfos(); eUnitRole = (UnitRoleTypes)(eUnitRole + 1))
	{
		changeUnitRoleCount(eUnitRole, kShareTeam.getUnitRoleCount(eUnitRole));
		//kShareTeam.changeUnitRoleCount(eUnitRole, getUnitRoleCount(eUnitRole) - kShareTeam.getUnitRoleCount(eUnitRole));
	}
	// MOD - END - Unit Roles
...
void CvTeam::updateYield()
{
	...
	*/
	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		GET_PLAYER(getLivingTeamMember(iMem)).updateYield();
	}
...
void CvTeam::updatePowerHealth()
{
	...
	*/
	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		GET_PLAYER(getLivingTeamMember(iMem)).updatePowerHealth();
	}
	// MOD - END - Cached Team Members
}

// MOD - START - Advanced Building Prerequisites (Power)
void CvTeam::updatePowerBuildings()
{
	// MOD - START - Cached Team Members
	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		GET_PLAYER(getLivingTeamMember(iMem)).updatePowerBuildings();
	}
	// MOD - END - Cached Team Members
}
...
void CvTeam::updateCommerce()
{
	...
	*/
	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		GET_PLAYER(getLivingTeamMember(iMem)).updateCommerce();
	}
	// MOD - END - Cached Team Members
}
...
bool CvTeam::canDeclareWar(TeamTypes eTeam) const
{
	...
	int iOurNukesAvailable = 0;
	int iOurNukesUsed = 0;
	int iTheirNukesUsed = 0;
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				iOurNukesAvailable += GET_PLAYER((PlayerTypes)iI).AI_totalUnitAIs(UNITAI_ICBM);
				iOurNukesUsed += GET_PLAYER((PlayerTypes)iI).getNukesUsed();
			}
			else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
			{
				iTheirNukesUsed += GET_PLAYER((PlayerTypes)iI).getNukesUsed();
			}
		}
	}

	if (iOurNukesUsed == 0 && iOurNukesAvailable == 0 && iTheirNukesUsed > 0)
	{
#if 0
		logBBAI("      Team %d (%S) blocked any warplan with team %d with nuclear veto, they used %d nukes so far while we used 0", getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), eTeam, iTheirNukesUsed);
#endif
		return false;
	}
	else
	{
#if 0
		logBBAI("      Team %d (%S) DID NOT block any warplan with team %d with nuclear veto, they used %d nukes so far while we used %d and we have %d available", getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), eTeam, iTheirNukesUsed, iOurNukesUsed, iOurNukesAvailable);
#endif
	}
...
void CvTeam::declareWar(TeamTypes eTeam, bool bNewDiplo, WarPlanTypes eWarPlan, bool bPrimaryDoW)
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				// Apply our detriments to our enemies
				GET_PLAYER((PlayerTypes)iI).applyDetriments(eTeam);
			}
			else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
			{
				// Our enemies also apply their detriments to us
				GET_PLAYER((PlayerTypes)iI).applyDetriments(getID());
			}
		}
	}
	// MOD - END - Detriments
	...
	if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
	{
		...
		bool const bPlaySound = ((!isAVassal() || isHuman()) &&
				(!GET_TEAM(eTeam).isAVassal() || GET_TEAM(eTeam).isHuman())); // </f1rpo>
				...
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
					{
						...
						gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer,
								/* <f1rpo> */ !bPlaySound ? NULL : /* </f1rpo> */ "AS2D_DECLAREWAR",
					...
					else if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
					{
						...
						gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
								/* <f1rpo> */ !bPlaySound ? NULL : /* </f1rpo> */ "AS2D_THEIRDECLAREWAR",
								MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
					}
				}
			}
	...
	{
		const CvPlayerAI& kPlayer = GET_PLAYER(i);
		...
		int iGlobalWarConscript = kPlayer.getWarConscript();
		int iUnitsConscripted = 0;

		CvCity* pLoopCity;
		int iLoop;
		for (pLoopCity = kPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kPlayer.nextCity(&iLoop))
		{
			int iTotalWarConscript = pLoopCity->getWarConscript() + iGlobalWarConscript;
			if (iTotalWarConscript > 0)
			{
				if (pLoopCity->canConscript(true))
				{
					for (int iCount = 0; iCount < iTotalWarConscript; iCount++)
					{
						CvUnit* pUnit = pLoopCity->initConscriptedUnit();
						if (pUnit != NULL)
						{
							iUnitsConscripted++;
						}
		...
		if (iUnitsConscripted != 0)
		{
			CvWString szBuffer;

			szBuffer = gDLL->getText("TXT_KEY_MISC_OUR_UNITS_CONSCRIPTED_FOR_WAR", iUnitsConscripted, GET_TEAM(eEnemyTeam).getName().GetCString());
			gDLL->getInterfaceIFace()->addHumanMessage(i, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WELOVEKING", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));

			for (PlayerTypes j = (PlayerTypes)0; j < MAX_CIV_PLAYERS; j=(PlayerTypes)(j+1))
			{
				const CvPlayerAI& kEnemyPlayer = GET_PLAYER(j);
				if (kEnemyPlayer.isAlive() && kEnemyPlayer.getTeam() == eEnemyTeam)
				{
					szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_UNITS_CONSCRIPTED_FOR_WAR", kPlayer.getCivilizationAdjective(0));
					gDLL->getInterfaceIFace()->addHumanMessage(j, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_ENEMY_TROOPS", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
				}
			}
		}
	}
...
/************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
				{
					// Remove our detriments from our enemies
					GET_PLAYER((PlayerTypes)iI).removeDetriments(eTeam);
				}
				else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
				{
					// Our enemies also remove their detriments from us
					GET_PLAYER((PlayerTypes)iI).removeDetriments(getID());
				}
			}
		}
		// MOD - END - Detriments
...
// MOD - START - Unit Roles
int CvTeam::getUnitRoleMaking(UnitRoleTypes eUnitRole) const
{
	int iCount = 0;

	// MOD - START - Cached Team Members
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).getUnitRoleMaking(eUnitRole);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getUnitRoleCountPlusMaking(UnitRoleTypes eIndex) const
{
...
int CvTeam::getUnitClassMaking(UnitClassTypes eUnitClass) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).getUnitClassMaking(eUnitClass);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getBuildingClassMaking(BuildingClassTypes eBuildingClass) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).getBuildingClassMaking(eBuildingClass);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getHasReligionCount(ReligionTypes eReligion) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).getHasReligionCount(eReligion);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getHasCorporationCount(CorporationTypes eCorporation) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).getHasCorporationCount(eCorporation);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::countTotalCulture() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).countTotalCulture();
	}
	// MOD - END - Cached Team Members
...
int CvTeam::countNumUnitsByArea(CvArea* pArea) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += pArea->getUnitsPerPlayer(*it);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::countNumCitiesByArea(CvArea* pArea) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += pArea->getCitiesPerPlayer(*it);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::countTotalPopulationByArea(CvArea* pArea) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += pArea->getPopulationPerPlayer(*it);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::countNumAIUnitsByArea(CvArea* pArea, UnitAITypes eUnitAI) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += pArea->getNumAIUnits(*it, eUnitAI);
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getTypicalUnitValue(UnitAITypes eUnitAI, DomainTypes eDomain) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iMax = std::max(iMax, GET_PLAYER(*it).getTypicalUnitValue(eUnitAI, eDomain));
	}
	// MOD - END - Cached Team Members
...
int CvTeam::getResearchCost(TechTypes eTech, bool bGlobalModifiers, bool bTeamSizeModifiers) const // K-Mod added bGlobalModifiers & bTeamSizeModifiers
{
	...
	const CvHandicapInfo& kHandicap = GC.getHandicapInfo(getHandicapType());

	iCost *= kHandicap.getResearchPercent();
	iCost /= 100;

	// MOD - START - Per City Research Cost
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_PER_CITY_RESEARCH_COST))
	{
		iCost *= 100 + (kHandicap.getPerCityResearchCostModifier() * getNumCities());
		iCost /= 100;
	}
	else
	{
		iCost *= 100 + (kHandicap.getPerCityResearchCostModifier() * (GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities()));
		iCost /= 100;
	}
	...
	if (bGlobalModifiers) // K-Mod
	{
		...
		if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_PER_CITY_RESEARCH_COST))
		{
			iCost *= 100 + (GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getPerCityResearchCostModifier() * getNumCities());
			iCost /= 100;
		}
		else
		{
			iCost *= 100 + (GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getPerCityResearchCostModifier() * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities());
			iCost /= 100;
		}
...
bool CvTeam::hasBonus(BonusTypes eBonus) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (GET_PLAYER(*it).hasBonus(eBonus))
		{
			return true;
		}
	}
...
HandicapTypes CvTeam::getHandicapType() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iGameHandicap += GET_PLAYER(*it).getHandicapType();
		iCount++;
	}
	// MOD - END - Cached Team Members
...
CvWString CvTeam::getName() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		setListHelp(szBuffer, L"", GET_PLAYER(*it).getName(), L"/", bFirst);
		bFirst = false;
	}
	// MOD - END - Cached Team Members
...
CvWString CvTeam::getReplayName() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		setListHelp(szBuffer, L"", GET_PLAYER(*it).getReplayName(), L"/", bFirst);
		bFirst = false;
	}
	// MOD - END - Cached Team Members
...
{
	if (!(GC.getGameINLINE().isOption(GAMEOPTION_ALLIANCES)))
	{
		return false;
	}	
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvTeam::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}

void CvTeam::setGlobalEpidemicModifier(int iNewValue)
{
	m_iGlobalEpidemicModifier = iNewValue;
}

void CvTeam::changeGlobalEpidemicModifier(int iChange)
{
...
void CvTeam::setOpenBorders(TeamTypes eIndex, bool bNewValue)
{
	...
	{
		// MOD - START - Tech Transfer
		CvTeam& kTeam = GET_TEAM((TeamTypes)eIndex);
		// MOD - END - Tech Transfer
		...
		// MOD - START - Tech Transfer
		if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_TECH_TRANSFER))
		{
			for (iI = 0; iI < GC.getNumTechInfos(); iI++)
			{
				// Give the other team access to our techs now that we have open borders
				if (isHasTech((TechTypes) iI))
				{
					kTeam.changeTechTransferCount((TechTypes) iI, bNewValue ? 1 : -1);
				}
			}
		}
		// MOD - END - Tech Transfer
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
				{
					GET_PLAYER((PlayerTypes)iI).updateOpenBordersCommerce();
				}
			}
		}
...
//Cancel deals between the master and the vassal
			m_bCapitulated = false;
            int iLoop;
            CLLNode<TradeData>* pNode;
            ...
            for (CvDeal* pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != NULL; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
            {
                //Cancel all resource/gold trade deals
                if (((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == getID()) && (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() != NULL)) || ((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() != NULL) && (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == getID())))
                {
                    if (pLoopDeal->getFirstTrades() != NULL)
                    {
                        for (pNode = pLoopDeal->getFirstTrades()->head(); pNode; pNode = pLoopDeal->getFirstTrades()->next(pNode)) {
                            if ((pNode->m_data.m_eItemType == TRADE_RESOURCES) || (pNode->m_data.m_eItemType == TRADE_GOLD_PER_TURN))
                            {
                                pLoopDeal->kill();
                            }
                        }
...
void CvTeam::changeProjectCount(ProjectTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		// MOD - START - Epidemics
		// Epidemic system by Mexico
		changeGlobalEpidemicModifier(kProject.getGlobalEpidemicModifier() * iChange);
		// MOD - END - Epidemics
...
// MOD - START - Unit Roles
int CvTeam::getUnitRoleCount(UnitRoleTypes eIndex) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitRoleInfos(), eIndex, "CvTeam::getUnitRoleCount");
	return m_paiUnitRoleCount[eIndex];
}


void CvTeam::changeUnitRoleCount(UnitRoleTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumUnitRoleInfos(), eIndex, "CvTeam::changeUnitRoleCount");
					...
					if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
					{
						for (pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
						{
							...
							*/
							...
							pLoopCity->updateNumActiveBuilding(eIndex);
							// MOD - END - Building Activation
						}
					}
				}
...
// MOD - START - Building Discontinuation
int CvTeam::getDiscontinuedBuildingCount(BuildingTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	return m_paiDiscontinuedBuildingCount[eIndex];
}

bool CvTeam::isDiscontinuedBuilding(BuildingTypes eIndex) const
{
	return (getDiscontinuedBuildingCount(eIndex) > 0);
}

void CvTeam::changeDiscontinuedBuildingCount(BuildingTypes eIndex, int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	...
	if (iChange != 0)
	{
		m_paiDiscontinuedBuildingCount[eIndex] += iChange;
		FAssert(getDiscontinuedBuildingCount(eIndex) >= 0);
	}
}
// MOD - END - Building Discontinuation
...
EraTypes CvTeam::getCurrentResearchEra() const
{
	EraTypes eEra = NO_ERA;

	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		const CvPlayer& kLoopPlayer = GET_PLAYER(getLivingTeamMember(iMem));
		TechTypes eResearchTech = kLoopPlayer.getCurrentResearch();
		if (eResearchTech != NO_TECH)
		{
			const CvTechInfo& kResearchTech = GC.getTechInfo(eResearchTech);
			if (eEra == NO_ERA || kResearchTech.getEra() > eEra)
			{
				eEra = (EraTypes)kResearchTech.getEra();
			}
		}
	}
...
int CvTeam::getTurnsUntilEra(EraTypes eEra) const
{
	int iTurns = MAX_INT;
	...
	for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
	{
		...
		if (kLoopPlayer.getCurrentEra() <= eEra)
		{
			return 0;
		}
		...
		{
			const CvTechInfo& kResearchTech = GC.getTechInfo(eResearchTech);
			if (kResearchTech.getEra() >= eEra)
			{
				int iResearchTurnsLeft = kLoopPlayer.getResearchTurnsLeft(eResearchTech, true);
				iTurns = std::min(iTurns, iResearchTurnsLeft);
			}
		}
	}
...
// MOD - START - Tech Path Caching
int CvTeam::getTechPathLength(TechTypes eTech) const
{
	FASSERT_BOUNDS(0, GC.getNumTechInfos(), eTech, "CvTeam::getTechPathLength");

	if (m_paiTechPathLength[eTech] == -1)
	{
		int iPathLength = 0;
		...
		if (!isHasTech(eTech))
		{
			...
			// Add one for this tech
			iPathLength++;

			//	Cycle through the AND paths and add their tech lengths
			for (int iPrereq = 0; iPrereq < GC.getNUM_AND_TECH_PREREQS(); iPrereq++)
			{
				TechTypes ePrereq = (TechTypes)kTech.getPrereqAndTechs(iPrereq);

				if (ePrereq != NO_TECH)
				{
					iPathLength += getTechPathLength(ePrereq);
				}
			}

			//	Cycle through the OR paths and add the tech length of the shortest route
			TechTypes eShortestOrTech = NO_TECH;
			int iShortestOrPathLength = MAX_INT;
			for (int iPrereq = 0; iPrereq < GC.getNUM_OR_TECH_PREREQS(); iPrereq++)
			{
				TechTypes ePrereq = (TechTypes)kTech.getPrereqOrTechs(iPrereq);

				if (ePrereq != NO_TECH)
				{
					int iCandidatePathLength = getTechPathLength(ePrereq);

					if (iCandidatePathLength < iShortestOrPathLength)
					{
						eShortestOrTech = ePrereq;
						iShortestOrPathLength = iCandidatePathLength;
					}
				}
			}

			if (eShortestOrTech != NO_TECH)
			{
				iPathLength += iShortestOrPathLength;
			}
		}

		m_paiTechPathLength[eTech] = iPathLength;
	}

	return m_paiTechPathLength[eTech];
}


int CvTeam::getTechPathCost(TechTypes eTech) const
{
	FASSERT_BOUNDS(0, GC.getNumTechInfos(), eTech, "CvTeam::getTechPathLength");

	if (m_paiTechPathCost[eTech] == -1)
	{
		int iPathCost = 0;
		...
		if (!isHasTech(eTech))
		{
			...
			// Add the cost of this tech
			iPathCost += getResearchCost(eTech);

			//	Cycle through the AND paths and add their tech costs
			for (int iPrereq = 0; iPrereq < GC.getNUM_AND_TECH_PREREQS(); iPrereq++)
			{
				TechTypes ePrereq = (TechTypes)kTech.getPrereqAndTechs(iPrereq);
				...
				if (ePrereq != NO_TECH)
				{
					iPathCost += getTechPathCost(ePrereq);
				}
			}

			//	Cycle through the OR paths and add the tech cost of the shortest route
			TechTypes eShortestOrTech = NO_TECH;
			int iShortestOrPathCost = MAX_INT;
			for (int iPrereq = 0; iPrereq < GC.getNUM_OR_TECH_PREREQS(); iPrereq++)
			{
				TechTypes ePrereq = (TechTypes)kTech.getPrereqOrTechs(iPrereq);
				...
				if (ePrereq != NO_TECH)
				{
					int iCandidatePathCost = getTechPathCost(ePrereq);

					if (iCandidatePathCost < iShortestOrPathCost)
					{
						eShortestOrTech = ePrereq;
						iShortestOrPathCost = iCandidatePathCost;
					}
				}
			}
			...
			if (eShortestOrTech != NO_TECH)
			{
				iPathCost += iShortestOrPathCost;
			}
		}

		m_paiTechPathCost[eTech] = iPathCost;
	}

	return m_paiTechPathCost[eTech];
}


void CvTeam::invalidateTechPathLengthCache()
{
	for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
	{
		m_paiTechPathLength[iI] = -1;
...
void CvTeam::invalidateTechPathCostCache()
{
	for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
	{
		m_paiTechPathCost[iI] = -1;
...
void CvTeam::setHasTech(TechTypes eIndex, bool bNewValue, PlayerTypes ePlayer, bool bFirst, bool bAnnounce)
{
	...
	if (isHasTech(eIndex) != bNewValue)
	{
		...
		else
		{
			...
			std::vector<BonusTypes> aAffectedCityBonuses;

			for (iI = 0; iI < GC.getNumBonusInfos(); ++iI)
			{
				// TODO: Evaluate this question
				// What about getTechCityTrade?
				if (GC.getBonusInfo((BonusTypes)iI).getTechObsolete() == eIndex)
				{
					aAffectedCityBonuses.push_back((BonusTypes)iI);
				}
			}
			// MOD - END - Obsolete City Free Bonus

			// MOD - START - Aid
			std::vector<AidTypes> aAffectedAidTypes;

			for (int iAid = 0; iAid < GC.getNumAidInfos(); iAid++)
			{
				const CvAidInfo& kAidInfo = GC.getAidInfo((AidTypes)iAid);

				for (int iPromotion = 0; iPromotion < kAidInfo.getNumPromotions(); iPromotion++)
				{
					PromotionTypes eAidPromotion = (PromotionTypes)kAidInfo.getPromotions(iPromotion);
					if (GC.getPromotionInfo(eAidPromotion).getTechPrereq() == eIndex)
					{
						aAffectedAidTypes.push_back((AidTypes)iAid);
						break;
					}
				}
			}
			...
			// MOD - START - Detriments
			std::vector<DetrimentTypes> aAffectedDetrimentTypes;

			for (int iDetriment = 0; iDetriment < GC.getNumDetrimentInfos(); iDetriment++)
			{
				const CvDetrimentInfo& kDetrimentInfo = GC.getDetrimentInfo((DetrimentTypes)iDetriment);

				for (int iPromotion = 0; iPromotion < kDetrimentInfo.getNumPromotions(); iPromotion++)
				{
					PromotionTypes eDetrimentPromotion = (PromotionTypes)kDetrimentInfo.getPromotions(iPromotion);
					if (GC.getPromotionInfo(eDetrimentPromotion).getTechPrereq() == eIndex)
					{
						aAffectedDetrimentTypes.push_back((DetrimentTypes)iDetriment);
						break;
					}
				}
			}
			...
			*/
			...
			// MOD - START - Tech Path Caching
			invalidateTechPathLengthCache();
			invalidateTechPathCostCache();
			// MOD - END - Tech Path Caching
			...
			for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
			{
				...
				*/
				pLoopPlot->updateBonusCityTradeForTech(getID(), eIndex, bNewValue);
				// MOD - END - PlotGroup Bonus Efficiency

				// MOD - START - Aid
				for (std::vector<AidTypes>::iterator it = aAffectedAidTypes.begin(); it != aAffectedAidTypes.end(); ++it)
				{
					pLoopPlot->updateAidEffects(getID(), *it);
				}
				// MOD - END - Aid

				// MOD - START - Detriments
				for (std::vector<DetrimentTypes>::iterator it = aAffectedDetrimentTypes.begin(); it != aAffectedDetrimentTypes.end(); ++it)
				{
					// TODO: Fix this so that detriments are applied based on the tech level of the supplier, not the receiver of the detriments
					pLoopPlot->updateDetrimentEffects(getID(), *it);
				}
				// MOD - END - Detriments
			}
			...
			for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
			{
				CvPlayerAI& kLoopPlayer = GET_PLAYER(*it);
				CvCity* pLoopCity;
				int iLoop;

				for (pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kLoopPlayer.nextCity(&iLoop))
				{
					pLoopCity->updateFreeBonusDistribution(aAffectedCityBonuses, !bNewValue);
				}
			}
			// MOD - END - Obsolete City Free Bonus
				...
				for (iI = 0; iI < MAX_TEAMS; iI++)
				{
					if (iI != getID())
					{
						if (isOpenBorders((TeamTypes)iI))
						{
							...
							if (kLoopTeam.isAlive())
							{
								kLoopTeam.changeTechTransferCount(eIndex, bNewValue ? 1 : -1);
							}
						}
					}
				}
...
// MOD - START - Tech Transfer
int CvTeam::getTechTransferCount(TechTypes eTech) const
{
	return m_paiTechTransferCount[eTech];
}


bool CvTeam::isTechTransfer(TechTypes eTech) const
{
	return (getTechTransferCount(eTech) > 0);
}


void CvTeam::changeTechTransferCount(TechTypes eTech, int iChange)
{
	if (iChange != 0)
	{
		m_paiTechTransferCount[eTech] = (m_paiTechTransferCount[eTech] + iChange);

		// MOD - START - Tech Transfer
		for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
		{
			PlayerTypes eLoopPlayer = getLivingTeamMember(iMem);
			if (GC.getGameINLINE().getActivePlayer() == eLoopPlayer)
			{
				if (GET_PLAYER(eLoopPlayer).isResearchingTech(eTech))
				{
					gDLL->getInterfaceIFace()->setDirty(GameData_DIRTY_BIT, true);
				}
			}
		...
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			...
			if (kPlayer.isAlive())
			{
				if (kPlayer.getTeam() == getID())
				{
					if (eTech == kPlayer.getCurrentResearch())
					{
						kPlayer.updateTechTransferCommerce();
					}
				}
			}
		}
...
// MOD - START - Advanced Yield and Commerce Effects
int CvTeam::getImprovementIrrigatedYieldChange(ImprovementTypes eImprovement, YieldTypes eYield) const
{
	FASSERT_BOUNDS(0, GC.getNumImprovementInfos(), eImprovement, "CvTeam::getImprovementIrrigatedYieldChange");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvTeam::getImprovementIrrigatedYieldChange");
	return m_ppaaiImprovementIrrigatedYieldChange[eImprovement][eYield];
}


void CvTeam::changeImprovementIrrigatedYieldChange(ImprovementTypes eImprovement, YieldTypes eYield, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumImprovementInfos(), eImprovement, "CvTeam::changeImprovementIrrigatedYieldChange");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvTeam::changeImprovementIrrigatedYieldChange");
	if (iChange != 0)
	{
		m_ppaaiImprovementIrrigatedYieldChange[eImprovement][eYield] += iChange;
		FAssert(getImprovementIrrigatedYieldChange(eImprovement, eYield) >= 0);
...
bool CvTeam::doesImprovementConnectBonus(ImprovementTypes eImprovement, BonusTypes eBonus, bool bCheckTechnology) const
{
	...
	// MOD - START - PlotGroup Bonus Efficiency
	if (bCheckTechnology && !isBonusConnected(eBonus))
		return false;
	// MOD - END - PlotGroup Bonus Efficiency
...
// MOD - START - PlotGroup Bonus Efficiency
bool CvTeam::isBonusConnected(BonusTypes eBonus) const
{
	FASSERT_BOUNDS(0, GC.getNumBonusInfos(), eBonus, "CvTeam::isBonusConnected");

	const CvBonusInfo& kBonus = GC.getBonusInfo(eBonus);

	TechTypes eTechCityTrade = (TechTypes)kBonus.getTechCityTrade();
	TechTypes eTechObsolete = (TechTypes)kBonus.getTechObsolete();
	TechTypes eTechReveal = (TechTypes)kBonus.getTechReveal();

	return
		isHasTech(eTechCityTrade) &&
		(eTechObsolete == NO_TECH || !isHasTech(eTechObsolete)) &&
		isBonusRevealed(eBonus);
}

bool CvTeam::isBonusConnectedWithTech(BonusTypes eBonus, TechTypes eTech, bool bHasTech) const
{
	FASSERT_BOUNDS(0, GC.getNumBonusInfos(), eBonus, "CvTeam::isBonusConnectedWithTech");
	FASSERT_BOUNDS(0, GC.getNumTechInfos(), eTech, "CvTeam::isBonusConnectedWithTech");

	const CvBonusInfo& kBonus = GC.getBonusInfo(eBonus);

	TechTypes eTechCityTrade = (TechTypes)kBonus.getTechCityTrade();
	if (eTechCityTrade != NO_TECH)
	{
		if (eTechCityTrade == eTech)
		{
			if (!bHasTech)
				return false;
		}
		else if (!isHasTech(eTechCityTrade))
			return false;
	}
	...
	{
		if (eTechObsolete == eTech)
		{
			if (bHasTech)
				return false;
		}
		else if (isHasTech(eTechObsolete))
			return false;
	}

	// TODO: Decide whether this check is necessary or not
	//       Notice that isBonusConnected doesn't look at this
	TechTypes eTechReveal = (TechTypes)kBonus.getTechReveal();
	if (eTechReveal != NO_TECH)
	{
		if (eTechReveal == eTech)
		{
			if (!bHasTech && !isForceRevealedBonus(eBonus))
				return false;
		}
		else if (!isHasTech(eTechReveal) && !isForceRevealedBonus(eBonus))
			return false;
	}
...
bool CvTeam::isBonusConnectedWithForceReveal(BonusTypes eBonus, bool bReveal) const
{
	FASSERT_BOUNDS(0, GC.getNumBonusInfos(), eBonus, "CvTeam::isBonusConnectedWithForceReveal");

	CvBonusInfo& kBonus = GC.getBonusInfo(eBonus);

	if (!isHasTech((TechTypes)kBonus.getTechCityTrade()))
		return false; // We don't have city trade yet

	TechTypes eTechObsolete = (TechTypes)kBonus.getTechObsolete();
	if (eTechObsolete != NO_TECH && isHasTech(eTechObsolete))
		return false; // It's already obsolete

	return isHasTech((TechTypes)kBonus.getTechReveal()) || bReveal;
}
// MOD - END - PlotGroup Bonus Efficiency
...
// MOD - START - Congestion
int CvTeam::getInsideCityCongestionThreshold() const
{
	return m_iInsideCityCongestionThreshold;
}

int CvTeam::getOutsideCityCongestionThreshold() const
{
	return m_iOutsideCityCongestionThreshold;
}

void CvTeam::updateInsideCityCongestionThreshold()
{
	int iOldValue = getInsideCityCongestionThreshold();
	int iNewValue = 0;

	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iNewValue = std::max(iNewValue, GET_PLAYER(*it).getInsideCityCongestionThreshold());
	}

	if (iNewValue != iOldValue)
	{
		m_iInsideCityCongestionThreshold = iNewValue;
		applyCongestionThresholdChange(true, iOldValue, iNewValue);
	}
}

void CvTeam::updateOutsideCityCongestionThreshold()
{
	int iOldValue = getOutsideCityCongestionThreshold();
	int iNewValue = 0;

	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iNewValue = std::max(iNewValue, GET_PLAYER(*it).getOutsideCityCongestionThreshold());
	}

	if (iNewValue != iOldValue)
	{
		m_iOutsideCityCongestionThreshold = iNewValue;
		applyCongestionThresholdChange(false, iOldValue, iNewValue);
	}
}

void CvTeam::applyCongestionThresholdChange(bool bCity, int iOldValue, int iNewValue)
{
	// Loop through every unit in the team and update its congestion if it has changed
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		const CvPlayer& kLoopPlayer = GET_PLAYER(*it);
		...
		for (CvUnit* pLoopUnit = kLoopPlayer.firstUnit(&iLoop); NULL != pLoopUnit; pLoopUnit = kLoopPlayer.nextUnit(&iLoop))
		{
			...
			const CvDomainInfo& kUnitDomain = GC.getDomainInfo(eDomain);
			int iNumCongestionTypes = kUnitDomain.getNumCongestionTypes(bCity);

			if (iNumCongestionTypes == 0)
			{
				continue;
			}
			...
			if (bCity != pPlot->isCity(true, getID()))
			{
				continue;
			}

			unsigned short iNumUnits = pPlot->getCombatUnitDomainCount(getID(), eDomain);

			for (int iI = 0; iI < iNumCongestionTypes; iI++)
			{
				CongestionTypes eLoopCongestion = (CongestionTypes)kUnitDomain.getCongestionTypes(bCity, iI);
				const CvCongestionInfo& kCongestion = GC.getCongestionInfo(eLoopCongestion);

				int iOldMinUnits = kCongestion.getMinUnits() + iOldValue;
				int iOldMaxUnits = kCongestion.getMaxUnits() == MAX_UNSIGNED_SHORT ? MAX_UNSIGNED_SHORT : kCongestion.getMaxUnits() + iOldValue;
				int iNewMinUnits = kCongestion.getMinUnits() + iNewValue;
				int iNewMaxUnits = kCongestion.getMaxUnits() == MAX_UNSIGNED_SHORT ? MAX_UNSIGNED_SHORT : kCongestion.getMaxUnits() + iNewValue;

				bool bWasApplicable = (iNumUnits >= iOldMinUnits) && (iNumUnits <= iOldMaxUnits);
				bool bWillBeApplicable = (iNumUnits >= iNewMinUnits) && (iNumUnits <= iNewMaxUnits);

				if (bWasApplicable != bWillBeApplicable)
				{
					if (bWillBeApplicable)
					{
						if (pLoopUnit->canAcquirePromotion(kCongestion.getPromotionType(), false, true))
						{
							pLoopUnit->setHasPromotion(kCongestion.getPromotionType(), true);
						}
					}
					else
					{
						pLoopUnit->setHasPromotion(kCongestion.getPromotionType(), false);
					}
				}
			}
		}
...
void CvTeam::setForceRevealedBonus(BonusTypes eBonus, bool bRevealed)
{
	...
	*/
	...
	{
		bool oldConnectedState = isBonusConnectedWithForceReveal(eBonus, !bRevealed);
		bool newConnectedState = isBonusConnectedWithForceReveal(eBonus, bRevealed);
		bool bConnectionChange = oldConnectedState != newConnectedState;

		for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); ++iI)
		{
			CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);

			// Note: updateYield and setLayoutDirty are handled in CvPlot::updateForceRevealedBonus now
			pLoopPlot->updateForceRevealedBonus(getID(), eBonus, bConnectionChange, newConnectedState);
		}
	}
}
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	for (int iRel = 0; iRel < kTech.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		for (int iMem = 0; iMem < kBuildingClass.getNumBuildingTypes(); iMem++)
		{
			...
			if (kLoopBuilding.getObsoleteTech() == eTech)
			{
				changeObsoleteBuildingCount(eLoopBuilding, iChange);
			}

			// MOD - START - Building Discontinuation
			if (kLoopBuilding.getDiscontinueTech() == eTech)
			{
				changeDiscontinuedBuildingCount(eLoopBuilding, iChange);
			}
			// MOD - END - Building Discontinuation

			if (kLoopBuilding.getSpecialBuildingType() != NO_SPECIALBUILDING)
			{
				const CvSpecialBuildingInfo& kLoopSpecialBuilding = GC.getSpecialBuildingInfo((SpecialBuildingTypes)kLoopBuilding.getSpecialBuildingType());

				if (kLoopSpecialBuilding.getObsoleteTech() == eTech)
				{
					changeObsoleteBuildingCount(eLoopBuilding, iChange);
				}

				// MOD - START - Building Discontinuation
				if (kLoopSpecialBuilding.getDiscontinueTech() == eTech)
				{
					changeDiscontinuedBuildingCount(eLoopBuilding, iChange);
				}
				// MOD - END - Building Discontinuation
	...
	{
		ImprovementTypes eLoopImprovement = (ImprovementTypes)kTech.getRelatedImprovements(iRel);
		const CvImprovementInfo& kLoopImprovement = GC.getImprovementInfo(eLoopImprovement);

		for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
		{
			changeImprovementYieldChange(eLoopImprovement, (YieldTypes)iYield, (kLoopImprovement.getTechYieldChanges(eTech, iYield) * iChange));
			// MOD - START - Advanced Yield and Commerce Effects
			changeImprovementIrrigatedYieldChange(eLoopImprovement, (YieldTypes)iYield, (kLoopImprovement.getTechIrrigatedYieldChanges(eTech, iYield) * iChange));
			// MOD - END - Advanced Yield and Commerce Effects
		}
	}
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (kLoopPlayer.getTeam() == getID())
		{
			...
			// MOD - START - Epidemics
			// Epidemic system by Mexico
			kLoopPlayer.changeGlobalEpidemicModifier(kTech.getGlobalEpidemicModifier() * iChange);
			// MOD - END - Epidemics
			...
			{
				kLoopPlayer.changeCommerceRateModifier(eCommerce, kTech.getCommerceModifier(eCommerce) * iChange);
				// MOD - START - Advanced Yield and Commerce Effects
				//kLoopPlayer.changeSpecialistExtraCommerce(eCommerce, kTech.getSpecialistExtraCommerce(eCommerce) * iChange);
				kLoopPlayer.changeAllSpecialistCommerceChange(eCommerce, kTech.getAllSpecialistCommerce(eCommerce) * iChange);
				// MOD - END - Advanced Yield and Commerce Effects
			}
			// K-Mod end
			// MOD - START - Congestion
			kLoopPlayer.changeInsideCityCongestionThreshold(kTech.getInsideCityCongestionThresholdChange() * iChange);
			kLoopPlayer.changeOutsideCityCongestionThreshold(kTech.getOutsideCityCongestionThresholdChange() * iChange);
			// MOD - END - Congestion
		}
	}

	// MOD - START - Relation Caching
	if (kTech.getNumRelatedBonuses() > 0)
	{
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
	...
	for (int iRel = 0; iRel < kTech.getNumRelatedBuilds(); iRel++)
	{
		BuildTypes eLoopBuild = (BuildTypes)kTech.getRelatedBuilds(iRel);
		const CvBuildInfo& kLoopBuild = GC.getBuildInfo(eLoopBuild);

		if (kLoopBuild.getTechPrereq() == eTech)
		{
			for (int iMem = 0; iMem < getNumLivingTeamMembers(); iMem++)
			{
				PlayerTypes eLoopPlayer = getLivingTeamMember(iMem);
				...
				kLoopPlayer.updateBuildCache(eLoopBuild);
...
void CvTeam::read(FDataStreamBase* pStream)
{
	...
	// MOD - START - Building Discontinuation
	pStream->Read(GC.getNumBuildingInfos(), m_paiDiscontinuedBuildingCount);
	// MOD - END - Building Discontinuation
	pStream->Read(GC.getNumTechInfos(), m_paiResearchProgress);
	pStream->Read(GC.getNumTechInfos(), m_paiTechCount);
	// MOD - START - Tech Path Caching
	pStream->Read(GC.getNumTechInfos(), m_paiTechPathLength);
	pStream->Read(GC.getNumTechInfos(), m_paiTechPathCost);
	// MOD - END - Tech Path Caching
	...
	// MOD - START - Tech Transfer
	pStream->Read(GC.getNumTechInfos(), m_paiTechTransferCount);
	// MOD - END - Tech Transfer
	...
	{
		pStream->Read(NUM_YIELD_TYPES, m_ppaaiImprovementIrrigatedYieldChange[iI]);
	}
	// MOD - END - Advanced Yield and Commerce Effects
	...
	for (int i = 0; i < iSize; ++i)
	{
		...
		m_aeLivingTeamMembers.push_back(ePlayer);
	}
	// MOD - END - Cached Team Members (poyuzhe)
}
...
void CvTeam::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(MAX_TEAMS, m_abHasSeen); // K-Mod
	...
	// MOD - START - Building Discontinuation
	pStream->Write(GC.getNumBuildingInfos(), m_paiDiscontinuedBuildingCount);
	// MOD - END - Building Discontinuation
	pStream->Write(GC.getNumTechInfos(), m_paiResearchProgress);
	pStream->Write(GC.getNumTechInfos(), m_paiTechCount);
	// MOD - START - Tech Path Caching
	pStream->Write(GC.getNumTechInfos(), m_paiTechPathLength);
	pStream->Write(GC.getNumTechInfos(), m_paiTechPathCost);
	// MOD - END - Tech Path Caching
	...
	// MOD - START - Tech Transfer
	pStream->Write(GC.getNumTechInfos(), m_paiTechTransferCount);
	// MOD - END - Tech Transfer
	...
	for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		pStream->Write(NUM_YIELD_TYPES, m_ppaaiImprovementIrrigatedYieldChange[iI]);
	}
	// MOD - END - Advanced Yield and Commerce Effects
	...
	// MOD - START - Cached Team Members (poyuzhe)
	pStream->Write(m_aeLivingTeamMembers.size());
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		pStream->Write(*it);
	}
...
// MOD - START - Cached Team Members (poyuzhe)
int CvTeam::getNumLivingTeamMembers() const
{
	return m_aeLivingTeamMembers.size();
}

PlayerTypes CvTeam::getLivingTeamMember(int iMember) const
{
	FASSERT_BOUNDS(0, getNumLivingTeamMembers(), iMember, "CvTeam::getLivingTeamMember");
	return m_aeLivingTeamMembers[iMember];
}

void CvTeam::setLivingTeamMember(PlayerTypes ePlayer, bool bNewValue)
{
	FASSERT_BOUNDS(0, MAX_PLAYERS, ePlayer, "CvTeam::setLivingTeamMember");

	bool bOldValue = (std::find(m_aeLivingTeamMembers.begin(), m_aeLivingTeamMembers.end(), ePlayer) != m_aeLivingTeamMembers.end());
	...
	if (bNewValue != bOldValue)
	{
		...
		{
			m_aeLivingTeamMembers.erase(find(m_aeLivingTeamMembers.begin(), m_aeLivingTeamMembers.end(), ePlayer));
		}

		// Always sort the player IDs in order to keep memory reads as sequential as possible
		std::sort(m_aeLivingTeamMembers.begin(), m_aeLivingTeamMembers.end());

		// MOD - START - Congestion
		updateInsideCityCongestionThreshold();
		updateOutsideCityCongestionThreshold();
		// MOD - END - Congestion
	}
}
// MOD - END - Cached Team Members (poyuzhe)
