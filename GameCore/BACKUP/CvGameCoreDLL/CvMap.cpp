...
void CvMap::init(CvMapInitData* pInitInfo/*=NULL*/)
{
	...
	// for forts pre-placed in scenarios
	rebuildFortIndex();
...
{
	m_forts.insertAtEnd(xy);
}

void CvMap::deleteFort(XYCoords xy)
{
	CLLNode<XYCoords>* pFortsNode = headFortsNode();

	while (pFortsNode != NULL)
	{
		if (pFortsNode->m_data.iX == xy.iX && pFortsNode->m_data.iY == xy.iY)
		{
			deleteFortsNode(pFortsNode);
			return;
		}
		else
		{
			pFortsNode = nextFortsNode(pFortsNode);
		}
	}

	FAssertMsg(false, "Attempted to remove fort plot, but it doesn't exist in the list");
}
...
CLLNode<XYCoords>* CvMap::deleteFortsNode(CLLNode<XYCoords>* pNode)
{
	...
	pPlotNode = m_forts.deleteNode(pNode);
...
{
	return m_forts.next(pNode);
}


int CvMap::getLengthForts() const
{
	return m_forts.getLength();
}
