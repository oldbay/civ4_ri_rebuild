			...
			for (int iY = -iMaxRange; iY <= iMaxRange; ++iY)
			{
				...
				if (NULL != pPlot)
				{
					if (pHeadUnit->plot()->canSeePlot(pPlot, pHeadUnit->getTeam(), iMaxRange - 1, NO_DIRECTION))
					{
						...
						{
							// MOD - START - Validate Sentry Domains
							if (pPlot->isValidDomainForLocation(*getHeadUnit()))
							// MOD - END - Validate Sentry Domains
							{
								return true;
							}
						}
...
bool CvSelectionGroup::canStartMission(int iMission, int iData1, int iData2, int iFlags, CvPlot* pPlot, bool bTestVisible, bool bUseCache)
{
	if (bUseCache)
	{
		if (m_bIsBusyCache)
	...
	return canDoMission(iMission, iData1, iData2, iFlags, pPlot, bTestVisible, false); // K-Mod. (original code merged into CvSelectionGroup::canDoMission
}
// MOD - END - Advanced Automations
...
void CvSelectionGroup::startMission()
{
	...
	if (!canStartMission(headMissionQueueNode()->m_data.eMissionType, headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2, headMissionQueueNode()->m_data.iFlags, plot()))
	// MOD - END - Advanced Automations
	{
		bDelete = true;
	}
		...
		case MISSION_SHADOW:
		{
			...
			if (headMissionQueueNode()->m_data.iFlags == MISSIONTARGET_PLOT)
			{
				...
				if (pShadowPlot != NULL)
				{
					...
					{
						CvUnit* pCandidateShadowUnit = ::getUnit(pShadowUnitNode->m_data);
						if (canShadowUnit(pCandidateShadowUnit))
						{
							pShadowUnit = pCandidateShadowUnit;
							iValidShadowUnits++;
					...
					if (iValidShadowUnits > 1)
					{
						// There is more than one valid unit on the plot so we'll create a popup for the user to choose the unit to follow
						CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_SELECT_SHADOW_UNIT, getID(), pShadowPlot->getX(), pShadowPlot->getY());
						if (pInfo)
						{
							gDLL->getInterfaceIFace()->addPopup(pInfo, getOwnerINLINE(), true);
						}
					}
					else if (pShadowUnit != NULL)
					{
						pushMission(MISSION_SHADOW, pShadowUnit->getOwnerINLINE(), pShadowUnit->getID(), MISSIONTARGET_UNIT, true);
					}
				}
			}
			break;
...
/************************************************************************************************/
				...
				case MISSION_WAIT_FOR_TECH:
					...
					break;
				case MISSION_SHADOW:
					if (headMissionQueueNode()->m_data.iFlags == MISSIONTARGET_UNIT)
					{
						CvUnit* pShadowUnit = ::getUnit(IDInfo((PlayerTypes)headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2));
						if (pLoopUnit->canShadowUnit(pShadowUnit))
						{
							pLoopUnit->setShadowUnit(pShadowUnit);
...
/************************************************************************************************/
		...
		case INTERFACEMODE_FENGAGE:
			...
			if (pLoopUnit->canFEngage(pLoopUnit->plot()))
			{
				return true;
			}
			break;
...
/************************************************************************************************/
		case INTERFACEMODE_SHADOW_UNIT:
			if (pLoopUnit != NULL)
			{
				if (pLoopUnit->canShadowPlot(pPlot))
				{
					return true;
				}
			}
...
bool CvSelectionGroup::isImmuneToDamageFromFeature(FeatureTypes eFeature) const
{
	CLLNode<IDInfo>* pUnitNode;
	CvUnit* pLoopUnit;
	...
	while (pUnitNode != NULL)
	{
		...
		if (!(pLoopUnit->isImmuneToDamageFromFeature(eFeature)) && !pLoopUnit->isCargo())
		{
			return false;
		}
	}
...
// MOD - START - Advanced Automations
bool CvSelectionGroup::canShadow() const
{
	if (getNumUnits() > 0)
	{
		for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = nextUnitNode(pUnitNode))
...
bool CvSelectionGroup::canShadowPlot(CvPlot* pShadowPlot) const
{
	if (getNumUnits() > 0)
	{
		for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = nextUnitNode(pUnitNode))
		{
			CvUnit* pLoopUnit = ::getUnit(pUnitNode->m_data);
			if (!pLoopUnit->canShadowPlot(pShadowPlot))
			{
				return false;
			}
		}
...
bool CvSelectionGroup::canShadowUnit(CvUnit* pShadowUnit) const
{
	if (getNumUnits() > 0)
	{
		for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = nextUnitNode(pUnitNode))
			...
			if (iFlags & (MOVE_THROUGH_ENEMY | MOVE_ATTACK_STACK | MOVE_DIRECT_ATTACK) || getDomainType() == DOMAIN_AIR || (final_path.GeneratePath(pDestPlot) && final_path.GetPathFirstPlot() == pDestPlot)) // K-Mod
			{
				...
				if (pBestAttackUnit)
				{
					...
					while (true)
					{
						...
						if (!isHuman())
						{
							if (iAttackOdds < GC.getSKIP_RANGE_ATTACK_MIN_BEST_ATTACK_ODDS())
							{
								CvUnit* pBestRangedUnit = AI_getBestGroupRangeAttacker(pDestPlot);

								bool bRangeStrike = false;
								while (pBestRangedUnit != NULL && pBestRangedUnit->rangeStrike(pDestPlot->getX_INLINE(), pDestPlot->getY_INLINE()))
								{
									bRangeStrike = true;
...
*/
							if ((!isHuman() || isAutomated()) && getNumUnits() > 1 && !(iFlags & MOVE_SINGLE_ATTACK)) // K-Mod
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
							// MOD - END - Advanced Automations
...
bool CvSelectionGroup::readyForMission()
{
	...
	return canDoMission(kData.eMissionType, kData.iData1, kData.iData2, kData.iFlags, plot(), false, bCheckMoves) || canAllMove();
	// MOD - END - Advanced Automations
...
bool CvSelectionGroup::canDoMission(int iMission, int iData1, int iData2, int iFlags, CvPlot* pPlot, bool bTestVisible, bool bCheckMoves)
// MOD - END - Advanced Automations
{
	if (!pPlot)
		pPlot = plot();
	...
	while (pUnitNode != NULL)
	{
		...
		// MOD - START - Inquisition
		case MISSION_REMOVE_RELIGION:
			if (pLoopUnit->canRemoveReligion(pPlot, bTestVisible))
				return true;
			break;
		// MOD - END - Inquisition
...
/************************************************************************************************/
		case MISSION_ESPIONAGE_SLEEP:
			if (pLoopUnit->canSleep(pPlot) && pLoopUnit->canEspionage(pPlot, true) && pLoopUnit->getFortifyTurns() != GC.getDefineINT("MAX_FORTIFY_TURNS"))
			{
				return true;
			}
			break;
		case MISSION_SHADOW:
			if (iFlags == MISSIONTARGET_PLOT)
			{
				// The mission is targeting a plot
				CvPlot* pShadowPlot = GC.getMapINLINE().plotINLINE(iData1, iData2);
				if (pLoopUnit->canShadowPlot(pShadowPlot))
				{
					return true;
				}
			}
			else if (iFlags == MISSIONTARGET_UNIT)
			{
				// The mission is targeting a unit
				CvUnit* pShadowUnit = ::getUnit(IDInfo((PlayerTypes)iData1, iData2));
				if (pLoopUnit->canShadowUnit(pShadowUnit))
				{
					return true;
				}
...
/************************************************************************************************/
		...
		// Dale - FE: Fighters START
		case MISSION_FENGAGE:
			if ((pLoopUnit->canFEngageAt(pPlot, iData1, iData2)) && (pLoopUnit->canMove()))
			{
				return true;
			}
			break;
