...
// MOD - END - Graphical Paging (45deg)
{
	...
	m_paBestHolyCityCandidate = NULL;
...
void CvGame::regenerateMap()
{
	...
	cycleSelectionGroups_delayed(1, false); // K-Mod
...
void CvGame::uninit()
{
	...
	// MOD - START - Holy City Migration
	SAFE_DELETE_ARRAY(m_paBestHolyCityCandidate);
	// MOD - END - Holy City Migration
...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	if (!bConstructorCall)
	{
		...
		// MOD - START - Holy City Migration
		FAssertMsg(m_paBestHolyCityCandidate==NULL, "about to leak memory, CvGame::m_paBestHolyCityCandidate");
		m_paBestHolyCityCandidate = new IDInfo[GC.getNumReligionInfos()];	
		// MOD - END - Holy City Migration
		for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
			...
			// MOD - START - Holy City Migration
			m_paBestHolyCityCandidate[iI].reset();
			// MOD - END - Holy City Migration
	...
	if (!bConstructorCall && getActivePlayer() == NO_PLAYER)
	{
		// End of game
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "updatePlayerContext");
	}
	...
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (pLoopPlot != pStartingPlot)
				{
							// MOD - START - Island Features
							if (!pLoopPlot->isWater())
							{
							// MOD - END - Island Features
								if (pLoopPlot->getBonusType() == NO_BONUS)
								{
									if (pLoopPlot->getFeatureType() == NO_FEATURE)
									{
...
// MOD - START - Graphical Paging (45deg)
void CvGame::updateGraphicalPaging()
{
	...
	if ( lookatPlot != NULL )
	{
		...
		// NOTE: Changed from BUG setting to graphic option by JDS 2015-08-13
		bool bPagingEnabled = GC.getGraphicOption(GRAPHICOPTION_GRAPHICAL_PAGING);
...
void CvGame::updateScore(bool bForce)
{
	...
	int aiPlayerScores[MAX_CIV_PLAYERS];
	int aiTeamScores[MAX_CIV_TEAMS];
	// MOD - END - Efficiency
	int iScore;
	int iBestScore;
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		// MOD - START - Efficiency
		aiPlayerScores[iI] = GET_PLAYER((PlayerTypes)iI).calculateScore(false);
		// MOD - END - Efficiency
	}
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		if (eTeam != NO_TEAM)
		{
			aiTeamScores[eTeam] += aiPlayerScores[iI];
		}
	}
	// MOD - END - Efficiency
...
void CvGame::cityPushOrder(CvCity* pCity, OrderTypes eOrder, int iData, bool bAlt, bool bShift, bool bCtrl) const
{
	if (pCity->getProduction() > 0)
	{
		...
		CvMessageControl::getInstance().sendPushOrder(pCity->getID(), eOrder, iData, bAlt, false, bShift ? -1 : 0); // K-Mod
	}
	else if ((eOrder == ORDER_TRAIN) && (pCity->getProductionUnit() == iData))
	{
		...
		CvMessageControl::getInstance().sendPushOrder(pCity->getID(), eOrder, iData, bAlt, false, bCtrl ? 0 : -1); // K-Mod
	}
	else
	{
		//CvMessageControl::getInstance().sendPushOrder(pCity->getID(), eOrder, iData, bAlt, bShift, bCtrl);
		CvMessageControl::getInstance().sendPushOrder(pCity->getID(), eOrder, iData, bAlt, !(bShift || bCtrl), bShift ? -1 : 0); // K-Mod
	}
}
...
EraTypes CvGame::getCurrentEra() const
{
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++) // K-Mod (dont' count the barbarians)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			// Ungomma: do not count tribal civs who are intended to lag behind in techs and eras
			CvCivilizationInfo& civInfo = GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)iI).getCivilizationType());
			bool bSpecialBackwardsCiv = !civInfo.isPlayable() && !civInfo.isDerivative();
			if (!bSpecialBackwardsCiv) {
...
/************************************************************************************************/
		...
		if (getVictory() != NO_VICTORY)
		{
			...
			{
				szBuffer = gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getReplayName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide()); // K-Mod
...
void CvGame::setHolyCity(ReligionTypes eIndex, CvCity* pNewValue, bool bAnnounce)
{
	...
	if (pOldValue != pNewValue)
	{
		...
		if (pOldValue != NULL)
		{
			...
			for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
			{
				if (GC.getBuildingInfo((BuildingTypes)iI).getHolyCity() == eIndex)
				{
					if (pOldValue->getNumBuilding((BuildingTypes)iI) > 0)
					{
						pOldValue->updateNumActiveBuilding((BuildingTypes)iI);
					}
				}
			}
			// MOD - END - Holy City Migration
		...
		if (getHolyCity(eIndex) != NULL)
		{
			...
			for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
			{
				if (GC.getBuildingInfo((BuildingTypes)iI).getHolyCity() == eIndex)
				{
					if (pHolyCity->getNumBuilding((BuildingTypes)iI) > 0)
					{
						pHolyCity->updateNumActiveBuilding((BuildingTypes)iI);
					}
				}
			}

			// TODO: Move this to a DEFINE
			GET_PLAYER(pHolyCity->getOwnerINLINE()).changeHolyCityInfluence(eIndex, 1000);
			// MOD - END - Holy City Migration
...
// MOD - START - Holy City Migration
int CvGame::getNumHolyCityCandidates(ReligionTypes eReligion, PlayerTypes eIgnorePlayer) const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (eLoopPlayer != eIgnorePlayer)
		{
			CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
			if (kLoopPlayer.isAlive() && kLoopPlayer.canAcquireHolyCity(eReligion))
			{
				CvCity* pLoopCandidate = kLoopPlayer.getHolyCityCandidate(eReligion);
				if (pLoopCandidate != NULL)
				{
					iNumCandidates++;
				}
			}
	...
	return iNumCandidates;
}


void CvGame::updateBestHolyCityCandidate(ReligionTypes eReligion)
{
	CvCity* pBestCandidate = NULL;
	int iBestCandidateValue = -1;
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (kLoopPlayer.isAlive() && kLoopPlayer.canAcquireHolyCity(eReligion))
		{
			...
			if (pLoopCandidate != NULL)
			{
				int iCandidateValue = pLoopCandidate->getHolyCityCandidateValue(eReligion);
				if (iCandidateValue > iBestCandidateValue)
				{
					iBestCandidateValue = iCandidateValue;
					pBestCandidate = pLoopCandidate;
				}
			}
		}
	}

	setBestHolyCityCandidate(eReligion, pBestCandidate);
}

CvCity* CvGame::getBestHolyCityCandidate(ReligionTypes eReligion) const
{
	FASSERT_BOUNDS(0, GC.getNumReligionInfos(), eReligion, "CvGame::updateBestHolyCityCandidate");
	return getCity(m_paBestHolyCityCandidate[eReligion]);
}

void CvGame::setBestHolyCityCandidate(ReligionTypes eReligion, CvCity* pNewValue)
{
	CvCity* pOldValue = getBestHolyCityCandidate(eReligion);

	if (pOldValue != pNewValue)
	{
		if (pNewValue != NULL)
		{
			m_paBestHolyCityCandidate[eReligion] = pNewValue->getIDInfo();
		}
		else
		{
			m_paBestHolyCityCandidate[eReligion].reset();
		}
	}
}

int CvGame::getHolyCityLossThreshold() const
{
	return 100;
}

int CvGame::getHolyCityAcquisitionThreshold() const
{
	return 500;
}
// MOD - START - Holy City Migration
...
void CvGame::doTurn()
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			// Open Borders Commerce is based on attitude, and we don't have events
			// about attitude changes to hook onto.  To keep the Open Borders Commerce
			// reasonably in sync with the player attitudes we update the state here
			// so that it at least gets updated at the start of every per turn.  The code is here,
			// rather than the player's doTurn, to make sure that there is no discrimination
			// due to turn order.  Note that new Open Borders trades will also trigger
			// a recalculation. - Josh Sjoding 2009-12-11
			kPlayer.updateOpenBordersCommerce();
			// MOD - END - Open Borders Commerce

			// MOD - START - Population Metrics
			kPlayer.updateMetricTurnValues();
...
// MOD - START - Better Nukes
void CvGame::doNuclearWinter()
{
	int m_iOldNuclearWinterLevel = m_iNuclearWinterLevel;
	if (m_iNuclearWinterLevel == 1)
	{
		m_iNuclearWinterLevel = 0;
	}	
	else
	{
		m_iNuclearWinterLevel = m_iNuclearWinterLevel * 100 / 105;
	}
	if (getNuclearWinterFoodYieldEffect(m_iOldNuclearWinterLevel) != getNuclearWinterFoodYieldEffect(m_iNuclearWinterLevel))
	{
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			PlayerTypes eLoopPlayer = (PlayerTypes)iI;
			...
			if (kLoopPlayer.isAlive())
			{
				CvWString szBuffer = gDLL->getText("TXT_KEY_NUCLEAR_WINTER_DECREASE");
				gDLL->getInterfaceIFace()->addHumanMessage(eLoopPlayer, false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, "AS2D_GLOBALWARMING", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
			}
		}
		for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
	...
	for (int iI = 0; iI < iGlobalWarmingRolls; iI++)
	{
		...
		if (getSorenRandNum(iLeftOdds, "Global Warming") < GC.getDefineINT("GLOBAL_WARMING_PROB"))
		{
			...
			if (pPlot != NULL)
			{
				...
				// TODO: Comment the code below back out if we re-enable this method
...
void CvGame::doHolyCity()
{
	...
	// MOD - START - Holy City Migration
	if (isOption(GAMEOPTION_HOLY_CITY_MIGRATION))
	{
		for (int iReligion = 0; iReligion < GC.getNumReligionInfos(); iReligion++)
		{
			...
			if (isReligionFounded(eReligion))
			{
				...
				// Update the holy city candidate at least once per turn
				updateBestHolyCityCandidate(eReligion);

				CvCity* pHolyCity = getHolyCity(eReligion);
				if (pHolyCity == NULL)
				{
					PlayerTypes eBestPlayer = NO_PLAYER;
					int iBestHolyCityInfluence = 0;
					for (int iI = 0; iI < MAX_PLAYERS; iI++)
					{
						PlayerTypes eLoopPlayer = (PlayerTypes)iI;
						CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
						if (kLoopPlayer.isAlive() && kLoopPlayer.canAcquireHolyCity(eReligion))
						{
							int iLoopInfluence = kLoopPlayer.getHolyCityInfluence(eReligion);
							if (iLoopInfluence >= getHolyCityAcquisitionThreshold() && iLoopInfluence > iBestHolyCityInfluence)
							{
								iBestHolyCityInfluence = iLoopInfluence;
					...
					if (eBestPlayer != NO_PLAYER)
					{
						...
						CvCity* pNewHolyCity = kBestPlayer.getHolyCityCandidate(eReligion);
						if (pNewHolyCity != NULL)
						{
							// Set the new holy city
							setHolyCity(eReligion, pNewHolyCity, false);

							// Announce the acquisition of the holy city
							for (int iI = 0; iI < MAX_PLAYERS; iI++)
							{
								PlayerTypes eLoopPlayer = (PlayerTypes)iI;
								CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
								if (kLoopPlayer.isAlive())
								{
									if (pNewHolyCity->isRevealed(kLoopPlayer.getTeam(), false))
									{
										CvWString szBuffer = gDLL->getText("TXT_KEY_RM_HOLYCITY_RESPAWN", pNewHolyCity->getNameKey(), kReligion.getTextKeyWide());
										gDLL->getInterfaceIFace()->addHumanMessage(eLoopPlayer, false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, kReligion.getSound(), MESSAGE_TYPE_MAJOR_EVENT, kReligion.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"), pNewHolyCity->getX_INLINE(), pNewHolyCity->getY_INLINE(), true, true);
									}
									else
									{
										CvWString szBuffer = gDLL->getText("TXT_KEY_RM_HOLYCITY_RESPAWN_UNKNOWN", kReligion.getTextKeyWide());
										gDLL->getInterfaceIFace()->addHumanMessage(eLoopPlayer, false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, kReligion.getSound(), MESSAGE_TYPE_MAJOR_EVENT, kReligion.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
									}
								}
							}
						}
				...
				{
					const CvPlayer& kOwner = GET_PLAYER(pHolyCity->getOwnerINLINE());

					if (!kOwner.canMaintainHolyCity(eReligion))
					{
						if (kOwner.getHolyCityInfluence(eReligion) < getHolyCityLossThreshold())
						{
							// Remove the holy city
							setHolyCity(eReligion, NULL, false);

							// Announce the loss of the holy city
							for (int iI = 0; iI < MAX_PLAYERS; iI++)
							{
								PlayerTypes eLoopPlayer = (PlayerTypes)iI;
								CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
								if (kLoopPlayer.isAlive())
								{
									if (pHolyCity->isRevealed(kLoopPlayer.getTeam(), false))
									{
										CvWString szBuffer = gDLL->getText("TXT_KEY_RM_HOLY_CITY_BANISHED", kReligion.getTextKeyWide(), pHolyCity->getNameKey());
										gDLL->getInterfaceIFace()->addHumanMessage(eLoopPlayer, false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, kReligion.getSound(), MESSAGE_TYPE_MAJOR_EVENT, kReligion.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"), pHolyCity->getX_INLINE(), pHolyCity->getY_INLINE(), true, true);
									}
									else
									{
										CvWString szBuffer = gDLL->getText("TXT_KEY_RM_HOLY_CITY_BANISHED_UNKNOWN", kReligion.getTextKeyWide());
										gDLL->getInterfaceIFace()->addHumanMessage(eLoopPlayer, false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, kReligion.getSound(), MESSAGE_TYPE_MAJOR_EVENT, kReligion.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
									}
								}
							}
				...
				if (GC.getLogging())
				{
					...
					{
						TCHAR szOut[1024];
						sprintf(szOut, "========%s Holy City Influence - START========\n", CvString(kReligion.getTextKeyWide()).GetCString());
						gDLL->messageControlLog(szOut);
						for (int iI = 0; iI < MAX_PLAYERS; iI++)
						{
							PlayerTypes eLoopPlayer = (PlayerTypes)iI;
							CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
							if (kLoopPlayer.isAlive() && kLoopPlayer.getHolyCityInfluence(eReligion) > 0)
							{
								sprintf(szOut, "Player %d: %d\n", eLoopPlayer, kLoopPlayer.getHolyCityInfluence(eReligion));
								gDLL->messageControlLog(szOut);
							}
						}
						sprintf(szOut, "========%s Holy City Influence - END==========\n", CvString(kReligion.getTextKeyWide()).GetCString());
						gDLL->messageControlLog(szOut);
					}
				}
				*/
...
void CvGame::createBarbarianUnits()
{
	...
	const bool bBarbCombatUnits = canCreateBarbarianCombatUnits();
	std::vector<UnitTypes> aUnits;

	if (bBarbCombatUnits)
	{
		getBarbarianCombatUnits(aUnits);
	}
	else if (canCreateAnimals())
	{
		getAnimals(aUnits);
	}
	else
	...
	for(CvArea* pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != NULL; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
	{
		if (bBarbCombatUnits)
		{
			createBarbarianCombatUnits(pLoopArea, aUnits);
		}
		else
		{
			createAnimals(pLoopArea, aUnits);
		}
	}

	if (bBarbCombatUnits)
	{
		// Remove one animal per turn after the units show up
...
bool CvGame::canCreateBarbarianCombatUnits() const
{
...
bool CvGame::canCreateAnimals() const
{
...
void CvGame::createBarbarianCombatUnits(CvArea* pArea, const std::vector<UnitTypes>& aBarbarianCombatUnits)
{
	...
	int iNeededBarbs = ((pArea->getNumUnownedTiles() / iDivisor) - pArea->getUnitsPerPlayer(BARBARIAN_PLAYER)); // XXX eventually need to measure how many barbs of eBarbUnitAI we have in this area...
	...
	if( pArea->isWater() )
	{
		...
		for( int iI = 0; iI < MAX_CIV_PLAYERS; iI++ )
		{
			if( GET_PLAYER((PlayerTypes)iI).isAlive() )
			{
				iPlayerSeaUnits += GET_PLAYER((PlayerTypes)iI).AI_totalWaterAreaUnitAIs(pArea,UNITAI_ATTACK_SEA);
				iPlayerSeaUnits += GET_PLAYER((PlayerTypes)iI).AI_totalWaterAreaUnitAIs(pArea,UNITAI_EXPLORE_SEA);
				iPlayerSeaUnits += GET_PLAYER((PlayerTypes)iI).AI_totalWaterAreaUnitAIs(pArea,UNITAI_ASSAULT_SEA);
				iPlayerSeaUnits += GET_PLAYER((PlayerTypes)iI).AI_totalWaterAreaUnitAIs(pArea,UNITAI_SETTLER_SEA);
			}
		}

		if( pArea->getUnitsPerPlayer(BARBARIAN_PLAYER) > (iPlayerSeaUnits/3 + 1) )
		{
			return;
		}
	}
	...
	{
		CvPlot* pPlot = GC.getMapINLINE().syncRandPlot((RANDPLOT_NOT_VISIBLE_TO_CIV | RANDPLOT_ADJACENT_LAND | RANDPLOT_PASSIBLE), pArea->getID(), GC.getDefineINT("MIN_BARBARIAN_STARTING_DISTANCE"));
		...
		for (std::vector<UnitTypes>::const_iterator it = aBarbarianCombatUnits.begin(); it != aBarbarianCombatUnits.end(); ++it)
		{
			UnitTypes eLoopUnit = *it;
			CvUnitInfo& kLoopUnit = GC.getUnitInfo(eLoopUnit);

			if (pArea->isWater() && kLoopUnit.getDomainType() != DOMAIN_SEA)
			{
				continue;
			}
			else if (!pArea->isWater() && kLoopUnit.getDomainType() != DOMAIN_LAND)
			{
				continue;
			}

			if (!kPlayer.canTrain(eLoopUnit))
			{
				continue;
			}
			...
			if (kLoopUnit.getUnitAIType(eBarbUnitAI))
			{
				iValue += 200;
			}
		...
		{
			kPlayer.initUnit(eBestUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), eBarbUnitAI);
		}
	}
}


void CvGame::createAnimals(CvArea* pArea, const std::vector<UnitTypes>& aAnimals)
{
	if (pArea->isWater())
	{
		return;
	...
	int iNeededAnimals = ((pArea->getNumUnownedTiles() / GC.getHandicapInfo(getHandicapType()).getUnownedTilesPerGameAnimal()) - pArea->getUnitsPerPlayer(BARBARIAN_PLAYER));
	...
	{
		CvPlot* pPlot = GC.getMapINLINE().syncRandPlot((RANDPLOT_NOT_VISIBLE_TO_CIV | RANDPLOT_PASSIBLE), pArea->getID(), GC.getDefineINT("MIN_ANIMAL_STARTING_DISTANCE"));
		...
		for (std::vector<UnitTypes>::const_iterator it = aAnimals.begin(); it != aAnimals.end(); ++it)
		{
			UnitTypes eLoopUnit = *it;
			CvUnitInfo& kLoopUnit = GC.getUnitInfo(eLoopUnit);

			if ((pPlot->getFeatureType() != NO_FEATURE) ? kLoopUnit.getFeatureNative(pPlot->getFeatureType()) : kLoopUnit.getTerrainNative(pPlot->getTerrainType()))
			{
...
void CvGame::getBarbarianCombatUnits(std::vector<UnitTypes>& aBarbarianCombatUnits) const
{
	CvPlayer& kPlayer = GET_PLAYER(BARBARIAN_PLAYER);
	...
	{
		UnitTypes eLoopUnit = (UnitTypes)kCivilization.getCivilizationUnits(iUnitClass);
		...
		if (kLoopUnit.getCombat() == 0)
			continue;
		
		if (kLoopUnit.isOnlyDefensive())
			continue;

		if (!kPlayer.canTrain(eLoopUnit))
			continue;

		CvTeam& kTeam = GET_TEAM(BARBARIAN_TEAM);

		// MOD - START - Advanced Unit Prerequisites (Bonus)
		bool bValid = true;
		for (int iPre = 0; iPre < kLoopUnit.getNumPrereqAndBonuses(); iPre++)
		{
			TechTypes ePrereqTech = (TechTypes)GC.getBonusInfo((BonusTypes)kLoopUnit.getPrereqAndBonuses(iPre)).getTechCityTrade();
		...
		{
			TechTypes eTech = (TechTypes)GC.getBonusInfo((BonusTypes)kLoopUnit.getPrereqOrBonuses(iPre)).getTechCityTrade();
			if (eTech != NO_TECH)
			{
				bRequires = true;
		...
		aBarbarianCombatUnits.push_back(eLoopUnit);
	}
}


void CvGame::getAnimals(std::vector<UnitTypes>& aAnimals) const
{
	CvPlayer& kPlayer = GET_PLAYER(BARBARIAN_PLAYER);
	CvCivilizationInfo& kCivilization = GC.getCivilizationInfo(kPlayer.getCivilizationType());
	...
	for (int iUnitClass = 0; iUnitClass < GC.getNumUnitClassInfos(); iUnitClass++)
	{
		...
		if (!kLoopUnit.getUnitAIType(UNITAI_ANIMAL))
			continue;

		aAnimals.push_back(eLoopUnit);
	}
}
// MOD - END - Efficiency
			...
			if (player.isTurnActive())
			{
						...
						else
						{
							if (GC.getLogging())
							{
								if (gDLL->getChtLvl() > 0)
								{
									{
										TCHAR szOut[1024];
										sprintf(szOut, "  Player %d is unable to start automoves (hasBusyUnit: %s, hasReadyUnit: %s)\n", player.getID(), player.hasBusyUnit() ? "true" : "false", player.hasReadyUnit(true) ? "true" : "false");
										gDLL->messageControlLog(szOut);
									}
									...
									{
										if (pLoopSelectionGroup->readyToMove(true) && !pLoopSelectionGroup->isAutomated())
										{
											...
											if (pUnitNode != NULL)
											{
												CvUnit* pLoopUnit = ::getUnit(pUnitNode->m_data);
												CvWString szString;
												getUnitAIString(szString, pLoopUnit->AI_getUnitAIType());

												TCHAR szOut[1024];
												sprintf(szOut, "  Player %d (%S) group %d lead by unit %d (%S / %S) claims that it is ready to move\n", player.getID(), player.getCivilizationDescription(0), pLoopSelectionGroup->getID(), pLoopUnit->getID(), pLoopUnit->getName(0).GetCString(), szString.GetCString());
												gDLL->messageControlLog(szOut);
											}
											else
											{
												TCHAR szOut[1024];
												sprintf(szOut, "  Player %d (%S) group %d claims that it is ready to move but it has no units\n", player.getID(), player.getCivilizationDescription(0), pLoopSelectionGroup->getID());
												gDLL->messageControlLog(szOut);
											}
											break;
										}
				...
				if (player.isAutoMoves())
				{
					...
					if (player.isHuman())
					{
						...
						*/
						//Units that are automated to follow need to move last
						std::vector<CvSelectionGroup*> shadowGroups;
						bool bBusy = false;
						...
						for (pLoopSelectionGroup = player.firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = player.nextSelectionGroup(&iLoop))
						{
							...
							{
								shadowGroups.push_back(pLoopSelectionGroup);
							}
						}

						if (!bBusy && shadowGroups.size() > 0)
						{
							for (std::vector<CvSelectionGroup*>::iterator it = shadowGroups.begin(); it != shadowGroups.end(); ++it)
							{
								if ((*it)->AI_update())
								{
									FAssert(player.hasBusyUnit());
...
void CvGame::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iInitialNumPlayers);
	pStream->Read(&m_iAheadOfTimeEra);	
	...
	for (iI=0;iI<GC.getNumReligionInfos();iI++)
	{
		pStream->Read((int*)&m_paBestHolyCityCandidate[iI].eOwner);
		pStream->Read(&m_paBestHolyCityCandidate[iI].iID);
	}
	// MOD - START - Holy City Migration
	...
	// MOD - START - Better Nukes
	pStream->Read(&m_iNuclearWinterLevel);
	// MOD - END - Better Nukes
}
...
void CvGame::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iInitialNumPlayers);
	pStream->Write(m_iAheadOfTimeEra);	
	...
	for (iI=0;iI<GC.getNumReligionInfos();iI++)
	{
		pStream->Write(m_paBestHolyCityCandidate[iI].eOwner);
		pStream->Write(m_paBestHolyCityCandidate[iI].iID);
	}
	// MOD - START - Holy City Migration
	...
	// MOD - START - Better Nukes
	pStream->Write(m_iNuclearWinterLevel);
	// MOD - END - Better Nukes
}
					...
					else if (GC.getVoteInfo(kData.eVote).isAssignCity())
					{
						for (int iPlayer1 = 0; iPlayer1 < MAX_CIV_PLAYERS; ++iPlayer1)
						{
							...
							for (CvCity* pLoopCity = kPlayer1.firstCity(&iLoop); NULL != pLoopCity; pLoopCity = kPlayer1.nextCity(&iLoop))
							{
								...
								if (eNewOwner != NO_PLAYER && GET_PLAYER(eNewOwner).isAlive())
								{
...
int CvGame::getInitialNumPlayers() const
{
//	FAssert(eVoteSource >= 0);
//	FAssert(eVoteSource < GC.getNumVoteSourceInfos());
	return m_iInitialNumPlayers;
}

void CvGame::setInitialNumPlayers(int iNewValue)
{
	FAssert(iNewValue >= 0);
	FAssert(iNewValue <= MAX_CIV_PLAYERS);
	m_iInitialNumPlayers = iNewValue;
}

int CvGame::getAheadOfTimeEra() const
{
	return m_iAheadOfTimeEra;
}

void CvGame::setAheadOfTimeEra( const int newAheadOfTimeEra )			
{
	FAssert(newAheadOfTimeEra >= 0);
	FAssert(newAheadOfTimeEra <= 11);
	m_iAheadOfTimeEra = newAheadOfTimeEra;
}

int CvGame::getForcedStyle() const
{
	return m_iForcedStyle;
}


void CvGame::setForcedStyle(int iNewValue)
{
	m_iForcedStyle = iNewValue;
}

// MOD - START - Better Nukes
int CvGame::getNuclearWinterLevel() const
{
	return m_iNuclearWinterLevel;
}

void CvGame::changeNuclearWinterLevel(int iNewValue)
{
	m_iNuclearWinterLevel += iNewValue;
}

int CvGame::getNuclearWinterUnhealthiness() const
{
	return m_iNuclearWinterLevel / 30;
}

int CvGame::getNuclearWinterFoodYieldEffect(int iNuclearWinterLevel) const
{
	return iNuclearWinterLevel / 100;
}
// MOD - END - Better Nukes
