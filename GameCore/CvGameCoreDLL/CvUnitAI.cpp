...
bool CvUnitAI::AI_update()
{
	...
	if (getGroup()->isAutomated() && isHuman()) // K-Mod. When the AI fills in for a human player, they should ignore automation
	{
...
bool CvUnitAI::AI_follow(bool bFirst)
{
	...
	//if (bFirst && AI_cityAttack(1, 70, 0, true)) -- K-Mod 1.36->1.37
...
void CvUnitAI::AI_upgrade()
{
	...
	*/
	...
	kCivInfo.getUpgradesForUnit(getUnitType(), aUpgradeUnits);
	for (std::vector<UnitTypes>::const_iterator it = aUpgradeUnits.begin(); it != aUpgradeUnits.end(); ++it)
	{
		int iValue = kPlayer.AI_unitValue(*it, eUnitAI, pArea);
		// use a random factor. less than 100, so that the upgrade must be better than the current unit.
		iValue *= 80 + GC.getGameINLINE().getSorenRandNum(21, "AI Upgrade");

		// (believe it or not, AI_unitValue is faster than canUpgrade.)
		if (iValue > iBestValue && canUpgrade(*it))
		{
			iBestValue = iValue;
			eBestUnit = *it;
		}
...
int CvUnitAI::AI_groupSecondVal()
{
	...
	else if (isRangeStrikeCapable() && isOnlyDefensive())
	{
		return airRange() * 100 + baseCombatStr();
	}
	else
	{
...
/************************************************************************************************/
	...
	if (GC.getGameINLINE().isOption(GAMEOPTION_PROTECT_VALUABLE_UNITS) && GC.getLFBUseCombatOdds())
	...
	return ((AI_getUnitAIType() == UNITAI_CITY_DEFENSE) ||
		      (AI_getUnitAIType() == UNITAI_CITY_COUNTER) ||
					(AI_getUnitAIType() == UNITAI_CITY_SPECIAL) ||
						// MOD - START - Fortification Units
						(AI_getUnitAIType() == UNITAI_FORTIFICATION) ||
						// MOD - END - Fortification Units
...
*/
		...
		// MOD - END - Protect Valuable Units / Lead From Behind
		{
			...
			iValue /= (10 + std::max(getSameTileHeal(), 0) + std::max(getAdjacentTileHeal(), 0));
...
void CvUnitAI::AI_workerMove()
{
    ...
    if (!(getGroup()->canDefend()))
	{
		...
		{
			if (m_pUnitInfo->getBaseHurry() > 0) // XXX if it's too dangerous outside to have slaves work improvements, use them to rush buildings
			{
...
//		}
	...
	if (m_pUnitInfo->getBaseHurry() > 0) // XXX before building routes everywhere with slaves check if there are buildings we can rush
	{
		if (AI_greatPersonMove())
		{
			return;
		...
		if (iStepDistToTarget <= 2)
		{
			...
			if (bTargetTooStrong)
			{
				...
				else
				{
					...
					// MOD - START - Ranged Strike AI
					if (canRangeStrikeAt(plot(), pTargetCity->plot()->getX_INLINE(), pTargetCity->plot()->getY_INLINE()))
					{
						getGroup()->pushMission(MISSION_RANGE_ATTACK, pTargetCity->plot()->getX_INLINE(), pTargetCity->plot()->getY_INLINE(), 0, false, false, MISSIONAI_ASSAULT, pTargetCity->plot());
						return;
					}
					// MOD - END - Ranged Strike AI
			...
			if (iStepDistToTarget == 1)
			{
				...
				/*
					...
					return;
				*/
				else if (iComparePostBombard >= iAttackRatio) // we're satisfied with our position already. But we still want to consider bombarding.
				{
					if (AI_bombardCity())
						return;
...
void CvUnitAI::AI_collateralMove()
{
	...
	// MOD - START - Non-Collateral Siege AI
	// Some siege equipment may have the ability to reduce city walls
	// but not to inflict collateral damage on troops in conventional
	// combat. This differs from Vanilla Civ4.
...
// MOD - START - Fortification Units
void CvUnitAI::AI_fortificationMove()
{
	AI_rangeAttackOrSkip();

	// Note: Enablie this and check it over if we ever have transportable fortifications
	/*
	if (AI_guardCityAirlift())
	{
		return;
...
*/
	{
		if (getDamage() > 0)
		{
			...
			if ((plot()->getFeatureType() == NO_FEATURE) || (GC.getFeatureInfo(plot()->getFeatureType()).getTurnDamage() == 0) || getGroup()->isImmuneToDamageFromFeature(plot()->getFeatureType()))
...
// MOD - START - Inquisition
void CvUnitAI::AI_inquisitorMove()
{
	PROFILE_FUNC();

	if (AI_removeReligion())
	{
		return;
	}
	...
	if (!isHuman() || (isAutomated() && GET_TEAM(getTeam()).getAtWarCount(true) == 0))
	{
		if (!isHuman())
		{
			if (AI_load(UNITAI_INQUISITOR_SEA, MISSIONAI_LOAD_SPECIAL, NO_UNITAI, -1, -1, -1, 0, MOVE_SAFE_TERRITORY))
			{
				return;
			}

			if (AI_load(UNITAI_INQUISITOR_SEA, MISSIONAI_LOAD_SPECIAL, NO_UNITAI, -1, -1, -1, 0, MOVE_NO_ENEMY_TERRITORY))
			{
				return;
			}
		}
...
void CvUnitAI::AI_prophetMove()
{
	...
	/* original bts code
	// MOD - START - Improved Construction AI
	//if (AI_construct(1))
	if (AI_construct())
	{
		return;
	}
	// MOD - END - Improved Construction AI
...
void CvUnitAI::AI_scientistMove()
{
	...
	// MOD - START - Improved Construction AI
	if ((GET_PLAYER(getOwnerINLINE()).getCurrentEra() <= (GC.getNumEraInfos() / 2)) || (GC.getGameINLINE().getSorenRandNum(5, "AI Scientist Construct") == 0))
	// MOD - END - Improved Construction AI
	{
		if (AI_construct())
		{
...
void CvUnitAI::AI_generalMove()
{
	...
	// MOD - START - Realism Great General Logic
	/*
	if (AI_construct(1))
	{
		return;
...
/************************************************************************************************/
	...
	*/
	// MOD - END - Realism Great General Logic

	// MOD - START - Realism Great General Logic

	// First of all, build doctrines and traditions if available
	if (AI_construct(MAX_INT, 1, 1))
	{
		return;
	}

	CvPlayerAI& kOwner = GET_PLAYER(getOwnerINLINE());
	int birthmark = AI_getBirthmark();

	bool bDagger = kOwner.AI_isDoStrategy(AI_STRATEGY_DAGGER);
	bool bCrush = kOwner.AI_isDoStrategy(AI_STRATEGY_CRUSH);

	// When in an offensive war or planning one, one third can go to city attack (plus one tenth all the time)
	if ((birthmark % 10 == 0) || ((bOffenseWar || bDagger || bCrush) && (birthmark % 3 == 0)))
	{
		aeUnitAITypes.clear();
		aeUnitAITypes.push_back(UNITAI_ATTACK_CITY);
		if (AI_lead(aeUnitAITypes))
	...
	// Determine whether our area needs a leader
	...
	// When in an offensive war or planning one, one third can go to general offense (plus one tenth all the time)
	if ((birthmark % 10 == 1) || ((bNeedLeader || bOffenseWar || bDagger || bCrush) && (birthmark % 3 == 1)))
	{
		aeUnitAITypes.clear();
		aeUnitAITypes.push_back(UNITAI_ATTACK);
		if (AI_lead(aeUnitAITypes))
	...
	bool bTurtle = kOwner.AI_isDoStrategy(AI_STRATEGY_TURTLE);
	bool bLastStand = kOwner.AI_isDoStrategy(AI_STRATEGY_LAST_STAND);
	bool bAlert2 = kOwner.AI_isDoStrategy(AI_STRATEGY_ALERT2);

	// When turtling or when under threat two thirds can go to city defense and counter
	if ((bTurtle || bLastStand || bAlert2) && (birthmark % 3 != 0))
	{
		aeUnitAITypes.clear();
	...
	// When planning attack one fifth can go to pillaging
	if (bNeedLeader && (bDagger || bCrush) && (birthmark % 5 == 0))
	{
		aeUnitAITypes.clear();
		aeUnitAITypes.push_back(UNITAI_PILLAGE);
		if (AI_lead(aeUnitAITypes))
		{
			return;
		}
	...
	// Randomly build more military academies
	if (GC.getGameINLINE().getSorenRandNum(3, "AI General Construct") == 0)
	{
		if (AI_construct())
		{
	...
	if ((bNeedLeader || bAlert1) && (birthmark % 2 == 1))
	{
		aeUnitAITypes.clear();
		aeUnitAITypes.push_back(UNITAI_ATTACK);
		aeUnitAITypes.push_back(UNITAI_COUNTER);
...
// TODO: Check for compliance with great people in Realism: Invictus
bool CvUnitAI::AI_greatPersonMove()
{
	...
	bool bCanWork = m_pUnitInfo->getWorkRate() > 0;
			...
			if (generatePath(pLoopCity->plot(), iMoveFlags, true, &iPathTurns))
			{
				...
				for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
				{
					...
					if (eBuilding != NO_BUILDING)
					{
						...
						else if (bCanHurry && pLoopCity->canConstruct(eBuilding) && (isWorldWonderClass((BuildingClassTypes)iI) || bCanWork))
						{
							...
							if (!bCanWork)
							{
								...
								if (iFraction > 10) // arbitary, and somewhat unneccessary.
								{
	...
	if (pBestTradePlot != NULL)
	{
		...
		iTradeValue /= std::max(1, kPlayer.AI_averageCommerceMultiplier(COMMERCE_GOLD));
...
//  Assert disabled as it would harmlessly trigger on slaves all the time 
...
void CvUnitAI::AI_spyMove()
{
		...
		if (plot()->isCity())
		{
			...
			if( !bTargetCity )
			{
				...
				else
				{
					...
					int iInterceptChance = getSpyInterceptPercent(plot()->getTeam(), false); // K-Mod
	...
	if (plot()->getTeam() == getTeam())
	{
		...
		if (!kOwner.AI_isDoStrategy(AI_STRATEGY_BIG_ESPIONAGE) && GET_TEAM(getTeam()).getAtWarCount(true) > 0 &&
			//GC.getGame().getSorenRandNum(100, "AI Spy pillage improvement") < (kOwner.AI_getStrategyRand(5) % 30)) -- K-Mod 1.36->1.37
...
/************************************************************************************************/
	...
	if (AI_improveLocalPlot(3, pCity))
	{
		return;		
	}
...
// MOD - START - Inquisition
void CvUnitAI::AI_inquisitorSeaMove()
{
	PROFILE_FUNC();
	const CvPlayerAI& kOwner = GET_PLAYER(getOwnerINLINE()); // K-Mod
	...
	if (getUnitAICargo(UNITAI_INQUISITOR) > 0)
	{
		if (AI_specialSeaTransportInquisitor())
		{
			return;
		}
	}
	...
	if (AI_pickup(UNITAI_INQUISITOR, true))
	{
		return;
	}
...
int CvUnitAI::AI_promotionValue(PromotionTypes ePromotion)
{
	...
	// TODO: Exclude a unit's current temporary benefits from these calculations
	...
	else if (AI_getUnitAIType() == UNITAI_CITY_SPECIAL || AI_getUnitAIType() == UNITAI_FORTIFICATION || AI_getUnitAIType() == UNITAI_CARRIER_AIR)
	// MOD - END - Fortification Units
	{
		iValue += (iTemp * 2);
	}
	...
	// TODO: Consider only the available increase of withdrawal, but
	// exclude the benefits of Temporary Promotions from this calculation.
	...
	if (iTemp != 0)
	{
		...
		iExtra = std::max(0, (m_pUnitInfo->getWithdrawalProbability() + (getExtraWithdrawal() * 4)));
...
/************************************************************************************************/
	...
	if ((AI_getUnitAIType() == UNITAI_CITY_DEFENSE) ||
		(AI_getUnitAIType() == UNITAI_CITY_COUNTER) ||
		// MOD - START - Fortification Units
		(AI_getUnitAIType() == UNITAI_FORTIFICATION && plot()->isCity()) ||
		// MOD - END - Fortification Units
		(AI_getUnitAIType() == UNITAI_CITY_SPECIAL))
	{
		if (iTemp > 0)
	...
	if (bRebels || !GC.getGameINLINE().isOption(GAMEOPTION_NO_BARBARIANS))
	{
		...
		{
			CvPlot* pPlot = plot();
			bool bWithinTerritory = pPlot != NULL && pPlot->getTeam() == kOwner.getTeam();

			// Scale threat by era
			iBarbarianThreatModifier = 100 / std::max(1, (int)eEra - 1);

			if (bRebels && bWithinTerritory)
			{
			...
			{
				if (bWithinTerritory && pPlot != NULL && pPlot->area()->isBorderObstacle(kOwner.getTeam()))
				{
	...
	iTemp = kPromotion.getExtraBarbarianMaxXPValue();
	if (iTemp != 0 && iBarbarianThreatModifier != 0)
	{
		const int iNumEvaluationLevels = 12;

		int iBarbarianMaxXpValue = GC.getDefineINT("BARBARIAN_MAX_XP_VALUE") + getExtraBarbarianMaxXPValue();
		int iModifier = kOwner.getLevelExperienceModifier();
		int iLevelsBeforeLimit = 0;
		int iLevelsAfterLimit = 0;

		for (int iLevelChange = 0; iLevelChange < iNumEvaluationLevels; iLevelChange++)
		{
			int iLevel = getLevel() + iLevelChange;
			int iExperienceNeeded = iLevel * iLevel + 1;

			if (iModifier != 0)
				iExperienceNeeded = (iExperienceNeeded * (100+iModifier) + 99) / 100;

			if (iExperienceNeeded <= iBarbarianMaxXpValue)
			{
				// We can attain this level before hitting the XP limit
				iLevelsBeforeLimit++;
			}
			else if (iExperienceNeeded <= iBarbarianMaxXpValue + iTemp)
			{
				// We can attain this level with this promotion
				iLevelsAfterLimit++;
			}
			else
			{
		...
		// Base promotion value per level
		iTemp = 6;

		// Double value for the same set of AI types as CombatPercent
		if ((AI_getUnitAIType() == UNITAI_ATTACK) ||
			(AI_getUnitAIType() == UNITAI_COUNTER) ||
			(AI_getUnitAIType() == UNITAI_CITY_COUNTER) ||
			(AI_getUnitAIType() == UNITAI_ATTACK_SEA) ||
		...
		// Scale by the levels to be gained and how far away from the cap we are right now
		iTemp *= iLevelsAfterLimit;
		iTemp /= std::max(1, iLevelsBeforeLimit);

		// Scale by the threat modifier
		iTemp *= iBarbarianThreatModifier;
		iTemp /= 100;
	...
	iTemp = kPromotion.getExtraBarbarianCombatModifier();
	if (iTemp != 0 && iBarbarianThreatModifier != 0)
	{
		if ((AI_getUnitAIType() == UNITAI_ATTACK) ||
			(AI_getUnitAIType() == UNITAI_COUNTER) ||
	...
	for (int iRel = 0; iRel < kPromotion.getNumRelatedDomains(); iRel++)
	{
		...
		iTemp = kPromotion.getDomainModifierPercent(eLoopDomain);
	...
	for (int iRel = 0; iRel < kPromotion.getNumRelatedTerrains(); iRel++)
	{
		...
		iTemp = kPromotion.getTerrainAttackPercent(eLoopTerrain);
		if (iTemp != 0)
		{
			...
			if ((AI_getUnitAIType() == UNITAI_ATTACK) ||
				(AI_getUnitAIType() == UNITAI_COUNTER))
			{
				if (plot()->getTerrainType() == eLoopTerrain)
				{
					iValue += (iTemp / 3);
				}
			// This is very crude, but only value terrain attack promo if standing on said terrain - then there is at least a realistic chance you'll get to use it in this terrain
				else
				{
					iValue += (iTemp / 64);
				}
		...
		iTemp = kPromotion.getTerrainDefensePercent(eLoopTerrain);
		if (iTemp > 0)
		{
			iExtra =  getExtraTerrainDefensePercent(eLoopTerrain);
			iTemp *= (100 + iExtra);
			iTemp /= 100;
			if ((AI_getUnitAIType() == UNITAI_CITY_DEFENSE) || (AI_getUnitAIType() == UNITAI_CITY_SPECIAL) || (AI_getUnitAIType() == UNITAI_FORTIFICATION))
			// Terrain defense promotion should only be valuable to units that are expected to stay on the tile with said terrain 
			{
				if (plot()->getTerrainType() == eLoopTerrain)
				{
					iValue += (iTemp);
	...
	for (int iRel = 0; iRel < kPromotion.getNumRelatedFeatures(); iRel++)
	{
		...
		iTemp = kPromotion.getFeatureAttackPercent(eLoopFeature);
		if (GC.getFeatureInfo(eLoopFeature).isTerrain(plot()->getTerrainType()))
		// If the feature is possible on the plot we're currently standing on - should be all coastal for water units (if we're standing on the ocean, we're oceangoing and thus less likely to be fighting on shore tiles/features anyway), most plots for forest except when we're in the desert/ice, should significantly limit the usefulness of jungle bonus (to grassland tiles only) and limit desert-only features to units that actually found themselves in the desert 
		{
			if (iTemp != 0)
			{
			...
			iTemp = kPromotion.getFeatureDefensePercent(eLoopFeature);
			if (iTemp > 0)
			{
				...
				if (!noDefensiveBonus())
				{
					if (AI_getUnitAIType() == UNITAI_COUNTER)
					{
						if (plot()->getFeatureType() == eLoopFeature)
						{
							iValue += (iTemp / 3);
						}
						else
	...
	for (int iRel = 0; iRel < kPromotion.getNumRelatedUnitCombats(); iRel++)
	{
		...
		iTemp = kPromotion.getUnitCombatModifierPercent(eLoopUnitCombat);
		if (iTemp == 0)
		{
			continue;
		}
		...
		iCombatWeight *= GET_PLAYER(getOwnerINLINE()).AI_getUnitCombatWeight(eLoopUnitCombat);
		iCombatWeight /= 100;		
...
bool CvUnitAI::AI_omniGroup(UnitAITypes eUnitAI, int iMaxGroup, int iMaxOwnUnitAI, bool bStackOfDoom, int iFlags, int iMaxPath, bool bMergeGroups, bool bSafeOnly, bool bIgnoreFaster, bool bIgnoreOwnUnitType, bool bBiggerOnly, int iMinUnitAI, bool bWithCargoOnly, bool bIgnoreBusyTransports)
{
	...
	//PromotionTypes eCrowdedPromotionType = getCrowdedPromotionType();
	//int iUncrowdedStackSize = getCrowdedStackSize() - 1;
	// MOD - END - Congestion
				...
				if (getDomainType() != DOMAIN_LAND || canMoveAllTerrain() || area() == pPlot->area())
				{
					if (AI_allowGroup(pLoopUnit, eUnitAI))
					{
						...
						if (true
							...
							// NOTE: Add a check for getUnitCombatType() != NO_UNITCOMBAT to prevent non-military units from being hampered
							// if ((iMaxGroup == -1) || ((pLoopGroup->getNumUnits() + GET_PLAYER(getOwnerINLINE()).AI_unitTargetMissionAIs(pLoopUnit, MISSIONAI_GROUP, getGroup())) <=  std::min(iUncrowdedStackSize,(iMaxGroup + ((bStackOfDoom) ? AI_stackOfDoomExtra() : 0)))))
							// MOD - END - Congestion
...
bool CvUnitAI::AI_guardCityBestDefender()
{
	...
	if (pCity != NULL)
	{
		if (pCity->getOwnerINLINE() == getOwnerINLINE())
		{
			...
			{
				// MOD - START - Ranged Strike AI
				AI_rangeAttackOrFortify(0, false, false, MISSIONAI_GUARD_CITY);
				// MOD - END - Ranged Strike AI
				return true;
			}
		}
...
bool CvUnitAI::AI_guardCityOnlyDefender()
{
	...
	if (pPlotCity && pPlotCity->getOwnerINLINE() == getOwnerINLINE())
	{
		...
		{
			// MOD - START - Ranged Strike AI
			AI_rangeAttackOrFortify(0, false, false, noDefensiveBonus() ? NO_MISSIONAI : MISSIONAI_GUARD_CITY);
			// MOD - END - Ranged Strike AI
			return true;
		}
	}
...
bool CvUnitAI::AI_guardCity(bool bLeave, bool bSearch, int iMaxPath, int iFlags)
{
	...
	// MOD - START - Immobile Defenders (Ungomma)
	// Barbarian forts *are* city defenders
	//FAssert(getDomainType() == DOMAIN_LAND);
	FAssert(getDomainType() == DOMAIN_LAND || getDomainType() == DOMAIN_IMMOBILE);
	// MOD - END - Immobile Defenders (Ungomma)
	FAssert(canDefend());
...
bool CvUnitAI::AI_guardFort(bool bSearch)
{
	...
	CLLNode<XYCoords>* pFortNode = GC.getMapINLINE().headFortsNode();

	while (pFortNode != NULL)
	{
		CvPlot* pLoopPlot = GC.getMapINLINE().plotSorenINLINE(pFortNode->m_data.iX, pFortNode->m_data.iY);
		...
		pFortNode = GC.getMapINLINE().nextFortsNode(pFortNode);
	}
...
bool CvUnitAI::AI_heal(int iDamagePercent, int iMaxPath)
{
	...
	if (plot()->getFeatureType() != NO_FEATURE)
	{
		...
		{
			// MOD - START - Feature Damage Immunity
			if (!getGroup()->isImmuneToDamageFromFeature(plot()->getFeatureType()))
			{
				//Pass through
...
// MOD - START - Inquisition
// Returns true if a mission was pushed...
bool CvUnitAI::AI_removeReligion()
{
	PROFILE_FUNC();

	CvCity* pLoopCity;
	CvPlot* pBestPlot;
	CvPlot* pBestRemovePlot;
	...
	iBestValue = 0;
	pBestPlot = NULL;
	pBestRemovePlot = NULL;

	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		CvPlayer& kLoopPlayer = GET_PLAYER((PlayerTypes)iI);
		if (kLoopPlayer.isAlive() && (canEnterTerritory(GET_PLAYER((PlayerTypes)iI).getTeam())) || (getTeam() == kLoopPlayer.getTeam()))
		{			
			for (pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kLoopPlayer.nextCity(&iLoop))
			{
				if (AI_plotValid(pLoopCity->plot()))
				{
					if (canRemoveReligion(pLoopCity->plot()))
					{
						if (!(pLoopCity->plot()->isVisibleEnemyUnit(getOwnerINLINE())))
						{
							if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopCity->plot(), MISSIONAI_REMOVE_RELIGION, getGroup()) == 0)
							{
								if (generatePath(pLoopCity->plot(), MOVE_NO_ENEMY_TERRITORY, true, &iPathTurns))
								{
									...
									if (iValue > iBestValue)
									{
										iBestValue = iValue;
										pBestPlot = getPathEndTurnPlot();
										pBestRemovePlot = pLoopCity->plot();
									}
								}
							}
						}
	...
	if ((pBestPlot != NULL) && (pBestRemovePlot != NULL))
	{
		if (atPlot(pBestRemovePlot))
		{
			getGroup()->pushMission(MISSION_REMOVE_RELIGION, eReligion);
			return true;
		}
		else
		{
			FAssert(!atPlot(pBestPlot));
			getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), MOVE_NO_ENEMY_TERRITORY, false, false, MISSIONAI_REMOVE_RELIGION, pBestRemovePlot);
			return true;
		}
	}
...
bool CvUnitAI::AI_lead(std::vector<UnitAITypes>& aeUnitAITypes)
{
	...
	// MOD - START - Realism Great General Logic
	// The caller will be the judge of whether joining is warranted
	/*
	bool bNeedLeader = false;
	for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
...
bool CvUnitAI::AI_construct(int iMaxCount, int iMaxSingleBuildingCount, int iThreshold)
{
	...
	/*
	CvWString szBuffer;
	szBuffer.Format(L"%s %s considering construction", GET_PLAYER(getOwnerINLINE()).getReplayName(), getName().GetCString());
	...
	*/
	...
	// Create a list of buildings that we can construct
	std::vector<BuildingTypes> aeBuildingTypes;

	// MOD - START - Relation Caching
	for (int iRel = 0; iRel < m_pUnitInfo->getNumRelatedBuildingClasses(); iRel++)
	{
		BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)m_pUnitInfo->getRelatedBuildingClasses(iRel);
		...
		{
			if ((m_pUnitInfo->getForceBuildings(eLoopBuilding)) || (m_pUnitInfo->getBuildings(eLoopBuilding)))
			{
				if (kOwner.getBuildingClassCount(eLoopBuildingClass) < iMaxSingleBuildingCount)
				{
					aeBuildingTypes.push_back(eLoopBuilding);
				}
			}
		}
	}
	...
	if (aeBuildingTypes.size() == 0)
	{
		// Nothing to construct
		// MOD - START - Debugging
		/*
		szBuffer.Format(L"%s has nothing to construct", GET_PLAYER(getOwnerINLINE()).getReplayName());
		gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NEWTURN", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
		*/
		// MOD - END - Debugging
		return false;
			...
			if (!(pLoopCity->plot()->isVisibleEnemyUnit(this)))
			{
				...
				// above line disabled by K-Mod, because there are different types of buildings to construct...
				{
					...
					*/
					...
					for (uint iJ = 0; iJ < aeBuildingTypes.size(); iJ++)
					{
						BuildingTypes eBuilding = aeBuildingTypes[iJ];

						// Note: Because the building list has already been filtered by
						//       iMaxSingleBuildingCount, this logic could come out a little
						//       differently than the original; but iMaxCount is never used
						//       with iMaxSingleBuildingCount.
						//
						//       iMaxCount is stupid anyway because it doesn't even check
						//       all cities, just the ones in this area that we can get
						//       to safely
						...
						if (canConstruct(pLoopCity->plot(), eBuilding) && generatePath(pLoopCity->plot(), MOVE_NO_ENEMY_TERRITORY, true))
						{
							...
							// MOD - START - Building Construction Flavor
							/*
							if ((iValue > iThreshold) && (iValue > iBestValue))
							{
								iBestValue = iValue;
							...
							*/
							...
							{
								iValue *= (GC.getGameINLINE().getSorenRandNum(25, "AI Construct Building Value") + 100);
								iValue /= 100;
							...
							// MOD - END - Building Construction Flavor
						}
					}
					// MOD - END - Efficiency
				}
	...
	if ((pBestPlot != NULL) && (pBestConstructPlot != NULL) && (eBestBuilding != NO_BUILDING))
	{
		if (atPlot(pBestConstructPlot))
		{
			// MOD - START - Debugging
			/*
			szBuffer.Format(L"%s constructing %s", GET_PLAYER(getOwnerINLINE()).getReplayName(), GC.getBuildingInfo(eBestBuilding).getTextKeyWide());
			gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NEWTURN", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			*/
			// MOD - END - Debugging
			getGroup()->pushMission(MISSION_CONSTRUCT, eBestBuilding);
		...
		else
		{
			// MOD - START - Debugging
			/*
			szBuffer.Format(L"%s moving to construct %s", GET_PLAYER(getOwnerINLINE()).getReplayName(), GC.getBuildingInfo(eBestBuilding).getTextKeyWide());
			gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NEWTURN", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			*/
			// MOD - END - Debugging
			FAssert(!atPlot(pBestPlot));
...
/************************************************************************************************/
	...
	// MOD - START - Debugging
	/*
	szBuffer.Format(L"%s couldn't find a suitable building to build", GET_PLAYER(getOwnerINLINE()).getReplayName());
	gDLL->getInterfaceIFace()->addMessage(GC.getGameINLINE().getActivePlayer(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NEWTURN", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
	*/
	// MOD - END - Debugging
	return false;
					...
					if (AI_potentialEnemy(GET_PLAYER((PlayerTypes)iI).getTeam(), pLoopCity->plot()))
					{
						if (kOwner.AI_deduceCitySite(pLoopCity))
						{
							...
							if (iPathTurns < iMaxPathTurns)
							{
								...
								{
									if (GET_PLAYER(pLoopCity->getOwnerINLINE()).isBarbarian())
									{
										iEnemyDefence = kOwner.AI_localDefenceStrength(pLoopCity->plot(), NO_TEAM, DOMAIN_LAND, true, 0); //Barbarians won't shift around their city defenses
									}	
									else 
									{
...
// MOD - START - Ranged Strike AI
CvPlot* CvUnitAI::AI_rangeStrikeTargetPlot(int iSearchRange) const
{
	int iBestValue = 0;
	CvPlot* pBestPlot = NULL;
...
// Returns true if a mission was pushed...
// MOD - START - Ranged Strike AI
bool CvUnitAI::AI_rangeAttack(int iFlags, bool bAppend, bool bManual, MissionAITypes eMissionAI, CvPlot* pMissionAIPlot, CvUnit* pMissionAIUnit)
{
	FAssert(canMove());
	...
	CvPlot* pBestPlot = AI_rangeStrikeTargetPlot(airRange());
	if (pBestPlot != NULL)
	{
		FAssert(!atPlot(pBestPlot));
		getGroup()->pushMission(MISSION_RANGE_ATTACK, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), iFlags, bAppend, bManual, eMissionAI, pMissionAIPlot, pMissionAIUnit);
		return true;
	}
...
bool CvUnitAI::AI_rangeAttackCity()
{
	...
	if (!canRangeStrikeAt(plot(), pTargetCityPlot->getX_INLINE(), pTargetCityPlot->getY_INLINE()))
		return false;

	int iAttackOdds = getGroup()->AI_attackOdds(pTargetCityPlot, true);
	int iStackComparison = getGroup()->AI_compareStacks(pTargetCityPlot, true);

	if (iAttackOdds > GC.getSKIP_RANGE_ATTACK_MIN_BEST_ATTACK_ODDS() && iStackComparison >= GC.getSKIP_RANGE_ATTACK_MIN_STACK_RATIO())
	{
	...
	getGroup()->pushMission(MISSION_RANGE_ATTACK, pTargetCityPlot->getX_INLINE(), pTargetCityPlot->getY_INLINE(), 0, false, false, MISSIONAI_ASSAULT, pTargetCityPlot);

	return true;
}

bool CvUnitAI::AI_rangeAttackOrSkip(int iFlags, bool bAppend, bool bManual, MissionAITypes eMissionAI, CvPlot* pMissionAIPlot, CvUnit* pMissionAIUnit)
{
...
bool CvUnitAI::AI_rangeAttackOrFortify(int iFlags, bool bAppend, bool bManual, MissionAITypes eMissionAI, CvPlot* pMissionAIPlot, CvUnit* pMissionAIUnit)
{
	if (AI_rangeAttack(iFlags, bAppend, bManual, eMissionAI, pMissionAIPlot, pMissionAIUnit))
	{
		return true;
...
bool CvUnitAI::AI_assaultSeaTransport(bool bAttackBarbs, bool bLocal)
{
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (iEnemyDefenders > 0)
		{
			...
			if (pCity == NULL)
				if (iAmphibiousAttackStrength != 0)
...
bool CvUnitAI::AI_assaultSeaReinforce(bool bAttackBarbs)
{
	...
	plot()->waterAreas(aWaterAreas, iNumWaterAreas, true);
	// MOD - END - Multiple City Water Areas
	...
	for (int iDX = -(iRange); iDX <= iRange; iDX++)
	{
		for (int iDY = -(iRange); iDY <= iRange; iDY++)
		{
			...
			if (pLoopPlot && isEnemy(pLoopPlot->getTeam(), pLoopPlot))
			{
				...
				bool bSameWaterArea = bCanMoveAllTerrain;
				if (!bSameWaterArea)
				{
					for (int iJ = 0; iJ < iNumWaterAreas; iJ++)
					{
						if (pLoopPlot->isAdjacentToArea(aWaterAreas[iJ]))
						{
							bSameWaterArea = true;
							break;
						}
	...
	for (CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
	{
		...
		if (!bSameWaterArea)
		{
			int iNumCityWaterAreas;
			CvArea* aCityWaterAreas[MAX_CITY_WATER_AREAS];
			pLoopCity->waterAreas(aCityWaterAreas, iNumCityWaterAreas, true);
			for (int iI = 0; iI < iNumCityWaterAreas; iI++)
			{
				for (int iJ = 0; iJ < iNumWaterAreas; iJ++)
				{
					if (aCityWaterAreas[iI] == aWaterAreas[iJ])
					{
						bSameWaterArea = true;
						break;
					}
...
// MOD - START - Inquisition
// Returns true if a mission was pushed...
bool CvUnitAI::AI_specialSeaTransportInquisitor()
{
	PROFILE_FUNC();

	CLLNode<IDInfo>* pUnitNode;
	CvCity* pCity;
	CvUnit* pInquisitorUnit;
	CvUnit* pLoopUnit;
	CvPlot* pLoopPlot;
	CvPlot* pPlot;
	CvPlot* pBestPlot;
	...
	FAssert(getCargo() > 0);
	FAssert(getUnitAICargo(UNITAI_INQUISITOR) > 0);
	...
	while (pUnitNode != NULL)
	{
		...
		if (pLoopUnit->getTransportUnit() == this)
		{
			if (pLoopUnit->AI_getUnitAIType() == UNITAI_INQUISITOR)
			{
				pInquisitorUnit = pLoopUnit;
				break;
			}
		}
	}

	if (pInquisitorUnit == NULL)
	{
		return false;
	}
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot->isCoastalLand())
		{
			...
			{
				iValue = 0;
				if (pInquisitorUnit->canRemoveReligion(pLoopPlot))
				{
					iValue += 3;
				}
				...
				if (iValue > 0)
				{
					...
					{
						if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_REMOVE_RELIGION, getGroup()) == 0)
						{
							if (generatePath(pLoopPlot, 0, true, &iPathTurns))
							{
								iValue *= pCity->getPopulation();
	...
	if ((pBestPlot != NULL) && (pBestRemovePlot != NULL))
	{
		...
		if ((pBestPlot == pBestRemovePlot) || (stepDistance(pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), pBestRemovePlot->getX_INLINE(), pBestRemovePlot->getY_INLINE()) == 1))
		{
			if (atPlot(pBestRemovePlot))
			{
				unloadAll(); // XXX is this dangerous (not pushing a mission...) XXX air units?
			...
			{
				getGroup()->pushMission(MISSION_MOVE_TO, pBestRemovePlot->getX_INLINE(), pBestRemovePlot->getY_INLINE(), 0, false, false, MISSIONAI_REMOVE_RELIGION, pBestRemovePlot);
				return true;
			}
		}
		else
		{
			FAssert(!atPlot(pBestPlot));
			getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), 0, false, false, MISSIONAI_REMOVE_RELIGION, pBestRemovePlot);
			return true;
		}
	}
...
bool CvUnitAI::AI_improveLocalPlot(int iRange, CvCity* pIgnoreCity)
{
	...
	for (iX = -iRange; iX <= iRange; iX++)
	{
		for (iY = -iRange; iY <= iRange; iY++)
		{
			...
			if (pCity == NULL || pCity->getOwnerINLINE() != getOwnerINLINE())
  			...
  			if (pLoopPlot->getFeatureType() != NO_FEATURE && pLoopPlot->isWater())
				{
					if (GC.getFeatureInfo(pLoopPlot->getFeatureType()).getGrowthProbability() > 0)
					{
						FAssertMsg(pLoopPlot->isWater(), "Water tile evaluated");
						if (canBuild(pLoopPlot, (BuildTypes)GC.getInfoTypeForString("BUILD_BREAK_ICE")))
							{
							FAssertMsg(canBuild(pLoopPlot, (BuildTypes)GC.getInfoTypeForString("BUILD_BREAK_ICE")), "Water tile evaluated, can build!");	
							eBestBuild = (BuildTypes)GC.getInfoTypeForString("BUILD_BREAK_ICE");
							getGroup()->pushMission(MISSION_MOVE_TO, pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), 0, false, false, MISSIONAI_BUILD, pLoopPlot);
							getGroup()->pushMission(MISSION_BUILD, eBestBuild, -1, 0, true, false, MISSIONAI_BUILD, pLoopPlot); 
							continue;
							}
					}
				}				
			// Means you're an icebreaker and you just found your life's purpose  
	...
	if (pBestPlot != NULL)
	{
		...
		if (NULL != pBestPlot->getWorkingCity())
		{
			...
			{
				pBestPlot->getWorkingCity()->AI_changeSeaWorkersHave(+1);

				if (plot()->getWorkingCity() != NULL)
				{
					plot()->getWorkingCity()->AI_changeSeaWorkersHave(-1);
				}
			}
			else
			{
...
/************************************************************************************************/
			...
			if (getDomainType() == DOMAIN_SEA)
			{
				iWorkersNeeded = pLoopCity->AI_getSeaWorkersNeeded();
				iWorkersHave = pLoopCity->AI_getSeaWorkersHave();
			}
			else
			{
				iWorkersNeeded = pLoopCity->AI_getWorkersNeeded();
				iWorkersHave = pLoopCity->AI_getWorkersHave();
			}
			// MOD - END - Sea Improvements
						...
						if ((eBestTempBuild != NO_BUILD) || (bCanRoute && !bIsConnected))
						{
							...
							if (generatePath(pLoopPlot, 0, true, &iPathTurns))
							{
								...
								if (bDoImprove)
								{
									...
									iValue += 5 * pLoopPlot->calculateImprovementYieldChange(eImprovement, YIELD_FOOD, getOwnerINLINE(), false, false, pWorkingCity);
		...
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (!pLoopPlot->isCity() && pLoopPlot->isOwned() && pLoopPlot != plot())
				{
					...
					{
						iValue = 0;
						iValue += GC.getGameINLINE().getSorenRandNum(3, "AI Air Bomb");
...
bool CvUnitAI::AI_nuke()
{
	...
	*/
	...
	int iWarRating = kTeam.AI_getWarSuccessRating();
	bool bDanger = kOwner.AI_getAnyPlotDanger(plot(), 2);
	bool bAggresive = kOwner.AI_isDoStrategy(AI_STRATEGY_CRUSH | AI_STRATEGY_DAGGER | AI_STRATEGY_FINAL_WAR) || 
				      kOwner.AI_isDoVictoryStrategy(AI_VICTORY_CONQUEST4 | AI_VICTORY_DOMINATION4);
	int iNuclearWinterLevel = GC.getGameINLINE().getNuclearWinterLevel();

	int iBaseWeight = 10;
	iBaseWeight += GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI) ? 20 : 0;
	iBaseWeight += bAggresive ? 20 : 0;
	iBaseWeight += std::max(0, -iWarRating);
	iBaseWeight -= std::max(0, iWarRating - 40); // Prevent complete desctruction

	int iBestValue = std::max(0, 8 * getUnitInfo().getProductionCost());
	iBestValue += bDanger || bAggresive ? 0 : 1000;
	iBestValue += iNuclearWinterLevel * 100;
	iBestValue *= 120 + std::max(-100, iWarRating * 3);
	iBestValue /= 120;

#ifdef BETTER_NUKES_LOGS
	logBBAI("      ======================================");
	logBBAI("      Finding suitable target for nuclear strike...");
	logBBAI("      iWarRating = %d, bDanger = %d, bAggresive = %d, iNuclearWinterLevel = %d, iBaseWeight = %d, iBestValue = %d",
				   iWarRating, (int)bDanger, (int)bAggresive, iNuclearWinterLevel, iBaseWeight, iBestValue);
	logBBAI("      ======================================");
#endif
	...
	{
#ifdef BETTER_NUKES_LOGS
		logBBAI("      Nuclear target accepted! iBestValue = %d, X = %d, Y = %d", iBestValue, pBestTarget->getX_INLINE(), pBestTarget->getY_INLINE());
#endif
	...
	{
#ifdef BETTER_NUKES_LOGS
		logBBAI("      No suitable targets for nuclear strike! iBestValue = %d", iBestValue);
#endif
	}
...
bool CvUnitAI::AI_nukeRange(int iRange)
{
	...
	int iThresholdValue = std::max(0, 10 * getUnitInfo().getProductionCost());
	iThresholdValue += bDanger || bAggresive ? 0 : 1000;
	iThresholdValue += iNuclearWinterLevel * 100;
	iThresholdValue *= 120 + std::max(-100, iWarRating * 3);
	iThresholdValue /= 120;

#ifdef BETTER_NUKES_LOGS
	logBBAI("      ======================================");
	logBBAI("      Finding suitable target for tactical nuclear strike...");
	logBBAI("      iWarRating = %d, bDanger = %d, bAggresive = %d, iNuclearWinterLevel = %d, iThresholdValue = %d",
				   iWarRating, (int)bDanger, (int)bAggresive, iNuclearWinterLevel, iThresholdValue);
	logBBAI("      ======================================");
#endif
	// MOD - END - Better Nukes
	...
	{
#ifdef BETTER_NUKES_LOGS
		logBBAI("      Tactical nuclear target accepted! iNukeValue = %d, X = %d, Y = %d", iNukeValue, pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE());
#endif
	...
	{
#ifdef BETTER_NUKES_LOGS
		logBBAI("      No suitable targets for tactical nuclear strike! iBestValue = %d", iNukeValue);
#endif
	}
...
int CvUnitAI::AI_pillageValue(CvPlot* pPlot, int iBonusValueThreshold)
{
	...
	if (eImprovement != NO_IMPROVEMENT)
	{
		...
		*/
		...
		if (pWorkingCity != NULL)
		{
			iValue += (pPlot->calculateImprovementYieldChange(eImprovement, YIELD_FOOD, pPlot->getOwnerINLINE(), false, false, pWorkingCity) * 5);
			iValue += (pPlot->calculateImprovementYieldChange(eImprovement, YIELD_PRODUCTION, pPlot->getOwnerINLINE(), false, false, pWorkingCity) * 4);
			iValue += (pPlot->calculateImprovementYieldChange(eImprovement, YIELD_COMMERCE, pPlot->getOwnerINLINE(), false, false, pWorkingCity) * 3);
		}
		// MOD - END - Advanced Yield and Commerce Effects
		...
		for (int iY = -iSearchRange; iY <= iSearchRange; iY++)
		{
			...
			*/
			...
			if (bValid)
			{
				...
				bool bAggresive = kOwner.AI_isDoStrategy(AI_STRATEGY_CRUSH | AI_STRATEGY_DAGGER | AI_STRATEGY_FINAL_WAR) ||
								  kOwner.AI_isDoVictoryStrategy(AI_VICTORY_CONQUEST4 | AI_VICTORY_DOMINATION4);
				int iWarRatingThreshold = bAggresive ? -25 : -50;
				int iWarRating = GET_TEAM(kOwner.getTeam()).AI_getWarSuccessRating();
				int iWarRatingScaled = std::min(25, std::max(0, iWarRating - iWarRatingThreshold));
				bool bPreventStrike = false;

				// Sometimes AI nukes its own territory with a direct neighbourhood of enemy city, 
				// as long as they didn't strike us first we try to not do it too
				for (int jX = -nukeRange(); !bPreventStrike && jX <= nukeRange(); jX++)
				{
					for (int jY = -nukeRange(); !bPreventStrike && jY <= nukeRange(); jY++)
					{
						CvPlot* pLoopPlot = plotXY(pLoopTarget, jX, jY);
						if (!pLoopPlot)
							continue;
						...
						{
							CvPlayerAI& kLoopPlayer = GET_PLAYER(pLoopPlot->getOwnerINLINE());
							if (kLoopPlayer.getID() != kOwner.getID())
							{
								int iNuclearWeaponsUsed =
									kOwner.AI_getMemoryCount(kLoopPlayer.getID(), MEMORY_NUKED_US) +
									kOwner.AI_getMemoryCount(kLoopPlayer.getID(), MEMORY_NUKED_FRIEND) +
									kLoopPlayer.AI_getMemoryCount(kOwner.getID(), MEMORY_NUKED_US) +
									kLoopPlayer.AI_getMemoryCount(kOwner.getID(), MEMORY_NUKED_FRIEND);

								if (iNuclearWeaponsUsed == 0 && kLoopPlayer.getNukesUsed() < 8)
								{
									bPreventStrike = true;
									break;
								}
						...
						while (pUnitNode)
						{
							...
							CvPlayerAI& kUnitOwner = GET_PLAYER(pLoopUnit->getOwnerINLINE());
							if (kUnitOwner.getID() != kOwner.getID())
							{
								int iNuclearWeaponsUsed =
									kOwner.AI_getMemoryCount(kUnitOwner.getID(), MEMORY_NUKED_US) +
									kOwner.AI_getMemoryCount(kUnitOwner.getID(), MEMORY_NUKED_FRIEND) +
									kUnitOwner.AI_getMemoryCount(kOwner.getID(), MEMORY_NUKED_US) +
									kUnitOwner.AI_getMemoryCount(kOwner.getID(), MEMORY_NUKED_FRIEND);

								if (iNuclearWeaponsUsed == 0 && kUnitOwner.getNukesUsed() < 8)
								{
									bPreventStrike = true;
									break;
								}
...
#ifdef BETTER_NUKES_LOGS
				logBBAI("      First nuclear strike parameters: bAggresive = %d, bPreventStrike = %d, iWarRatingThreshold = %d, iWarRating = %d, iWarRatingScaled %d", 
								(int)bAggresive, (int)bPreventStrike, iWarRatingThreshold, iWarRating, iWarRatingScaled);
#endif

				// Try to not use nuclear weapon first unless the situation gets bad or the enemy is known from using it too much, scale iTargetValue with war rating
				if (bPreventStrike && iWarRating > iWarRatingThreshold)
				{
					iTargetValue /= iWarRatingScaled;
#ifdef BETTER_NUKES_LOGS
					logBBAI("      We are not going to start nuclear war first, iTargetValue divided by %d to %d", iWarRatingScaled, iTargetValue);
#endif
				}
...
bool CvUnitAI::AI_plotValid(CvPlot* pPlot)
{
	...
	case DOMAIN_IMMOBILE:
		// MOD - START - Immobile Defenders (Ungomma)
		// Note from Josh (2012-07-26): I'm not sure what the implications of doing this are.
		//                              Would this let someone move a missile from a city to a fort?
		// TODO: Check behavior of this in-game
		//FAssert(false);
		return pPlot->isFriendlyCity(*this, true);
		// MOD - END - Immobile Defenders (Ungomma)
		break;
...
int CvUnitAI::AI_getWeightedOdds(CvPlot* pPlot, bool bPotentialEnemy)
{
	...
	if (pDefender->isAlwaysHostile(pPlot) && getDomainType() == DOMAIN_SEA && pPlot->getNumVisiblePotentialEnemyDefenders(pAttacker) < pPlot->getNumUnits())
	{
		return 0;
	}
	...
	if (pBestPlot)
	{
		...
		if (atPlot(pBestPlot))
		{
			...
			if (canPillage(plot()))
				...
				AI_rangeAttackOrSkip(iFlags, false, false, MISSIONAI_CHOKE, pChokedCityPlot);
				// MOD - END - Ranged Strike AI
...
// Note from Ungomma (2012-07-17): I am not sure this is necessary, K-Mod has good hurry logic and this one "avoids using great people" anyway.
// Note from Josh (2012-07-26): I agree
...
// MOD - START - Improved Automations
bool CvUnitAI::AI_AutomatedPillage(int iBonusValueThreshold)
{
	...
	int iOddsThreshold = 75; // TODO: Make this depend on personality?
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot) && canPillage(pLoopPlot))
		{
			if (potentialWarAction(pLoopPlot))
			{
				int iBaseValue = AI_pillageValue(pLoopPlot, iBonusValueThreshold);
				iBaseValue *= 1000;

				// Prefer to pillage enemy players, not barbarians
				if (pLoopPlot->getOwnerINLINE() == BARBARIAN_PLAYER)
				{
					iBaseValue /= 4;
				}

				int iMaxWorthwhileDistance = -1; // The maximum distance this plot can be and still beat the best value

				// Check if there's no way this is better than our current best value
				if (iBestValue > 0)
				{
					iMaxWorthwhileDistance = iBaseValue / iBestValue - 1;
					if (stepDistance(pLoopPlot, plot()) > iMaxWorthwhileDistance)
					{
						// Even as the crow flies this is too far away to be worth it
						continue;
					}
				}

				// Don't go after plots with more units than our group has
				if (pLoopPlot->isVisibleEnemyUnit(this))
				{
				...
				if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_PILLAGE, getGroup(), iMaxWorthwhileDistance) == 0)
				{
					int iPathTurns;
					if (generatePath(pLoopPlot, 0, true, &iPathTurns, iMaxWorthwhileDistance))
					{
						int iValue = iBaseValue / (iPathTurns + 1);
						if (iValue <= iBestValue)
						{
							continue;
						}
						...
						if (pLoopPlot->isVisibleEnemyUnit(this))
						{
							...
							// Scale the value by our odds of victory (we intentionally do this twice to amplify the effect)
							iValue *= iOdds;
							iValue /= 100;
							iValue *= iOdds;
							iValue /= 100;
			...
			if (pLoopPlot != NULL)
			{
				if (AI_plotValid(pLoopPlot))
				{
					if (!bStayInBorders || (pLoopPlot->getOwnerINLINE() == getOwnerINLINE()))
					{
						if (pLoopPlot->isVisibleEnemyUnit(this))
						{
							// TODO: Simply pass iRange in as a max path to generatePath?
							if (!atPlot(pLoopPlot) && getGroup()->canMoveInto(pLoopPlot, true) && generatePath(pLoopPlot, 0, true, &iPathTurns) && (iPathTurns <= iRange))
							{
								if (pLoopPlot->getNumVisibleEnemyDefenders(this) <= getGroup()->getNumUnits())
								{
									...
									{
										//iValue = getGroup()->AI_attackOdds(pLoopPlot, true);
										iValue = AI_getWeightedOdds(pLoopPlot, false); // K-Mod
...
bool CvUnitAI::AI_patrolBorders()
{
	...
	for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)
	{
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				//if (getGroup()->canMoveInto(pLoopPlot, false, false, true))
				if (getGroup()->canMoveOrAttackInto(pLoopPlot, false, false, true))
				{
...
void CvUnitAI::AI_autoAirStrike()
{
	...
	// MOD - START - K-Mod Combined Air Combat AI (Ungomma)
	/*
	...
	*/
	// MOD - END - K-Mod Combined Air Combat AI (Ungomma)
	...
	*/
	...
	// MOD - START - K-Mod Combined Air Combat AI (Ungomma)
	// Note from Ungomma (2012-07-17): Should this be merged into AI_airStrike?
	/*
	if (AI_airBombCities())
	{
		return;
...
// MOD - START - K-Mod Combined Air Combat AI (Ungomma)
// Note from Ungomma (2012-07-17): AI_airStike handles attacking cities, is this necessary?
// Note from Josh (2012-07-26): Based on Karadoc's notes I don't think it is
/*
// Returns true if a mission was pushed...
bool CvUnitAI::AI_airBombCities()
{
	...
	for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)
	{
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (getGroup()->canMoveInto(pLoopPlot, true))
				{
					iValue = 0;
					...
					{
						{
							if (pLoopPlot->isVisibleEnemyUnit(this))
							{
								if (!atPlot(pLoopPlot) && getGroup()->canMoveInto(pLoopPlot, true) && generatePath(pLoopPlot, 0, true, &iPathTurns) && (iPathTurns <= iSearchRange))
								{
									if (pLoopPlot->getNumVisibleEnemyDefenders(this) <= getGroup()->getNumUnits())
									{
										if (pLoopPlot->getNumVisibleAdjacentEnemyDefenders(this) <= ((getGroup()->getNumUnits() * 3) / 2))
...
// MOD - END - Advanced Automations
// Dale - FE: Fighters START
bool CvUnitAI::AI_fighterEngage()
{
	PROFILE_FUNC();
	CvPlot* pLoopPlot;
	CvPlot* pBestPlot;
