...
void CvDllTranslator::initializeTags(CvWString& szTagStartIcon, CvWString& szTagStartOur, CvWString& szTagStartCT, CvWString& szTagStartColor, CvWString& szTagStartLink, CvWString& szTagEndLink, CvWString& szEndLinkReplacement, std::map<std::wstring, CvWString>& aIconMap, std::map<std::wstring, CvWString>& aColorMap)
{
	...
	aIconMap[L"[ICON_CANCEL]"] = std::wstring(1, (wchar)gDLL->getSymbolID(CANCEL_CHAR));
	// MOD - END - Advanced Civilopedia
	aIconMap[L"[ICON_EPIDEMIC]"] = std::wstring(1, (wchar)gDLL->getSymbolID(EPIDEMIC_START_CHAR));
	aIconMap[L"[ICON_EPIDEMIC_END]"] = std::wstring(1, (wchar)gDLL->getSymbolID(EPIDEMIC_END_CHAR));
	aIconMap[L"[ICON_SEPARATISM]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_CHAR));
	aIconMap[L"[ICON_SEPARATISM_POS]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_POS_CHAR));
	aIconMap[L"[ICON_SEPARATISM_NEG]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_NEG_CHAR));
	aIconMap[L"[ICON_SEPARATISM_GREEN]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_GREEN_CHAR));
	aIconMap[L"[ICON_SEPARATISM_YELLOW]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_YELLOW_CHAR));
	aIconMap[L"[ICON_SEPARATISM_ORANGE]"] = std::wstring(1, (wchar)gDLL->getSymbolID(SEPARATISM_ORANGE_CHAR));
