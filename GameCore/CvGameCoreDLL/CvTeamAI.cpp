...
CvTeamAI::CvTeamAI()
{
	...
	m_aiAdjacentLandPlotsCache = new int[MAX_TEAMS];
...
CvTeamAI::~CvTeamAI()
{
	...
	// MOD - START - Border Statistics Cache
	SAFE_DELETE_ARRAY(m_aiAdjacentLandPlotsCache);
	// MOD - END - Border Statistics Cache
...
void CvTeamAI::AI_reset(bool bConstructor)
{
	...
	for (int iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		// MOD - START - Border Statistics Cache
		m_aiAdjacentLandPlotsCache[iI] = 0;
		// MOD - END - Border Statistics Cache
		...
		if (!bConstructor && getID() != NO_TEAM)
		{
			...
			// MOD - START - Border Statistics Cache
			kLoopTeam.m_aiAdjacentLandPlotsCache[getID()] = 0;
			// MOD - END - Border Statistics Cache
...
void CvTeamAI::AI_makeAssignWorkDirty()
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		GET_PLAYER(*it).AI_makeAssignWorkDirty();
	}
...
void CvTeamAI::AI_updateAreaTargets()
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		CvPlayerAI& kLoopPlayer = GET_PLAYER(*it); // K-Mod
		if (kLoopPlayer.AI_getCityTargetTimer() == 0) // K-Mod added timer check.
		{
...
int CvTeamAI::AI_countFinancialTrouble() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (GET_PLAYER(*it).AI_isFinancialTrouble())
		{
			iCount++;
		}
	}
...
int CvTeamAI::AI_countMilitaryWeight(CvArea* pArea) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iCount += GET_PLAYER(*it).AI_militaryWeight(pArea);
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_estimateTotalYieldRate(YieldTypes eYield) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		const CvPlayerAI& kLoopPlayer = GET_PLAYER(*it);
	// MOD - END - Cached Team Members
		int iSubTotal = 0;
		int iBase = 0;
...
bool CvTeamAI::AI_isAnyCapitalAreaAlone() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (GET_PLAYER(*it).AI_isCapitalAreaAlone())
		{
			return true;
		}
	}
...
bool CvTeamAI::AI_isPrimaryArea(CvArea* pArea) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (GET_PLAYER(*it).AI_isPrimaryArea(pArea))
		{
			return true;
		}
	}
...
AreaAITypes CvTeamAI::AI_calculateAreaAIType(CvArea* pArea, bool bPreparingTotal) const
{
	...
	if (bTargets)
	{
		...
		// MOD - END - Cached Team Members (poyuzhe)
			...
			{
				if (pArea->getCitiesPerPlayer(*it) > 0)
				{
					return AREAAI_MASSING;
				}
			}
...
// MOD - START - Border Statistics Cache
void CvTeamAI::AI_updateBorderStatistics()
{
	PROFILE_FUNC();

	const TeamTypes eOurTeam = getID();

	// Clear the cache
	for (TeamTypes i = (TeamTypes)0; i < MAX_TEAMS; i=(TeamTypes)(i+1))
	{
		AI_setAdjacentLandPlots(i, 0);
	}
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		...
		{
			kPlayer.AI_setStolenCityRadiusPlots(i, 0);
		}
	}

	// Pass over the entire map to collect statistics for the team as a whole and for its members
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);

		PlayerTypes ePlotOwner = pLoopPlot->getOwnerINLINE();
		if (ePlotOwner != NO_PLAYER)
		{
			TeamTypes ePlotTeam = GET_PLAYER(ePlotOwner).getTeam();

			if (ePlotTeam != eOurTeam)
			{
				if (!pLoopPlot->isWater())
				{
					if (pLoopPlot->isAdjacentTeam(eOurTeam, true))
					{
						AI_changeAdjacentLandPlots(ePlotTeam, 1);
					}
				}

				for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
				{
					if (pLoopPlot->isPlayerCityRadius(*it))
					{
						GET_PLAYER(*it).AI_changeStolenCityRadiusPlots(ePlotOwner, 1);
					}
				}
			}
		}
...
int CvTeamAI::AI_calculateCapitalProximity(TeamTypes eTeam) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		pOurCapitalCity = GET_PLAYER(*it).getCapitalCity();
	// MOD - END - Cached Team Members (poyuzhe)
...
bool CvTeamAI::AI_haveSeenCities(TeamTypes eTeam, bool bPrimaryAreaOnly, int iMinimum) const
{
	...
	for (int iTheirMember = 0; iTheirMember < kTheirTeam.getNumLivingTeamMembers(); iTheirMember++)
	{
		const CvPlayerAI& kLoopPlayer = GET_PLAYER(kTheirTeam.getLivingTeamMember(iTheirMember));
	// MOD - END - Cached Team Members
		int iLoop;
...
bool CvTeamAI::AI_isLandTarget(TeamTypes eTeam) const
{
	...
	// MOD - START - Cached Team Members
	for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
	{
		const CvPlayerAI& kOurPlayer = GET_PLAYER(getLivingTeamMember(iOurMember));
	// MOD - END - Cached Team Members
		...
		for (CvCity* pOurCity = kOurPlayer.firstCity(&iL1); pOurCity; pOurCity = kOurPlayer.nextCity(&iL1))
		{
			...
			// MOD - START - Cached Team Members
			for (int iTheirMember = 0; iTheirMember < kOtherTeam.getNumLivingTeamMembers(); iTheirMember++)
			{
				const CvPlayerAI& kTheirPlayer = GET_PLAYER(kOtherTeam.getLivingTeamMember(iTheirMember));
				if (!kTheirPlayer.AI_isPrimaryArea(pOurCity->area()))
					continue;
			// MOD - END - Cached Team Members
...
AttitudeTypes CvTeamAI::AI_getAttitude(TeamTypes eTeam, bool bForced) const
{
	...
	// TODO: Just use AI_getAttitudeVal instead of duplicating the code here? What would that mean for the default attitude?
	...
	for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
	{
		...
		for (int iTheirMember = 0; iTheirMember < kOtherTeam.getNumLivingTeamMembers(); iTheirMember++)
		{
			PlayerTypes eTheirPlayer = kOtherTeam.getLivingTeamMember(iTheirMember);

			// K-Mod. Removed attitude averaging between vassals and masters
			iAttitude += kOurPlayer.AI_getAttitudeVal(eTheirPlayer, bForced); // bbai. Average values rather than attitudes directly.
			iCount++;
		}
	}
	// MOD - END - Cached Team Members
...
int CvTeamAI::AI_getAttitudeVal(TeamTypes eTeam, bool bForced) const
{
	...
	for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
	{
		...
		for (int iTheirMember = 0; iTheirMember < kOtherTeam.getNumLivingTeamMembers(); iTheirMember++)
		{
			...
			iAttitudeVal += kOurPlayer.AI_getAttitudeVal(eTheirPlayer, bForced);
			iCount++;
		}
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_getMemoryCount(TeamTypes eTeam, MemoryTypes eMemory) const
{
	...
	for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
	{
		...
		for (int iTheirMember = 0; iTheirMember < kOtherTeam.getNumLivingTeamMembers(); iTheirMember++)
		{
			...
			iMemoryCount += kOurPlayer.AI_getMemoryCount(eTheirPlayer, eMemory);
			iCount++;
		}
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_chooseElection(const VoteSelectionData& kVoteSelectionData) const
{
	...
	for (int iI = 0; iI < (int)kVoteSelectionData.aVoteOptions.size(); ++iI)
	{
		...
		if (!GC.getGameINLINE().isTeamVote(eVote))
		{
			...
			for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
			{
				PlayerVoteTypes eVote = GET_PLAYER(*it).AI_diploVote(kVoteSelectionData.aVoteOptions[iI], eVoteSource, true);
...
int CvTeamAI::AI_warSpoilsValue(TeamTypes eTarget, WarPlanTypes eWarPlan) const
{
	...
	std::set<int> nearby_cities; // Ungomma: set of city IDs located in the same plot groups (thus, connected) to close cities (AI_highestTeamCloseness > 0).

	// MOD - START - Cached Team Members
	for (int iTargetMember = 0; iTargetMember < kTargetTeam.getNumLivingTeamMembers(); iTargetMember++)
	{
		PlayerTypes eLoopPlayer = kTargetTeam.getLivingTeamMember(iTargetMember);
		const CvPlayerAI& kLoopPlayer = GET_PLAYER(eLoopPlayer);
	// MOD - END - Cached Team Members
		...
		for (CvCity* pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity; pLoopCity = kLoopPlayer.nextCity(&iLoop))
		{
			...
			// Ungomma: prefer close cities and nearby: closer than 22 plots (close enough) in the same area
			bool bIsClose = pLoopCity->AI_highestTeamCloseness(getID()) > 0;
			bool bIsNearby = false;
			if (!bIsClose && AI_isPrimaryArea(pLoopCity->area()))
			{
				const CvCity* pOurClosestCity = GC.getMap().findCity(pLoopCity->plot()->getX(), pLoopCity->plot()->getY(), NO_PLAYER, getID());
				int iDistance = plotDistance(pOurClosestCity->getX_INLINE(), pOurClosestCity->getY_INLINE(), pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE());
				if (iDistance < 10) // Ungomma: closeness is determined by _path_ length if we are in the same area; path might be long while we are separated by a few plots of water
					bIsClose = true;
				else if (iDistance < 25) // Ungomma: world size should probably be taken into account, but on giant maps I still don't want this value to exceed a distance of 10-20 turns
					bIsNearby = true;

			}
			if (bIsClose || bIsNearby)
			{
				nearby_cities.insert(pLoopCity->getID());
			}
	...
	for (int i = 0; i < GC.getMapINLINE().numPlotsINLINE(); i++)
	{
		...
		if (pLoopPlot->getTeam() == eTarget)
		{
			...
			if (eBonus != NO_BONUS)
			{
				// Ungomma: value close bonuses which are already in our plotGroups

				bool close = false;
				if (pLoopPlot->isRevealed(getID(), false))
				{
					for (int iOurPlayer = 0; iOurPlayer < getNumLivingTeamMembers(); iOurPlayer++)
					{
						PlayerTypes eLoopPlayer = kTargetTeam.getLivingTeamMember(iOurPlayer);
						CvPlotGroup* pPlotGroup = pLoopPlot->getPlotGroup(eLoopPlayer);
						if (pPlotGroup!=NULL)
						{
							close = true;
							break;
				...
				if (AI_isPrimaryArea(pLoopPlot->area())) 
					bonuses[eBonus] += (bTotalWar ? 30 : 15) * (close ? 2 : 1);
				else
					bonuses[eBonus] += (bTotalWar ? 15 : 0) * (close ? 5/2 : 1);
			}
		}
	}
	...
	// magnify the gained value based on how many of the target's cities are nearby
	int iNearbyCities = nearby_cities.size();

	iGainedValue *= 75 + 50 * iNearbyCities / std::max(1, kTargetTeam.getNumCities());
	iGainedValue /= 100;
...
int CvTeamAI::AI_warCommitmentCost(TeamTypes eTarget, WarPlanTypes eWarPlan) const
{
	...
	// Estimate of military production costs
	{
		...
		// scale based on the relative strengths of our units
		{
			...
			int iOurCityAttackUnit = std::max(30, getTypicalUnitValue(UNITAI_ATTACK_CITY, DOMAIN_LAND));
			int iOurDefenceUnit = std::max(30, getTypicalUnitValue(UNITAI_CITY_DEFENSE, DOMAIN_LAND));
			int iHighScale = 30 + 70 * std::max(std::max(iOurAttackUnit, iOurCityAttackUnit), iOurDefenceUnit) / iEnemyUnit;
			int iLowScale = 10 + 90 * std::min(std::min(iOurAttackUnit, iOurCityAttackUnit), iOurDefenceUnit) / iEnemyUnit;
...
DenialTypes CvTeamAI::AI_techTrade(TechTypes eTech, TeamTypes eTeam) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getTechRefuseAttitudeThreshold())
		{
...
DenialTypes CvTeamAI::AI_mapTrade(TeamTypes eTeam) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMapRefuseAttitudeThreshold())
		{
			return DENIAL_ATTITUDE;
		}
	}
...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	if (!GET_TEAM(eTeam).isParent(getID()))
	{
		...
		for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
		{
			iPersonalityModifier += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getVassalPowerModifier();
			++iMembers;
		}
		// MOD - END - Cached Team Members (poyuzhe)
...
/************************************************************************************************/
	...
	if (!isAtWar(eTeam))
	{
		if (!GET_TEAM(eTeam).isParent(getID()))
		{
			...
			if (!AI_hasCitiesInPrimaryArea(eTeam) && AI_getAdjacentLandPlots(eTeam) == 0)
			{
		...
		for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
		{
			...
			if (eModifiedAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getVassalRefuseAttitudeThreshold())
			{
				return DENIAL_ATTITUDE;
			}
		}
...
int CvTeamAI::AI_getWarSuccessRating() const
{
	...
	iScore = range(iScore / std::max(1, iThisTeamPower * iSuccessScale / 20), -99, 99);
	return iScore;
}
// K-Mod end
...
bool CvTeamAI::AI_acceptSurrender( TeamTypes eSurrenderTeam ) const
{
	...
	// MOD - START - Cached Team Members
	for (int iSurrenderMember = 0; iSurrenderMember < kSurrenderTeam.getNumLivingTeamMembers(); iSurrenderMember++)
	{
		PlayerTypes eLoopPlayer = kSurrenderTeam.getLivingTeamMember(iSurrenderMember);
		const CvPlayerAI& kLoopPlayer = GET_PLAYER(eLoopPlayer);
	// MOD - END - Cached Team Members
		int iLoop;
		...
		for (CvCity* pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kLoopPlayer.nextCity(&iLoop))
		{
			...
			int iOwnerPower = kLoopPlayer.AI_localDefenceStrength(pLoopCity->plot(), eSurrenderTeam, DOMAIN_LAND, 2, true, false, true);
			...
			if( bValuable )
			{
				...
				else
				{
					...
					for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
					{
						PlayerTypes eOurPlayer = getLivingTeamMember(iOurMember);

						if( pLoopCity->AI_playerCloseness(eOurPlayer) > 5 )
						{
							iValuableCities++;
							break;
						}
...
/************************************************************************************************/
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getDeclareWarRefuseAttitudeThreshold())
		{
			return DENIAL_ATTITUDE;
		}
	}
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitudeThem > GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getDeclareWarThemRefuseAttitudeThreshold())
		{
			return DENIAL_ATTITUDE_THEM;
		}
	}
...
// This function returns a value estimating positive effects of open borders
// It is mostly designed for normal one-civ teams
float CvTeamAI::AI_openBordersPositive(TeamTypes eTeam) const
{
	bool bAlreadyOpen = isOpenBorders(eTeam);
	float iValue = 0.35f;
	const CvPlayerAI& team_leader = GET_PLAYER(getLeaderID());

	int leader_missing_techs = 0;

	// A civ that's slightly behind may have no new tech for us,
	// or we may have several new techs for it and it several for us
	for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
	{
		if (team_leader.canEverResearch((TechTypes)iI) &&
			!isHasTech((TechTypes)iI)                  &&
			team_leader.AI_teamCouldBoostTech(eTeam, iI)  )
		{
			if( gTeamLogLevel >= 3 )
			{
				logBBAI("      Team %d (%S) is missing tech %S that team %d (%S) has", getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), GC.getTechInfo((TechTypes)iI).getDescription(), eTeam, GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).getCivilizationDescription(0));
			}
			leader_missing_techs++;
		}
	}

	iValue += std::min(6, leader_missing_techs) * 0.25f + std::min(2, leader_missing_techs) * 0.25f;

	// open borders improve attitude, so for weak nations they are a form of non-aggression pact
	// This only works, however, if relations are not too poor to begin with
	if (AI_teamCloseness(eTeam) > 0 &&
		AI_getAttitude(eTeam) != ATTITUDE_FURIOUS)
	{
		int iOurPower = getPower(false);
		int iTheirPower = GET_TEAM(eTeam).getPower(false);

		if (iOurPower < iTheirPower * 3 / 4 )
			iValue += 0.4f;
	}

	// Open borders are also valuable if they give us more foreign trade routes
	// We assume that the trade networks are connected, as if they are not already, it's likely only temporary.

	// First, check that foreign trade routes are possible with this team
	bool canTradeWithTeam = true;
	for (int iLoopPlayer = 0; iLoopPlayer < MAX_PLAYERS; iLoopPlayer++)
	{
		if (GET_PLAYER((PlayerTypes)iLoopPlayer).isNoForeignTrade())
		{
			// A player on our team or on their team has protectionnism
			if (GET_PLAYER((PlayerTypes)iLoopPlayer).getTeam() == eTeam ||
				GET_PLAYER((PlayerTypes)iLoopPlayer).getTeam() == (TeamTypes)getID())
			{
				canTradeWithTeam = false;
				break;
			}
		}
	}

	// Foreign trade routes are possible, so check how much trade routes we get from other agreements
	// and compare with how much foreign trade routes we need.
	// TODO: account for intercontinental trade bonuses
	if (canTradeWithTeam)
	{
		...
		for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
		{
			int currPlayerTradeRoutes = 0;
			for (CvCity* pLoopCity = GET_PLAYER(*it).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(*it).nextCity(&iLoop))
			{
				currPlayerTradeRoutes += pLoopCity->getTradeRoutes();
			}
			iTotalTradeRoutes = std::max(iTotalTradeRoutes, currPlayerTradeRoutes); // The limit of 1 trade route per foreign city is per-civ, not per-team
		}

		int iTotalForeignTradeCities = 0;

		// Note: If a team has multiple members, trade between them counts as domestic trade
		for (int iLoopPlayer = 0; iLoopPlayer < MAX_PLAYERS; iLoopPlayer++)
		{
			if (!GET_PLAYER((PlayerTypes)iLoopPlayer).isNoForeignTrade() &&
				GET_PLAYER((PlayerTypes)iLoopPlayer).getTeam() != eTeam &&
				GET_PLAYER((PlayerTypes)iLoopPlayer).getTeam() != (TeamTypes)getID() &&
				isOpenBorders(GET_PLAYER((PlayerTypes)iLoopPlayer).getTeam()))
			{
				iTotalForeignTradeCities += GET_PLAYER((PlayerTypes)iLoopPlayer).getNumCities();
			}
		}

		float trade_ratio = (float)iTotalForeignTradeCities / (float)iTotalTradeRoutes;

		// If our trade routes are already saturated from other open border agreements, the value is much lower,
		// but keep some margin to be more resilient to the potential cancellation of another agreement.
		// We deliberately don't valuate based on the number of foreign cities,
		// because several agreements with smaller states are no worse than one agreement with a big state
		if (trade_ratio < 2.5f)
			iValue += (2.5f - trade_ratio) * 0.4f;

		if( gTeamLogLevel >= 3 )
		{
			logBBAI("      Team %d (%S) has %i trade routes and has %i foreign cities available to trade with (except for team %d (%S) cities), for a ratio of %f", getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), iTotalTradeRoutes, iTotalForeignTradeCities, eTeam, GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).getCivilizationDescription(0), trade_ratio);
		}
	} // if canTradeWithTeam

	if (bAlreadyOpen)
		iValue += 0.4f; // Reduce flip-flopping

	return iValue;
} // AI_openBordersPositive

// This function returns a value estimating negative effects of open borders
float CvTeamAI::AI_openBordersNegative(TeamTypes eTeam) const
{
	float iValue = 0.4f;

	int iOurTechScore = getBestKnownTechScorePercent();
	int iTheirTechScore = GET_TEAM(eTeam).getBestKnownTechScorePercent();

	if (iTheirTechScore < iOurTechScore)
		iValue += 0.4f;
	if (iTheirTechScore < iOurTechScore * 95 / 100)
		iValue += 0.35f;
	if (iTheirTechScore < iOurTechScore * 90 / 100)
		iValue += 0.3f;
	if (iTheirTechScore < iOurTechScore * 85 / 100)
		iValue += 0.25f;
	if (iTheirTechScore < iOurTechScore * 80 / 100)
		iValue += 0.2f;

	// We don't want to help a rival civilization
	if (AI_teamCloseness(eTeam) > 0 &&
		AI_getAttitude(eTeam) != ATTITUDE_FRIENDLY)
	{
		int iOurPower = getPower(false);
		int iTheirPower = GET_TEAM(eTeam).getPower(false);

		if (iOurPower > iTheirPower * 5 / 6 && iOurPower < iTheirPower)
		{
			iValue *= 1.5f;
		}
	}

	// If a civilization is weak and not next to our borders,
	// there is less risk in giving it open borders, as the weak civ
	// is not going to threaten us but could potentially be an ally
	// in the future or at least a thorn in the side of a rival civ.
	else if (AI_teamCloseness(eTeam) == 0 &&
		AI_getAttitude(eTeam) != ATTITUDE_FURIOUS)
	{
		int iOurPower = getPower(false);
		int iTheirPower = GET_TEAM(eTeam).getPower(false);

		if (iTheirPower < iOurPower * 2 / 3 )
			iValue *= 0.65f;
	}

	return iValue;
} // AI_openBordersNegative
...
DenialTypes CvTeamAI::AI_openBordersTrade(TeamTypes eTeam) const
{
	...
	// TODO - have some denials during wars
	// Otherwise, a civilization that's safe from fighting could declare a fake war
	// just to get an open border deal that would otherwise never be accepted.
	...
	float positive = AI_openBordersPositive(eTeam);
	float negative = AI_openBordersNegative(eTeam);
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		// Typical values are 0 for leaders that like open borders, 2 for leaders that dislike them, and 1 for most others.
		// A higher value increases rejection of open borders.
		int personality_threshold = GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getOpenBordersRefuseAttitudeThreshold();

		// Be more open to open borders with team with which we have a good relationship
		int net_attitude = (int)eAttitude - personality_threshold;

		float base_evaluation = positive - negative; // Used to pick the denial reason

		if (net_attitude > 0)
		{
			positive = positive * (1.0f + (float)net_attitude / 10);
			negative = negative * (1.0f - (float)net_attitude / 10);
		}
		if (net_attitude < 0)
		{
			negative = negative * (1.0f - (float)net_attitude / 10); // net_attitude is negative so the factor is > 1
		}

		float final_evaluation = positive - negative;
			
		if (gTeamLogLevel >= 3) 
		{
			logBBAI("     Team %d (%S) has a base evaluation of %f and a final evaluation of %f (positive: %f, negative: %f) for open borders with team %d (%S)",
				getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), base_evaluation, final_evaluation, positive, negative, eTeam, GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).getCivilizationDescription(0));
		}

		if (final_evaluation < 0.0f)
		{
			if (base_evaluation > 0.0f)
				return DENIAL_ATTITUDE;
			else
				return DENIAL_NO_GAIN;
		}
...
DenialTypes CvTeamAI::AI_defensivePactTrade(TeamTypes eTeam) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold())
		{
			return DENIAL_ATTITUDE;
		}
	}
...
DenialTypes CvTeamAI::AI_permanentAllianceTrade(TeamTypes eTeam) const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		if (eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getPermanentAllianceRefuseAttitudeThreshold())
		{
			return DENIAL_ATTITUDE;
		}
	}
...
// MOD - START - Border Statistics Cache
void CvTeamAI::AI_changeAdjacentLandPlots(TeamTypes eIndex, int iChange)
{
	AI_setAdjacentLandPlots(eIndex, (AI_getAdjacentLandPlots(eIndex) + iChange));
}


int CvTeamAI::AI_getAdjacentLandPlots(TeamTypes eIndex) const
{
	FASSERT_BOUNDS(0, MAX_TEAMS, eIndex, "CvTeamAI::AI_getAdjacentLandPlots");
	return m_aiAdjacentLandPlotsCache[eIndex];
}


void CvTeamAI::AI_setAdjacentLandPlots(TeamTypes eIndex, int iNewValue)
{
	FASSERT_BOUNDS(0, MAX_TEAMS, eIndex, "CvTeamAI::AI_setAdjacentLandPlots");
	m_aiAdjacentLandPlotsCache[eIndex] = iNewValue;
	FAssert(AI_getAdjacentLandPlots(eIndex) >= 0);
}
// MOD - END - Border Statistics Cache
...
int CvTeamAI::AI_teamCloseness(TeamTypes eIndex, int iMaxDistance) const
{
	...
	int iValue = 0;
	// MOD - START - Cached Team Members (poyuzhe)
	const CvTeamAI& kOtherTeam = GET_TEAM(eIndex);
	for (int iOurMember = 0; iOurMember < getNumLivingTeamMembers(); iOurMember++)
	{
		const CvPlayerAI& kOurPlayer = GET_PLAYER(getLivingTeamMember(iOurMember));
		...
		{
			PlayerTypes eTheirPlayer = kOtherTeam.getLivingTeamMember(iTheirMember);
			iValue += kOurPlayer.AI_playerCloseness(eTheirPlayer, iMaxDistance);
		}
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_noTechTradeThreshold() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getNoTechTradeThreshold();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_techTradeKnownPercent() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getTechTradeKnownPercent();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_maxWarRand() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMaxWarRand();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_maxWarNearbyPowerRatio() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMaxWarNearbyPowerRatio();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_maxWarDistantPowerRatio() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMaxWarDistantPowerRatio();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_maxWarMinAdjacentLandPercent() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMaxWarMinAdjacentLandPercent();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_limitedWarRand() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getLimitedWarRand();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_limitedWarPowerRatio() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getLimitedWarPowerRatio();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_dogpileWarRand() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getDogpileWarRand();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
int CvTeamAI::AI_makePeaceRand() const
{
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		iRand += GC.getLeaderHeadInfo(GET_PLAYER(*it).getPersonalityType()).getMakePeaceRand();
		iCount++;
	}
	// MOD - END - Cached Team Members (poyuzhe)
...
/************************************************************************************************/
	...
	for (PlayerIterator it = m_aeLivingTeamMembers.begin(); it != m_aeLivingTeamMembers.end(); ++it)
	{
		const CvPlayerAI& kLoopPlayer = GET_PLAYER(*it);
	// MOD - END - Cached Team Members
		iProb += GC.getLeaderHeadInfo(kLoopPlayer.getPersonalityType()).getNoWarAttitudeProb(eAttitude);
		iCount++;
		...
		else if( kLoopPlayer.AI_isDoVictoryStrategy(AI_VICTORY_DOMINATION4) )
		{
...
bool CvTeamAI::AI_isOkayVassalTarget( TeamTypes eTeam ) const
{
	/* if( GET_TEAM(eTeam).isAVassal() )
	{
		if( !(AI_hasCitiesInPrimaryArea(eTeam)) || AI_getAdjacentLandPlots(eTeam) == 0 )
		{
			for( int iI = 0; iI < MAX_CIV_TEAMS; iI++ )
			{
				...
				{
					if( AI_hasCitiesInPrimaryArea((TeamTypes)iI) && AI_getAdjacentLandPlots((TeamTypes)iI) > 0)
					{
						return false;
					}
				}
	...
	if ((!AI_hasCitiesInPrimaryArea(eTeam) || AI_getAdjacentLandPlots(eTeam) == 0) &&
		(AI_hasCitiesInPrimaryArea(eMasterTeam) && AI_getAdjacentLandPlots(eMasterTeam) > 0))
		return false;
						...
						if (canDeclareWar((TeamTypes)iI) && AI_haveSeenCities((TeamTypes)iI))
						{
							...
							if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)) && (!bVassal || iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude(eLoopMasterTeam))))
							{
								...
								if (iDefensivePower < ((iOurPower * ((iPass > 1) ? AI_maxWarDistantPowerRatio() : AI_maxWarNearbyPowerRatio())) / 100))
								{
									...
									{
										if ((iPass > 0) || (AI_getAdjacentLandPlots((TeamTypes)iI) >= ((getTotalLand() * AI_maxWarMinAdjacentLandPercent()) / 100)) || GET_TEAM((TeamTypes)iI).AI_isAnyMemberDoVictoryStrategyLevel4())
										{
											int iValue = AI_startWarVal((TeamTypes)iI, WARPLAN_TOTAL);

											if( iValue > 0 && gTeamLogLevel >= 2 )
											{
												logBBAI("      Team %d (%S) considering starting TOTAL warplan with team %d with value %d on pass %d with %d adjacent plots", getID(), GET_PLAYER(getLeaderID()).getCivilizationDescription(0), iI, iValue, iPass, AI_getAdjacentLandPlots((TeamTypes)iI) );
											}
...
int CvTeamAI::AI_getTechMonopolyValue(TechTypes eTech, TeamTypes eTeam) const
{
	...
	// Efficiency improvements are made throughout this section
	...
	for (int iRel = 0; iRel < kTech.getNumRelatedUnitClasses(); iRel++)
	{
		...
		UnitTypes eLoopUnit = ((UnitTypes)GC.getUnitClassInfo(eLoopUnitClass).getDefaultUnitIndex());

		// MOD - START - Unit Classes Without Default Units
		// TODO: Take really unique units into account
		...
		if (eLoopUnit != NO_UNIT)
		{
			if (isTechRequiredForUnit((eTech), eLoopUnit))
			{
				...
				if (kLoopUnit.getPrimaryPrereqTech() == eTech)
				{
					int iNavalValue = 0;
					
					int iCombatRatio = (kLoopUnit.getCombat() * 100) / std::max(1, GC.getGameINLINE().getBestLandUnitCombat());
					if (iCombatRatio > 50)
					{
	...
	for (int iRel = 0; iRel < kTech.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		BuildingTypes eLoopBuilding = (BuildingTypes)GC.getBuildingClassInfo(eLoopBuildingClass).getDefaultBuildingIndex();
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			if (isTechRequiredForBuilding(eTech, eLoopBuilding))
			{
				...
				{
					if (!GC.getGameINLINE().isBuildingClassMaxedOut(eLoopBuildingClass))
					{
						iValue += 50;
					}
				}
