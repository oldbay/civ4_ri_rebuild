...
bool CyUnit::canRemoveReligion(CyPlot* pPlot, bool bTestVisible)
{
	return m_pUnit ? m_pUnit->canRemoveReligion(pPlot->getPlot(), bTestVisible) : false;
}

bool CyUnit::removeReligion()
{
	return m_pUnit ? m_pUnit->removeReligion() : false;
}
// MOD - END - Inquisition
...
int /*UnitCombatTypes*/ CyUnit::getUnitPromoType()
{
	return m_pUnit ? (int) m_pUnit->getUnitPromoType() : (int) NO_UNITCOMBAT;
}
...
int CyUnit::maxXPValue(CyUnit* pAttacker)
{
	return m_pUnit ? m_pUnit->maxXPValue(pAttacker != NULL ? pAttacker->getUnit() : NULL) : -1;
}
...
// MOD - START - Advanced Experience Limits
int CyUnit::getExtraBarbarianMaxXPValue() const
{
	return m_pUnit ? m_pUnit->getExtraBarbarianMaxXPValue() : -1;
}
// MOD - END - Advanced Experience Limits

// MOD - START - Extra Barbarian Combat Modifier
int CyUnit::getExtraBarbarianCombatModifier() const
{
	return m_pUnit ? m_pUnit->getExtraBarbarianMaxXPValue() : -1;
}
...
bool CyUnit::isImmuneToDamageFromFeature(int /*FeatureTypes*/ eFeature)
{
	return m_pUnit ? m_pUnit->isImmuneToDamageFromFeature((FeatureTypes) eFeature) : false;
}
...
// MOD - START - Slavery
// Update for slavery rebellion component
// rebels can ignore borderObstacles tag (GreatWall)
bool CyUnit::isIgnoreObstacle()
{
	return m_pUnit ? m_pUnit->isIgnoreObstacle() : false;
}
void CyUnit::setIgnoreObstacle(bool bNewValue)
{
	if (m_pUnit)
	{
		m_pUnit->setIgnoreObstacle(bNewValue);
	}
}
// chance to consume unit after improvement build by Mexico
int CyUnit::getBuildKillChance()
{
	return m_pUnit ? m_pUnit->getBuildKillChance() : 0;
}
// MOD - END - Slavery
