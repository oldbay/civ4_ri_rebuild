...
bool isPromotionValid(PromotionTypes ePromotion, UnitTypes eUnit, bool bLeader)
{
	...
	{
		if ((kUnit.getUnitPromoType() == NO_UNITCOMBAT) || !(kPromotion.getUnitCombat(kUnit.getUnitPromoType())))
		{
			return false;
		}	
	}
...
bool isTechRequiredForUnit(TechTypes eTech, UnitTypes eUnit)
{
	...
	// Old code deleted
	for (int iPre = 0; iPre < kUnit.getNumPrereqAndTechs(); iPre++)
	{
		if (kUnit.getPrereqAndTechs(iPre) == eTech)
		{
			return true;
		}
	}
...
bool isTechRequiredForBuilding(TechTypes eTech, BuildingTypes eBuilding)
{
	...
	{
		if (kBuilding.getPrereqAndTechs(iPre) == eTech)
		{
			return true;
		}
	}
...
bool isCivicRequiredForBuilding(CivicTypes eCivic, BuildingTypes eBuilding)
{
	for (int iPrereqCivicIndex = 0; iPrereqCivicIndex < GC.getBuildingInfo(eBuilding).getNumPrereqAndCivics(); iPrereqCivicIndex++)
	{
		CivicTypes ePrereqAndCivic = (CivicTypes)GC.getBuildingInfo(eBuilding).getPrereqAndCivics(iPrereqCivicIndex);
		if (ePrereqAndCivic == eCivic)
		{
			return true;
		}
	}
...
bool isTraitRelatedToImprovement(TraitTypes eTrait, ImprovementTypes eImprovement)
{
	...
	{
		if (kTrait.getImprovementYieldChanges(eImprovement, iI) != 0)
		{
			return true;
		}
	}
...
bool isTraitRelatedToSpecialist(TraitTypes eTrait, SpecialistTypes eSpecialist)
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		if (kTrait.getSpecialistYieldChanges(eSpecialist, iI) != 0)
		{
			return true;
		}
	}
	...
	{
		if (kTrait.getSpecialistCommerceChanges(eSpecialist, iI) != 0)
		{
			return true;
		}
	}
...
bool isTechRelatedToBonus(TechTypes eTech, BonusTypes eBonus)
{
	...
	if (kBonus.getTechCityTrade() == eTech)
	{
		return true;
	}

	if (kBonus.getTechObsolete() == eTech)
	{
		return true;
	}
...
bool isTechRelatedToImprovement(TechTypes eTech, ImprovementTypes eImprovement)
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		if (kImprovement.getTechYieldChanges(eTech, iI) != 0)
		{
			return true;
		}

		// MOD - START - Advanced Yield and Commerce Effects
		if (kImprovement.getTechIrrigatedYieldChanges(eTech, iI) != 0)
		{
			return true;
		}
...
bool isTechRelatedToBuilding(TechTypes eTech, BuildingTypes eBuilding)
{
	...
	if (kBuilding.getSpecialBuildingType() != NO_SPECIALBUILDING)
	{
		...
		if (kSpecialBuilding.getObsoleteTech() == eTech)
		{
			return true;
		}

		// MOD - START - Building Discontinuation
		if (kSpecialBuilding.getDiscontinueTech() == eTech)
		{
			return true;
		}
	...
	// MOD - START - Building Discontinuation
	if (kBuilding.getDiscontinueTech() == eTech)
	{
		return true;
	}
	// MOD - END - Building Discontinuation
...
bool isReligionRelatedToUnit(ReligionTypes eReligion, UnitTypes eUnit)
{
	...
	if (kUnit.getReligionType() == eReligion)
	{
		return true;
	}

	if (kUnit.getPrereqReligion() == eReligion)
	{
		return true;
	}

	if (kUnit.getStateReligion() == eReligion)
	{
		return true;
	}

	if (kUnit.getHolyCity() == eReligion)
	{
		return true;
	}
...
bool isReligionRelatedToBuilding(ReligionTypes eReligion, BuildingTypes eBuilding)
{
	...
	if (kBuilding.getPrereqReligion() == eReligion)
	{
		return true;
	}

	if (kBuilding.getStateReligion() == eReligion)
	{
		return true;
	}
	...
	for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		if (kBuilding.getStateReligionCommerce(iI) != 0)
		{
			return true;
		}
	}
...
bool isCivicRelatedToFeature(CivicTypes eCivic, FeatureTypes eFeature)
{
	...
	if (kCivic.getFeatureHappinessChanges(eFeature) != 0)
	{
		return true;
	}
...
bool isCivicRelatedToImprovement(CivicTypes eCivic, ImprovementTypes eImprovement)
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		if (kCivic.getImprovementYieldChanges(eImprovement, iI) != 0)
		{
			return true;
		}
	}
	...
	const CvImprovementInfo& kImprovement = GC.getImprovementInfo(eImprovement);
	if (kImprovement.getImpPrereqCivic() == eCivic)
	{
		return true;
	}
...
bool isCivicRelatedToSpecialist(CivicTypes eCivic, SpecialistTypes eSpecialist)
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		if (kCivic.getSpecialistYieldChanges(eSpecialist, iI) != 0)
		{
			return true;
		}
	}
	...
	for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		if (kCivic.getSpecialistCommerceChanges(eSpecialist, iI) != 0)
		{
			return true;
		}
	}
...
bool isCivicRelatedToUnit(CivicTypes eCivic, UnitTypes eUnit)
{
	...
	if (kUnit.getPrereqCivic() == eCivic)
	{
		return true;
	}
...
bool isCivicRelatedToBuilding(CivicTypes eCivic, BuildingTypes eBuilding)
{
	...
	if (kCivic.getBuildingHealthChanges(eBuildingClass) != 0)
	{
		return true;
	}

	if (kCivic.getBuildingHappinessChanges(eBuildingClass) != 0)
	{
		return true;
	}
	...
	{
		if (kBuilding.getPrereqAndCivics(iPre) == eCivic)
		{
			return true;
		}
	}
...
bool isBonusRelatedToUnit(BonusTypes eBonus, UnitTypes eUnit)
{
	...
	{
		BonusTypes ePrereqBonus = (BonusTypes)kUnit.getPrereqAndBonuses(iI);
	...
	{
		BonusTypes ePrereqBonus = (BonusTypes)kUnit.getPrereqOrBonuses(iI);
		if (ePrereqBonus == eBonus)
		{
			return true;
		}
...
bool isBonusRelatedToBuilding(BonusTypes eBonus, BuildingTypes eBuilding)
{
	...
	if (kBuilding.getNoBonus() == eBonus)
	{
		return true;
	}
	...
	{
		if (kBuilding.getPrereqAndBonuses(iPre) == eBonus)
		{
			return true;
		}
	}
	...
	{
		if (kBuilding.getPrereqOrBonuses(iPre) == eBonus)
		{
			return true;
		}
	}
	...
	int iProducedBonusIndex = kBuilding.getProducedBonusIndex(eBonus);
	...
	{
		if (kBuilding.getBonusYieldModifierBonus(iMem) == eBonus)
		{
			return true;
		}
	}
...
bool isSpecialistRelatedToBuilding(SpecialistTypes eSpecialist, BuildingTypes eBuilding)
{
	...
	if (kBuilding.getSpecialistCount(eSpecialist) != 0)
	{
		return true;
	}
	...
	{
		if ((SpecialistTypes)kBuilding.getSpecialistYieldChangeSpecialist(iMem) == eSpecialist)
		{
			return true;
		}
	}
	...
	{
		if ((SpecialistTypes)kBuilding.getSpecialistCommerceChangeSpecialist(iMem) == eSpecialist)
		{
			return true;
		}
	}
	...
	{
		if ((SpecialistTypes)kBuilding.getGlobalSpecialistYieldChangeSpecialist(iMem) == eSpecialist)
		{
			return true;
		}
	}
	...
	{
		if ((SpecialistTypes)kBuilding.getGlobalSpecialistCommerceChangeSpecialist(iMem) == eSpecialist)
		{
			return true;
		}
	}
...
bool isBuildingRelatedToUnitCombat(BuildingTypes eBuilding, UnitCombatTypes eUnitCombat)
{
	...
	if (kBuilding.getUnitCombatHealRateChanges(eUnitCombat) != 0)
	{
		return true;
	}
	...
	if (kBuilding.getUnitCombatDefenseModifier(eUnitCombat) != 0)
	{
		return true;
	}
...
bool isBuildingRelatedToUnit(BuildingTypes eBuilding, UnitTypes eUnit)
{
	...
	// MOD - START - Building Heal Rate
	if (kBuilding.getUnitClassHealRateChanges(eUnitClass) != 0)
	{
		return true;
	}
	// MOD - END - Building Heal Rate
	...
	// MOD - START - Defense Modifiers
	if (kBuilding.getUnitClassDefenseModifier(eUnitClass) != 0)
	{
		return true;
	}
	// MOD - END - Defense Modifiers
	...
	// MOD - START - Advanced Unit Prerequisites
	for (int iPre = 0; iPre < kUnit.getNumPrereqAndBuildingClasses(); iPre++)
	{
		if (((BuildingClassTypes)kUnit.getPrereqAndBuildingClasses(iPre)) == eBuildingClass)
		{
			return true;
		}
	}

	for (int iPre = 0; iPre < kUnit.getNumPrereqOrBuildingClasses(); iPre++)
	{
		if (((BuildingClassTypes)kUnit.getPrereqOrBuildingClasses(iPre)) == eBuildingClass)
		{
			return true;
		}
	}
	...
	if (kUnit.getBuildings(eBuilding))
	{
		return true;
	}

	if (kUnit.getForceBuildings(eBuilding))
	{
		return true;
	}
...
{
	const CvBuildingInfo& kBuildingA = GC.getBuildingInfo(eBuildingA);
	const CvBuildingInfo& kBuildingB = GC.getBuildingInfo(eBuildingB);

	BuildingClassTypes eBuildingClassA = (BuildingClassTypes)kBuildingA.getBuildingClassType();
	BuildingClassTypes eBuildingClassB = (BuildingClassTypes)kBuildingB.getBuildingClassType();

	if ((kBuildingA.getBuildingHappinessChanges(eBuildingClassB) != 0) || (kBuildingB.getBuildingHappinessChanges(eBuildingClassA) != 0))
	{
		return true;
	}

	if ((kBuildingA.getPrereqNumOfBuildingClass(eBuildingClassB) != 0) || (kBuildingB.getPrereqNumOfBuildingClass(eBuildingClassA) != 0))
	{
		return true;
	}

	if (kBuildingA.isBuildingClassNeededInCity(eBuildingClassB) || kBuildingB.isBuildingClassNeededInCity(eBuildingClassA))
	{
		return true;
	}

	if ((kBuildingA.getFreeBuildingClass() == eBuildingClassB) || (kBuildingB.getFreeBuildingClass() == eBuildingClassA))
	{
		return true;
	}

	// MOD - START - Building Replacement
	if ((kBuildingA.getObsoleteBuildingClass() == eBuildingClassB) || (kBuildingB.getObsoleteBuildingClass() == eBuildingClassA))
	{
		return true;
	}
	...
	// MOD - START - Advanced Yield and Commerce Effects
	for (int iMem = 0; iMem < kBuildingA.getNumBuildingYieldChanges(); iMem++)
	{
		if (kBuildingA.getBuildingYieldChangeBuildingClass(iMem) == eBuildingClassB)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingA.getNumBuildingCommerceChanges(); iMem++)
	{
		if (kBuildingA.getBuildingCommerceChangeBuildingClass(iMem) == eBuildingClassB)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingA.getNumGlobalBuildingYieldChanges(); iMem++)
	{
		if (kBuildingA.getGlobalBuildingYieldChangeBuildingClass(iMem) == eBuildingClassB)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingA.getNumGlobalBuildingCommerceChanges(); iMem++)
	{
		if (kBuildingA.getGlobalBuildingCommerceChangeBuildingClass(iMem) == eBuildingClassB)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingB.getNumBuildingYieldChanges(); iMem++)
	{
		if (kBuildingB.getBuildingYieldChangeBuildingClass(iMem) == eBuildingClassA)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingB.getNumBuildingCommerceChanges(); iMem++)
	{
		if (kBuildingB.getBuildingCommerceChangeBuildingClass(iMem) == eBuildingClassA)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingB.getNumGlobalBuildingYieldChanges(); iMem++)
	{
		if (kBuildingB.getGlobalBuildingYieldChangeBuildingClass(iMem) == eBuildingClassA)
		{
			return true;
		}
	}

	for (int iMem = 0; iMem < kBuildingB.getNumGlobalBuildingCommerceChanges(); iMem++)
	{
		if (kBuildingB.getGlobalBuildingCommerceChangeBuildingClass(iMem) == eBuildingClassA)
		{
			return true;
		}
	}
...
bool isUnitRelatedToFeature(UnitTypes eUnit, FeatureTypes eFeature)
{
	...
	if (kUnit.getFeatureDamageImmunity(eFeature))
	{
		return true;
	}
...
{
	return GC.getUnitInfo(eUnit).getBuilds(eBuild);
}
...
bool isUnitRelatedToUnitCombat(UnitTypes eUnit, UnitCombatTypes eUnitCombat)
{
	...
	if (kUnit.getUnitCombatAttackModifier(eUnitCombat) != 0)
	{
		return true;
	}

	if (kUnit.getUnitCombatDefenseModifier(eUnitCombat) != 0)
	{
		return true;
	}
...
{
	const CvUnitInfo& kUnitA = GC.getUnitInfo(eUnitA);
	const CvUnitInfo& kUnitB = GC.getUnitInfo(eUnitB);

	UnitClassTypes eUnitClassA = (UnitClassTypes)kUnitA.getUnitClassType();
	UnitClassTypes eUnitClassB = (UnitClassTypes)kUnitB.getUnitClassType();

	if ((kUnitA.getUnitClassAttackModifier(eUnitClassB) != 0) || (kUnitB.getUnitClassAttackModifier(eUnitClassA) != 0))
	{
		return true;
	}

	if ((kUnitA.getUnitClassDefenseModifier(eUnitClassB) != 0) || (kUnitB.getUnitClassDefenseModifier(eUnitClassA) != 0))
	{
		return true;
	}

	if (kUnitA.getTargetUnitClass(eUnitClassB) || kUnitB.getTargetUnitClass(eUnitClassA))
	{
		return true;
	}

	if (kUnitA.getDefenderUnitClass(eUnitClassB) || kUnitB.getDefenderUnitClass(eUnitClassA))
	{
		return true;
	}

	if ((kUnitA.getFlankingStrikeUnitClass(eUnitClassB) != 0) || (kUnitB.getFlankingStrikeUnitClass(eUnitClassA) != 0))
	{
		return true;
	}
...
bool isPromotionRelatedToDomain(PromotionTypes ePromotion, DomainTypes eDomain)
{
...
bool isPromotionRelatedToFeature(PromotionTypes ePromotion, FeatureTypes eFeature)
{
	...
	// MOD - START - Feature Damage Immunity
	if (kPromotion.getFeatureDamageImmunity(eFeature))
	{
		return true;
	}
	// MOD - END - Feature Damage Immunity
...
{
	const CvPromotionInfo& kPromotionA = GC.getPromotionInfo(ePromotionA);
	const CvPromotionInfo& kPromotionB = GC.getPromotionInfo(ePromotionB);

	if ((kPromotionA.getPrereqPromotion() == ePromotionB) || (kPromotionB.getPrereqPromotion() == ePromotionA))
	{
		return true;
	}

	if ((kPromotionA.getPrereqOrPromotion1() == ePromotionB) || (kPromotionB.getPrereqOrPromotion1() == ePromotionA))
	{
		return true;
	}

	if ((kPromotionA.getPrereqOrPromotion2() == ePromotionB) || (kPromotionB.getPrereqOrPromotion2() == ePromotionA))
	{
		return true;
	}

	if ((kPromotionA.getPrereqOrPromotion3() == ePromotionB) || (kPromotionB.getPrereqOrPromotion3() == ePromotionA))
	{
		return true;
	}
...
{
	const CvEventInfo& kEventA = GC.getEventInfo(eEventA);
	const CvEventInfo& kEventB = GC.getEventInfo(eEventB);

	if ((kEventA.getClearEventChance(eEventB) != 0) || (kEventB.getClearEventChance(eEventA) != 0))
	{
		return true;
	}

	if ((kEventA.getAdditionalEventChance(eEventB) != 0) || (kEventB.getAdditionalEventChance(eEventA) != 0))
	{
		return true;
	}

	if ((kEventA.getAdditionalEventTime(eEventB) != 0) || (kEventB.getAdditionalEventTime(eEventA) != 0))
	{
		return true;
	}
...
bool isEventRelatedToUnitCombat(EventTypes eEvent, UnitCombatTypes eUnitCombat)
{
	...
	if (kEvent.getUnitCombatPromotion(eUnitCombat) != NO_PROMOTION)
	{
		return true;
	}
...
bool isEventRelatedToUnit(EventTypes eEvent, UnitTypes eUnit)
{
	...
	if (kEvent.getUnitClass() == eUnitClass)
	{
		return true;
	}

	if (kEvent.getUnitClassPromotion(eUnitClass) != NO_PROMOTION)
	{
		return true;
	}
...
bool isEventRelatedToBuilding(EventTypes eEvent, BuildingTypes eBuilding)
{
	...
	if (kEvent.getNumBuildingYieldChanges() > 0)
	{
		...
		{
			if (kEvent.getBuildingYieldChange(eBuildingClass, iYield) != 0)
			{
				return true;
			}
		}
	...
	if (kEvent.getNumBuildingCommerceChanges() > 0)
	{
		...
		{
			if (kEvent.getBuildingCommerceChange(eBuildingClass, iCommerce) != 0)
			{
				return true;
			}				
		}
...
int getCombatOdds(const CvUnit* pAttacker, const CvUnit* pDefender)
{
	...
	/*	<f1rpo> Not enough to reach the limit, needs to be exceeded.
		This bugfix should be equivalent to the ACO bugfix in
		CvGameTextMgr::setCombatPlotHelp (see comment "A catapult that deals 25HP"...) */
	if (iDefenderHitLimit > 0)
		iDefenderHitLimit--; // </f1rpo>
...
#undef WRAP_Y
	...
	if (iWorstMovesLeft <= 0)
	{
		...
		if (0 != GC.getPATH_DAMAGE_WEIGHT())
		{
			...
			{
				// MOD - START - Feature Damage Immunity
				if (GC.getFeatureInfo(pToPlot->getFeatureType()).getTurnDamage() != 0)
				{
					if (!pSelectionGroup->isImmuneToDamageFromFeature(pToPlot->getFeatureType()))
					{
...
void getMissionTypeString(CvWString& szString, MissionTypes eMissionType)
{
	...
	case MISSION_REMOVE_RELIGION: szString = L"MISSION_REMOVE_RELIGION"; break;
	// MOD - END - Inquisition
...
void getUnitAIString(CvWString& szString, UnitAITypes eUnitAI)
{
	...
	case UNITAI_FORTIFICATION: szString = L"fortification"; break;
	...
	case UNITAI_INQUISITOR_SEA: szString = L"inquisitor sea"; break;
	// MOD - END - Inquisition
...
// MOD - START - Improved Civilopedia
void doPediaLink(std::wstring szLink)
{
	for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
	{
		if (GC.getTechInfo((TechTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_TECHNOLOGY, iI);
			return;
		}
	}
	...
	{
		if (GC.getUnitInfo((UnitTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_UNIT, iI);
			return;
		}
	}
	...
	{
		if (GC.getCorporationInfo((CorporationTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_CORPORATION, iI);
			return;
		}
	}
	...
	{
		if (GC.getBuildingInfo((BuildingTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_BUILDING, iI);
			return;
		}
	}
	...
	{
		if (GC.getPromotionInfo((PromotionTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_PROMOTION, iI);
			return;
		}
	}
	...
	{
		if (GC.getAidInfo((AidTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_AID, iI);
			return;
		}
	}
	// MOD - END - Aid
	...
	{
		if (GC.getDetrimentInfo((DetrimentTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_DETRIMENT, iI);
			return;
		}
	}
	// MOD - END - Detriments
	...
	{
		if (GC.getUnitCombatInfo((UnitCombatTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_UNIT_COMBAT, iI);
			return;
		}
	}
	...
	{
		if (GC.getBonusInfo((BonusTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_BONUS, iI);
			return;
		}
	}
	...
	{
		if (GC.getTerrainInfo((TerrainTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_TERRAIN, iI);
			return;
		}
	}
	...
	{
		if (GC.getFeatureInfo((FeatureTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_FEATURE, iI);
			return;
		}
	}
	...
	{
		if (GC.getImprovementInfo((ImprovementTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_IMPROVEMENT, iI);
			return;
		}
	}
	...
	{
		if (GC.getCivicInfo((CivicTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_CIVIC, iI);
			return;
		}
	}
	...
	{
		if (GC.getCivilizationInfo((CivilizationTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_CIVILIZATION, iI);
			return;
		}
	}
	...
	{
		if (GC.getLeaderHeadInfo((LeaderHeadTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_LEADER, iI);
			return;
		}
	}
	...
	{
		if (GC.getSpecialistInfo((SpecialistTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_SPECIALIST, iI);
			return;
		}
	}
	...
	{
		if (GC.getProjectInfo((ProjectTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_PROJECT, iI);
			return;
		}
	}
	...
	{
		if (GC.getReligionInfo((ReligionTypes)iI).isMatchForLink(szLink, false))
		{
			doPediaAction(CIVILOPEDIA_PAGE_RELIGION, iI);
			return;
		}
	}
}
...
{
	CyArgsList argsList;
	argsList.add(eSection);
	argsList.add(NO_CIVILOPEDIA_VIEW);
	argsList.add(-1);
	argsList.add(-1);
	argsList.add(NO_CIVILOPEDIA_PAGE);
	argsList.add(-1);
	argsList.add(-1);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "pediaAction", argsList.makeFunctionArgs());
}

void doPediaAction(CivilopediaViewTypes eView, int iViewData1, int iViewData2)
{
	CyArgsList argsList;
	argsList.add(NO_CIVILOPEDIA_SECTION);
	argsList.add(eView);
	argsList.add(iViewData1);
	argsList.add(iViewData2);
	argsList.add(NO_CIVILOPEDIA_PAGE);
	argsList.add(-1);
	argsList.add(-1);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "pediaAction", argsList.makeFunctionArgs());
...
void doPediaAction(CivilopediaPageTypes ePage, int iPageData1, int iPageData2)
{
	CyArgsList argsList;
	argsList.add(NO_CIVILOPEDIA_SECTION);
	argsList.add(NO_CIVILOPEDIA_VIEW);
	...
	argsList.add(-1);
	argsList.add(ePage);
	argsList.add(iPageData1);
	argsList.add(iPageData2);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "pediaAction", argsList.makeFunctionArgs());
}
// MOD - END - Improved Civilopedia
...
{
	FASSERT_BOUNDS(0, NUM_DOMAIN_TYPES, eDomain, "getRelevantCongestionTypes");

	std::vector< std::pair<int, CongestionTypes> > aSortableWorkingSet;

	for (int iI = 0; iI < GC.getNumCongestionInfos(); iI++)
	{
		const CvCongestionInfo& kCongestion = GC.getCongestionInfo((CongestionTypes)iI);

		if (kCongestion.isApplicable(eDomain, bInsideCity))
		{
			aSortableWorkingSet.push_back(std::make_pair(kCongestion.getMinUnits(), (CongestionTypes)iI));
		}
	}

	if (aSortableWorkingSet.size() == 0)
	{
		return;
	}

	std::sort(aSortableWorkingSet.begin(), aSortableWorkingSet.end());

	for (std::vector< std::pair<int, CongestionTypes> >::const_iterator it = aSortableWorkingSet.begin(); it != aSortableWorkingSet.end(); ++it)
	{
		aRelevantCongestionTypes.push_back((*it).second);
	}
}
...
CongestionTypes getCongestionRelativeToCurrent(const CvPlot* pPlot, TeamTypes eTeam, DomainTypes eDomain, int iOffset)
{
	...
	for (int iI = 0; iI < (int)aRelevantCongestionTypes.size(); iI++)
	{
		const CvCongestionInfo& kCongestion = GC.getCongestionInfo(aRelevantCongestionTypes[iI]);
		...
		bool bActive = iMinUnits <= iNumUnits && iNumUnits <= iMaxUnits;

		if (bActive)
		{
			int iIndex = iI + iOffset;
			if (iIndex < 0 || iIndex >= (int)aRelevantCongestionTypes.size())
			{
				return NO_CONGESTION;
			}
			else
			{
				return aRelevantCongestionTypes[iIndex];
			}
		}
	}
	...
	if (iOffset > 0)
	{
		int iIndex = iOffset - 1;
		if (iIndex >= (int)aRelevantCongestionTypes.size())
		{
			return NO_CONGESTION;
		}
		else
