...
void CvInfoBase::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_bRebellionRelated);	
...
void CvInfoBase::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_bRebellionRelated);	
...
// MOD - START - Rebellion Help
bool CvInfoBase::isRebellionRelated() const
{
	return m_bRebellionRelated;
}
// MOD - END - Rebellion Help
...
{
// MOD - START - Rebellion Help
	if ((GC.getGameINLINE().isOption(GAMEOPTION_NO_REBELLIONS) && (m_bRebellionRelated == true)))
	{
		m_szCachedHelp.clear();
...
bool CvInfoBase::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_bRebellionRelated, "bRebellionRelated");
...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   CvHotKeyClassInfo()
//
//  PURPOSE :   Default constructor
//
//------------------------------------------------------------------------------------------------------
CvHotKeyClassInfo::CvHotKeyClassInfo() :
m_iNumActions(0),
m_bCatchUnassigned(false),
m_bCatchAll(false),
m_piActions(NULL)
{
}
...
CvHotKeyClassInfo::~CvHotKeyClassInfo()
{
	SAFE_DELETE_ARRAY(m_piActions);
}

int CvHotKeyClassInfo::getNumActions() const
{
	return m_iNumActions;
}

bool CvHotKeyClassInfo::isCatchUnassigned() const
{
	return m_bCatchUnassigned;
}

bool CvHotKeyClassInfo::isCatchAll() const
{
	return m_bCatchAll;
}

int CvHotKeyClassInfo::getActions(int i) const
{
	FASSERT_BOUNDS(0, getNumActions(), i, "CvHotKeyClassInfo::getActions");
	return m_piActions ? m_piActions[i] : -1;
}

bool CvHotKeyClassInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_bCatchUnassigned, "bCatchUnassigned");
	pXML->GetChildXmlValByName(&m_bCatchAll, "bCatchAll");

	return true;
}

bool CvHotKeyClassInfo::readPass3()
{
	HotKeyClassTypes eHotKeyClass = (HotKeyClassTypes)GC.getInfoTypeForString(getType());

	std::vector<int> aActions;
	for(int i = 0; i < GC.getNumActionInfos(); i++)
	{
		const CvActionInfo& kAction = GC.getActionInfo(i);

		if (
			(isCatchAll() && ((kAction.getNumHotKeyClasses() > 0 ) || (kAction.getHotKeyVal() > -1))) ||
			(isCatchUnassigned() && kAction.getNumHotKeyClasses() == 0) && (kAction.getHotKeyVal() > -1))
		{
			if (kAction.getHotKeyVal() > -1)
			{
				aActions.push_back(i);
			}
		}
		else
		{
			for (int iMem = 0; iMem < kAction.getNumHotKeyClasses(); iMem++)
			{
				if (kAction.getHotKeyClasses(iMem) == eHotKeyClass)
				{
					aActions.push_back(i);
					break;
				}
	...
	m_iNumActions = (int)aActions.size();
	SAFE_DELETE_ARRAY(m_piActions);
	if (m_iNumActions > 0)
	{
		m_piActions = new int[m_iNumActions];
		int iRel = 0;
		for (std::vector<int>::const_iterator it=aActions.begin(); it!=aActions.end(); ++it)
		{
			m_piActions[iRel++] = *it;
		}
	}
...
{
	// MOD - START - Improved Civilopedia
	SAFE_DELETE_ARRAY(m_piHotKeyClasses);
	// MOD - END - Improved Civilopedia
}
...
bool CvHotkeyInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Improved Civilopedia
	pXML->SetInfoArray(&m_iNumHotKeyClasses, &m_piHotKeyClasses, "HotKeyClasses");
	// MOD - END - Improved Civilopedia
...
void CvHotkeyInfo::read(FDataStreamBase* pStream)
{
	...
	// MOD - START - Improved Civilopedia
	pStream->Read(&m_iNumHotKeyClasses);
	SAFE_DELETE_ARRAY(m_piHotKeyClasses);
	if (m_iNumHotKeyClasses > 0)
	{
		m_piHotKeyClasses = new int[m_iNumHotKeyClasses];
		pStream->Read(m_iNumHotKeyClasses, m_piHotKeyClasses);
	}
	// MOD - END - Improved Civilopedia
}
...
void CvHotkeyInfo::write(FDataStreamBase* pStream)
{
	...
	// MOD - START - Improved Civilopedia
	pStream->Write(m_iNumHotKeyClasses);
	if (m_iNumHotKeyClasses > 0)
	{
		pStream->Write(m_iNumHotKeyClasses, m_piHotKeyClasses);
	}
	// MOD - END - Improved Civilopedia
}
#endif
...
// MOD - START - Improved Civilopedia
int CvHotkeyInfo::getNumHotKeyClasses() const
{
	return m_iNumHotKeyClasses;
}

int CvHotkeyInfo::getHotKeyClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumHotKeyClasses(), i, "CvHotkeyInfo::getHotKeyClasses");
	return m_piHotKeyClasses ? m_piHotKeyClasses[i] : -1;
}
// MOD - END - Improved Civilopedia
...
m_iNumRelatedTraits(0),
m_iNumRelatedCivics(0),
m_iNumRelatedBuildingClasses(0),
...
// MOD - START - Relation Caching
m_piRelatedTraits(NULL),
m_piRelatedCivics(NULL),
m_piRelatedBuildingClasses(NULL),
// MOD - END - Relation Caching
m_iExperience(0)
{
}
...
CvSpecialistInfo::~CvSpecialistInfo()
{
	...
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piRelatedTraits);
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	// MOD - END - Relation Caching
}
...
// MOD - START - Relation Caching
int CvSpecialistInfo::getNumRelatedTraits() const
{
	return m_iNumRelatedTraits;
}

int CvSpecialistInfo::getNumRelatedCivics() const
{
	return m_iNumRelatedCivics;
}

int CvSpecialistInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int CvSpecialistInfo::getRelatedTraits(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedTraits(), i, "CvSpecialistInfo::getRelatedTraits");
	return m_piRelatedTraits ? m_piRelatedTraits[i] : -1;
}

int CvSpecialistInfo::getRelatedCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedCivics(), i, "CvSpecialistInfo::getRelatedCivics");
	return m_piRelatedCivics ? m_piRelatedCivics[i] : -1;
}

int CvSpecialistInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvSpecialistInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
void CvSpecialistInfo::cacheRelations()
{
	SpecialistTypes eSpecialist = (SpecialistTypes) GC.getInfoTypeForString(getType());

	// Traits
	std::set<TraitTypes> aRelatedTraits;
	for (int iTrait = 0; iTrait < GC.getNumTraitInfos(); iTrait++)
	{
		if (isTraitRelatedToSpecialist((TraitTypes)iTrait, eSpecialist))
		{
			aRelatedTraits.insert((TraitTypes)iTrait);
		}
	}

	m_iNumRelatedTraits = (int)aRelatedTraits.size();
	SAFE_DELETE_ARRAY(m_piRelatedTraits);
	if (m_iNumRelatedTraits > 0)
	{
		m_piRelatedTraits = new int[m_iNumRelatedTraits];
		int iRel = 0;
		for (std::set<TraitTypes>::const_iterator it=aRelatedTraits.begin(); it!=aRelatedTraits.end(); ++it)
		{
			m_piRelatedTraits[iRel++] = *it;
		}
	}
	aRelatedTraits.clear();

	// Civics
	std::set<CivicTypes> aRelatedCivics;
	for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
	{
		if (isCivicRelatedToSpecialist((CivicTypes)iCivic, eSpecialist))
		{
			aRelatedCivics.insert((CivicTypes)iCivic);
		}
	}

	m_iNumRelatedCivics = (int)aRelatedCivics.size();
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	if (m_iNumRelatedCivics > 0)
	{
		m_piRelatedCivics = new int[m_iNumRelatedCivics];
		int iRel = 0;
		for (std::set<CivicTypes>::const_iterator it=aRelatedCivics.begin(); it!=aRelatedCivics.end(); ++it)
		{
			m_piRelatedCivics[iRel++] = *it;
		}
	}
	aRelatedCivics.clear();

	// Buildings
	std::set<BuildingClassTypes> aRelatedBuildingClasses;
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isSpecialistRelatedToBuilding(eSpecialist, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}

	m_iNumRelatedBuildingClasses = (int)aRelatedBuildingClasses.size();
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	if (m_iNumRelatedBuildingClasses > 0)
	{
		m_piRelatedBuildingClasses = new int[m_iNumRelatedBuildingClasses];
		int iRel = 0;
		for (std::set<BuildingClassTypes>::const_iterator it=aRelatedBuildingClasses.begin(); it!=aRelatedBuildingClasses.end(); ++it)
		{
			m_piRelatedBuildingClasses[iRel++] = *it;
		}
	}
	aRelatedBuildingClasses.clear();
}
// MOD - END - Relation Caching
...
m_iGridY(0),
// MOD - START - Relation Caching
m_iNumRelatedBonuses(0),
m_iNumRelatedBuilds(0),
m_iNumRelatedImprovements(0),
m_iNumRelatedUnitClasses(0),
m_iNumRelatedBuildingClasses(0),
// MOD - END - Relation Caching
m_bRepeat(false),
m_bTrade(false),
...
m_piAllSpecialistCommerce(NULL),
// MOD - END - Advanced Yield and Commerce Effects
// MOD - START - Relation Caching
m_piRelatedBonuses(NULL),
m_piRelatedBuilds(NULL),
m_piRelatedImprovements(NULL),
m_piRelatedUnitClasses(NULL),
m_piRelatedBuildingClasses(NULL),
// MOD - END - Relation Caching
...
CvTechInfo::~CvTechInfo()
{
	...
	SAFE_DELETE_ARRAY(m_piAllSpecialistCommerce);
	// MOD - END - Advanced Yield and Commerce Effects
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piRelatedBonuses);
	SAFE_DELETE_ARRAY(m_piRelatedBuilds);
	SAFE_DELETE_ARRAY(m_piRelatedImprovements);
	SAFE_DELETE_ARRAY(m_piRelatedUnitClasses);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	// MOD - END - Relation Caching
...
int CvTechInfo::getAheadOfTime() const
{
	return m_iAheadOfTime;
}

void CvTechInfo::setAheadOfTime( const int newAheadOfTime )			
{
	m_iAheadOfTime = newAheadOfTime;
}
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvTechInfo::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}
// MOD - END - Epidemics
...
int CvTechInfo::getInsideCityCongestionThresholdChange() const
{
	return m_iInsideCityCongestionThresholdChange;
}

int CvTechInfo::getOutsideCityCongestionThresholdChange() const
{
	return m_iOutsideCityCongestionThresholdChange;
}
// MOD - END - Congestion
...
// MOD - START - Relation Caching
int CvTechInfo::getNumRelatedBonuses() const
{
	return m_iNumRelatedBonuses;
}

int CvTechInfo::getNumRelatedBuilds() const
{
	return m_iNumRelatedBuilds;
}

int CvTechInfo::getNumRelatedImprovements() const
{
	return m_iNumRelatedImprovements;
}

int CvTechInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvTechInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
int CvTechInfo::getAllSpecialistCommerce(int i) const
{
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, i, "CvTechInfo::getAllSpecialistCommerce");
	return m_piAllSpecialistCommerce ? m_piAllSpecialistCommerce[i] : -1;
}

int* CvTechInfo::getAllSpecialistCommerceArray() const
{
	return m_piAllSpecialistCommerce;
}
// MOD - END - Advanced Yield and Commerce Effects
// K-Mod end

// MOD - START - Relation Caching
int CvTechInfo::getRelatedBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBonuses(), i, "CvTechInfo::getRelatedBonuses");
	return m_piRelatedBonuses ? m_piRelatedBonuses[i] : -1;
}

int CvTechInfo::getRelatedBuilds(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuilds(), i, "CvTechInfo::getRelatedBuilds");
	return m_piRelatedBuilds ? m_piRelatedBuilds[i] : -1;
}

int CvTechInfo::getRelatedImprovements(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedImprovements(), i, "CvTechInfo::getRelatedImprovements");
	return m_piRelatedImprovements ? m_piRelatedImprovements[i] : -1;
}

int CvTechInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvTechInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvTechInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvTechInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
void CvTechInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	stream->Read(&m_iGlobalEpidemicModifier);
	// MOD - END - Epidemics
	stream->Read(&m_iFirstFreeTechs);
	stream->Read(&m_iAssetValue);
	stream->Read(&m_iPowerValue);
	// MOD - START - Congestion
	stream->Read(&m_iInsideCityCongestionThresholdChange);
	stream->Read(&m_iOutsideCityCongestionThresholdChange);
	// MOD - END - Congestion
	...
	if (uiFlag >= 1)
	{
		...
		*/
		SAFE_DELETE_ARRAY(m_piAllSpecialistCommerce);
		m_piAllSpecialistCommerce = new int[NUM_COMMERCE_TYPES];
		stream->Read(NUM_COMMERCE_TYPES, m_piAllSpecialistCommerce);
		// MOD - END - Advanced Yield and Commerce Effects
	}
	// K-Mod end

	// MOD - START - Relation Caching
	stream->Read(&m_iNumRelatedBonuses);
	SAFE_DELETE_ARRAY(m_piRelatedBonuses);
	if (m_iNumRelatedBonuses > 0)
	{
		m_piRelatedBonuses = new int[m_iNumRelatedBonuses];
		stream->Read(m_iNumRelatedBonuses, m_piRelatedBonuses);
	}

	stream->Read(&m_iNumRelatedBuilds);
	SAFE_DELETE_ARRAY(m_piRelatedBuilds);
	if (m_iNumRelatedBuilds > 0)
	{
		m_piRelatedBuilds = new int[m_iNumRelatedBuilds];
		stream->Read(m_iNumRelatedBuilds, m_piRelatedBuilds);
	}

	stream->Read(&m_iNumRelatedImprovements);
	SAFE_DELETE_ARRAY(m_piRelatedImprovements);
	if (m_iNumRelatedImprovements > 0)
	{
		m_piRelatedImprovements = new int[m_iNumRelatedImprovements];
		stream->Read(m_iNumRelatedImprovements, m_piRelatedImprovements);
	}

	stream->Read(&m_iNumRelatedUnitClasses);
	SAFE_DELETE_ARRAY(m_piRelatedUnitClasses);
	if (m_iNumRelatedUnitClasses > 0)
	{
		m_piRelatedUnitClasses = new int[m_iNumRelatedUnitClasses];
		stream->Read(m_iNumRelatedUnitClasses, m_piRelatedUnitClasses);
	}

	stream->Read(&m_iNumRelatedBuildingClasses);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	if (m_iNumRelatedBuildingClasses > 0)
	{
		m_piRelatedBuildingClasses = new int[m_iNumRelatedBuildingClasses];
		stream->Read(m_iNumRelatedBuildingClasses, m_piRelatedBuildingClasses);
	}
	// MOD - END - Relation Caching
...
void CvTechInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	stream->Write(m_iGlobalEpidemicModifier);
	// MOD - END - Epidemics
	stream->Write(m_iFirstFreeTechs);
	stream->Write(m_iAssetValue);
	stream->Write(m_iPowerValue);
	// MOD - START - Congestion
	stream->Write(m_iInsideCityCongestionThresholdChange);
	stream->Write(m_iOutsideCityCongestionThresholdChange);
	// MOD - END - Congestion
	...
	stream->Write(NUM_COMMERCE_TYPES, m_piAllSpecialistCommerce); // uiFlag >= 1
	// MOD - END - Advanced Yield and Commerce Effects

	// MOD - START - Relation Caching
	stream->Write(m_iNumRelatedBonuses);
	if (m_iNumRelatedBonuses > 0)
	{
		stream->Write(m_iNumRelatedBonuses, m_piRelatedBonuses);
	}

	stream->Write(m_iNumRelatedBuilds);
	if (m_iNumRelatedBuilds > 0)
	{
		stream->Write(m_iNumRelatedBuilds, m_piRelatedBuilds);
	}

	stream->Write(m_iNumRelatedImprovements);
	if (m_iNumRelatedImprovements > 0)
	{
		stream->Write(m_iNumRelatedImprovements, m_piRelatedImprovements);
	}

	stream->Write(m_iNumRelatedUnitClasses);
	if (m_iNumRelatedUnitClasses > 0)
	{
		stream->Write(m_iNumRelatedUnitClasses, m_piRelatedUnitClasses);
	}

	stream->Write(m_iNumRelatedBuildingClasses);
	if (m_iNumRelatedBuildingClasses > 0)
	{
		stream->Write(m_iNumRelatedBuildingClasses, m_piRelatedBuildingClasses);
	}
	// MOD - END - Relation Caching
...
bool CvTechInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	pXML->GetChildXmlValByName(&m_iGlobalEpidemicModifier, "iGlobalEpidemicModifier", 0);
	// MOD - END - Epidemics
	pXML->GetChildXmlValByName(&m_iFirstFreeTechs, "iFirstFreeTechs");
	pXML->GetChildXmlValByName(&m_iAssetValue, "iAsset");
	pXML->GetChildXmlValByName(&m_iPowerValue, "iPower");
	// MOD - START - Congestion
	pXML->GetChildXmlValByName(&m_iInsideCityCongestionThresholdChange, "iInsideCityCongestionThresholdChange");
	pXML->GetChildXmlValByName(&m_iOutsideCityCongestionThresholdChange, "iOutsideCityCongestionThresholdChange");
	// MOD - END - Congestion
	...
	*/
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"AllSpecialistCommerces"))
	{
		pXML->SetCommerce(&m_piAllSpecialistCommerce);
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
	}
	else
	{
		pXML->InitList(&m_piAllSpecialistCommerce, NUM_COMMERCE_TYPES);
	}
	// MOD - END - Advanced Yield and Commerce Effects
	// K-Mod end
...
// MOD - START - Relation Caching
void CvTechInfo::cacheRelations()
{
	TechTypes eTech = (TechTypes) GC.getInfoTypeForString(getType());

	// Bonuses
	std::set<BonusTypes> aRelatedBonuses;
	for (int iBonus = 0; iBonus < GC.getNumBonusInfos(); iBonus++)
	{
		if (isTechRelatedToBonus(eTech, (BonusTypes)iBonus))
		{
			aRelatedBonuses.insert((BonusTypes)iBonus);
		}
	}

	m_iNumRelatedBonuses = (int)aRelatedBonuses.size();
	SAFE_DELETE_ARRAY(m_piRelatedBonuses);
	if (m_iNumRelatedBonuses > 0)
	{
		m_piRelatedBonuses = new int[m_iNumRelatedBonuses];
		int iRel = 0;
		for (std::set<BonusTypes>::const_iterator it=aRelatedBonuses.begin(); it!=aRelatedBonuses.end(); ++it)
		{
			m_piRelatedBonuses[iRel++] = *it;
		}
	}
	aRelatedBonuses.clear();

	// Builds
	std::set<BuildTypes> aRelatedBuilds;
	for (int iBuild = 0; iBuild < GC.getNumBuildInfos(); iBuild++)
	{
		if (isTechRelatedToBuild(eTech, (BuildTypes)iBuild))
		{
			aRelatedBuilds.insert((BuildTypes)iBuild);
		}
	}

	m_iNumRelatedBuilds = (int)aRelatedBuilds.size();
	SAFE_DELETE_ARRAY(m_piRelatedBuilds);
	if (m_iNumRelatedBuilds > 0)
	{
		m_piRelatedBuilds = new int[m_iNumRelatedBuilds];
		int iRel = 0;
		for (std::set<BuildTypes>::const_iterator it=aRelatedBuilds.begin(); it!=aRelatedBuilds.end(); ++it)
		{
			m_piRelatedBuilds[iRel++] = *it;
		}
	}
	aRelatedBuilds.clear();

	// Improvements
	std::set<ImprovementTypes> aRelatedImprovements;
	for (int iImprovement = 0; iImprovement < GC.getNumImprovementInfos(); iImprovement++)
	{
		if (isTechRelatedToImprovement(eTech, (ImprovementTypes)iImprovement))
		{
			aRelatedImprovements.insert((ImprovementTypes)iImprovement);
		}
	}

	m_iNumRelatedImprovements = (int)aRelatedImprovements.size();
	SAFE_DELETE_ARRAY(m_piRelatedImprovements);
	if (m_iNumRelatedImprovements > 0)
	{
		m_piRelatedImprovements = new int[m_iNumRelatedImprovements];
		int iRel = 0;
		for (std::set<ImprovementTypes>::const_iterator it=aRelatedImprovements.begin(); it!=aRelatedImprovements.end(); ++it)
		{
			m_piRelatedImprovements[iRel++] = *it;
		}
	}
	aRelatedImprovements.clear();

	// Units
	std::set<UnitClassTypes> aRelatedUnitClasses;
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isTechRelatedToUnit(eTech, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}

	m_iNumRelatedUnitClasses = (int)aRelatedUnitClasses.size();
	SAFE_DELETE_ARRAY(m_piRelatedUnitClasses);
	if (m_iNumRelatedUnitClasses > 0)
	{
		m_piRelatedUnitClasses = new int[m_iNumRelatedUnitClasses];
		int iRel = 0;
		for (std::set<UnitClassTypes>::const_iterator it=aRelatedUnitClasses.begin(); it!=aRelatedUnitClasses.end(); ++it)
		{
			m_piRelatedUnitClasses[iRel++] = *it;
		}
	}
	aRelatedUnitClasses.clear();

	// Buildings
	std::set<BuildingClassTypes> aRelatedBuildingClasses;
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isTechRelatedToBuilding(eTech, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
m_iExtraBarbarianMaxXPValue(0),
// MOD - END - Advanced Experience Limits
// MOD - START - Extra Barbarian Combat Modifier
m_iExtraBarbarianCombatModifier(0),
// MOD - END - Extra Barbarian Combat Modifier
m_iKamikazePercent(0),
// MOD - START - Relation Caching
m_iNumRelatedDomains(0),
m_iNumRelatedTerrains(0),
m_iNumRelatedFeatures(0),
m_iNumRelatedUnitCombats(0),
m_iNumRelatedPromotions(0),
// MOD - END - Relation Caching
m_bLeader(false),
m_bBlitz(false),
m_bAmphib(false),
...
// MOD - START - Relation Caching
m_piRelatedDomains(NULL),
m_piRelatedTerrains(NULL),
m_piRelatedFeatures(NULL),
m_piRelatedUnitCombats(NULL),
m_piRelatedPromotions(NULL),
// MOD - END - Relation Caching
m_pbTerrainDoubleMove(NULL),
m_pbFeatureDoubleMove(NULL),
// MOD - START - Feature Damage Immunity
m_pbFeatureDamageImmunity(NULL),
...
CvPromotionInfo::~CvPromotionInfo()
{
	...
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piRelatedDomains);
	SAFE_DELETE_ARRAY(m_piRelatedTerrains);
	SAFE_DELETE_ARRAY(m_piRelatedFeatures);
	SAFE_DELETE_ARRAY(m_piRelatedUnitCombats);
	SAFE_DELETE_ARRAY(m_piRelatedPromotions);
	// MOD - END - Relation Caching
	SAFE_DELETE_ARRAY(m_pbTerrainDoubleMove);
	SAFE_DELETE_ARRAY(m_pbFeatureDoubleMove);
	// MOD - START - Feature Damage Immunity
	SAFE_DELETE_ARRAY(m_pbFeatureDamageImmunity);
	// MOD - END - Feature Damage Immunity
...
// MOD - START - Advanced Experience Limits
int CvPromotionInfo::getExtraBarbarianMaxXPValue() const
{
	return m_iExtraBarbarianMaxXPValue;
}
// MOD - END - Advanced Experience Limits

// MOD - START - Extra Barbarian Combat Modifier
int CvPromotionInfo::getExtraBarbarianCombatModifier() const
{
	return m_iExtraBarbarianCombatModifier;
}
// MOD - END - Extra Barbarian Combat Modifier
...
// MOD - START - Relation Caching
int CvPromotionInfo::getNumRelatedDomains() const
{
	return m_iNumRelatedDomains;
}

int CvPromotionInfo::getNumRelatedTerrains() const
{
	return m_iNumRelatedTerrains;
}

int CvPromotionInfo::getNumRelatedFeatures() const
{
	return m_iNumRelatedFeatures;
}

int CvPromotionInfo::getNumRelatedUnitCombats() const
{
	return m_iNumRelatedUnitCombats;
}

int CvPromotionInfo::getNumRelatedPromotions() const
{
	return m_iNumRelatedPromotions;
}
// MOD - END - Relation Caching
...
// MOD - START - Special Promotions
bool CvPromotionInfo::isSpecial() const
{
	return m_bSpecial;
}

bool CvPromotionInfo::isSpecialRequired() const
{
	if (getPrereqPromotion() != NO_PROMOTION)
	{
		const CvPromotionInfo& kPromotion = GC.getPromotionInfo((PromotionTypes)getPrereqPromotion());
		if (kPromotion.isSpecial() || kPromotion.isSpecialRequired())
		{
			return true;
		}
	}

	bool bSpecialRequired = false;

	if (getPrereqOrPromotion1() != NO_PROMOTION)
	{
		const CvPromotionInfo& kPromotion = GC.getPromotionInfo((PromotionTypes)getPrereqOrPromotion1());
		if (kPromotion.isSpecial() || kPromotion.isSpecialRequired())
		{
			bSpecialRequired = true;
		}
		else
		{
			return false;
	...
	if (getPrereqOrPromotion2() != NO_PROMOTION)
	{
		const CvPromotionInfo& kPromotion = GC.getPromotionInfo((PromotionTypes)getPrereqOrPromotion2());
		if (kPromotion.isSpecial() || kPromotion.isSpecialRequired())
		{
			bSpecialRequired = true;
		}
	...
	if (getPrereqOrPromotion3() != NO_PROMOTION)
	{
		const CvPromotionInfo& kPromotion = GC.getPromotionInfo((PromotionTypes)getPrereqOrPromotion3());
		if (kPromotion.isSpecial() || kPromotion.isSpecialRequired())
		{
			bSpecialRequired = true;
		}
	...
	return bSpecialRequired;
}
// MOD - END - Special Promotions

// MOD - START - Temporary Promotions
bool CvPromotionInfo::isTemporary() const
{
	return m_bTemporary;
}
// MOD - END - Temporary Promotions
...
// MOD - START - Relation Caching
int CvPromotionInfo::getRelatedDomains(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedDomains(), i, "CvPromotionInfo::getRelatedDomains");
	return m_piRelatedDomains ? m_piRelatedDomains[i] : -1;
}

int CvPromotionInfo::getRelatedTerrains(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedTerrains(), i, "CvPromotionInfo::getRelatedTerrains");
	return m_piRelatedTerrains ? m_piRelatedTerrains[i] : -1;
}

int CvPromotionInfo::getRelatedFeatures(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedFeatures(), i, "CvPromotionInfo::getRelatedFeatures");
	return m_piRelatedFeatures ? m_piRelatedFeatures[i] : -1;
}

int CvPromotionInfo::getRelatedUnitCombats(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitCombats(), i, "CvPromotionInfo::getRelatedUnitCombats");
	return m_piRelatedUnitCombats ? m_piRelatedUnitCombats[i] : -1;
}

int CvPromotionInfo::getRelatedPromotions(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedPromotions(), i, "CvPromotionInfo::getRelatedPromotions");
	return m_piRelatedPromotions ? m_piRelatedPromotions[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Feature Damage Immunity
bool CvPromotionInfo::getFeatureDamageImmunity(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumFeatureInfos(), i, "CvPromotionInfo::getFeatureDamageImmunity");
	return m_pbFeatureDamageImmunity ? m_pbFeatureDamageImmunity[i] : false;
}
// MOD - END - Feature Damage Immunity
...
void CvPromotionInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Advanced Experience Limits
	stream->Read(&m_iExtraBarbarianMaxXPValue);
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	stream->Read(&m_iExtraBarbarianCombatModifier);
	// MOD - END - Extra Barbarian Combat Modifier
	...
	// MOD - START - Special Promotions
	stream->Read(&m_bSpecial);
	// MOD - END - Special Promotions
	// MOD - START - Temporary Promotions
	stream->Read(&m_bTemporary);
	// MOD - END - Temporary Promotions
	...
	// MOD - START - Feature Damage Immunity
	SAFE_DELETE_ARRAY(m_pbFeatureDamageImmunity);
	m_pbFeatureDamageImmunity = new bool[GC.getNumFeatureInfos()];
	stream->Read(GC.getNumFeatureInfos(), m_pbFeatureDamageImmunity);
	// MOD - END - Feature Damage Immunity
...
void CvPromotionInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Advanced Experience Limits
	stream->Write(m_iExtraBarbarianMaxXPValue);
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	stream->Write(m_iExtraBarbarianCombatModifier);
	// MOD - END - Extra Barbarian Combat Modifier
	...
	// MOD - START - Special Promotions
	stream->Write(m_bSpecial);
	// MOD - END - Special Promotions
	// MOD - START - Temporary Promotions
	stream->Write(m_bTemporary);
	// MOD - END - Temporary Promotions
	...
	// MOD - START - Feature Damage Immunity
	stream->Write(GC.getNumFeatureInfos(), m_pbFeatureDamageImmunity);
	// MOD - END - Feature Damage Immunity
...
bool CvPromotionInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Special Promotions
	pXML->GetChildXmlValByName(&m_bSpecial, "bSpecial");
	// MOD - END - Special Promotions
	// MOD - START - Temporary Promotions
	pXML->GetChildXmlValByName(&m_bTemporary, "bTemporary");
	// MOD - END - Temporary Promotions
	...
	// MOD - START - Advanced Experience Limits
	pXML->GetChildXmlValByName(&m_iExtraBarbarianMaxXPValue, "iExtraBarbarianMaxXPValue");
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	pXML->GetChildXmlValByName(&m_iExtraBarbarianCombatModifier, "iExtraBarbarianCombatModifier");
	// MOD - END - Extra Barbarian Combat Modifier
	...
	// MOD - START - Feature Damage Immunity
	pXML->SetVariableListTagPair(&m_pbFeatureDamageImmunity, "FeatureDamageImmunities", sizeof(GC.getFeatureInfo((FeatureTypes)0)), GC.getNumFeatureInfos());
	// MOD - END - Feature Damage Immunity
...
// MOD - START - Relation Caching
void CvPromotionInfo::cacheRelations()
{
	PromotionTypes ePromotion = (PromotionTypes)GC.getInfoTypeForString(getType());

	// Domains
	std::set<DomainTypes> aRelatedDomains;
	for (int iDomain = 0; iDomain < NUM_DOMAIN_TYPES; iDomain++)
	{
		if (isPromotionRelatedToDomain(ePromotion, (DomainTypes)iDomain))
		{
			aRelatedDomains.insert((DomainTypes)iDomain);
		}
	}

	m_iNumRelatedDomains = (int)aRelatedDomains.size();
	SAFE_DELETE_ARRAY(m_piRelatedDomains);
	if (m_iNumRelatedDomains > 0)
	{
		m_piRelatedDomains = new int[m_iNumRelatedDomains];
		int iRel = 0;
		for (std::set<DomainTypes>::const_iterator it=aRelatedDomains.begin(); it!=aRelatedDomains.end(); ++it)
		{
			m_piRelatedDomains[iRel++] = *it;
		}
	}
	aRelatedDomains.clear();

	// Terrains
	std::set<TerrainTypes> aRelatedTerrains;
	for (int iTerrain = 0; iTerrain < GC.getNumTerrainInfos(); iTerrain++)
	{
		if (isPromotionRelatedToTerrain(ePromotion, (TerrainTypes)iTerrain))
		{
			aRelatedTerrains.insert((TerrainTypes)iTerrain);
		}
	}

	m_iNumRelatedTerrains = (int)aRelatedTerrains.size();
	SAFE_DELETE_ARRAY(m_piRelatedTerrains);
	if (m_iNumRelatedTerrains > 0)
	{
		m_piRelatedTerrains = new int[m_iNumRelatedTerrains];
		int iRel = 0;
		for (std::set<TerrainTypes>::const_iterator it=aRelatedTerrains.begin(); it!=aRelatedTerrains.end(); ++it)
		{
			m_piRelatedTerrains[iRel++] = *it;
		}
	}
	aRelatedTerrains.clear();

	// Features
	std::set<FeatureTypes> aRelatedFeatures;
	for (int iFeature = 0; iFeature < GC.getNumFeatureInfos(); iFeature++)
	{
		if (isPromotionRelatedToFeature(ePromotion, (FeatureTypes)iFeature))
		{
			aRelatedFeatures.insert((FeatureTypes)iFeature);
		}
	}

	m_iNumRelatedFeatures = (int)aRelatedFeatures.size();
	SAFE_DELETE_ARRAY(m_piRelatedFeatures);
	if (m_iNumRelatedFeatures > 0)
	{
		m_piRelatedFeatures = new int[m_iNumRelatedFeatures];
		int iRel = 0;
		for (std::set<FeatureTypes>::const_iterator it=aRelatedFeatures.begin(); it!=aRelatedFeatures.end(); ++it)
		{
			m_piRelatedFeatures[iRel++] = *it;
		}
	}
	aRelatedFeatures.clear();

	// Unit Combats
	std::set<UnitCombatTypes> aRelatedUnitCombats;
	for (int iUnitCombat = 0; iUnitCombat < GC.getNumUnitCombatInfos(); iUnitCombat++)
	{
		if (isPromotionRelatedToUnitCombat(ePromotion, (UnitCombatTypes)iUnitCombat))
		{
			aRelatedUnitCombats.insert((UnitCombatTypes)iUnitCombat);
		}
	}

	m_iNumRelatedUnitCombats = (int)aRelatedUnitCombats.size();
	SAFE_DELETE_ARRAY(m_piRelatedUnitCombats);
	if (m_iNumRelatedUnitCombats > 0)
	{
		m_piRelatedUnitCombats = new int[m_iNumRelatedUnitCombats];
		int iRel = 0;
		for (std::set<UnitCombatTypes>::const_iterator it=aRelatedUnitCombats.begin(); it!=aRelatedUnitCombats.end(); ++it)
		{
			m_piRelatedUnitCombats[iRel++] = *it;
		}
	}
	aRelatedUnitCombats.clear();

	// Promotions
	std::set<PromotionTypes> aRelatedPromotions;
	for (int iPromotion = 0; iPromotion < GC.getNumPromotionInfos(); iPromotion++)
	{
		if (isPromotionRelatedToPromotion(ePromotion, (PromotionTypes)iPromotion))
		{
			aRelatedPromotions.insert((PromotionTypes)iPromotion);
		}
	}

	m_iNumRelatedPromotions = (int)aRelatedPromotions.size();
	SAFE_DELETE_ARRAY(m_piRelatedPromotions);
	if (m_iNumRelatedPromotions > 0)
	{
		m_piRelatedPromotions = new int[m_iNumRelatedPromotions];
		int iRel = 0;
		for (std::set<PromotionTypes>::const_iterator it=aRelatedPromotions.begin(); it!=aRelatedPromotions.end(); ++it)
		{
			m_piRelatedPromotions[iRel++] = *it;
		}
	}
	aRelatedPromotions.clear();
}
// MOD - END - Relation Caching
...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   CvAidAndDetrimentInfoBase()
//
//  PURPOSE :   Default constructor
//
//------------------------------------------------------------------------------------------------------
CvAidAndDetrimentInfoBase::CvAidAndDetrimentInfoBase() :
...
bool CvAidAndDetrimentInfoBase::isCoastalOnly() const
{
	return m_bCoastalOnly;
}

int CvAidAndDetrimentInfoBase::getRange() const
{
	return m_iRange;
}

bool CvAidAndDetrimentInfoBase::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(&m_bCoastalOnly, "bCoastalOnly");
...
//------------------------------------------------------------------------------------------------------
CvAidInfo::CvAidInfo() :
m_eAidStyle(NO_AIDSTYLE),
m_iNumSupersededAidTypes(0),
m_iNumSupersededByAidTypes(0),
m_iNumPromotions(0),
m_piSupersededAidTypes(NULL),
m_piSupersededByAidTypes(NULL),
m_piPromotions(NULL)
{
}
...
CvAidInfo::~CvAidInfo()
{
	SAFE_DELETE_ARRAY(m_piSupersededAidTypes);
	SAFE_DELETE_ARRAY(m_piSupersededByAidTypes);
	SAFE_DELETE_ARRAY(m_piPromotions);
}

AidStyleTypes CvAidInfo::getAidStyleType() const
{
	return m_eAidStyle;
}

int CvAidInfo::getNumSupersededAidTypes() const
{
	return m_iNumSupersededAidTypes;
}

int CvAidInfo::getNumSupersededByAidTypes() const
{
	return m_iNumSupersededByAidTypes;
}

int CvAidInfo::getNumPromotions() const
{
...
int CvAidInfo::getSupersededAidTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumSupersededAidTypes(), i, "CvAidInfo::getSupersededAidTypes");
	return m_piSupersededAidTypes ? m_piSupersededAidTypes[i] : -1;
}

int CvAidInfo::getSupersededByAidTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumSupersededByAidTypes(), i, "CvAidInfo::getSupersededByAidTypes");
	return m_piSupersededByAidTypes ? m_piSupersededByAidTypes[i] : -1;
}


int CvAidInfo::getPromotions(int i) const
{
	FASSERT_BOUNDS(0, getNumPromotions(), i, "CvAidInfo::getPromotions");
	return m_piPromotions ? m_piPromotions[i] : -1;
}

bool CvAidInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvAidAndDetrimentInfoBase::read(pXML))
	{
		return false;
	}
	...
	pXML->GetChildXmlValByName(szTextVal, "AidStyleType");
	m_eAidStyle = (AidStyleTypes)(pXML->FindInInfoClass(szTextVal));

	pXML->SetInfoArray(&m_iNumPromotions, &m_piPromotions, "Promotions");

	return true;
}

bool CvAidInfo::readPass2(CvXMLLoadUtility* pXML)
{
	pXML->SetInfoArray(&m_iNumSupersededAidTypes, &m_piSupersededAidTypes, "SupersedesAidTypes");

	return true;
}

bool CvAidInfo::readPass3()
{
	AidTypes eAidType = (AidTypes)GC.getInfoTypeForString(getType());
	
	std::vector<int> aSupersededBy;

	m_iNumSupersededByAidTypes = 0;
	SAFE_DELETE_ARRAY(m_piSupersededByAidTypes);

	for (int i = 0; i < GC.getNumAidInfos(); i++)
	{
		const AidTypes eLoopAid = (AidTypes)i;
		const CvAidInfo& kLoopAid = GC.getAidInfo(eLoopAid);
		for (int s = 0; s < kLoopAid.getNumSupersededAidTypes(); s++)
		{
			if (kLoopAid.getSupersededAidTypes(s) == eAidType)
			{
				aSupersededBy.push_back(eLoopAid);
				break;
			}
		}
	}

	if (aSupersededBy.size() > 0)
	{
		m_iNumSupersededByAidTypes = (int)aSupersededBy.size();
		m_piSupersededByAidTypes = new int[aSupersededBy.size()];
		for (uint i = 0; i < aSupersededBy.size(); i++)
		{
			m_piSupersededByAidTypes[i] = aSupersededBy[i];
		}
	}
...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   CvDetrimentInfo()
//
//  PURPOSE :   Default constructor
//
//------------------------------------------------------------------------------------------------------
CvDetrimentInfo::CvDetrimentInfo() :
m_eDetrimentStyle(NO_DETRIMENTSTYLE),
m_iNumSupersededDetrimentTypes(0),
m_iNumSupersededByDetrimentTypes(0),
m_iNumPromotions(0),
m_piSupersededDetrimentTypes(NULL),
m_piSupersededByDetrimentTypes(NULL),
m_piPromotions(NULL)
{
}
...
CvDetrimentInfo::~CvDetrimentInfo()
{
	SAFE_DELETE_ARRAY(m_piSupersededDetrimentTypes);
	SAFE_DELETE_ARRAY(m_piSupersededByDetrimentTypes);
	SAFE_DELETE_ARRAY(m_piPromotions);
}

DetrimentStyleTypes CvDetrimentInfo::getDetrimentStyleType() const
{
	return m_eDetrimentStyle;
}

int CvDetrimentInfo::getNumSupersededDetrimentTypes() const
{
	return m_iNumSupersededDetrimentTypes;
}

int CvDetrimentInfo::getNumSupersededByDetrimentTypes() const
{
	return m_iNumSupersededByDetrimentTypes;
}

int CvDetrimentInfo::getNumPromotions() const
{
	return m_iNumPromotions;
}
...
int CvDetrimentInfo::getSupersededDetrimentTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumSupersededDetrimentTypes(), i, "CvDetrimentInfo::getSupersededDetrimentTypes");
	return m_piSupersededDetrimentTypes ? m_piSupersededDetrimentTypes[i] : -1;
}

int CvDetrimentInfo::getSupersededByDetrimentTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumSupersededByDetrimentTypes(), i, "CvDetrimentInfo::getSupersededByDetrimentTypes");
	return m_piSupersededByDetrimentTypes ? m_piSupersededByDetrimentTypes[i] : -1;
}

int CvDetrimentInfo::getPromotions(int i) const
{
	FASSERT_BOUNDS(0, getNumPromotions(), i, "CvDetrimentInfo::getPromotions");
	return m_piPromotions ? m_piPromotions[i] : -1;
}

bool CvDetrimentInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvAidAndDetrimentInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(szTextVal, "DetrimentStyleType");
	m_eDetrimentStyle = (DetrimentStyleTypes)(pXML->FindInInfoClass(szTextVal));
...
bool CvDetrimentInfo::readPass2(CvXMLLoadUtility* pXML)
{
	pXML->SetInfoArray(&m_iNumSupersededDetrimentTypes, &m_piSupersededDetrimentTypes, "SupersedesDetrimentTypes");

	return true;
}

bool CvDetrimentInfo::readPass3()
{
	DetrimentTypes eDetrimentType = (DetrimentTypes)GC.getInfoTypeForString(getType());
	...
	for (int i = 0; i < GC.getNumDetrimentInfos(); i++)
	{
		const DetrimentTypes eLoopDetriment = (DetrimentTypes)i;
		const CvDetrimentInfo& kLoopDetriment = GC.getDetrimentInfo(eLoopDetriment);
		for (int s = 0; s < kLoopDetriment.getNumSupersededDetrimentTypes(); s++)
		{
			if (kLoopDetriment.getSupersededDetrimentTypes(s) == eDetrimentType)
			{
				aSupersededBy.push_back(eLoopDetriment);
				break;
			}
		}
	}
	...
	if (aSupersededBy.size() > 0)
	{
		m_iNumSupersededByDetrimentTypes = (int)aSupersededBy.size();
		m_piSupersededByDetrimentTypes = new int[aSupersededBy.size()];
		for (uint i = 0; i < aSupersededBy.size(); i++)
		{
			m_piSupersededByDetrimentTypes[i] = aSupersededBy[i];
		}
	}
...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   CvCongestionInfo()
//
//  PURPOSE :   Default constructor
//
//------------------------------------------------------------------------------------------------------
CvCongestionInfo::CvCongestionInfo() :
m_iNumDomains(0),
m_iMinUnits(0),
m_iMaxUnits(MAX_UNSIGNED_SHORT),
m_bApplyInsideCity(false),
m_bApplyOutsideCity(false),
m_ePromotionType(NO_PROMOTION),
m_eColorType(NO_COLOR),
m_piDomains(NULL)
{
	for (int iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
	{
		m_abDomains[iI] = false;
	}
}
...
CvCongestionInfo::~CvCongestionInfo()
{
	SAFE_DELETE_ARRAY(m_piDomains);
}

int CvCongestionInfo::getNumDomains() const
{
	return m_iNumDomains;
}

int CvCongestionInfo::getDomains(int i) const
{
	FASSERT_BOUNDS(0, getNumDomains(), i, "CvCongestionInfo::getDomains");
	return m_piDomains ? m_piDomains[i] : -1;
}

int CvCongestionInfo::getMinUnits() const
{
	return m_iMinUnits;
}

int CvCongestionInfo::getMaxUnits() const
{
	return m_iMaxUnits;
}

bool CvCongestionInfo::isDomain(int i) const
{
	return m_abDomains[i];
}

bool CvCongestionInfo::isAppliedInsideCities() const
{
	return m_bApplyInsideCity;
}

bool CvCongestionInfo::isAppliedOutsideCities() const
{
	return m_bApplyOutsideCity;
}

bool CvCongestionInfo::isApplicable(int iDomain, bool bInsideCity) const
{
	if (!isDomain((DomainTypes)iDomain))
	{
		return false;
	}

	if (!isAppliedInsideCities() && bInsideCity)
	{
		return false;
	}
			
	if (!isAppliedOutsideCities() && !bInsideCity)
	{
		return false;
	}
...
PromotionTypes CvCongestionInfo::getPromotionType() const
{
	return m_ePromotionType;
}

ColorTypes CvCongestionInfo::getColorType() const
{
	return m_eColorType;
}

bool CvCongestionInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->SetInfoArray(&m_iNumDomains, &m_piDomains, "DomainTypes");
	for (int iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
	{
		m_abDomains[iI] = false;
	}
	for (int iI = 0; iI < m_iNumDomains; iI++)
	{
		m_abDomains[m_piDomains[iI]] = true;
	}

	pXML->GetChildXmlValByName(&m_iMinUnits, "iMinUnits");
	pXML->GetChildXmlValByName(&m_iMaxUnits, "iMaxUnits");
	if (m_iMaxUnits < 0 || m_iMaxUnits > MAX_UNSIGNED_SHORT)
	{
		m_iMaxUnits = MAX_UNSIGNED_SHORT;
	}

	pXML->GetChildXmlValByName(&m_bApplyInsideCity, "bApplyInsideCity");
	pXML->GetChildXmlValByName(&m_bApplyOutsideCity, "bApplyOutsideCity");

	CvString szTextVal;
	if (pXML->GetChildXmlValByName(szTextVal, "PromotionType"))
	{
		m_ePromotionType = (PromotionTypes)pXML->FindInInfoClass(szTextVal);
	}

	if (pXML->GetChildXmlValByName(szTextVal, "ColorType"))
	{
		m_eColorType = (ColorTypes)pXML->FindInInfoClass(szTextVal);
	}
...
// MOD - START - Improved Civilopedia
int CvActionInfo::getNumHotKeyClasses() const
{
	if (getHotkeyInfo())
	{
		return getHotkeyInfo()->getNumHotKeyClasses();
	}

	return 0;
}

int CvActionInfo::getHotKeyClasses(int i) const
{
	if (getHotkeyInfo())
	{
		return getHotkeyInfo()->getHotKeyClasses(i);
	}

	return NO_HOTKEYCLASS;
}
// MOD - END - Improved Civilopedia
...
m_iUnitPromoCombatType(NO_UNITCOMBAT),
...
m_iNumPrereqAndBuildingClasses(0),
m_iNumPrereqOrBuildingClasses(0),
m_iNumPrereqAndTechs(0),
m_iNumPrereqAndBonuses(0),
m_iNumPrereqOrBonuses(0),
// MOD - END - Advanced Unit Prerequisites
// MOD - START - Relation Caching
m_iNumRelatedReligions(0),
m_iNumRelatedCivics(0),
m_iNumRelatedDomains(0),
m_iNumRelatedTerrains(0),
m_iNumRelatedFeatures(0),
...
m_bRemoveNonStateReligion(false),
// MOD - END - Inquisition
...
// MOD - START - Advanced Unit Prerequisites
m_piPrereqAndBuildingClasses(NULL),
m_piPrereqOrBuildingClasses(NULL),
m_piPrereqAndTechs(NULL),
...
// MOD - START - Slavery
m_iBuildKill(0)				// chance to consume unit after improvement built
// MOD - END - Slavery
{
}
...
CvUnitInfo::~CvUnitInfo()
{
	...
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piRelatedReligions);
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	SAFE_DELETE_ARRAY(m_piRelatedDomains);
	SAFE_DELETE_ARRAY(m_piRelatedTerrains);
	SAFE_DELETE_ARRAY(m_piRelatedFeatures);
	...
	// MOD - START - Advanced Unit Prerequisites
	SAFE_DELETE_ARRAY(m_piPrereqAndBuildingClasses);
	SAFE_DELETE_ARRAY(m_piPrereqOrBuildingClasses);
	SAFE_DELETE_ARRAY(m_piPrereqAndTechs);
...
// MOD - START - Unit AI
int CvUnitInfo::getMaxTechEra() const
{
	int iMaxEra = 0;
	for (int iPre = 0; iPre < getNumPrereqAndTechs(); iPre++)
	{
		int iEra = GC.getTechInfo((TechTypes)getPrereqAndTechs(iPre)).getEra();
		iMaxEra = std::max(iMaxEra, iEra);
	}
	return iMaxEra;
}
// MOD - END - Unit AI
...
int CvUnitInfo::getUnitPromoType() const
{
	return m_iUnitPromoCombatType;
}
...
// MOD - START - Advanced Unit Prerequisites
// Replaced with new array methods
/*
...
int CvUnitInfo::getPrimaryPrereqTech() const
{
	return getNumPrereqAndTechs() > 0 ? getPrereqAndTechs(0) : NO_TECH;
}
// MOD - END - Advanced Unit Prerequisites

// MOD - START - Unit Global Tech
int CvUnitInfo::getGlobalTech() const
{
	return m_iGlobalTech;
}
// MOD - END - Unit Global Tech
...
// MOD - START - Advanced Unit Prerequisites
int CvUnitInfo::getNumPrereqAndBuildingClasses() const
{
	return m_iNumPrereqAndBuildingClasses;
}

int CvUnitInfo::getNumPrereqOrBuildingClasses() const
{
	return m_iNumPrereqOrBuildingClasses;
}

int CvUnitInfo::getNumPrereqAndTechs() const
{
	return m_iNumPrereqAndTechs;
}

int CvUnitInfo::getNumPrereqAndBonuses() const
{
	return m_iNumPrereqAndBonuses;
}

int CvUnitInfo::getNumPrereqOrBonuses() const
{
	return m_iNumPrereqOrBonuses;
}
// MOD - END - Advanced Unit Prerequisites

// MOD - START - Relation Caching
int CvUnitInfo::getNumRelatedReligions() const
{
	return m_iNumRelatedReligions;
}

int CvUnitInfo::getNumRelatedCivics() const
{
	return m_iNumRelatedCivics;
}

int CvUnitInfo::getNumRelatedDomains() const
{
	return m_iNumRelatedDomains;
}

int CvUnitInfo::getNumRelatedTerrains() const
{
	return m_iNumRelatedTerrains;
}

int CvUnitInfo::getNumRelatedFeatures() const
{
	return m_iNumRelatedFeatures;
}

int CvUnitInfo::getNumRelatedBonuses() const
{
	return m_iNumRelatedBonuses;
}

int CvUnitInfo::getNumRelatedBuilds() const
{
	return m_iNumRelatedBuilds;
}

int CvUnitInfo::getNumRelatedUnitCombats() const
{
	return m_iNumRelatedUnitCombats;
}

int CvUnitInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvUnitInfo::getNumRelatedPromotions() const
{
	return m_iNumRelatedPromotions;
}

int CvUnitInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching

bool CvUnitInfo::isIgnoreUpgrades() const
{
	return m_bIgnoreUpgrades;
}
...
// MOD - START - Unique Units
bool CvUnitInfo::isUniqueUnit() const
{
	return m_bUniqueUnit;
}
// MOD - END -  Unique Units
...
// MOD - START - Untransportable Units
bool CvUnitInfo::isNoTransport() const
{
	return m_bNoTransport;
}
// MOD - END - Untransportable Units
...
bool CvUnitInfo::isStrategic() const
{
	return m_bStrategic;
}
...
// MOD - START - Inquisition
bool CvUnitInfo::isRemoveNonStateReligion() const
{
	return m_bRemoveNonStateReligion;
}
// MOD - END - Inquisition

// MOD - START - Player Build Caching
bool CvUnitInfo::isBuilder() const
{
	return m_bBuilder;
}
// MOD - END - Player Build Caching
...
// MOD - START - Slavery
int CvUnitInfo::getBuildKillChance() const
{
	return m_iBuildKill;
}
// MOD - END - Slavery
...
// MOD - START - Advanced Unit Prerequisites
int CvUnitInfo::getPrereqAndBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndBuildingClasses(), i, "CvUnitInfo::getPrereqAndBuildingClasses");
	return m_piPrereqAndBuildingClasses ? m_piPrereqAndBuildingClasses[i] : -1;
}

int CvUnitInfo::getPrereqOrBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqOrBuildingClasses(), i, "CvUnitInfo::getPrereqOrBuildingClasses");
	return m_piPrereqOrBuildingClasses ? m_piPrereqOrBuildingClasses[i] : -1;
}

int CvUnitInfo::getPrereqAndTechs(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndTechs(), i, "CvUnitInfo::getPrereqAndTechs");
	return m_piPrereqAndTechs ? m_piPrereqAndTechs[i] : -1;
}

int CvUnitInfo::getPrereqAndBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndBonuses(), i, "CvUnitInfo::getPrereqAndBonuses");
	return m_piPrereqAndBonuses ? m_piPrereqAndBonuses[i] : -1;
}

int CvUnitInfo::getPrereqOrBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqOrBonuses(), i, "CvUnitInfo::getPrereqOrBonuses");
...
// MOD - START - Relation Caching
int CvUnitInfo::getRelatedReligions(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedReligions(), i, "CvUnitInfo::getRelatedReligions");
	return m_piRelatedReligions ? m_piRelatedReligions[i] : -1;
}

int CvUnitInfo::getRelatedCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedCivics(), i, "CvUnitInfo::getRelatedCivics");
	return m_piRelatedCivics ? m_piRelatedCivics[i] : -1;
}

int CvUnitInfo::getRelatedDomains(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedDomains(), i, "CvUnitInfo::getRelatedDomains");
	return m_piRelatedDomains ? m_piRelatedDomains[i] : -1;
}

int CvUnitInfo::getRelatedTerrains(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedTerrains(), i, "CvUnitInfo::getRelatedTerrains");
	return m_piRelatedTerrains ? m_piRelatedTerrains[i] : -1;
}

int CvUnitInfo::getRelatedFeatures(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedFeatures(), i, "CvUnitInfo::getRelatedFeatures");
	return m_piRelatedFeatures ? m_piRelatedFeatures[i] : -1;
}

int CvUnitInfo::getRelatedBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBonuses(), i, "CvUnitInfo::getRelatedBonuses");
	return m_piRelatedBonuses ? m_piRelatedBonuses[i] : -1;
}

int CvUnitInfo::getRelatedBuilds(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuilds(), i, "CvUnitInfo::getRelatedBuilds");
	return m_piRelatedBuilds ? m_piRelatedBuilds[i] : -1;
}

int CvUnitInfo::getRelatedUnitCombats(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitCombats(), i, "CvUnitInfo::getRelatedUnitCombats");
	return m_piRelatedUnitCombats ? m_piRelatedUnitCombats[i] : -1;
}

int CvUnitInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvUnitInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvUnitInfo::getRelatedPromotions(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedPromotions(), i, "CvUnitInfo::getRelatedPromotions");
	return m_piRelatedPromotions ? m_piRelatedPromotions[i] : -1;
}

int CvUnitInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvUnitInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Feature Damage Immunity
bool CvUnitInfo::getFeatureDamageImmunity(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumFeatureInfos(), i, "CvUnitInfo::getFeatureDamageImmunity");
	return m_pbFeatureDamageImmunity ? m_pbFeatureDamageImmunity[i] : false;
}
// MOD - END - Feature Damage Immunity
...
const std::vector<UnitClassTypes>& CvUnitInfo::getUpgradeUnitClassTypes() const
{
...
const TCHAR* CvUnitInfo::getStyledButton(int i) const
{
	CvString szStyledButton; 
	szStyledButton = getArtInfo(0, NO_ERA, (UnitArtStyleTypes) GC.getCivilizationInfo((CivilizationTypes)i).getUnitArtStyleType())->getButton();
	return szStyledButton;
}
#if SERIALIZE_CVINFOS
void CvUnitInfo::read(FDataStreamBase* stream)
{
	...
	if ((GC.getGameINLINE().isOption(GAMEOPTION_NO_REBELLIONS) && (m_bRebellionRelated)))
	{
		m_bGraphicalOnly = true;  // don't show in Civilopedia rebel units if rebellions are turned off
	}	
	...
	stream->Read(&m_iUnitPromoCombatType);	
	...
	m_aiUpgradeUnitClassTypes.clear();
	m_aiUpgradeUnitClassTypes.reserve(iNumUpgradeUnitClassTypes);
	for(int i=0; i<iNumUpgradeUnitClassTypes;i++)
	{
		int iUnitClassType;
		stream->Read(&iUnitClassType);
		m_aiUpgradeUnitClassTypes.push_back((UnitClassTypes)iUnitClassType);
	}
	// MOD - END - Unit Upgrade Cache
	...
	stream->Read(&m_iCommandType);
	// MOD - START - Slavery
	stream->Read(&m_iBuildKill);
	// MOD - END - Slavery
	stream->Read(&m_bIgnoreUpgrades);
	stream->Read(&m_bAnimal);
	// MOD - START - Unique Units
	stream->Read(&m_bUniqueUnit);
	// MOD - END -  Unique Units
	...
	stream->Read(&m_bStrategic);	
	stream->Read(&m_bNoRevealMap);
	// MOD - START - Inquisition
	stream->Read(&m_bRemoveNonStateReligion);
	// MOD - END - Inquisition
	// MOD - START - Player Build Caching
	stream->Read(&m_bBuilder);
	// MOD - END - Player Build Caching
	...
	// MOD - START - Advanced Unit Prerequisites
	stream->Read(&m_iNumPrereqAndBuildingClasses);
	SAFE_DELETE_ARRAY(m_piPrereqAndBuildingClasses);
	if (m_iNumPrereqAndBuildingClasses > 0)
	{
		m_piPrereqAndBuildingClasses = new int[m_iNumPrereqAndBuildingClasses];
		stream->Read(m_iNumPrereqAndBuildingClasses, m_piPrereqAndBuildingClasses);
	}
	
	stream->Read(&m_iNumPrereqOrBuildingClasses);
	SAFE_DELETE_ARRAY(m_piPrereqOrBuildingClasses);
	if (m_piPrereqOrBuildingClasses > 0)
	{
		m_piPrereqOrBuildingClasses = new int[m_iNumPrereqOrBuildingClasses];
		stream->Read(m_iNumPrereqOrBuildingClasses, m_piPrereqOrBuildingClasses);
	}
	...
	*/
	stream->Read(&m_iNumPrereqAndTechs);
	SAFE_DELETE_ARRAY(m_piPrereqAndTechs);
	if (m_iNumPrereqAndTechs > 0)
	{
		m_piPrereqAndTechs = new int[m_iNumPrereqAndTechs];
		stream->Read(m_iNumPrereqAndTechs, m_piPrereqAndTechs);
	}

	stream->Read(&m_iNumPrereqAndBonuses);
	SAFE_DELETE_ARRAY(m_piPrereqAndBonuses);
	if (m_iNumPrereqAndBonuses > 0)
	{
		m_piPrereqAndBonuses = new int[m_iNumPrereqAndBonuses];
		stream->Read(m_iNumPrereqAndBonuses, m_piPrereqAndBonuses);
	}
	...
	*/
	stream->Read(&m_iNumPrereqOrBonuses);
	SAFE_DELETE_ARRAY(m_piPrereqOrBonuses);
	if (m_iNumPrereqOrBonuses > 0)
	{
		m_piPrereqOrBonuses = new int[m_iNumPrereqOrBonuses];
		stream->Read(m_iNumPrereqOrBonuses, m_piPrereqOrBonuses);
	}
	// MOD - END - Advanced Unit Prerequisites
	...
	// MOD - START - Relation Caching
	stream->Read(&m_iNumRelatedReligions);
	SAFE_DELETE_ARRAY(m_piRelatedReligions);
	if (m_iNumRelatedReligions > 0)
	{
		m_piRelatedReligions = new int[m_iNumRelatedReligions];
		stream->Read(m_iNumRelatedReligions, m_piRelatedReligions);
	}

	stream->Read(&m_iNumRelatedCivics);
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	if (m_iNumRelatedCivics > 0)
	{
		m_piRelatedCivics = new int[m_iNumRelatedCivics];
		stream->Read(m_iNumRelatedCivics, m_piRelatedCivics);
	}

	stream->Read(&m_iNumRelatedDomains);
	SAFE_DELETE_ARRAY(m_piRelatedDomains);
	if (m_iNumRelatedDomains > 0)
	{
		m_piRelatedDomains = new int[m_iNumRelatedDomains];
		stream->Read(m_iNumRelatedDomains, m_piRelatedDomains);
	}

	stream->Read(&m_iNumRelatedTerrains);
	SAFE_DELETE_ARRAY(m_piRelatedTerrains);
	if (m_iNumRelatedTerrains > 0)
	{
		m_piRelatedTerrains = new int[m_iNumRelatedTerrains];
		stream->Read(m_iNumRelatedTerrains, m_piRelatedTerrains);
	}

	stream->Read(&m_iNumRelatedFeatures);
	SAFE_DELETE_ARRAY(m_piRelatedFeatures);
	if (m_iNumRelatedFeatures > 0)
	{
		m_piRelatedFeatures = new int[m_iNumRelatedFeatures];
		stream->Read(m_iNumRelatedFeatures, m_piRelatedFeatures);
	}
	...
	stream->Read(&m_iNumRelatedUnitCombats);
	SAFE_DELETE_ARRAY(m_piRelatedUnitCombats);
	if (m_iNumRelatedUnitCombats > 0)
	{
		m_piRelatedUnitCombats = new int[m_iNumRelatedUnitCombats];
		stream->Read(m_iNumRelatedUnitCombats, m_piRelatedUnitCombats);
	}
	...
	stream->Read(&m_iNumRelatedPromotions);
	SAFE_DELETE_ARRAY(m_piRelatedPromotions);
	if (m_iNumRelatedPromotions > 0)
	{
		m_piRelatedPromotions = new int[m_iNumRelatedPromotions];
		stream->Read(m_iNumRelatedPromotions, m_piRelatedPromotions);
	}
...
void CvUnitInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(m_iUnitPromoCombatType);	
	...
	{
		stream->Write((int)m_aiUpgradeUnitClassTypes[i]);
	}
	// MOD - END - Unit Upgrade Cache
	...
	stream->Write(m_iCommandType);
	// MOD - START - Slavery
	stream->Write(m_iBuildKill);
	// MOD - END - Slavery

	stream->Write(m_bIgnoreUpgrades);
	stream->Write(m_bAnimal);
	// MOD - START - Unique Units
	stream->Write(&m_bUniqueUnit);
	// MOD - END -  Unique Units
	...
	stream->Write(m_bStrategic);	
	stream->Write(m_bNoRevealMap);
	// MOD - START - Inquisition
	stream->Write(m_bRemoveNonStateReligion);
	// MOD - END - Inquisition
	// MOD - START - Player Build Caching
	stream->Write(m_bBuilder);
	// MOD - END - Player Build Caching
	...
	// MOD - START - Advanced Unit Prerequisites
	stream->Write(m_iNumPrereqAndBuildingClasses);
	if (m_iNumPrereqAndBuildingClasses > 0)
	{
		stream->Write(m_iNumPrereqAndBuildingClasses, m_piPrereqAndBuildingClasses);
	}

	stream->Write(m_iNumPrereqOrBuildingClasses);
	if (m_iNumPrereqOrBuildingClasses > 0)
	{
		stream->Write(m_iNumPrereqOrBuildingClasses, m_piPrereqOrBuildingClasses);
	}

	//stream->Write(GC.getNUM_UNIT_AND_TECH_PREREQS(), m_piPrereqAndTechs);
	stream->Write(m_iNumPrereqAndTechs);
	if (m_iNumPrereqAndTechs > 0)
	{
		stream->Write(m_iNumPrereqAndTechs, m_piPrereqAndTechs);
	}

	stream->Write(m_iNumPrereqAndBonuses);
	if (m_iNumPrereqAndBonuses > 0)
	{
		stream->Write(m_iNumPrereqAndBonuses, m_piPrereqAndBonuses);
	}

	//stream->Write(GC.getNUM_UNIT_PREREQ_OR_BONUSES(), m_piPrereqOrBonuses);
	stream->Write(m_iNumPrereqOrBonuses);
	if (m_iNumPrereqOrBonuses > 0)
	{
		stream->Write(m_iNumPrereqOrBonuses, m_piPrereqOrBonuses);
	}
	// MOD - END - Advanced Unit Prerequisites
	...
	// MOD - START - Relation Caching
	stream->Write(m_iNumRelatedReligions);
	if (m_iNumRelatedReligions > 0)
	{
		stream->Write(m_iNumRelatedReligions, m_piRelatedReligions);
	}

	stream->Write(m_iNumRelatedCivics);
	if (m_iNumRelatedCivics > 0)
	{
		stream->Write(m_iNumRelatedCivics, m_piRelatedCivics);
	}

	stream->Write(m_iNumRelatedDomains);
	if (m_iNumRelatedDomains > 0)
	{
		stream->Write(m_iNumRelatedDomains, m_piRelatedDomains);
	}

	stream->Write(m_iNumRelatedTerrains);
	if (m_iNumRelatedTerrains > 0)
	{
		stream->Write(m_iNumRelatedTerrains, m_piRelatedTerrains);
	}

	stream->Write(m_iNumRelatedFeatures);
	if (m_iNumRelatedFeatures > 0)
	{
		stream->Write(m_iNumRelatedFeatures, m_piRelatedFeatures);
	}
	...
	stream->Write(m_iNumRelatedUnitCombats);
	if (m_iNumRelatedUnitCombats > 0)
	{
		stream->Write(m_iNumRelatedUnitCombats, m_piRelatedUnitCombats);
	}
	...
	stream->Write(m_iNumRelatedPromotions);
	if (m_iNumRelatedPromotions > 0)
	{
		stream->Write(m_iNumRelatedPromotions, m_piRelatedPromotions);
	}
...
bool CvUnitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(szTextVal, "PromoCombat");
	m_iUnitPromoCombatType = pXML->FindInInfoClass(szTextVal);	
	...
	pXML->GetChildXmlValByName(&m_bIgnoreUpgrades, "bIgnoreUpgrades");
	pXML->GetChildXmlValByName(&m_bAnimal, "bAnimal");
	// MOD - START - Unique Units
	pXML->GetChildXmlValByName(&m_bUniqueUnit, "bUniqueUnit");
	// MOD - END -  Unique Units
	...
	// MOD - START - Untransportable Units
	pXML->GetChildXmlValByName(&m_bNoTransport, "bNoTransport");
	// MOD - END - Untransportable Units
	...
	pXML->GetChildXmlValByName(&m_bStrategic,"bStrategic",false);	
	pXML->GetChildXmlValByName(&m_bNoRevealMap,"bNoRevealMap",false);
	// MOD - START - Inquisition
	pXML->GetChildXmlValByName(&m_bRemoveNonStateReligion,"bRemoveNonStateReligion",false);
	// MOD - END - Inquisition
	...
	{
		if (getUpgradeUnitClass(i))
		{
			m_aiUpgradeUnitClassTypes.push_back((UnitClassTypes)i);
		}
	}
	// MOD - END - Unit Upgrade Cache
	...
	// MOD - END - Player Build Caching
	// MOD - START - Slavery
	pXML->GetChildXmlValByName(&m_iBuildKill,"iBuildKill",0);
	// MOD - END - SLavery
	...
	// MOD - START - Unit civic prereq
	if ( pXML->GetChildXmlValByName(szTextVal, "PrereqCivic") )
	{
	...
	// MOD - START - Advanced Unit Prerequisites
	// Note: The old PrereqBuilding, PrereqTech and BonusType tags are no longer supported
	pXML->SetInfoArray(&m_iNumPrereqAndBuildingClasses, &m_piPrereqAndBuildingClasses, "PrereqAndBuildings");
	pXML->SetInfoArray(&m_iNumPrereqOrBuildingClasses, &m_piPrereqOrBuildingClasses, "PrereqOrBuildings");
	pXML->SetInfoArray(&m_iNumPrereqAndTechs, &m_piPrereqAndTechs, "PrereqTechs");
	pXML->SetInfoArray(&m_iNumPrereqAndBonuses, &m_piPrereqAndBonuses, "PrereqAndBonuses");
	pXML->SetInfoArray(&m_iNumPrereqOrBonuses, &m_piPrereqOrBonuses, "PrereqOrBonuses");
	// MOD - END - Advanced Unit Prerequisites

	// MOD - START - Unit Global Tech
	pXML->GetChildXmlValByName(szTextVal, "GlobalTech", "NONE");
	m_iGlobalTech = pXML->FindInInfoClass(szTextVal);
	// MOD - END - Unit Global Tech
	...
	// MOD - START - Feature Damage Immunity
	pXML->SetVariableListTagPair(&m_pbFeatureDamageImmunity, "FeatureDamageImmunities", sizeof(GC.getFeatureInfo((FeatureTypes)0)), GC.getNumFeatureInfos(), false);
	// MOD - END - Feature Damage Immunity
...
// MOD - START - Relation Caching
void CvUnitInfo::cacheRelations()
{
	UnitTypes eUnit = (UnitTypes)GC.getInfoTypeForString(getType());

	// Religions
	std::set<ReligionTypes> aRelatedReligions;
	for (int iReligion = 0; iReligion < GC.getNumReligionInfos(); iReligion++)
	{
		if (isReligionRelatedToUnit((ReligionTypes)iReligion, eUnit))
		{
			aRelatedReligions.insert((ReligionTypes)iReligion);
		}
	}

	m_iNumRelatedReligions = (int)aRelatedReligions.size();
	SAFE_DELETE_ARRAY(m_piRelatedReligions);
	if (m_iNumRelatedReligions > 0)
	{
		m_piRelatedReligions = new int[m_iNumRelatedReligions];
		int iRel = 0;
		for (std::set<ReligionTypes>::const_iterator it=aRelatedReligions.begin(); it!=aRelatedReligions.end(); ++it)
		{
			m_piRelatedReligions[iRel++] = *it;
		}
	}
	aRelatedReligions.clear();

	// Civics
	std::set<CivicTypes> aRelatedCivics;
	for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
	{
		if (isCivicRelatedToUnit((CivicTypes)iCivic, eUnit))
		{
			aRelatedCivics.insert((CivicTypes)iCivic);
		}
	}
	...
	for (int iDomain = 0; iDomain < NUM_DOMAIN_TYPES; iDomain++)
	{
		if (isUnitRelatedToDomain(eUnit, (DomainTypes)iDomain))
		{
			aRelatedDomains.insert((DomainTypes)iDomain);
		}
	}
	...
	for (int iTerrain = 0; iTerrain < GC.getNumTerrainInfos(); iTerrain++)
	{
		if (isUnitRelatedToTerrain(eUnit, (TerrainTypes)iTerrain))
		{
			aRelatedTerrains.insert((TerrainTypes)iTerrain);
		}
	}
	...
	for (int iFeature = 0; iFeature < GC.getNumFeatureInfos(); iFeature++)
	{
		if (isUnitRelatedToFeature(eUnit, (FeatureTypes)iFeature))
		{
			aRelatedFeatures.insert((FeatureTypes)iFeature);
		}
	}
	...
	for (int iBonus = 0; iBonus < GC.getNumBonusInfos(); iBonus++)
	{
		if (isBonusRelatedToUnit((BonusTypes)iBonus, eUnit))
		{
			aRelatedBonuses.insert((BonusTypes)iBonus);
		}
	}
	...
	for (int iBuild = 0; iBuild < GC.getNumBuildInfos(); iBuild++)
	{
		if (isUnitRelatedToBuild(eUnit, (BuildTypes)iBuild))
		{
			aRelatedBuilds.insert((BuildTypes)iBuild);
		}
	}
	...
	for (int iUnitCombat = 0; iUnitCombat < GC.getNumUnitCombatInfos(); iUnitCombat++)
	{
		if (isUnitRelatedToUnitCombat(eUnit, (UnitCombatTypes)iUnitCombat))
		{
			aRelatedUnitCombats.insert((UnitCombatTypes)iUnitCombat);
		}
	}
	...
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isUnitRelatedToUnit(eUnit, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iPromotion = 0; iPromotion < GC.getNumPromotionInfos(); iPromotion++)
	{
		if (isUnitRelatedToPromotion(eUnit, (PromotionTypes)iPromotion))
		{
			aRelatedPromotions.insert((PromotionTypes)iPromotion);
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isBuildingRelatedToUnit((BuildingTypes)iBuilding, eUnit))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
// MOD - START - Congestion
m_iInsideCityCongestionThresholdChange(0),
m_iOutsideCityCongestionThresholdChange(0),
// MOD - END - Congestion
// MOD - START - Advanced Yield and Commerce Effects
m_iNumImprovementYieldChanges(0),
m_iNumSpecialistYieldChanges(0),
m_iNumSpecialistCommerceChanges(0),
// MOD - END - Advanced Yield and Commerce Effects
// MOD - START - Relation Caching
m_iNumRelatedFeatures(0),
m_iNumRelatedImprovements(0),
m_iNumRelatedSpecialists(0),
m_iNumRelatedUnitClasses(0),
m_iNumRelatedBuildingClasses(0),
// MOD - END - Relation Caching
...
m_bCanAcquireHolyCityWithoutStateReligion(false),
m_bCanMaintainHolyCityWithoutStateReligion(false),
...
m_paImprovementYieldChanges(NULL),
m_paSpecialistYieldChanges(NULL),
m_paSpecialistCommerceChanges(NULL)
// MOD - END - Advanced Yield and Commerce Effects
{
}
...
CvCivicInfo::~CvCivicInfo()
{
	...
	SAFE_DELETE_ARRAY(m_piRelatedFeatures);
	SAFE_DELETE_ARRAY(m_piRelatedImprovements);
	SAFE_DELETE_ARRAY(m_piRelatedSpecialists);
	SAFE_DELETE_ARRAY(m_piRelatedUnitClasses);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	// MOD - END - Relation Caching
	...
	SAFE_DELETE_ARRAY(m_paImprovementYieldChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistYieldChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistCommerceChanges);
	// MOD - END - Advanced Yield and Commerce Effects
}
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvCivicInfo::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}
// MOD - END - Epidemics
...
// MOD - START - Wonder production cost modifier
int CvCivicInfo::getWonderCostModifier() const
{
	return m_iWonderCostModifier;
}
// MOD - END - Wonder production cost modifier
...
// MOD - START - Congestion
int CvCivicInfo::getInsideCityCongestionThresholdChange() const
{
	return m_iInsideCityCongestionThresholdChange;
}

int CvCivicInfo::getOutsideCityCongestionThresholdChange() const
{
	return m_iOutsideCityCongestionThresholdChange;
}
// MOD - END - Congestion
...
// MOD - START - Advanced Yield and Commerce Effects
int CvCivicInfo::getNumImprovementYieldChanges() const
{
	return m_iNumImprovementYieldChanges;
}

int CvCivicInfo::getNumSpecialistYieldChanges() const
{
	return m_iNumSpecialistYieldChanges;
}

int CvCivicInfo::getNumSpecialistCommerceChanges() const
{
	return m_iNumSpecialistCommerceChanges;
}
// MOD - END - Advanced Yield and Commerce Effects

// MOD - START - Relation Caching
int CvCivicInfo::getNumRelatedFeatures() const
{
	return m_iNumRelatedFeatures;
}

int CvCivicInfo::getNumRelatedImprovements() const
{
	return m_iNumRelatedImprovements;
}

int CvCivicInfo::getNumRelatedSpecialists() const
{
	return m_iNumRelatedSpecialists;
}

int CvCivicInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvCivicInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
bool CvCivicInfo::isCommunist() const
{
	return m_bCommunist;
}
...
// MOD - START - Holy City Migration
bool CvCivicInfo::isCanAcquireHolyCityWithoutStateReligion() const
{
	return m_bCanAcquireHolyCityWithoutStateReligion;
}

bool CvCivicInfo::isCanMaintainHolyCityWithoutStateReligion() const
{
	return m_bCanMaintainHolyCityWithoutStateReligion;
}
// MOD - END - Holy City Migration
...
int CvCivicInfo::getAllSpecialistCommerce(int i) const
{
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, i, "CvBuildingInfo::getAllSpecialistCommerce");
	return m_piAllSpecialistCommerce ? m_piAllSpecialistCommerce[i] : -1;
}

int* CvCivicInfo::getAllSpecialistCommerceArray() const
{
	return m_piAllSpecialistCommerce;
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Advanced Yield and Commerce Effects
int CvCivicInfo::getImprovementYieldChangeImprovement(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvCivicInfo::getImprovementYieldChangeImprovement");
	return m_paImprovementYieldChanges[i].eImprovement;
}

int CvCivicInfo::getImprovementYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvCivicInfo::getImprovementYieldChangeYield");
	return m_paImprovementYieldChanges[i].eYield;
}

int CvCivicInfo::getImprovementYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvCivicInfo::getImprovementYieldChangeValue");
	return m_paImprovementYieldChanges[i].iChange;
}

int CvCivicInfo::getSpecialistYieldChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvCivicInfo::getSpecialistYieldChangeSpecialist");
	return m_paSpecialistYieldChanges[i].eSpecialist;
}

int CvCivicInfo::getSpecialistYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvCivicInfo::getSpecialistYieldChangeYield");
	return m_paSpecialistYieldChanges[i].eYield;
}

int CvCivicInfo::getSpecialistYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvCivicInfo::getSpecialistYieldChangeValue");
	return m_paSpecialistYieldChanges[i].iChange;
}

int CvCivicInfo::getSpecialistCommerceChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvCivicInfo::getSpecialistCommerceChangeSpecialist");
	return m_paSpecialistCommerceChanges[i].eSpecialist;
}

int CvCivicInfo::getSpecialistCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvCivicInfo::getSpecialistCommerceChangeCommerce");
	return m_paSpecialistCommerceChanges[i].eCommerce;
}

int CvCivicInfo::getSpecialistCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvCivicInfo::getSpecialistCommerceChangeValue");
	return m_paSpecialistCommerceChanges[i].iChange;
}
// MOD - END - Advanced Yield and Commerce Effects

// MOD - START - Relation Caching
int CvCivicInfo::getRelatedFeatures(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedFeatures(), i, "CvBuildingInfo::getRelatedFeatures");
	return m_piRelatedFeatures ? m_piRelatedFeatures[i] : -1;
}

int CvCivicInfo::getRelatedImprovements(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedImprovements(), i, "CvBuildingInfo::getRelatedImprovements");
	return m_piRelatedImprovements ? m_piRelatedImprovements[i] : -1;
}

int CvCivicInfo::getRelatedSpecialists(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedSpecialists(), i, "CvBuildingInfo::getRelatedSpecialists");
	return m_piRelatedSpecialists ? m_piRelatedSpecialists[i] : -1;
}

int CvCivicInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvBuildingInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvCivicInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvBuildingInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
int CvCivicInfo::getImprovementYieldChanges(int i, int j) const
{
	...
	return m_ppiImprovementYieldChanges ? m_ppiImprovementYieldChanges[i][j] : 0;
}

// MOD - START - Advanced Yield and Commerce Effects
int CvCivicInfo::getSpecialistYieldChanges(int i, int j) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), i, "CvCivicInfo::getSpecialistYieldChanges");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, j, "CvCivicInfo::getSpecialistYieldChanges");
	return m_ppiSpecialistYieldChanges ? m_ppiSpecialistYieldChanges[i][j] : 0;
}

int CvCivicInfo::getSpecialistCommerceChanges(int i, int j) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), i, "CvCivicInfo::getSpecialistCommerceChanges");
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, j, "CvCivicInfo::getSpecialistCommerceChanges");
	return m_ppiSpecialistCommerceChanges ? m_ppiSpecialistCommerceChanges[i][j] : 0;
}

void CvCivicInfo::getImprovementYieldChangeArray(ImprovementTypes eImprovement, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumImprovementYieldChanges(); iMem++)
	{
		if (getImprovementYieldChangeImprovement(iMem) == eImprovement)
		{
			piYieldChanges[getImprovementYieldChangeYield(iMem)] += getImprovementYieldChangeValue(iMem);
		}
	}
}

void CvCivicInfo::getSpecialistYieldChangeArray(SpecialistTypes eSpecialist, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumSpecialistYieldChanges(); iMem++)
	{
		if (getSpecialistYieldChangeSpecialist(iMem) == eSpecialist)
		{
			piYieldChanges[getSpecialistYieldChangeYield(iMem)] += getSpecialistYieldChangeValue(iMem);
		}
	}
}

void CvCivicInfo::getSpecialistCommerceChangeArray(SpecialistTypes eSpecialist, int* piCommerceChanges) const
{
	for (int iMem = 0; iMem < getNumSpecialistCommerceChanges(); iMem++)
	{
		if (getSpecialistCommerceChangeSpecialist(iMem) == eSpecialist)
		{
			piCommerceChanges[getSpecialistCommerceChangeCommerce(iMem)] += getSpecialistCommerceChangeValue(iMem);
		}
	}
}
// MOD - END - Advanced Yield and Commerce Effects
...
void CvCivicInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&m_iExtraHappiness); // K-Mod
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	stream->Read(&m_iGlobalEpidemicModifier);
	// MOD - END - Epidemics
	...
	// MOD - START - Wonder production cost modifier
	stream->Read(&m_iWonderCostModifier);
	// MOD - END - Wonder production cost modifier
	stream->Read(&m_iWorkerSpeedModifier);
	...
	stream->Read(&m_bCommunist);	
	stream->Read(&m_bNoNonStateReligionSpread);
	// MOD - START - Holy City Migration
	stream->Read(&m_bCanAcquireHolyCityWithoutStateReligion);
	stream->Read(&m_bCanMaintainHolyCityWithoutStateReligion);
	// MOD - END - Holy City Migration
	...
	*/
	...
	stream->Read(&m_iNumRelatedSpecialists);
	SAFE_DELETE_ARRAY(m_piRelatedSpecialists);
	if (m_iNumRelatedSpecialists > 0)
	{
		m_piRelatedSpecialists = new int[m_iNumRelatedSpecialists];
		stream->Read(m_iNumRelatedSpecialists, m_piRelatedSpecialists);
	}
	...
	for(int iI=0; iI < GC.getNumImprovementInfos(); iI++)
	{
		...
		stream->Read(NUM_YIELD_TYPES, m_ppiImprovementYieldChanges[iI]);
	}
	...
	for(int iI=0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		m_ppiSpecialistYieldChanges[iI]  = new int[NUM_YIELD_TYPES];
		stream->Read(NUM_YIELD_TYPES, m_ppiSpecialistYieldChanges[iI]);
	}
	...
	for(int iI=0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		m_ppiSpecialistCommerceChanges[iI]  = new int[NUM_COMMERCE_TYPES];
		stream->Read(NUM_COMMERCE_TYPES, m_ppiSpecialistCommerceChanges[iI]);
	}

	stream->Read(&m_iNumImprovementYieldChanges);
	SAFE_DELETE_ARRAY(m_paImprovementYieldChanges);
	if (m_iNumImprovementYieldChanges > 0)
	{
		m_paImprovementYieldChanges = new ImprovementYieldChange[m_iNumImprovementYieldChanges];
		for (int iI = 0; iI < m_iNumImprovementYieldChanges; iI++)
		{
			m_paImprovementYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumSpecialistYieldChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistYieldChanges);
	if (m_iNumSpecialistYieldChanges > 0)
	{
		m_paSpecialistYieldChanges = new SpecialistYieldChange[m_iNumSpecialistYieldChanges];
		for (int iI = 0; iI < m_iNumSpecialistYieldChanges; iI++)
		{
			m_paSpecialistYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumSpecialistCommerceChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistCommerceChanges);
	if (m_iNumSpecialistCommerceChanges > 0)
	{
		m_paSpecialistCommerceChanges = new SpecialistCommerceChange[m_iNumSpecialistCommerceChanges];
		for (int iI = 0; iI < m_iNumSpecialistCommerceChanges; iI++)
		{
			m_paSpecialistCommerceChanges[iI].read(stream);
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
void CvCivicInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Wonder production cost modifier
	stream->Write(m_iWonderCostModifier);
	// MOD - END - Wonder production cost modifier
	stream->Write(m_iWorkerSpeedModifier);
	...
	stream->Write(m_bCommunist);	
	stream->Write(m_bNoNonStateReligionSpread);
	// MOD - START - Holy City Migration
	stream->Write(m_bCanAcquireHolyCityWithoutStateReligion);
	stream->Write(m_bCanMaintainHolyCityWithoutStateReligion);
	// MOD - END - Holy City Migration
	...
	stream->Write(NUM_COMMERCE_TYPES, m_piAllSpecialistCommerce);
	// MOD - END - Advanced Yield and Commerce Effects
	// MOD - START - Flavored Civics
    /*** GRAVE'S FLAVORED CIVICS, 4 October 2007 by Grave START ***/
	stream->Write(GC.getNumFlavorTypes(), m_piFlavorValue);
	...
	stream->Write(m_iNumRelatedSpecialists);
	if (m_iNumRelatedSpecialists > 0)
	{
		stream->Write(m_iNumRelatedSpecialists, m_piRelatedSpecialists);
	}
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		stream->Write(NUM_YIELD_TYPES, m_ppiImprovementYieldChanges[iI]);
	}

	// MOD - START - Advanced Yield and Commerce Effects
	for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		stream->Write(NUM_YIELD_TYPES, m_ppiSpecialistYieldChanges[iI]);
	}
	for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		stream->Write(NUM_COMMERCE_TYPES, m_ppiSpecialistCommerceChanges[iI]);
	}

	stream->Write(m_iNumImprovementYieldChanges);
	if (m_iNumImprovementYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumImprovementYieldChanges; iI++)
		{
			m_paImprovementYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumSpecialistYieldChanges);
	if (m_iNumSpecialistYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumSpecialistYieldChanges; iI++)
		{
			m_paSpecialistYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumSpecialistCommerceChanges);
	if (m_iNumSpecialistCommerceChanges > 0)
	{
		for (int iI = 0; iI < m_iNumSpecialistCommerceChanges; iI++)
		{
			m_paSpecialistCommerceChanges[iI].write(stream);
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
bool CvCivicInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Wonder production cost modifier
	pXML->GetChildXmlValByName(&m_iWonderCostModifier, "iWonderCostModifier");
	// MOD - END - Wonder production cost modifier
	pXML->GetChildXmlValByName(&m_iWorkerSpeedModifier, "iWorkerSpeedModifier");
	...
	pXML->GetChildXmlValByName(&m_bCommunist, "bCommunist");	
	pXML->GetChildXmlValByName(&m_bNoNonStateReligionSpread, "bNoNonStateReligionSpread");
	// MOD - START - Holy City Migration
	pXML->GetChildXmlValByName(&m_bCanAcquireHolyCityWithoutStateReligion, "bCanAcquireHolyCityWithoutStateReligion");
	pXML->GetChildXmlValByName(&m_bCanMaintainHolyCityWithoutStateReligion, "bCanMaintainHolyCityWithoutStateReligion");
	// MOD - END - Holy City Migration
	...
	*/
	...
	FAssertMsg(GC.getNumImprovementInfos() > 0 && NUM_YIELD_TYPES > 0, "Either the number of improvement infos is zero or less or the number of yield types is zero or less");
	ImprovementYieldChangeMap aImprovementYieldChangeMap;
	pXML->SetEconomicEffectMap<ImprovementTypes, YieldTypes>(aImprovementYieldChangeMap, "ImprovementYieldChanges", "ImprovementYieldChange", "ImprovementType", "ImprovementYields", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectLookup<ImprovementTypes, YieldTypes>(aImprovementYieldChangeMap, &m_ppiImprovementYieldChanges, GC.getNumImprovementInfos(), NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<ImprovementYieldChange, ImprovementTypes, YieldTypes>(aImprovementYieldChangeMap, &m_paImprovementYieldChanges, &m_iNumImprovementYieldChanges);
	aImprovementYieldChangeMap.clear();
	...
	SpecialistYieldChangeMap aSpecialistYieldChangeMap;
	pXML->SetEconomicEffectMap<SpecialistTypes, YieldTypes>(aSpecialistYieldChangeMap, "SpecialistYieldChanges", "SpecialistYieldChange", "SpecialistType", "SpecialistYields", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectLookup<SpecialistTypes, YieldTypes>(aSpecialistYieldChangeMap, &m_ppiSpecialistYieldChanges, GC.getNumSpecialistInfos(), NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<SpecialistYieldChange, SpecialistTypes, YieldTypes>(aSpecialistYieldChangeMap, &m_paSpecialistYieldChanges, &m_iNumSpecialistYieldChanges);
	aSpecialistYieldChangeMap.clear();
	...
	SpecialistCommerceChangeMap aSpecialistCommerceMap;
	pXML->SetEconomicEffectMap<SpecialistTypes, CommerceTypes>(aSpecialistCommerceMap, "SpecialistCommerceChanges", "SpecialistCommerceChange", "SpecialistType", "SpecialistCommerce", "iCommerce", NUM_COMMERCE_TYPES);
	pXML->SetEconomicEffectLookup<SpecialistTypes, CommerceTypes>(aSpecialistCommerceMap, &m_ppiSpecialistCommerceChanges, GC.getNumSpecialistInfos(), NUM_COMMERCE_TYPES);
	pXML->SetEconomicEffectEnumeration<SpecialistCommerceChange, SpecialistTypes, CommerceTypes>(aSpecialistCommerceMap, &m_paSpecialistCommerceChanges, &m_iNumSpecialistCommerceChanges);
	aSpecialistCommerceMap.clear();
	// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Relation Caching
void CvCivicInfo::cacheRelations()
{
	...
	for (int iFeature = 0; iFeature < GC.getNumFeatureInfos(); iFeature++)
	{
		if (isCivicRelatedToFeature(eCivic, (FeatureTypes)iFeature))
		{
			aRelatedFeatures.insert((FeatureTypes)iFeature);
		}
	}
	...
	for (int iImprovement = 0; iImprovement < GC.getNumImprovementInfos(); iImprovement++)
	{
		if (isCivicRelatedToImprovement(eCivic, (ImprovementTypes)iImprovement))
		{
			aRelatedImprovements.insert((ImprovementTypes)iImprovement);
		}
	}
	...
	// Specialists
	std::set<SpecialistTypes> aRelatedSpecialists;
	for (int iSpecialist = 0; iSpecialist < GC.getNumSpecialistInfos(); iSpecialist++)
	{
		if (isCivicRelatedToSpecialist(eCivic, (SpecialistTypes)iSpecialist))
		{
			aRelatedSpecialists.insert((SpecialistTypes)iSpecialist);
		}
	}

	m_iNumRelatedSpecialists = (int)aRelatedSpecialists.size();
	SAFE_DELETE_ARRAY(m_piRelatedSpecialists);
	if (m_iNumRelatedSpecialists > 0)
	{
		m_piRelatedSpecialists = new int[m_iNumRelatedSpecialists];
		int iRel = 0;
		for (std::set<SpecialistTypes>::const_iterator it=aRelatedSpecialists.begin(); it!=aRelatedSpecialists.end(); ++it)
		{
			m_piRelatedSpecialists[iRel++] = *it;
		}
	}
	aRelatedSpecialists.clear();

	// Unit Classes
	std::set<UnitClassTypes> aRelatedUnitClasses;
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isCivicRelatedToUnit(eCivic, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isCivicRelatedToBuilding(eCivic, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   CvUnitRoleInfo()
//
//  PURPOSE :   Default constructor
//
//------------------------------------------------------------------------------------------------------
CvUnitRoleInfo::CvUnitRoleInfo()
...
/*
int CvUnitRoleInfo::getInstanceCostModifier() const
{
	return m_iInstanceCostModifier;
}
*/

bool CvUnitRoleInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
// MOD - END - Aid
// MOD - START - Detriments
m_iNumDetrimentTypes(0),
m_piDetrimentTypes(NULL),
// MOD - END - Detriments
// MOD - START - Relation Caching
m_iNumUnitTypes(0),
m_piUnitTypes(NULL)
...
{
	// MOD - START - Unit Roles
	SAFE_DELETE_ARRAY(m_piRoles);
	// MOD - END - Unit Roles

	// MOD - START - Aid
	SAFE_DELETE_ARRAY(m_piAidTypes);
	// MOD - END - Aid

	// MOD - START - Detriments
	SAFE_DELETE_ARRAY(m_piDetrimentTypes);
	// MOD - END - Detriments

	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piUnitTypes);
	// MOD - END - Relation Caching
}
...
// MOD - START - Unit Roles
int CvUnitClassInfo::getRoleCostModifier() const
{
	return m_iRoleCostModifier;
}
// MOD - END - Unit Roles
...
// MOD - START - Improved Civilopedia
int CvUnitClassInfo::getRepresentativeUnit() const
{
	int iDefaultUnit = getDefaultUnitIndex();
	if (iDefaultUnit < 0)
	{
		if (getNumUnitTypes() > 0)
		{
			return getUnitTypes(0);
		}
	}
...
// MOD - START - Unit Roles
int CvUnitClassInfo::getNumRoles() const
{
	return m_iNumRoles;
}
// MOD - END - Unit Roles
...
// MOD - START - Aid
int CvUnitClassInfo::getNumAidTypes() const
{
	return m_iNumAidTypes;
}
// MOD - END - Aid

// MOD - START - Detriments
int CvUnitClassInfo::getNumDetrimentTypes() const
{
	return m_iNumDetrimentTypes;
}
// MOD - END - Detriments

// MOD - START - Relation Caching
int CvUnitClassInfo::getNumUnitTypes() const
{
	return m_iNumUnitTypes;
}
// MOD - END - Relation Caching

// MOD - START - Unit Roles
int CvUnitClassInfo::getRoles(int i) const
{
	FASSERT_BOUNDS(0, getNumRoles(), i, "CvBuildingInfo::getRoles");
	return m_piRoles[i];
}
// MOD - END - Unit Roles

// MOD - START - Aid
int CvUnitClassInfo::getAidTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumAidTypes(), i, "CvBuildingInfo::getAidTypes");
	return m_piAidTypes[i];
}
// MOD - END - Aid

// MOD - START - Detriments
int CvUnitClassInfo::getDetrimentTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumDetrimentTypes(), i, "CvBuildingInfo::getDetrimentTypes");
	return m_piDetrimentTypes[i];
}
// MOD - END - Detriments

// MOD - START - Relation Caching
int CvUnitClassInfo::getUnitTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumUnitTypes(), i, "CvBuildingInfo::getUnitTypes");
	return m_piUnitTypes[i];
}
// MOD - END - Relation Caching
...
bool CvUnitClassInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iInstanceCostModifier, "iInstanceCostModifier");
	// MOD - START - Unit Roles
	pXML->GetChildXmlValByName(&m_iRoleCostModifier, "iRoleCostModifier");
	// MOD - END - Unit Roles
	...
	// MOD - START - Unit Roles
	pXML->SetInfoArray(&m_iNumRoles, &m_piRoles, "Roles");
	// MOD - END - Unit Roles

	// MOD - START - Aid
	pXML->CollectInfoArray(m_aszAidTypes, "AidTypes");
	// MOD - END - Aid

	// MOD - START - Detriments
	pXML->CollectInfoArray(m_aszDetrimentTypes, "DetrimentTypes");
	// MOD - END - Detriments
...
bool CvUnitClassInfo::readPass3()
{
	...
	UnitClassTypes eClassType = (UnitClassTypes)GC.getInfoTypeForString(getType());

	UnitTypes eDefaultUnit = (UnitTypes)GC.getInfoTypeForString(m_aszExtraXMLforPass3[0]);
	if (eDefaultUnit == NO_UNIT || GC.getUnitInfo(eDefaultUnit).getUnitClassType() == eClassType)
	{
		m_iDefaultUnitIndex = eDefaultUnit;
	}
	else
	{
		m_iDefaultUnitIndex = NO_UNIT;
	...
	for (int iI = 0; iI < GC.getNumUnitInfos(); iI++)
	{
		...
		if (GC.getUnitInfo(eUnit).getUnitClassType() == eClassType)
		{
			aiUnitTypes.push_back(eUnit);
		}
	}
	
	if (aiUnitTypes.size() > 0)
	{
		m_iNumUnitTypes = (int)aiUnitTypes.size();
		m_piUnitTypes = new int[aiUnitTypes.size()];
		for (uint i = 0; i < aiUnitTypes.size(); i++)
		{
			m_piUnitTypes[i] = aiUnitTypes[i];
		}
	}
	// MOD - END - Relation Caching

	// MOD - START - Aid
	CvXMLLoadUtility::SetInfoArray(&m_iNumAidTypes, &m_piAidTypes, m_aszAidTypes);
	m_aszAidTypes.clear();
	// MOD - END - Aid

	// MOD - START - Detriments
	CvXMLLoadUtility::SetInfoArray(&m_iNumDetrimentTypes, &m_piDetrimentTypes, m_aszDetrimentTypes);
	m_aszDetrimentTypes.clear();
	// MOD - END - Detriments
...
m_iNumPrereqAndTechs(0),
m_iNumPrereqAndCivics(0),
m_iNumPrereqAndBonuses(0),
m_iNumPrereqOrBonuses(0),
m_iNumPrereqGlobalBuildingClasses(0),
m_iNumPrereqAndBuildingClasses(0),
...
m_iObsoleteBuildingClass(NO_BUILDINGCLASS),
// MOD - END - Building Replacement
m_iObsoleteTech(NO_TECH),
// MOD - START - Building Discontinuation
m_iDiscontinueTech(NO_TECH),
...
m_iNumConsumedBonuses(0),
m_iNumProducedBonuses(0),
...
// MOD - START - Epidemics
// Epidemic system by Mexico
m_iEpidemicModifier(0),
m_iGlobalEpidemicModifier(0),
// MOD - END - Epidemics
m_iGlobalPopulationChange(0),
m_iFreeTechs(0),
m_iDefenseModifier(0),
m_iNaturalDefenseModifier(0),
// MOD - START - Obsolete Defense
m_iObsoleteSafeDefenseModifier(0),
// MOD - END - Obsolete Defense
m_iBombardDefenseModifier(0),
// MOD - START - Obsolete Defense
m_iObsoleteSafeBombardDefenseModifier(0),
// MOD - END - Obsolete Defense
...
// MOD - START - Advanced Yield and Commerce Effects
//m_iNumBonusYieldChanges(0),
m_iNumBonusYieldModifiers(0),
m_iNumImprovementYieldChanges(0),
m_iNumSpecialistYieldChanges(0),
m_iNumSpecialistCommerceChanges(0),
m_iNumBuildingYieldChanges(0),
m_iNumBuildingCommerceChanges(0),
//m_iNumGlobalBonusYieldChanges(0),
m_iNumGlobalImprovementYieldChanges(0),
m_iNumGlobalSpecialistYieldChanges(0),
m_iNumGlobalSpecialistCommerceChanges(0),
m_iNumGlobalBuildingYieldChanges(0),
m_iNumGlobalBuildingCommerceChanges(0),
// MOD - END - Advanced Yield and Commerce Effects
// MOD - START - Relation Caching
m_iNumBuilderUnitClasses(0),
m_iNumBuilderUnits(0),
m_iNumRelatedReligions(0),
m_iNumRelatedCivics(0),
m_iNumRelatedBonuses(0),
m_iNumRelatedImprovements(0),
...
m_bUniqueBuilding(false),
// MOD - END - Unique Buildings
m_bIgnoreWorks(false),
// MOD - START - Military Disciplines
m_bMilitaryDiscipline(false),
...
m_piBonusProductionModifier(NULL),
// MOD - START - Bonus Converters
m_piConsumedBonusType(NULL),
m_piConsumedBonusAmount(NULL),
m_piConsumedBonusIndex(NULL),
m_piProducedBonusType(NULL),
m_piProducedBonusAmount(NULL),
m_piProducedBonusIndex(NULL),
// MOD - END - Bonus Converters
...
m_piDomainHealRateChanges(NULL),
m_piUnitClassHealRateChanges(NULL),
m_piUnitCombatHealRateChanges(NULL),
...
m_piUnitCombatSimultaneousProduction(NULL),
// MOD - END - Simultaneous Production
// MOD - START - Relation Caching
m_piBuilderUnitClasses(NULL),
m_piBuilderUnits(NULL),
m_piRelatedReligions(NULL),
m_piRelatedCivics(NULL),
m_piRelatedBonuses(NULL),
m_piRelatedImprovements(NULL),
...
m_pbGlobalTerrainDoubleMove(NULL),
...
/************************************************************************************************/
//m_paBonusYieldChanges(NULL),
m_paBonusYieldModifiers(NULL),
m_paImprovementYieldChanges(NULL),
m_paSpecialistYieldChanges(NULL),
m_paSpecialistCommerceChanges(NULL),
m_paBuildingYieldChanges(NULL),
m_paBuildingCommerceChanges(NULL),
//m_paGlobalBonusYieldChanges(NULL),
m_paGlobalImprovementYieldChanges(NULL),
m_paGlobalSpecialistYieldChanges(NULL),
m_paGlobalSpecialistCommerceChanges(NULL),
m_paGlobalBuildingYieldChanges(NULL),
m_paGlobalBuildingCommerceChanges(NULL)
// MOD - END - Advanced Yield and Commerce Effects
{
}
...
CvBuildingInfo::~CvBuildingInfo()
{
	// MOD - START - Advanced Building Prerequisites
	SAFE_DELETE_ARRAY(m_piPrereqAndTechs);
	SAFE_DELETE_ARRAY(m_piPrereqAndCivics);
	SAFE_DELETE_ARRAY(m_piPrereqAndBonuses);
	SAFE_DELETE_ARRAY(m_piPrereqOrBonuses);
	SAFE_DELETE_ARRAY(m_paPrereqGlobalBuildingClasses);
	SAFE_DELETE_ARRAY(m_piPrereqAndBuildingClasses);
	// MOD - END - Advanced Building Prerequisites
	SAFE_DELETE_ARRAY(m_piProductionTraits);
	...
	SAFE_DELETE_ARRAY(m_piBonusProductionModifier);
	// MOD - START - Bonus Converters
	SAFE_DELETE_ARRAY(m_piConsumedBonusType);
	SAFE_DELETE_ARRAY(m_piConsumedBonusAmount);
	SAFE_DELETE_ARRAY(m_piConsumedBonusIndex);
	SAFE_DELETE_ARRAY(m_piProducedBonusType);
	SAFE_DELETE_ARRAY(m_piProducedBonusAmount);
	SAFE_DELETE_ARRAY(m_piProducedBonusIndex);
	// MOD - END - Bonus Converters
	...
	// MOD - START - Building Heal Rate
	SAFE_DELETE_ARRAY(m_piDomainHealRateChanges);
	SAFE_DELETE_ARRAY(m_piUnitClassHealRateChanges);
	SAFE_DELETE_ARRAY(m_piUnitCombatHealRateChanges);
	// MOD - END - Building Heal Rate
	// MOD - START - Defense Modifiers (Mexico)
	SAFE_DELETE_ARRAY(m_piUnitCombatDefenseModifier);
	SAFE_DELETE_ARRAY(m_piUnitClassDefenseModifier);
	...
	// MOD - START - Simultaneous Production
	SAFE_DELETE_ARRAY(m_piUnitCombatSimultaneousProduction);
	// MOD - END - Simultaneous Production
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piBuilderUnitClasses);
	SAFE_DELETE_ARRAY(m_piBuilderUnits);
	SAFE_DELETE_ARRAY(m_piRelatedReligions);
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	SAFE_DELETE_ARRAY(m_piRelatedBonuses);
	SAFE_DELETE_ARRAY(m_piRelatedImprovements);
	...
	// MOD - START - Global Terrain Double Moves
	SAFE_DELETE_ARRAY(m_pbGlobalTerrainDoubleMove);
	// MOD - END - Global Terrain Double Moves
	// MOD - START - Advanced Building Prerequisites (Civic)
	...
	*/
	//SAFE_DELETE_ARRAY(m_paBonusYieldChanges);
	SAFE_DELETE_ARRAY(m_paBonusYieldModifiers);
	SAFE_DELETE_ARRAY(m_paImprovementYieldChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistYieldChanges);
	SAFE_DELETE_ARRAY(m_paSpecialistCommerceChanges);
	SAFE_DELETE_ARRAY(m_paBuildingYieldChanges);
	SAFE_DELETE_ARRAY(m_paBuildingCommerceChanges);
	//SAFE_DELETE_ARRAY(m_paGlobalBonusYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalImprovementYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalSpecialistYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalSpecialistCommerceChanges);
	SAFE_DELETE_ARRAY(m_paGlobalBuildingYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalBuildingCommerceChanges);
	// MOD - END - Advanced Yield and Commerce Effects

	// MOD - START - Advanced Building Prerequisites (Civic)
	m_aszExtraXMLforCivicPrereqsPass3.clear();
	// MOD - END - Advanced Building Prerequisites (Civic)
}
...
int CvBuildingInfo::getMaxTechEra() const
{
	int iMaxEra = 0;
	for (int iPre = 0; iPre < getNumPrereqAndTechs(); iPre++)
	{
	...
	{
		TechTypes ePrereqTech = (TechTypes)GC.getSpecialBuildingInfo((SpecialBuildingTypes)getSpecialBuildingType()).getTechPrereq();
		if (ePrereqTech != NO_TECH)
		{
			int iEra = GC.getTechInfo(ePrereqTech).getEra();
			iMaxEra = std::max(iMaxEra, iEra);
		}
	}
...
// MOD - START - Building Replacement
int CvBuildingInfo::getObsoleteBuildingClass() const
{
	return m_iObsoleteBuildingClass;
}
// MOD - END - Building Replacement
...
// MOD - START - Building Discontinuation
int CvBuildingInfo::getDiscontinueTech() const
{
	return m_iDiscontinueTech;
}
// MOD - END - Building Discontinuation

// MOD - START - Advanced Building Prerequisites
int CvBuildingInfo::getPrimaryPrereqTech() const
{
	return getNumPrereqAndTechs() > 0 ? getPrereqAndTechs(0) : NO_TECH;
}
...
int CvBuildingInfo::getNumPrereqAndBonuses() const
{
	return m_iNumPrereqAndBonuses;
}

int CvBuildingInfo::getNumPrereqGlobalBuildingClasses() const
{
	return m_iNumPrereqGlobalBuildingClasses;
}
...
// MOD - START - Bonus Converters
int CvBuildingInfo::getNumConsumedBonuses() const
{
	return m_iNumConsumedBonuses;
}

int CvBuildingInfo::getNumProducedBonuses() const
{
	return m_iNumProducedBonuses;
}
// MOD - END - Bonus Converters
...
// MOD - START - Wartime Conscription
int CvBuildingInfo::getWarConscript() const
{
	return m_iWarConscript;
}

int CvBuildingInfo::getGlobalWarConscript() const
{
	return m_iGlobalWarConscript;
}
// MOD - END - Wartime Conscription
...
int CvBuildingInfo::getSeparatism() const
{
	return m_iSeparatism;
}

int CvBuildingInfo::getGlobalSeparatism() const
{
	return m_iGlobalSeparatism;
}
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvBuildingInfo::getEpidemicModifier() const
{
	return m_iEpidemicModifier;
}

int CvBuildingInfo::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}
// MOD - END - Epidemics
...
int CvBuildingInfo::getNaturalDefenseModifier() const
{
	return m_iNaturalDefenseModifier;
}

// MOD - START - Obsolete Defense
int CvBuildingInfo::getObsoleteSafeDefenseModifier() const
{
	return m_iObsoleteSafeDefenseModifier;
}
// MOD - END - Obsolete Defense
...
// MOD - START - Obsolete Defense
int CvBuildingInfo::getObsoleteSafeBombardDefenseModifier() const
{
	return m_iObsoleteSafeBombardDefenseModifier;
}
// MOD - END - Obsolete Defense
...
// MOD - START - Relation Caching
int CvBuildingInfo::getNumBuilderUnitClasses() const
{
	return m_iNumBuilderUnitClasses;
}

int CvBuildingInfo::getNumBuilderUnits() const
{
	return m_iNumBuilderUnits;
}

int CvBuildingInfo::getNumRelatedReligions() const
{
	return m_iNumRelatedReligions;
}

int CvBuildingInfo::getNumRelatedCivics() const
{
	return m_iNumRelatedCivics;
}

int CvBuildingInfo::getNumRelatedBonuses() const
{
	return m_iNumRelatedBonuses;
}

int CvBuildingInfo::getNumRelatedImprovements() const
{
	return m_iNumRelatedImprovements;
}

int CvBuildingInfo::getNumRelatedSpecialists() const
{
	return m_iNumRelatedSpecialists;
}

int CvBuildingInfo::getNumRelatedUnitCombats() const
{
	return m_iNumRelatedUnitCombats;
}

int CvBuildingInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvBuildingInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
// MOD - START - Advanced Yield and Commerce Effects
int CvBuildingInfo::getNumBonusYieldModifiers() const
{
	return m_iNumBonusYieldModifiers;
}

int CvBuildingInfo::getNumImprovementYieldChanges() const
{
	return m_iNumImprovementYieldChanges;
}

int CvBuildingInfo::getNumSpecialistYieldChanges() const
{
	return m_iNumSpecialistYieldChanges;
}

int CvBuildingInfo::getNumSpecialistCommerceChanges() const
{
	return m_iNumSpecialistCommerceChanges;
}

int CvBuildingInfo::getNumBuildingYieldChanges() const
{
	return m_iNumBuildingYieldChanges;
}

int CvBuildingInfo::getNumBuildingCommerceChanges() const
{
	return m_iNumBuildingCommerceChanges;
}

int CvBuildingInfo::getNumGlobalImprovementYieldChanges() const
{
	return m_iNumGlobalImprovementYieldChanges;
}

int CvBuildingInfo::getNumGlobalSpecialistYieldChanges() const
{
	return m_iNumGlobalSpecialistYieldChanges;
}

int CvBuildingInfo::getNumGlobalSpecialistCommerceChanges() const
{
	return m_iNumGlobalSpecialistCommerceChanges;
}

int CvBuildingInfo::getNumGlobalBuildingYieldChanges() const
{
	return m_iNumGlobalBuildingYieldChanges;
}

int CvBuildingInfo::getNumGlobalBuildingCommerceChanges() const
{
	return m_iNumGlobalBuildingCommerceChanges;
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Congestion
int CvBuildingInfo::getInsideCityCongestionThresholdChange() const
{
	return m_iInsideCityCongestionThresholdChange;
}

int CvBuildingInfo::getOutsideCityCongestionThresholdChange() const
{
	return m_iOutsideCityCongestionThresholdChange;
}
// MOD - END - Congestion
...
// MOD - START - Unique Buildings
bool CvBuildingInfo::isUniqueBuilding() const
{
	return m_bUniqueBuilding;
}
// MOD - END - Unique Buildings

// MOD - START - Great Works
bool CvBuildingInfo::isGreatWork() const
{
	if (getProductionCost() > -1)
	{
		return false;
	}

	if (getNumBuilderUnits() == 0)
	{
		return false;
	}

	if (isMilitaryDiscipline())
	{
		return false;
	}

	if (!isWorldWonderClass((BuildingClassTypes)getBuildingClassType()) && !isNationalWonderClass((BuildingClassTypes)getBuildingClassType()))
	{
		return false;
	}
...
bool CvBuildingInfo::isIgnoreWorks() const
{
	return m_bIgnoreWorks;
}

// MOD - START - Military Disciplines
bool CvBuildingInfo::isMilitaryDiscipline() const
{
	return m_bMilitaryDiscipline;
	...
	BuildingTypes eBuilding = (BuildingTypes) GC.getInfoTypeForString(getType());

	// MOD - START - Relation Caching
	// Military Discipline Buildings
	for (int iRel = 0; iRel < getNumRelatedUnitClasses(); iRel++)
	{
		UnitClassTypes eLoopUnitClass = (UnitClassTypes)getRelatedUnitClasses(iRel);
		const CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(eLoopUnitClass);
		...
		for (int iMem = 0; iMem < kUnitClass.getNumUnitTypes(); iMem++)
		{
			...
			if (kLoopUnit.getBuildings(eBuilding) || kLoopUnit.getForceBuildings(eBuilding))
			{
				if (kLoopUnit.getUnitAIType(UNITAI_GENERAL))
				{
					return true;
				}
			}
	...
	// Military Discipline Effect Buildings
	for (int iRel = 0; iRel < getNumRelatedBuildingClasses(); iRel++)
	{
		BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)getRelatedBuildingClasses(iRel);
		const CvBuildingClassInfo& kBuildingClass = GC.getBuildingClassInfo(eLoopBuildingClass);

		for (int iMem = 0; iMem < kBuildingClass.getNumBuildingTypes(); iMem++)
		{
			BuildingTypes eLoopBuilding = (BuildingTypes)kBuildingClass.getBuildingTypes(iMem);
			const CvBuildingInfo& kLoopBuilding = GC.getBuildingInfo(eLoopBuilding);
			if (kLoopBuilding.getFreeBuildingClass() == getBuildingClassType())
			{
				if (kLoopBuilding.isMilitaryDiscipline())
				{
					return true;
				}
			}
...
bool CvBuildingInfo::isMilitaryDoctrine() const
{
	if (isMilitaryDiscipline() && isWorldWonderClass((BuildingClassTypes)getBuildingClassType()))
	{
		return true;
	}
	...
	for (int iRel = 0; iRel < getNumRelatedBuildingClasses(); iRel++)
	{
		...
		for (int iMem = 0; iMem < kBuildingClass.getNumBuildingTypes(); iMem++)
		{
			...
			if (kLoopBuilding.getFreeBuildingClass() == getBuildingClassType())
			{
				if (kLoopBuilding.isMilitaryDoctrine())
				{
					return true;
				}
			}
...
bool CvBuildingInfo::isMilitaryTradition() const
{
	if (isWorldWonderClass((BuildingClassTypes)getBuildingClassType()))
	{
		return false;
	}
	...
	for (int iRel = 0; iRel < getNumRelatedBuildingClasses(); iRel++)
	{
		...
		for (int iMem = 0; iMem < kBuildingClass.getNumBuildingTypes(); iMem++)
		{
			...
			if (kLoopBuilding.getFreeBuildingClass() == getBuildingClassType())
			{
				if (kLoopBuilding.isMilitaryTradition())
				{
					return true;
				}
			}
...
// MOD - START - Fresh Water Aquifers
bool CvBuildingInfo::isFreshWaterSource() const
{
...
// MOD - START - Unlimited Conscription
bool CvBuildingInfo::isGlobalUnlimitedConscript() const
{
	return m_bGlobalUnlimitedConscript;
}
// MOD - END - Unlimited Conscription

// MOD - START - Bonus Converters
bool CvBuildingInfo::isBonusConverter() const
{
	return m_bBonusConverter;
}
// MOD - END - Bonus Converters
...
bool CvBuildingInfo::isInactiveWithoutPower() const
{
	return m_bInactiveWithoutPower;
}
// MOD - END - Advanced Building Prerequisites (Power)

// MOD - START - Advanced Building Prerequisites (Religion)
bool CvBuildingInfo::isInactiveWithoutPrereqStateReligion() const
{
	return m_bInactiveWithoutPrereqStateReligion;
}
// MOD - END - Advanced Building Prerequisites (Religion)

// MOD - START - Advanced Building Prerequisites (Civic)
bool CvBuildingInfo::isInactiveWithoutCivicPrereqs() const
{
	return m_bInactiveWithoutPrereqCivics;
}
// MOD - END - Advanced Building Prerequisites (Civic)
...
{
	FASSERT_BOUNDS(0, getNumPrereqAndTechs(), i, "CvBuildingInfo::getPrereqAndTechs");
	return m_piPrereqAndTechs ? m_piPrereqAndTechs[i] : -1;
}

int CvBuildingInfo::getPrereqAndCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndCivics(), i, "CvBuildingInfo::getPrereqAndCivics");
	return m_piPrereqAndCivics ? m_piPrereqAndCivics[i] : -1;
}

int CvBuildingInfo::getPrereqAndBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndBonuses(), i, "CvBuildingInfo::getPrereqAndBonuses");
	return m_piPrereqAndBonuses ? m_piPrereqAndBonuses[i] : -1;
}

int CvBuildingInfo::getPrereqOrBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqOrBonuses(), i, "CvBuildingInfo::getPrereqOrBonuses");
	return m_piPrereqOrBonuses ? m_piPrereqOrBonuses[i] : -1;
}

int CvBuildingInfo::getPrereqGlobalBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqGlobalBuildingClasses(), i, "CvBuildingInfo::getPrereqGlobalBuildingClasses");
	return m_paPrereqGlobalBuildingClasses ? m_paPrereqGlobalBuildingClasses[i].eBuildingClass : -1;
}

int CvBuildingInfo::getPrereqGlobalBuildingCounts(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqGlobalBuildingClasses(), i, "CvBuildingInfo::getPrereqGlobalBuildingCounts");
	return m_paPrereqGlobalBuildingClasses ? m_paPrereqGlobalBuildingClasses[i].iCount : -1;
}

int CvBuildingInfo::getPrereqAndBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndBuildingClasses(), i, "CvBuildingInfo::getPrereqAndBuildingClasses");
	return m_piPrereqAndBuildingClasses ? m_piPrereqAndBuildingClasses[i] : -1;
}
// MOD - END - Advanced Building Prerequisites
...
int CvBuildingInfo::getAllSpecialistCommerce(int i) const
{
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, i, "CvBuildingInfo::getAllSpecialistCommerce");
	return m_piAllSpecialistCommerce ? m_piAllSpecialistCommerce[i] : -1;
}

int* CvBuildingInfo::getAllSpecialistCommerceArray() const
{
	return m_piAllSpecialistCommerce;
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Building Heal Rate
int CvBuildingInfo::getDomainHealRateChanges(int i) const
{
	FAssertMsg(i < NUM_DOMAIN_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piDomainHealRateChanges ? m_piDomainHealRateChanges[i] : -1;
}

int CvBuildingInfo::getUnitClassHealRateChanges(int i) const
{
	FAssertMsg(i < GC.getNumUnitClassInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piUnitClassHealRateChanges ? m_piUnitClassHealRateChanges[i] : -1;
}

int CvBuildingInfo::getUnitCombatHealRateChanges(int i) const
{
	FAssertMsg(i < GC.getNumUnitCombatInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piUnitCombatHealRateChanges ? m_piUnitCombatHealRateChanges[i] : -1;
}
// MOD - END - Building Heal Rate

// MOD - START - Defense Modifiers (Mexico)
int CvBuildingInfo::getUnitCombatDefenseModifier(int i) const
{
	FAssertMsg(i < GC.getNumUnitCombatInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piUnitCombatDefenseModifier ? m_piUnitCombatDefenseModifier[i] : 0;
}

int CvBuildingInfo::getUnitClassDefenseModifier(int i) const
{
	FAssertMsg(i < GC.getNumUnitClassInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
...
int CvBuildingInfo::getPrereqNumOfBuildingClass(int i) const
{
	...
	for (int iPre = 0; iPre < getNumPrereqGlobalBuildingClasses(); iPre++)
	{
		if (m_paPrereqGlobalBuildingClasses[iPre].eBuildingClass == i)
		{
			return m_paPrereqGlobalBuildingClasses[iPre].iCount;
		}
	}
	return 0;
}
...
// MOD - START - Bonus Converters
int CvBuildingInfo::getConsumedBonusType(int i) const
{
	FASSERT_BOUNDS(0, getNumConsumedBonuses(), i, "CvBuildingInfo::getConsumedBonusType");
	return m_piConsumedBonusType ? m_piConsumedBonusType[i] : -1;
}

int CvBuildingInfo::getConsumedBonusAmount(int i) const
{
	FASSERT_BOUNDS(0, getNumConsumedBonuses(), i, "CvBuildingInfo::getConsumedBonusAmount");
	return m_piConsumedBonusAmount ? m_piConsumedBonusAmount[i] : -1;
}

int CvBuildingInfo::getConsumedBonusIndex(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumBonusInfos(), i, "CvBuildingInfo::getConsumedBonusIndex");
	return m_piConsumedBonusIndex ? m_piConsumedBonusIndex[i] : -1;
}

int CvBuildingInfo::getProducedBonusType(int i) const
{
	FASSERT_BOUNDS(0, getNumProducedBonuses(), i, "CvBuildingInfo::getProducedBonusType");
	return m_piProducedBonusType ? m_piProducedBonusType[i] : -1;
}

int CvBuildingInfo::getProducedBonusAmount(int i) const
{
	FASSERT_BOUNDS(0, getNumProducedBonuses(), i, "CvBuildingInfo::getProducedBonusAmount");
	return m_piProducedBonusAmount ? m_piProducedBonusAmount[i] : -1;
}

int CvBuildingInfo::getProducedBonusIndex(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumBonusInfos(), i, "CvBuildingInfo::getProducedBonusIndex");
	return m_piProducedBonusIndex ? m_piProducedBonusIndex[i] : -1;
}
// MOD - END - Bonus Converters

// MOD - START - Simultaneous Production
int CvBuildingInfo::getUnitCombatSimultaneousProduction(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitCombatInfos(), i, "CvBuildingInfo::getUnitCombatSimultaneousProduction");
	return m_piUnitCombatSimultaneousProduction ? m_piUnitCombatSimultaneousProduction[i] : 0;
}
// MOD - END - Simultaneous Production

// MOD - START - Relation Caching
int CvBuildingInfo::getBuilderUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumBuilderUnitClasses(), i, "CvBuildingInfo::getBuilderUnitClasses");
	return m_piBuilderUnitClasses ? m_piBuilderUnitClasses[i] : -1;
}

int CvBuildingInfo::getBuilderUnits(int i) const
{
	FASSERT_BOUNDS(0, getNumBuilderUnits(), i, "CvBuildingInfo::getBuilderUnits");
	return m_piBuilderUnits ? m_piBuilderUnits[i] : -1;
}

int CvBuildingInfo::getRelatedReligions(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedReligions(), i, "CvBuildingInfo::getRelatedReligions");
	return m_piRelatedReligions ? m_piRelatedReligions[i] : -1;
}

int CvBuildingInfo::getRelatedCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedCivics(), i, "CvBuildingInfo::getRelatedCivics");
	return m_piRelatedCivics ? m_piRelatedCivics[i] : -1;
}

int CvBuildingInfo::getRelatedBonuses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBonuses(), i, "CvBuildingInfo::getRelatedBonuses");
	return m_piRelatedBonuses ? m_piRelatedBonuses[i] : -1;
}

int CvBuildingInfo::getRelatedImprovements(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedImprovements(), i, "CvBuildingInfo::getRelatedImprovements");
	return m_piRelatedImprovements ? m_piRelatedImprovements[i] : -1;
}

int CvBuildingInfo::getRelatedSpecialists(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedSpecialists(), i, "CvBuildingInfo::getRelatedSpecialists");
	return m_piRelatedSpecialists ? m_piRelatedSpecialists[i] : -1;
}

int CvBuildingInfo::getRelatedUnitCombats(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitCombats(), i, "CvBuildingInfo::getRelatedUnitCombats");
	return m_piRelatedUnitCombats ? m_piRelatedUnitCombats[i] : -1;
}

int CvBuildingInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvBuildingInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvBuildingInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvBuildingInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
...
bool CvBuildingInfo::isBuildingClassNeededInCity(int i) const
{
	...
	for (int iPre = 0; iPre < getNumPrereqAndBuildingClasses(); iPre++)
	{
		if (m_piPrereqAndBuildingClasses[iPre] == i)
		{
			return true;
		}
	}
...
// MOD - START - Bonus Converters
bool CvBuildingInfo::isConsumerOfBonus(int i) const
{
	return (getConsumedBonusIndex(i) >= 0);
}

bool CvBuildingInfo::isProducerOfBonus(int i) const
{
	return (getProducedBonusIndex(i) >= 0);
}
// MOD - END - Bonus Converters

// MOD - START - Global Terrain Double Moves
bool CvBuildingInfo::getGlobalTerrainDoubleMove(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumTerrainInfos(), i, "CvBuildingInfo::getGlobalTerrainDoubleMove");
	return m_pbGlobalTerrainDoubleMove ? m_pbGlobalTerrainDoubleMove[i] : false;
}
// MOD - END - Global Terrain Double Moves
...
{
	FASSERT_BOUNDS(0, GC.getNumCivicInfos(), i, "CvBuildingInfo::isPrereqCivic");
...
/*
int CvBuildingInfo::getBonusYieldChangeBonus(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldChanges(), i, "CvBuildingInfo::getBonusYieldChangeBonus");
	return m_paBonusYieldChanges[i].eBonus;
}

int CvBuildingInfo::getBonusYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldChanges(), i, "CvBuildingInfo::getBonusYieldChangeYield");
	return m_paBonusYieldChanges[i].eYield;
}

int CvBuildingInfo::getBonusYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldChanges(), i, "CvBuildingInfo::getBonusYieldChangeValue");
	return m_paBonusYieldChanges[i].iChange;
}
*/

int CvBuildingInfo::getBonusYieldModifierBonus(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldModifiers(), i, "CvBuildingInfo::getBonusYieldModifierBonus");
	return m_paBonusYieldModifiers[i].eBonus;
}

int CvBuildingInfo::getBonusYieldModifierYield(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldModifiers(), i, "CvBuildingInfo::getBonusYieldModifierYield");
	return m_paBonusYieldModifiers[i].eYield;
}

int CvBuildingInfo::getBonusYieldModifierValue(int i) const
{
	FASSERT_BOUNDS(0, getNumBonusYieldModifiers(), i, "CvBuildingInfo::getBonusYieldModifierValue");
	return m_paBonusYieldModifiers[i].iModifier;
}


int CvBuildingInfo::getImprovementYieldChangeImprovement(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvBuildingInfo::getImprovementYieldChangeImprovement");
	return m_paImprovementYieldChanges[i].eImprovement;
}

int CvBuildingInfo::getImprovementYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvBuildingInfo::getImprovementYieldChangeYield");
	return m_paImprovementYieldChanges[i].eYield;
}

int CvBuildingInfo::getImprovementYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvBuildingInfo::getImprovementYieldChangeValue");
	return m_paImprovementYieldChanges[i].iChange;
}


int CvBuildingInfo::getSpecialistYieldChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvBuildingInfo::getSpecialistYieldChangeSpecialist");
	return m_paSpecialistYieldChanges[i].eSpecialist;
}

int CvBuildingInfo::getSpecialistYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvBuildingInfo::getSpecialistYieldChangeYield");
	return m_paSpecialistYieldChanges[i].eYield;
}

int CvBuildingInfo::getSpecialistYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvBuildingInfo::getSpecialistYieldChangeValue");
	return m_paSpecialistYieldChanges[i].iChange;
}

int CvBuildingInfo::getSpecialistCommerceChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvBuildingInfo::getSpecialistCommerceChangeSpecialist");
	return m_paSpecialistCommerceChanges[i].eSpecialist;
}

int CvBuildingInfo::getSpecialistCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvBuildingInfo::getSpecialistCommerceChangeCommerce");
	return m_paSpecialistCommerceChanges[i].eCommerce;
}

int CvBuildingInfo::getSpecialistCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvBuildingInfo::getSpecialistCommerceChangeValue");
	return m_paSpecialistCommerceChanges[i].iChange;
}


int CvBuildingInfo::getBuildingYieldChangeBuildingClass(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingYieldChanges(), i, "CvBuildingInfo::getBuildingYieldChangeBuildingClass");
	return m_paBuildingYieldChanges[i].eBuildingClass;
}

int CvBuildingInfo::getBuildingYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingYieldChanges(), i, "CvBuildingInfo::getBuildingYieldChangeYield");
	return m_paBuildingYieldChanges[i].eYield;
}

int CvBuildingInfo::getBuildingYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingYieldChanges(), i, "CvBuildingInfo::getBuildingYieldChangeValue");
	return m_paBuildingYieldChanges[i].iChange;
}

int CvBuildingInfo::getBuildingCommerceChangeBuildingClass(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingCommerceChanges(), i, "CvBuildingInfo::getBuildingCommerceChangeBuildingClass");
	return m_paBuildingCommerceChanges[i].eBuildingClass;
}

int CvBuildingInfo::getBuildingCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingCommerceChanges(), i, "CvBuildingInfo::getBuildingCommerceChangeCommerce");
	return m_paBuildingCommerceChanges[i].eCommerce;
}

int CvBuildingInfo::getBuildingCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumBuildingCommerceChanges(), i, "CvBuildingInfo::getBuildingCommerceChangeValue");
	return m_paBuildingCommerceChanges[i].iChange;
}


/*
int CvBuildingInfo::getGlobalBonusYieldChangeBonus(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBonusYieldChanges(), i, "CvBuildingInfo::getGlobalBonusYieldChangeBonus");
	return m_paGlobalBonusYieldChanges[i].eBonus;
}

int CvBuildingInfo::getGlobalBonusYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBonusYieldChanges(), i, "CvBuildingInfo::getGlobalBonusYieldChangeYield");
	return m_paGlobalBonusYieldChanges[i].eYield;
}

int CvBuildingInfo::getGlobalBonusYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBonusYieldChanges(), i, "CvBuildingInfo::getGlobalBonusYieldChangeValue");
	return m_paGlobalBonusYieldChanges[i].iChange;
}
*/

int CvBuildingInfo::getGlobalImprovementYieldChangeImprovement(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalImprovementYieldChanges(), i, "CvBuildingInfo::getGlobalImprovementYieldChangeImprovement");
	return m_paGlobalImprovementYieldChanges[i].eImprovement;
}

int CvBuildingInfo::getGlobalImprovementYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalImprovementYieldChanges(), i, "CvBuildingInfo::getGlobalImprovementYieldChangeYield");
	return m_paGlobalImprovementYieldChanges[i].eYield;
}

int CvBuildingInfo::getGlobalImprovementYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalImprovementYieldChanges(), i, "CvBuildingInfo::getGlobalImprovementYieldChangeValue");
	return m_paGlobalImprovementYieldChanges[i].iChange;
}


int CvBuildingInfo::getGlobalSpecialistYieldChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistYieldChanges(), i, "CvBuildingInfo::getGlobalSpecialistYieldChangeSpecialist");
	return m_paGlobalSpecialistYieldChanges[i].eSpecialist;
}

int CvBuildingInfo::getGlobalSpecialistYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistYieldChanges(), i, "CvBuildingInfo::getGlobalSpecialistYieldChangeYield");
	return m_paGlobalSpecialistYieldChanges[i].eYield;
}

int CvBuildingInfo::getGlobalSpecialistYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistYieldChanges(), i, "CvBuildingInfo::getGlobalSpecialistYieldChangeValue");
	return m_paGlobalSpecialistYieldChanges[i].iChange;
}

int CvBuildingInfo::getGlobalSpecialistCommerceChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistCommerceChanges(), i, "CvBuildingInfo::getGlobalSpecialistCommerceChangeSpecialist");
	return m_paGlobalSpecialistCommerceChanges[i].eSpecialist;
}

int CvBuildingInfo::getGlobalSpecialistCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistCommerceChanges(), i, "CvBuildingInfo::getGlobalSpecialistCommerceChangeCommerce");
	return m_paGlobalSpecialistCommerceChanges[i].eCommerce;
}

int CvBuildingInfo::getGlobalSpecialistCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalSpecialistCommerceChanges(), i, "CvBuildingInfo::getGlobalSpecialistCommerceChangeValue");
	return m_paGlobalSpecialistCommerceChanges[i].iChange;
}


int CvBuildingInfo::getGlobalBuildingYieldChangeBuildingClass(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingYieldChanges(), i, "CvBuildingInfo::getGlobalBuildingYieldChangeBuildingClass");
	return m_paGlobalBuildingYieldChanges[i].eBuildingClass;
}

int CvBuildingInfo::getGlobalBuildingYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingYieldChanges(), i, "CvBuildingInfo::getGlobalBuildingYieldChangeYield");
	return m_paGlobalBuildingYieldChanges[i].eYield;
}

int CvBuildingInfo::getGlobalBuildingYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingYieldChanges(), i, "CvBuildingInfo::getGlobalBuildingYieldChangeValue");
	return m_paGlobalBuildingYieldChanges[i].iChange;
}

int CvBuildingInfo::getGlobalBuildingCommerceChangeBuildingClass(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingCommerceChanges(), i, "CvBuildingInfo::getGlobalBuildingCommerceChangeBuildingClass");
	return m_paGlobalBuildingCommerceChanges[i].eBuildingClass;
}

int CvBuildingInfo::getGlobalBuildingCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingCommerceChanges(), i, "CvBuildingInfo::getGlobalBuildingCommerceChangeBuildingClass");
	return m_paGlobalBuildingCommerceChanges[i].eCommerce;
}

int CvBuildingInfo::getGlobalBuildingCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumGlobalBuildingCommerceChanges(), i, "CvBuildingInfo::getGlobalBuildingCommerceChangeBuildingClass");
	return m_paGlobalBuildingCommerceChanges[i].iChange;
}

/*
void CvBuildingInfo::getBonusYieldChangeArray(BonusTypes eBonus, int* piYieldChanges) const
{
	for (std::vector<BonusYieldChange>::const_iterator it = m_aBonusYieldChanges.begin(); it != m_aBonusYieldChanges.end(); ++it)
	{
		if ((*it).eBonus == eBonus)
		{
			piYieldModifiers[(*it).eYield] += (*it).iChange;
		}
	}
}
*/

void CvBuildingInfo::getBonusYieldModifierArray(BonusTypes eBonus, int* piYieldModifiers) const
{
	for (int iMem = 0; iMem < getNumBonusYieldModifiers(); iMem++)
	{
		if (getBonusYieldModifierBonus(iMem) == eBonus)
		{
			piYieldModifiers[getBonusYieldModifierYield(iMem)] += getBonusYieldModifierValue(iMem);
		}
	}
}

void CvBuildingInfo::getImprovementYieldChangeArray(ImprovementTypes eImprovement, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumImprovementYieldChanges(); iMem++)
	{
		if (getImprovementYieldChangeImprovement(iMem) == eImprovement)
...
void CvBuildingInfo::getSpecialistYieldChangeArray(SpecialistTypes eSpecialist, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumSpecialistYieldChanges(); iMem++)
	{
		if (getSpecialistYieldChangeSpecialist(iMem) == eSpecialist)
...
void CvBuildingInfo::getSpecialistCommerceChangeArray(SpecialistTypes eSpecialist, int* piCommerceChanges) const
{
	for (int iMem = 0; iMem < getNumSpecialistCommerceChanges(); iMem++)
	{
		if (getSpecialistCommerceChangeSpecialist(iMem) == eSpecialist)
...
void CvBuildingInfo::getBuildingYieldChangeArray(BuildingClassTypes eBuildingClass, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumBuildingYieldChanges(); iMem++)
	{
		if (getBuildingYieldChangeBuildingClass(iMem) == eBuildingClass)
		{
			piYieldChanges[getBuildingYieldChangeYield(iMem)] += getBuildingYieldChangeValue(iMem);
		}
	}
}

void CvBuildingInfo::getBuildingCommerceChangeArray(BuildingClassTypes eBuildingClass, int* piCommerceChanges) const
{
	for (int iMem = 0; iMem < getNumBuildingCommerceChanges(); iMem++)
	{
		if (getBuildingCommerceChangeBuildingClass(iMem) == eBuildingClass)
		{
			piCommerceChanges[getBuildingCommerceChangeCommerce(iMem)] += getBuildingCommerceChangeValue(iMem);
		}
	}
}

/*
void CvBuildingInfo::getGlobalBonusYieldChangeArray(BonusTypes eBonus, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalBonusYieldChanges(); iMem++)
	{
		if (getGlobalBonusYieldChangeBonus(iMem) == eBonus)
		{
			piYieldChanges[getGlobalBonusYieldChangeYield(iMem)] += getGlobalBonusYieldChangeValue(iMem);
		}
	}
}
*/

void CvBuildingInfo::getGlobalImprovementYieldChangeArray(ImprovementTypes eImprovement, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalImprovementYieldChanges(); iMem++)
	{
		if (getGlobalImprovementYieldChangeImprovement(iMem) == eImprovement)
		{
			piYieldChanges[getGlobalImprovementYieldChangeYield(iMem)] += getGlobalImprovementYieldChangeValue(iMem);
		}
	}
}

void CvBuildingInfo::getGlobalSpecialistYieldChangeArray(SpecialistTypes eSpecialist, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalSpecialistYieldChanges(); iMem++)
	{
		if (getGlobalSpecialistYieldChangeSpecialist(iMem) == eSpecialist)
		{
			piYieldChanges[getGlobalSpecialistYieldChangeYield(iMem)] += getGlobalSpecialistYieldChangeValue(iMem);
		}
	}
}

void CvBuildingInfo::getGlobalSpecialistCommerceChangeArray(SpecialistTypes eSpecialist, int* piCommerceChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalSpecialistCommerceChanges(); iMem++)
	{
		if (getGlobalSpecialistCommerceChangeSpecialist(iMem) == eSpecialist)
		{
			piCommerceChanges[getGlobalSpecialistCommerceChangeCommerce(iMem)] += getGlobalSpecialistCommerceChangeValue(iMem);
		}
	}
}

void CvBuildingInfo::getGlobalBuildingYieldChangeArray(BuildingClassTypes eBuildingClass, int* piYieldChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalBuildingYieldChanges(); iMem++)
	{
		if (getGlobalBuildingYieldChangeBuildingClass(iMem) == eBuildingClass)
		{
			piYieldChanges[getGlobalBuildingYieldChangeYield(iMem)] += getGlobalBuildingYieldChangeValue(iMem);
		}
	}
}

void CvBuildingInfo::getGlobalBuildingCommerceChangeArray(BuildingClassTypes eBuildingClass, int* piCommerceChanges) const
{
	for (int iMem = 0; iMem < getNumGlobalBuildingCommerceChanges(); iMem++)
	{
		if (getGlobalBuildingCommerceChangeBuildingClass(iMem) == eBuildingClass)
		{
			piCommerceChanges[getGlobalBuildingCommerceChangeCommerce(iMem)] += getGlobalBuildingCommerceChangeValue(iMem);
		}
	}
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Building Timer
int CvBuildingInfo::getTimer() const
{
	return m_iTimer;
}
// MOD - END - Building Timer
...
void CvBuildingInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Building Replacement
	stream->Read(&m_iObsoleteBuildingClass);
	// MOD - END - Building Replacement
	stream->Read(&m_iObsoleteTech);
	// MOD - START - Building Discontinuation
	stream->Read(&m_iDiscontinueTech);
	// MOD - END - Building Discontinuation
	...
	// MOD - START - Wartime Conscription
	stream->Read(&m_iWarConscript);
	stream->Read(&m_iGlobalWarConscript);
	// MOD - END - Wartime Conscription
	stream->Read(&m_iHealRateChange);
	// MOD - START - Revolutions (separatism)
	stream->Read(&m_iSeparatism);
	stream->Read(&m_iGlobalSeparatism);
	// MOD - END - Revolutions (separatism)
	stream->Read(&m_iHealth);
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	stream->Read(&m_iEpidemicModifier);
	stream->Read(&m_iGlobalEpidemicModifier);
	// MOD - END - Epidemics
	stream->Read(&m_iGlobalPopulationChange);
	stream->Read(&m_iFreeTechs);
	stream->Read(&m_iDefenseModifier);
	stream->Read(&m_iNaturalDefenseModifier);	
	// MOD - START - Obsolete Defense
	stream->Read(&m_iObsoleteSafeDefenseModifier);
	// MOD - END - Obsolete Defense
	stream->Read(&m_iBombardDefenseModifier);
	// MOD - START - Obsolete Defense
	stream->Read(&m_iObsoleteSafeBombardDefenseModifier);
	// MOD - END - Obsolete Defense
...
// MOD - END - Congestion
	// MOD - START - Unique Buildings
	stream->Read(&m_bUniqueBuilding);
	// MOD - END - Unique Buildings
	stream->Read(&m_bIgnoreWorks);	
	// MOD - START - Military Disciplines
	stream->Read(&m_bMilitaryDiscipline);
	// MOD - END - Military Disciplines
	stream->Read(&m_bTeamShare);
	stream->Read(&m_bWater);
	stream->Read(&m_bRiver);
	// MOD - START - Fresh Water Aquifers
	stream->Read(&m_bFreshWaterSource);
	// MOD - END - Fresh Water Aquifers
	...
	// MOD - START - Unlimited Conscription
	stream->Read(&m_bGlobalUnlimitedConscript);
	// MOD - END - Unlimited Conscription
	// MOD - START - Bonus Converters
	stream->Read(&m_bBonusConverter);
	// MOD - END - Bonus Converters
	// MOD - START - Advanced Building Prerequisites (Power)
	stream->Read(&m_bPrereqPower);
	stream->Read(&m_bInactiveWithoutPower);
	// MOD - END - Advanced Building Prerequisites (Power)
	// MOD - START - Advanced Building Prerequisites (Religion)
	stream->Read(&m_bInactiveWithoutPrereqStateReligion);
	// MOD - END - Advanced Building Prerequisites (Religion)
	// MOD - START - Advanced Building Prerequisites (Civic)
	stream->Read(&m_bInactiveWithoutPrereqCivics);
	// MOD - END - Advanced Building Prerequisites (Civic)
	...
	stream->Read(&m_iNumPrereqAndCivics);
	SAFE_DELETE_ARRAY(m_piPrereqAndCivics);
	if (m_iNumPrereqAndCivics > 0)
	{
		m_piPrereqAndCivics = new int[m_iNumPrereqAndCivics];
		stream->Read(m_iNumPrereqAndCivics, m_piPrereqAndCivics);
	}
	...
	stream->Read(&m_iNumPrereqGlobalBuildingClasses);
	SAFE_DELETE_ARRAY(m_paPrereqGlobalBuildingClasses);
	if (m_iNumPrereqGlobalBuildingClasses > 0)
	{
		m_paPrereqGlobalBuildingClasses = new BuildingClassCount[m_iNumPrereqGlobalBuildingClasses];
		for (int iI = 0; iI < m_iNumPrereqGlobalBuildingClasses; iI++)
		{
			m_paPrereqGlobalBuildingClasses[iI].read(stream);
		}
	}
	...
	*/
	...
	// MOD - START - Bonus Converters
	stream->Read(&m_iNumConsumedBonuses);
	SAFE_DELETE_ARRAY(m_piConsumedBonusType);
	SAFE_DELETE_ARRAY(m_piConsumedBonusAmount);
	if (m_iNumConsumedBonuses > 0)
	{
		m_piConsumedBonusType = new int[m_iNumConsumedBonuses];
		m_piConsumedBonusAmount = new int[m_iNumConsumedBonuses];
		stream->Read(m_iNumConsumedBonuses, m_piConsumedBonusType);
		stream->Read(m_iNumConsumedBonuses, m_piConsumedBonusAmount);
	}

	SAFE_DELETE_ARRAY(m_piConsumedBonusIndex);
	m_piConsumedBonusIndex = new int[GC.getNumBonusInfos()];
	stream->Read(GC.getNumBonusInfos(), m_piConsumedBonusIndex);

	stream->Read(&m_iNumProducedBonuses);
	SAFE_DELETE_ARRAY(m_piProducedBonusType);
	SAFE_DELETE_ARRAY(m_piProducedBonusAmount);
	if (m_iNumProducedBonuses > 0)
	{
		m_piProducedBonusType = new int[m_iNumProducedBonuses];
		m_piProducedBonusAmount = new int[m_iNumProducedBonuses];
		stream->Read(m_iNumProducedBonuses, m_piProducedBonusType);
		stream->Read(m_iNumProducedBonuses, m_piProducedBonusAmount);
	}

	SAFE_DELETE_ARRAY(m_piProducedBonusIndex);
	m_piProducedBonusIndex = new int[GC.getNumBonusInfos()];
	stream->Read(GC.getNumBonusInfos(), m_piProducedBonusIndex);
	// MOD - END - Bonus Converters
	...
	// MOD - START -Building Heal Rate
	SAFE_DELETE_ARRAY(m_piDomainHealRateChanges);
	m_piDomainHealRateChanges = new int[NUM_DOMAIN_TYPES];
	stream->Read(NUM_DOMAIN_TYPES, m_piDomainHealRateChanges);

	SAFE_DELETE_ARRAY(m_piUnitClassHealRateChanges);
	m_piUnitClassHealRateChanges = new int[GC.getNumUnitClassInfos()];
	stream->Read(GC.getNumUnitClassInfos(), m_piUnitClassHealRateChanges);

	SAFE_DELETE_ARRAY(m_piUnitCombatHealRateChanges);
	m_piUnitCombatHealRateChanges = new int[GC.getNumUnitCombatInfos()];
	stream->Read(GC.getNumUnitCombatInfos(), m_piUnitCombatHealRateChanges);
	// MOD - END - Building Heal Rate
	...
	// MOD - START - Simultaneous Production
	SAFE_DELETE_ARRAY(m_piUnitCombatSimultaneousProduction);
	m_piUnitCombatSimultaneousProduction = new int[GC.getNumUnitCombatInfos()];
	stream->Read(GC.getNumUnitCombatInfos(), m_piUnitCombatSimultaneousProduction);
	// MOD - END - Simultaneous Production

	// MOD - START - Relation Caching
	stream->Read(&m_iNumBuilderUnitClasses);
	SAFE_DELETE_ARRAY(m_piBuilderUnitClasses);
	if (m_iNumBuilderUnitClasses > 0)
	{
		m_piBuilderUnitClasses = new int[m_iNumBuilderUnitClasses];
		stream->Read(m_iNumBuilderUnitClasses, m_piBuilderUnitClasses);
	}

	stream->Read(&m_iNumBuilderUnits);
	SAFE_DELETE_ARRAY(m_piBuilderUnits);
	if (m_iNumBuilderUnits > 0)
	{
		m_piBuilderUnits = new int[m_iNumBuilderUnits];
		stream->Read(m_iNumBuilderUnits, m_piBuilderUnits);
	}
	...
	// MOD - START - Global Terrain Double Moves
	SAFE_DELETE_ARRAY(m_pbGlobalTerrainDoubleMove);
	m_pbGlobalTerrainDoubleMove = new bool[GC.getNumTerrainInfos()];
	stream->Read(GC.getNumTerrainInfos(), m_pbGlobalTerrainDoubleMove);
	// MOD - END - Global Terrain Double Moves
...
/************************************************************************************************/
	...
	/*
	stream->Read(&m_iNumBonusYieldChanges);
	SAFE_DELETE_ARRAY(m_paBonusYieldChanges);
	if (m_iNumBonusYieldChanges > 0)
	{
		m_paBonusYieldChanges = new BonusYieldChange[m_iNumBonusYieldChanges];
		for (int iI = 0; iI < m_iNumBonusYieldChanges; iI++)
		{
			m_paBonusYieldChanges[iI].read(stream);
		}
	}
	*/

	stream->Read(&m_iNumBonusYieldModifiers);
	SAFE_DELETE_ARRAY(m_paBonusYieldModifiers);
	if (m_iNumBonusYieldModifiers > 0)
	{
		m_paBonusYieldModifiers = new BonusYieldModifier[m_iNumBonusYieldModifiers];
		for (int iI = 0; iI < m_iNumBonusYieldModifiers; iI++)
		{
			m_paBonusYieldModifiers[iI].read(stream);
		}
	}
	...
	stream->Read(&m_iNumBuildingYieldChanges);
	SAFE_DELETE_ARRAY(m_paBuildingYieldChanges);
	if (m_iNumBuildingYieldChanges > 0)
	{
		m_paBuildingYieldChanges = new BuildingYieldChange[m_iNumBuildingYieldChanges];
		for (int iI = 0; iI < m_iNumBuildingYieldChanges; iI++)
		{
			m_paBuildingYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumBuildingCommerceChanges);
	SAFE_DELETE_ARRAY(m_paBuildingCommerceChanges);
	if (m_iNumBuildingCommerceChanges > 0)
	{
		m_paBuildingCommerceChanges = new BuildingCommerceChange[m_iNumBuildingCommerceChanges];
		for (int iI = 0; iI < m_iNumBuildingCommerceChanges; iI++)
		{
			m_paBuildingCommerceChanges[iI].read(stream);
		}
	}

	/*
	stream->Read(&m_iNumGlobalBonusYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalBonusYieldChanges);
	if (m_iNumGlobalBonusYieldChanges > 0)
	{
		m_paGlobalBonusYieldChanges = new BonusYieldChange[m_iNumGlobalBonusYieldChanges];
		for (int iI = 0; iI < m_iNumGlobalBonusYieldChanges; iI++)
		{
			m_paGlobalBonusYieldChanges[iI].read(stream);
		}
	}
	*/

	stream->Read(&m_iNumGlobalImprovementYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalImprovementYieldChanges);
	if (m_iNumGlobalImprovementYieldChanges > 0)
	{
		m_paGlobalImprovementYieldChanges = new ImprovementYieldChange[m_iNumGlobalImprovementYieldChanges];
		for (int iI = 0; iI < m_iNumGlobalImprovementYieldChanges; iI++)
		{
			m_paGlobalImprovementYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumGlobalSpecialistYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalSpecialistYieldChanges);
	if (m_iNumGlobalSpecialistYieldChanges > 0)
	{
		m_paGlobalSpecialistYieldChanges = new SpecialistYieldChange[m_iNumGlobalSpecialistYieldChanges];
		for (int iI = 0; iI < m_iNumGlobalSpecialistYieldChanges; iI++)
		{
			m_paGlobalSpecialistYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumGlobalSpecialistCommerceChanges);
	SAFE_DELETE_ARRAY(m_paGlobalSpecialistCommerceChanges);
	if (m_iNumGlobalSpecialistCommerceChanges > 0)
	{
		m_paGlobalSpecialistCommerceChanges = new SpecialistCommerceChange[m_iNumGlobalSpecialistCommerceChanges];
		for (int iI = 0; iI < m_iNumGlobalSpecialistCommerceChanges; iI++)
		{
			m_paGlobalSpecialistCommerceChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumGlobalBuildingYieldChanges);
	SAFE_DELETE_ARRAY(m_paGlobalBuildingYieldChanges);
	if (m_iNumGlobalBuildingYieldChanges > 0)
	{
		m_paGlobalBuildingYieldChanges = new BuildingYieldChange[m_iNumGlobalBuildingYieldChanges];
		for (int iI = 0; iI < m_iNumGlobalBuildingYieldChanges; iI++)
		{
			m_paGlobalBuildingYieldChanges[iI].read(stream);
		}
	}

	stream->Read(&m_iNumGlobalBuildingCommerceChanges);
	SAFE_DELETE_ARRAY(m_paGlobalBuildingCommerceChanges);
	if (m_iNumGlobalBuildingCommerceChanges > 0)
	{
		m_paGlobalBuildingCommerceChanges = new BuildingCommerceChange[m_iNumGlobalBuildingCommerceChanges];
		for (int iI = 0; iI < m_iNumGlobalBuildingCommerceChanges; iI++)
		{
			m_paGlobalBuildingCommerceChanges[iI].read(stream);
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
}
...
void CvBuildingInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Building Replacement
	stream->Write(m_iObsoleteBuildingClass);
	// MOD - END - Building Replacement
	stream->Write(m_iObsoleteTech);
	// MOD - START - Building Discontinuation
	stream->Write(m_iDiscontinueTech);
	// MOD - END - Building Discontinuation
	...
	// MOD - START - Wartime Conscription
	stream->Write(m_iWarConscript);
	stream->Write(m_iGlobalWarConscript);
	// MOD - END - Wartime Conscription
	stream->Write(m_iHealRateChange);
	// MOD - START - Revolutions (separatism)
	stream->Write(m_iSeparatism);
	stream->Write(m_iGlobalSeparatism);
	// MOD - END - Revolutions (separatism)
	stream->Write(m_iHealth);
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	stream->Write(m_iEpidemicModifier);
	stream->Write(m_iGlobalEpidemicModifier);
	// MOD - END - Epidemics
	stream->Write(m_iGlobalPopulationChange);
	stream->Write(m_iFreeTechs);
	stream->Write(m_iDefenseModifier);
	stream->Write(m_iNaturalDefenseModifier);	
	// MOD - START - Obsolete Defense
	stream->Write(m_iObsoleteSafeDefenseModifier);
	// MOD - END - Obsolete Defense
	stream->Write(m_iBombardDefenseModifier);
	// MOD - START - Obsolete Defense
	stream->Write(m_iObsoleteSafeBombardDefenseModifier);
	// MOD - END - Obsolete Defense
	...
	stream->Write(m_iOutsideCityCongestionThresholdChange);
	// MOD - START - Unique Buildings
	stream->Write(m_bUniqueBuilding);
	// MOD - END - Unique Buildings
	stream->Write(m_bIgnoreWorks);	
	// MOD - START - Military Disciplines
	stream->Write(m_bMilitaryDiscipline);
	// MOD - END - Military Disciplines
	stream->Write(m_bTeamShare);
	stream->Write(m_bWater);
	stream->Write(m_bRiver);
	// MOD - START - Fresh Water Aquifers
	stream->Write(m_bFreshWaterSource);
	// MOD - END - Fresh Water Aquifers
	...
	// MOD - START - Unlimited Conscription
	stream->Write(m_bGlobalUnlimitedConscript);
	// MOD - END - Unlimited Conscription
	// MOD - START - Bonus Converters
	stream->Write(m_bBonusConverter);
	// MOD - END - Bonus Converters
	// MOD - START - Advanced Building Prerequisites (Power)
	stream->Write(m_bPrereqPower);
	stream->Write(m_bInactiveWithoutPower);
	// MOD - END - Advanced Building Prerequisites (Power)
	// MOD - START - Advanced Building Prerequisites (Religion)
	stream->Write(m_bInactiveWithoutPrereqStateReligion);
	// MOD - END - Advanced Building Prerequisites (Religion)
	// MOD - START - Advanced Building Prerequisites (Civic)
	stream->Write(m_bInactiveWithoutPrereqCivics);
	// MOD - END - Advanced Building Prerequisites (Civic)
	...
	stream->Write(m_iNumPrereqAndCivics);
	if (m_iNumPrereqAndCivics > 0)
	{
		stream->Write(m_iNumPrereqAndCivics, m_piPrereqAndCivics);
	}
	...
	stream->Write(m_iNumPrereqGlobalBuildingClasses);
	if (m_iNumPrereqGlobalBuildingClasses > 0)
	{
		for (int iI = 0; iI < m_iNumPrereqGlobalBuildingClasses; iI++)
		{
			m_paPrereqGlobalBuildingClasses[iI].write(stream);
		}
	}
	...
	stream->Write(GC.getNumBonusInfos(), m_piBonusProductionModifier);
	// MOD - START - Bonus Converters
	stream->Write(m_iNumConsumedBonuses);
	if (m_iNumConsumedBonuses > 0)
	{
		stream->Write(m_iNumConsumedBonuses, m_piConsumedBonusType);
		stream->Write(m_iNumConsumedBonuses, m_piConsumedBonusAmount);
	}
	stream->Write(GC.getNumBonusInfos(), m_piConsumedBonusIndex);

	stream->Write(m_iNumProducedBonuses);
	if (m_iNumProducedBonuses > 0)
	{
		stream->Write(m_iNumProducedBonuses, m_piProducedBonusType);
		stream->Write(m_iNumProducedBonuses, m_piProducedBonusAmount);
	}
	stream->Write(GC.getNumBonusInfos(), m_piProducedBonusIndex);
	// MOD - END - Bonus Converters
	...
	// MOD - START - Building Heal Rate
	stream->Write(NUM_DOMAIN_TYPES, m_piDomainHealRateChanges);
	stream->Write(GC.getNumUnitClassInfos(), m_piUnitClassHealRateChanges);
	stream->Write(GC.getNumUnitCombatInfos(), m_piUnitCombatHealRateChanges);
	// MOD - END - Building Heal Rate
	...
	// MOD - START - Simultaneous Production
	stream->Write(GC.getNumUnitCombatInfos(), m_piUnitCombatSimultaneousProduction);
	// MOD - END - Simultaneous Production

	// MOD - START - Relation Caching
	stream->Write(m_iNumBuilderUnitClasses);
	if (m_iNumBuilderUnitClasses > 0)
	{
		stream->Write(m_iNumBuilderUnitClasses, m_piBuilderUnitClasses);
	}

	stream->Write(m_iNumBuilderUnits);
	if (m_iNumBuilderUnits > 0)
	{
		stream->Write(m_iNumBuilderUnits, m_piBuilderUnits);
	}
	...
	// MOD - START - Global Terrain Double Moves
	stream->Write(GC.getNumTerrainInfos(), m_pbGlobalTerrainDoubleMove);
	// MOD - END - Global Terrain Double Moves
	// MOD - START - Advanced Building Prerequisites (Civic)
	...
	*/
	...
	/*
	stream->Write(m_iNumBonusYieldChanges);
	if (m_iNumBonusYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumBonusYieldChanges; iI++)
		{
			m_paBonusYieldChanges[iI].write(stream);
		}
	}
	*/

	stream->Write(m_iNumBonusYieldModifiers);
	if (m_iNumBonusYieldModifiers > 0)
	{
		for (int iI = 0; iI < m_iNumBonusYieldModifiers; iI++)
		{
			m_paBonusYieldModifiers[iI].write(stream);
		}
	}
	...
	stream->Write(m_iNumBuildingYieldChanges);
	if (m_iNumBuildingYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumBuildingYieldChanges; iI++)
		{
			m_paBuildingYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumBuildingCommerceChanges);
	if (m_iNumBuildingCommerceChanges > 0)
	{
		for (int iI = 0; iI < m_iNumBuildingCommerceChanges; iI++)
		{
			m_paBuildingCommerceChanges[iI].write(stream);
		}
	}

	/*
	stream->Write(m_iNumGlobalBonusYieldChanges);
	if (m_iNumGlobalBonusYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalBonusYieldChanges; iI++)
		{
			m_paGlobalBonusYieldChanges[iI].write(stream);
		}
	}
	*/

	stream->Write(m_iNumGlobalImprovementYieldChanges);
	if (m_iNumGlobalImprovementYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalImprovementYieldChanges; iI++)
		{
			m_paGlobalImprovementYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumGlobalSpecialistYieldChanges);
	if (m_iNumGlobalSpecialistYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalSpecialistYieldChanges; iI++)
		{
			m_paGlobalSpecialistYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumGlobalSpecialistCommerceChanges);
	if (m_iNumGlobalSpecialistCommerceChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalSpecialistCommerceChanges; iI++)
		{
			m_paGlobalSpecialistCommerceChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumGlobalBuildingYieldChanges);
	if (m_iNumGlobalBuildingYieldChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalBuildingYieldChanges; iI++)
		{
			m_paGlobalBuildingYieldChanges[iI].write(stream);
		}
	}

	stream->Write(m_iNumGlobalBuildingCommerceChanges);
	if (m_iNumGlobalBuildingCommerceChanges > 0)
	{
		for (int iI = 0; iI < m_iNumGlobalBuildingCommerceChanges; iI++)
		{
			m_paGlobalBuildingCommerceChanges[iI].write(stream);
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
}
...
bool CvBuildingInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Advanced Building Prerequisites
	// Note: The old PrereqTech and Bonus tags are no longer supported
	pXML->SetInfoArray(&m_iNumPrereqAndTechs, &m_piPrereqAndTechs, "PrereqAndTechs");
	pXML->SetInfoArray(&m_iNumPrereqAndCivics, &m_piPrereqAndCivics, "PrereqAndCivics");
	pXML->SetInfoArray(&m_iNumPrereqAndBonuses, &m_piPrereqAndBonuses, "PrereqAndBonuses");
	pXML->SetInfoArray(&m_iNumPrereqOrBonuses, &m_piPrereqOrBonuses, "PrereqOrBonuses");
	pXML->SetInfoValueArray<BuildingClassCount, BuildingClassTypes>(&m_iNumPrereqGlobalBuildingClasses, &m_paPrereqGlobalBuildingClasses, "PrereqGlobalBuildings", "PrereqGlobalBuilding", "BuildingClassType", "iNumBuildingNeeded");
	pXML->SetInfoArray(&m_iNumPrereqAndBuildingClasses, &m_piPrereqAndBuildingClasses, "PrereqAndBuildings");

	/*
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "PrereqCivics"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "PrereqCivic"))
		{
			do
			{
				pXML->GetXmlVal(szTextVal);
				m_aszExtraXMLforCivicPrereqsPass3.push_back(szTextVal);
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "PrereqCivic"));
	...
	*/
	...
	{
		m_pbPrereqCivics[iI] = false;
	}

	for (int iPre = 0; iPre < getNumPrereqAndCivics(); iPre++)
	{
		CivicTypes eCivicType = (CivicTypes)getPrereqAndCivics(iPre);
		if (eCivicType != NO_CIVIC)
		{
			m_pbPrereqCivics[eCivicType] = true;
		}
	}
	...
	pXML->GetChildXmlValByName(szTextVal, "PrereqVictory");
	...
	pXML->GetChildXmlValByName(&m_bStateReligion, "bPrereqCurrentStateReligion");
	pXML->GetChildXmlValByName(&m_bInactiveWithoutPower, "bInactiveWithoutPower", false);
	pXML->GetChildXmlValByName(&m_bInactiveWithoutPrereqStateReligion, "bInactiveWithoutPrereqStateReligion", false);
	pXML->GetChildXmlValByName(&m_bInactiveWithoutPrereqCivics, "bInactiveWithoutPrereqCivics", false);
	// MOD - END - Advanced Building Prerequisites
	...
	// MOD - START - Building Replacement
	pXML->GetChildXmlValByName(szTextVal, "ObsoleteBuildingClass");
	m_iObsoleteBuildingClass = pXML->FindInInfoClass(szTextVal);
	// MOD - END - Building Replacement
	...
	// MOD - START - Building Discontinuation
	pXML->GetChildXmlValByName(szTextVal, "DiscontinueTech");
	m_iDiscontinueTech = pXML->FindInInfoClass(szTextVal);
	// MOD - END - Building Discontinuation
	...
	pXML->GetChildXmlValByName(&m_iOutsideCityCongestionThresholdChange, "iOutsideCityCongestionThresholdChange");	
	// MOD - START - Unique Buildings
	pXML->GetChildXmlValByName(&m_bUniqueBuilding, "bUniqueBuilding");
	// MOD - END - Unique Buildings
	pXML->GetChildXmlValByName(&m_bIgnoreWorks, "bIgnoreWorks");	
	// MOD - START - Military Disciplines
	pXML->GetChildXmlValByName(&m_bMilitaryDiscipline, "bMilitaryDiscipline");
	// MOD - END - Military Disciplines	
	pXML->GetChildXmlValByName(&m_bTeamShare, "bTeamShare");
	pXML->GetChildXmlValByName(&m_bWater, "bWater");
	pXML->GetChildXmlValByName(&m_bRiver, "bRiver");
	// MOD - START - Fresh Water Aquifers
	pXML->GetChildXmlValByName(&m_bFreshWaterSource, "bFreshWaterSource");
	// MOD - END - Fresh Water Aquifers
	...
	// MOD - START - Unlimited Conscription
	pXML->GetChildXmlValByName(&m_bGlobalUnlimitedConscript, "bGlobalUnlimitedConscript");
	// MOD - END - Unlimited Conscription
	pXML->GetChildXmlValByName(&m_bMapCentering, "bMapCentering");
	...
	pXML->GetChildXmlValByName(&m_iProductionCost, "iCost");
	// MOD - START - Building Timer
	pXML->GetChildXmlValByName(&m_iTimer, "iTimer");
	// MOD - END - Building Timer
	pXML->GetChildXmlValByName(&m_iHurryCostModifier, "iHurryCostModifier");
	...
	// MOD - START - Wartime Conscription
	pXML->GetChildXmlValByName(&m_iWarConscript, "iWarConscript");
	pXML->GetChildXmlValByName(&m_iGlobalWarConscript, "iGlobalWarConscript");
	// MOD - END - Wartime Conscription
	pXML->GetChildXmlValByName(&m_iHealRateChange, "iHealRateChange");
	// MOD - START - Revolutions (separatism)
	pXML->GetChildXmlValByName(&m_iSeparatism, "iSeparatism");
	pXML->GetChildXmlValByName(&m_iGlobalSeparatism, "iGlobalSeparatism");
	// MOD - END - Revolutions (separatism)
	pXML->GetChildXmlValByName(&m_iHealth, "iHealth");
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	pXML->GetChildXmlValByName(&m_iEpidemicModifier, "iEpidemicModifier", 0);
	pXML->GetChildXmlValByName(&m_iGlobalEpidemicModifier, "iGlobalEpidemicModifier", 0);
	// MOD - END - Epidemics
	pXML->GetChildXmlValByName(&m_iHappiness, "iHappiness");
	...
	pXML->GetChildXmlValByName(&m_iNaturalDefenseModifier, "iNaturalDefense");	
	// MOD - START - Obsolete Defense
	pXML->GetChildXmlValByName(&m_iObsoleteSafeDefenseModifier, "iObsoleteSafeDefense");
	// MOD - END - Obsolete Defense
	pXML->GetChildXmlValByName(&m_iBombardDefenseModifier, "iBombardDefense");
	// MOD - START - Obsolete Defense
	pXML->GetChildXmlValByName(&m_iObsoleteSafeBombardDefenseModifier, "iObsoleteSafeBombardDefense");
	// MOD - END - Obsolete Defense
	...
	*/
	...
	{
		m_piConsumedBonusIndex[j] = -1;
	}
	
	std::vector<CvBonusAmount> aConsumedBonus;
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "ConsumesBonuses"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "ConsumesBonus"))
		{
			do
			{
				BonusTypes eConsumedBonus = NO_BONUS;
				int iConsumedAmount = 0;

				if (pXML->GetChildXmlValByName(szTextVal, "BonusType"))
				{
					eConsumedBonus = (BonusTypes)(pXML->FindInInfoClass(szTextVal));
				}

				if (pXML->GetChildXmlValByName(&iConsumedAmount, "iBonusAmount"))
				{
					if (eConsumedBonus != NO_BONUS && iConsumedAmount > 0)
					{
						aConsumedBonus.push_back(CvBonusAmount(eConsumedBonus, iConsumedAmount));
					}
				}
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "ConsumesBonus"));
	...
	m_iNumConsumedBonuses = aConsumedBonus.size();
	SAFE_DELETE_ARRAY(m_piConsumedBonusType);
	SAFE_DELETE_ARRAY(m_piConsumedBonusAmount);
	if (m_iNumConsumedBonuses > 0)
	{
		m_piConsumedBonusType = new int[m_iNumConsumedBonuses];
		m_piConsumedBonusAmount = new int[m_iNumConsumedBonuses];
		for (int iConsumedBonusIndex = 0; iConsumedBonusIndex < m_iNumConsumedBonuses; iConsumedBonusIndex++)
		{
			CvBonusAmount kBonusAmount = aConsumedBonus[iConsumedBonusIndex];
			m_piConsumedBonusType[iConsumedBonusIndex] = kBonusAmount.m_eBonus;
			m_piConsumedBonusAmount[iConsumedBonusIndex] = kBonusAmount.m_iAmount;
			m_piConsumedBonusIndex[kBonusAmount.m_eBonus] = iConsumedBonusIndex; // Store the index so that we can quickly look up by Bonus as well
		}
	}
	aConsumedBonus.clear();
	
	SAFE_DELETE_ARRAY(m_piProducedBonusIndex);
	m_piProducedBonusIndex = new int[GC.getNumBonusInfos()];
	for (j=0;j<GC.getNumBonusInfos();j++)
	{
		m_piProducedBonusIndex[j] = -1;
	}

	std::vector<CvBonusAmount> aProducedBonus;
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "ProducesBonuses"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "ProducesBonus"))
		{
			do
			{
				BonusTypes eProducedBonus = NO_BONUS;
				int iProducedAmount = 0;

				if (pXML->GetChildXmlValByName(szTextVal, "BonusType"))
				{
					eProducedBonus = (BonusTypes)(pXML->FindInInfoClass(szTextVal));
				}

				if (pXML->GetChildXmlValByName(&iProducedAmount, "iBonusAmount"))
				{
					if (eProducedBonus != NO_BONUS && iProducedAmount > 0)
					{
						aProducedBonus.push_back(CvBonusAmount(eProducedBonus, iProducedAmount));
					}
				}
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "ProducesBonus"));
	...
	m_iNumProducedBonuses = aProducedBonus.size();
	SAFE_DELETE_ARRAY(m_piProducedBonusType);
	SAFE_DELETE_ARRAY(m_piProducedBonusAmount);
	if (m_iNumProducedBonuses > 0)
	{
		m_piProducedBonusType = new int[m_iNumProducedBonuses];
		m_piProducedBonusAmount = new int[m_iNumProducedBonuses];
		for (int iProducedBonusIndex = 0; iProducedBonusIndex < m_iNumProducedBonuses; iProducedBonusIndex++)
		{
			CvBonusAmount kBonusAmount = aProducedBonus[iProducedBonusIndex];
			m_piProducedBonusType[iProducedBonusIndex] = kBonusAmount.m_eBonus;
			m_piProducedBonusAmount[iProducedBonusIndex] = kBonusAmount.m_iAmount;
			m_piProducedBonusIndex[kBonusAmount.m_eBonus] = iProducedBonusIndex; // Store the index so that we can quickly look up by Bonus as well
		}
	}
	aProducedBonus.clear();

	if ((m_iNumConsumedBonuses > 0) || (m_iNumProducedBonuses > 0))
	{
		m_bBonusConverter = true;
	}
	else
	{
		m_bBonusConverter = false;
	...
	// MOD - START - Building Heal Rate
	pXML->SetVariableListTagPair(&m_piDomainHealRateChanges, "DomainHealRateChanges", sizeof(GC.getDomainInfo((DomainTypes)0)), NUM_DOMAIN_TYPES);
	pXML->SetVariableListTagPair(&m_piUnitClassHealRateChanges, "UnitClassHealRateChanges", sizeof(GC.getUnitClassInfo((UnitClassTypes)0)), GC.getNumUnitClassInfos());
	pXML->SetVariableListTagPair(&m_piUnitCombatHealRateChanges, "UnitCombatHealRateChanges", sizeof(GC.getUnitCombatInfo((UnitCombatTypes)0)), GC.getNumUnitCombatInfos());
	// MOD - END - Building Heal Rate

	// MOD - START - Global Terrain Double Moves
	pXML->SetVariableListTagPair(&m_pbGlobalTerrainDoubleMove, "GlobalTerrainDoubleMoves", sizeof(GC.getTerrainInfo((TerrainTypes)0)), GC.getNumTerrainInfos());
	// MOD - END - Global Terrain Double Moves
...
/************************************************************************************************/
	...
	FAssertMsg(NUM_YIELD_TYPES > 0, "The are no YieldTypes defined");
	FAssertMsg(NUM_COMMERCE_TYPES > 0, "There are no CommerceTypes defined");
	FAssertMsg(GC.getNumBonusInfos() > 0, "There are no BonusInfos defined");
	FAssertMsg(GC.getNumImprovementInfos() > 0, "There are no ImprovementInfos defined");
	FAssertMsg(GC.getNumSpecialistInfos() > 0, "There are no SpecialistInfos defined");
	FAssertMsg(GC.getNumBuildingClassInfos() > 0, "There are no BuildingClassInfos defined");

	//pXML->SetEconomicEffectEnumeration<BonusYieldChangeMap, BonusYieldChange, BonusTypes, YieldTypes>(&m_paBonusYieldChanges, &m_iNumBonusYieldChanges, "BonusYieldChanges", "BonusYieldChange", "BonusType", "YieldChanges", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<BonusYieldModifierMap, BonusYieldModifier, BonusTypes, YieldTypes>(&m_paBonusYieldModifiers, &m_iNumBonusYieldModifiers, "BonusYieldModifiers", "BonusYieldModifier", "BonusType", "YieldModifiers", "iYield", NUM_YIELD_TYPES);

	pXML->SetEconomicEffectEnumeration<ImprovementYieldChangeMap, ImprovementYieldChange, ImprovementTypes, YieldTypes>(&m_paImprovementYieldChanges, &m_iNumImprovementYieldChanges, "ImprovementYieldChanges", "ImprovementYieldChange", "ImprovementType", "YieldChanges", "iYield", NUM_YIELD_TYPES);

	pXML->SetEconomicEffectEnumeration<SpecialistYieldChangeMap, SpecialistYieldChange, SpecialistTypes, YieldTypes>(&m_paSpecialistYieldChanges, &m_iNumSpecialistYieldChanges, "SpecialistYieldChanges", "SpecialistYieldChange", "SpecialistType", "YieldChanges", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<SpecialistCommerceChangeMap, SpecialistCommerceChange, SpecialistTypes, CommerceTypes>(&m_paSpecialistCommerceChanges, &m_iNumSpecialistCommerceChanges, "SpecialistCommerceChanges", "SpecialistCommerceChange", "SpecialistType", "CommerceChanges", "iCommerce", NUM_COMMERCE_TYPES);

	pXML->SetEconomicEffectEnumeration<BuildingYieldChangeMap, BuildingYieldChange, BuildingClassTypes, YieldTypes>(&m_paBuildingYieldChanges, &m_iNumBuildingYieldChanges, "BuildingYieldChanges", "BuildingYieldChange", "BuildingClassType", "YieldChanges", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<BuildingCommerceChangeMap, BuildingCommerceChange, BuildingClassTypes, CommerceTypes>(&m_paBuildingCommerceChanges, &m_iNumBuildingCommerceChanges, "BuildingCommerceChanges", "BuildingCommerceChange", "BuildingClassType", "CommerceChanges", "iCommerce", NUM_COMMERCE_TYPES);

	//pXML->SetEconomicEffectEnumeration<BonusYieldChangeMap, BonusYieldChange, BonusClassTypes, YieldTypes>(&m_paGlobalBonusYieldChanges, &m_iNumGlobalBonusYieldChanges, "GlobalBonusYieldChanges", "BonusYieldChange", "BonusType", "YieldChanges", "iYield", NUM_YIELD_TYPES);

	pXML->SetEconomicEffectEnumeration<ImprovementYieldChangeMap, ImprovementYieldChange, ImprovementTypes, YieldTypes>(&m_paGlobalImprovementYieldChanges, &m_iNumGlobalImprovementYieldChanges, "GlobalImprovementYieldChanges", "ImprovementYieldChange", "ImprovementType", "YieldChanges", "iYield", NUM_YIELD_TYPES);

	pXML->SetEconomicEffectEnumeration<SpecialistYieldChangeMap, SpecialistYieldChange, SpecialistTypes, YieldTypes>(&m_paGlobalSpecialistYieldChanges, &m_iNumGlobalSpecialistYieldChanges, "GlobalSpecialistYieldChanges", "SpecialistYieldChange", "SpecialistType", "YieldChanges", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<SpecialistCommerceChangeMap, SpecialistCommerceChange, SpecialistTypes, CommerceTypes>(&m_paGlobalSpecialistCommerceChanges, &m_iNumGlobalSpecialistCommerceChanges, "GlobalSpecialistCommerceChanges", "SpecialistCommerceChange", "SpecialistType", "CommerceChanges", "iCommerce", NUM_COMMERCE_TYPES);

	pXML->SetEconomicEffectEnumeration<BuildingYieldChangeMap, BuildingYieldChange, BuildingClassTypes, YieldTypes>(&m_paGlobalBuildingYieldChanges, &m_iNumGlobalBuildingYieldChanges, "GlobalBuildingYieldChanges", "BuildingYieldChange", "BuildingClassType", "YieldChanges", "iYield", NUM_YIELD_TYPES);
	pXML->SetEconomicEffectEnumeration<BuildingCommerceChangeMap, BuildingCommerceChange, BuildingClassTypes, CommerceTypes>(&m_paGlobalBuildingCommerceChanges, &m_iNumGlobalBuildingCommerceChanges, "GlobalBuildingCommerceChanges", "BuildingCommerceChange", "BuildingClassType", "CommerceChanges", "iCommerce", NUM_COMMERCE_TYPES);
	// MOD - END - Advanced Yield and Commerce Effects
	...
	// MOD - START - Defense Modifiers (Mexico)
	pXML->SetVariableListTagPair(&m_piUnitCombatDefenseModifier, "UnitCombatDefenses", sizeof(GC.getUnitCombatInfo((UnitCombatTypes)0)), GC.getNumUnitCombatInfos());
	pXML->SetVariableListTagPair(&m_piUnitClassDefenseModifier, "UnitClassDefenses", sizeof(GC.getUnitClassInfo((UnitClassTypes)0)), GC.getNumUnitClassInfos());
	// MOD - END - Defense Modifiers (Mexico)

	// MOD - START - Simultaneous Production
	pXML->SetVariableListTagPair(&m_piUnitCombatSimultaneousProduction, "UnitCombatSimultaneousProductions", sizeof(GC.getUnitCombatInfo((UnitCombatTypes)0)), GC.getNumUnitCombatInfos());
	// MOD - END - Simultaneous Production
...
// MOD - START - Relation Caching
void CvBuildingInfo::cacheRelations()
{
	BuildingTypes eBuilding = (BuildingTypes) GC.getInfoTypeForString(getType());

	// Builders
	std::set<UnitClassTypes> aBuilderUnitClasses;
	std::vector<UnitTypes> aBuilderUnits;
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		...
		{
			aBuilderUnits.push_back(eUnit);
			aBuilderUnitClasses.insert((UnitClassTypes)kUnit.getUnitClassType());
		}
	}

	m_iNumBuilderUnitClasses = (int)aBuilderUnitClasses.size();
	SAFE_DELETE_ARRAY(m_piBuilderUnitClasses);
	if (m_iNumBuilderUnitClasses > 0)
	{
		m_piBuilderUnitClasses = new int[m_iNumBuilderUnitClasses];
		int iRel = 0;
		for (std::set<UnitClassTypes>::const_iterator it=aBuilderUnitClasses.begin(); it!=aBuilderUnitClasses.end(); ++it)
		{
			m_piBuilderUnitClasses[iRel++] = *it;
		}
	}
	aBuilderUnitClasses.clear();

	m_iNumBuilderUnits = (int)aBuilderUnits.size();
	SAFE_DELETE_ARRAY(m_piBuilderUnits);
	if (m_iNumBuilderUnits > 0)
	{
		m_piBuilderUnits = new int[m_iNumBuilderUnits];
		int iRel = 0;
		for (std::vector<UnitTypes>::const_iterator it=aBuilderUnits.begin(); it!=aBuilderUnits.end(); ++it)
		{
			m_piBuilderUnits[iRel++] = *it;
		}
	}
	aBuilderUnits.clear();
	...
	for (int iReligion = 0; iReligion < GC.getNumReligionInfos(); iReligion++)
	{
		if (isReligionRelatedToBuilding((ReligionTypes)iReligion, eBuilding))
		{
			aRelatedReligions.insert((ReligionTypes)iReligion);
		}
	}
	...
	for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
	{
		if (isCivicRelatedToBuilding((CivicTypes)iCivic, eBuilding))
		{
			aRelatedCivics.insert((CivicTypes)iCivic);
		}
	}
	...
	for (int iBonus = 0; iBonus < GC.getNumBonusInfos(); iBonus++)
	{
		if (isBonusRelatedToBuilding((BonusTypes)iBonus, eBuilding))
		{
			aRelatedBonuses.insert((BonusTypes)iBonus);
		}
	}
	...
	for (int iImprovement = 0; iImprovement < GC.getNumImprovementInfos(); iImprovement++)
	{
		if (isImprovementRelatedToBuilding((ImprovementTypes)iImprovement, eBuilding))
		{
			aRelatedImprovements.insert((ImprovementTypes)iImprovement);
		}
	}
	...
	for (int iSpecialist = 0; iSpecialist < GC.getNumSpecialistInfos(); iSpecialist++)
	{
		if (isSpecialistRelatedToBuilding((SpecialistTypes)iSpecialist, eBuilding))
		{
			aRelatedSpecialists.insert((SpecialistTypes)iSpecialist);
		}
	}
	...
	for (int iUnitCombat = 0; iUnitCombat < GC.getNumUnitCombatInfos(); iUnitCombat++)
	{
		if (isBuildingRelatedToUnitCombat(eBuilding, (UnitCombatTypes)iUnitCombat))
		{
			aRelatedUnitCombats.insert((UnitCombatTypes)iUnitCombat);
		}
	}
	...
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isBuildingRelatedToUnit(eBuilding, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isBuildingRelatedToBuilding(eBuilding, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
// MOD - START - Building Discontinuation
int CvSpecialBuildingInfo::getDiscontinueTech() const
{
	return m_iDiscontinueTech;
}
// MOD - END - Building Discontinuation
...
bool CvSpecialBuildingInfo::isExclusive( void ) const
{
	return m_bExclusive;
}
...
// MOD - START - Building Replacement
m_iNumReplacedBuildings(0),
// MOD - END - Building Replacement
m_bNoLimit(false),
m_bMonument(false),
// MOD - START - Building Replacement
m_piReplacedBuildings(NULL),
// MOD - END - Building Replacement
...
{
	// MOD - START - Building Replacement
	SAFE_DELETE_ARRAY(m_piReplacedBuildings);
	// MOD - END - Building Replacement
...
// MOD - START - Building Replacement
int CvBuildingClassInfo::getNumReplacedBuildings() const
{
	return m_iNumReplacedBuildings;
}

int CvBuildingClassInfo::getReplacedBuildings(int i) const
{
	FASSERT_BOUNDS(0, getNumReplacedBuildings(), i, "CvBuildingClassInfo::getReplacedBuildings");
	return m_piReplacedBuildings ? m_piReplacedBuildings[i] : -1;
}
// MOD - END - Building Replacement

// MOD - START - Relation Caching
int CvBuildingClassInfo::getRepresentativeBuilding() const
{
	int iDefaultBuilding = getDefaultBuildingIndex();
	if (iDefaultBuilding < 0)
	{
		if (getNumBuildingTypes() > 0)
		{
			return getBuildingTypes(0);
		}
	}
...
int CvBuildingClassInfo::getNumBuildingTypes() const
{
	return (int)m_aiBuildingTypes.size();
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int CvBuildingClassInfo::getBuildingTypes(int i) const
{
	return m_aiBuildingTypes[i];
}
// MOD - END - Relation Caching
...
bool CvBuildingClassInfo::readPass3()
{
	...
	// MOD - START - Robust XML Loader
	BuildingClassTypes eClassType = (BuildingClassTypes)GC.getInfoTypeForString(getType());

	BuildingTypes eDefaultBuilding = (BuildingTypes)GC.getInfoTypeForString(m_aszExtraXMLforPass3[0]);
	if (eDefaultBuilding == NO_BUILDING || GC.getBuildingInfo(eDefaultBuilding).getBuildingClassType() == eClassType)
	{
		m_iDefaultBuildingIndex = eDefaultBuilding;
	}
	else
	{
		m_iDefaultBuildingIndex = NO_BUILDING;
	...
	for (int iI = 0; iI < GC.getNumBuildingInfos(); iI++)
	{
		...
		if (GC.getBuildingInfo(eBuilding).getBuildingClassType() == eClassType)
		{
			m_aiBuildingTypes.push_back(eBuilding);
		}
	}
	// MOD - END - Relation Caching
...
// MOD - START - Building Replacement
void CvBuildingClassInfo::cacheReplacements(const std::multimap<BuildingClassTypes, BuildingTypes> &buildingReplacementMap)
{
	// This function builds up a list of all descendent buildings replaced by this class.
	// If C replaces B which replaces A, this will cache the replacements of both B and A within C.

	std::set<BuildingClassTypes> aProcessed;    // Building classes we've processed
	std::set<BuildingClassTypes> aQueued;       // Building classes we've yet to process
	std::set<BuildingTypes> aReplacedBuildings; // All of the buildings this class replaces

	aQueued.insert((BuildingClassTypes)GC.getInfoTypeForString(getType()));
	do
	{
		// Pull the next item out of the queue
		std::set<BuildingClassTypes>::iterator next = aQueued.begin();
		BuildingClassTypes eBuildingClass = *(next);
		aQueued.erase(next);

		// Make sure we don't needlessly process classes more than once
		aProcessed.insert(eBuildingClass);

		// Get the set of buildings replaced by the current item
		std::pair<std::multimap<BuildingClassTypes, BuildingTypes>::const_iterator, std::multimap<BuildingClassTypes, BuildingTypes>::const_iterator> bounds = buildingReplacementMap.equal_range(eBuildingClass);
	
		for (std::multimap<BuildingClassTypes, BuildingTypes>::const_iterator it=bounds.first; it != bounds.second; ++it)
		{
			BuildingTypes eLoopBuilding = it->second;

			// Note the replacement of the building
			aReplacedBuildings.insert(eLoopBuilding);

			// Check to see whether the replaced building's class also replaces other buildings
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)GC.getBuildingInfo(eLoopBuilding).getBuildingClassType();
			if (aProcessed.find(eLoopBuildingClass) != aProcessed.end())
			{
				continue;
			}
			if (buildingReplacementMap.find(eLoopBuildingClass) == buildingReplacementMap.end())
			{
				continue;
			}

			// Add the downstream building class to the queue
			aQueued.insert(eLoopBuildingClass);
		}
	}
	while (!aQueued.empty());

	m_iNumReplacedBuildings = (int)aReplacedBuildings.size();
	SAFE_DELETE_ARRAY(m_piReplacedBuildings);
	if (m_iNumReplacedBuildings > 0)
	{
		m_piReplacedBuildings = new int[m_iNumReplacedBuildings];
		int iRel = 0;
		for (std::set<BuildingTypes>::const_iterator it=aReplacedBuildings.begin(); it!=aReplacedBuildings.end(); ++it)
		{
			m_piReplacedBuildings[iRel++] = *it;
		}
	}
}
// MOD - END - Building Replacement
...
bool CvCivilizationInfo::isDerivative() const
{
	return m_bDerivative;
}
...
int CvCivilizationInfo::getNumDerivativeCivs() const
{
	return m_aDerivativeCivs.size();
}

CivilizationTypes CvCivilizationInfo::getFirstDerivative() const
{
	CivilizationTypes eBestCiv = NO_CIVILIZATION;
	int iBestValue = -1;
	for (int i = 0; i < (int)m_aDerivativeCivs.size(); i++)
		{
			CvDerivativeCivData kDerivativeCiv = m_aDerivativeCivs.at(i);
			int iValue = (1 + GC.getGameINLINE().getSorenRandNum(100, "Choosing Split Personality"));
			iValue += kDerivativeCiv.m_iWeight;

			if (iValue > iBestValue)
				{
					iBestValue = iValue;
					eBestCiv = kDerivativeCiv.m_eCivilization;
				}
		}
	return eBestCiv;
}

CivilizationTypes CvCivilizationInfo::getNDerivative(int i) const
{
	CvDerivativeCivData kDerivativeCiv = m_aDerivativeCivs.at(i);
	return kDerivativeCiv.m_eCivilization;
}

CvDerivativeCivData CvCivilizationInfo::getDerivativeCiv(int i) const
{
	return m_aDerivativeCivs[i];
}
// MOD - END - Multiple Derivative Civs

// MOD - START - Unit Upgrade Cache
void CvCivilizationInfo::getUpgradesForUnit(UnitTypes eUnit, std::vector<UnitTypes>& aUpgradeUnits) const
{
	// TODO: Consider computing the max tree size on load?
	int maxUpgradeTreeSize = GC.getNumUnitInfos();
	int i = 0;

	while(i < maxUpgradeTreeSize)
	{
		const std::vector<UnitClassTypes>& upgradeClasses = GC.getUnitInfo(eUnit).getUpgradeUnitClassTypes();
		for (std::vector<UnitClassTypes>::const_iterator it = upgradeClasses.begin(); it != upgradeClasses.end(); ++it)
		{
			UnitTypes eLoopUnit = (UnitTypes)getCivilizationUnits(*it);
			if (eLoopUnit != NO_UNIT)
			{
				if (std::find(aUpgradeUnits.begin(), aUpgradeUnits.end(), eLoopUnit) == aUpgradeUnits.end())
				{
					aUpgradeUnits.push_back(eLoopUnit);
				}
			}
		}

		if ((int)aUpgradeUnits.size() > i)
			eUnit = aUpgradeUnits[i++];
		else
			break;
	}
}
...
const std::vector<UnitTypes>& CvCivilizationInfo::getBuilderUnits() const
{
	return m_aeBuilderUnits;
}

bool CvCivilizationInfo::canBuild(int i) const
{
	return m_abBuilds[i];
}

bool CvCivilizationInfo::canProduceImprovement(int i) const
{
	return m_abImprovements[i];
}
...
void CvCivilizationInfo::read(FDataStreamBase* stream)
{
	...
	m_aDerivativeCivs.clear();
	m_aDerivativeCivs.reserve(iNumElements);
	for (int i = 0; i < iNumElements; i++)
	{
		CvDerivativeCivData kData;
		kData.read(stream);
		m_aDerivativeCivs.push_back(kData);
	}
	// MOD - END - Multiple Derivative Civs

	// MOD - START - Unique Improvements
	int iNumBuilderUnits;
	stream->Read(&iNumBuilderUnits);
	m_aeBuilderUnits.reserve(iNumBuilderUnits);
	for (int i = 0; i < iNumBuilderUnits; i++)
	{
		int iUnitType;
		stream->Read(&iUnitType);
		m_aeBuilderUnits.push_back((UnitTypes)iUnitType);
	}

	m_abBuilds.resize(GC.getNumBuildInfos(), false);
	...
	m_abImprovements.resize(GC.getNumImprovementInfos(), false);
	for (int i = 0; i < GC.getNumImprovementInfos(); i++)
	{
		bool bValue;
		stream->Read(&bValue);
		m_abImprovements[i] = bValue;
	}
	// MOD - END - Unique Improvements
}
...
void CvCivilizationInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Multiple Derivative Civs
	stream->Write(m_aDerivativeCivs.size());
	for (std::vector<CvDerivativeCivData>::const_iterator it = m_aDerivativeCivs.begin(); it != m_aDerivativeCivs.end(); ++it)
	{
		(*it).write(stream);
	}
	// MOD - END - Multiple Derivative Civs
	...
	// MOD - START - Unique Improvements
	stream->Write((int)m_aeBuilderUnits.size());
	for (std::vector<UnitTypes>::const_iterator it = m_aeBuilderUnits.begin(); it != m_aeBuilderUnits.end(); ++it)
	{
		stream->Write((int)*it);
	}

	for (std::vector<bool>::const_iterator it = m_abBuilds.begin(); it != m_abBuilds.end(); ++it)
	{
		stream->Write(*it);
	}

	for (std::vector<bool>::const_iterator it = m_abImprovements.begin(); it != m_abImprovements.end(); ++it)
	{
		stream->Write(*it);
	}
	// MOD - END - Unique Improvements
...
bool CvCivilizationInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Robust XML Loader
	char szTypeVal[256];					// holds the text value of the relevant typeinfo
	// MOD - END - Robust XML Loader
	...
	// MOD - START - Multiple Derivative Civs
	pXML->GetChildXmlValByName(&m_bDerivative, "bDerivative");
	// MOD - END - Multiple Derivative Civs
	...
	// MOD - START - Robust XML Loader
	// Setup default civilization buildings
	pXML->InitBuildingDefaults(&m_piCivilizationBuildings);

	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Buildings"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Building"))
		{
			do
			{
				if (pXML->GetChildXmlValByName(szClassVal, "BuildingClassType") &&
					pXML->GetChildXmlValByName(szTypeVal, "BuildingType"))
				{
					BuildingClassTypes eBuildingClass = (BuildingClassTypes)pXML->FindInInfoClass(szClassVal);
					if (eBuildingClass > -1)
					{
						// Note: Assigns -1 if not found
						BuildingTypes eCivBuilding = (BuildingTypes)pXML->FindInInfoClass(szTypeVal);
						if (eCivBuilding == NO_BUILDING || GC.getBuildingInfo(eCivBuilding).getBuildingClassType() == eBuildingClass)
						{
							m_piCivilizationBuildings[eBuildingClass] = eCivBuilding;
						}
						else
						{
							TCHAR	szMessage[1024];
							snprintf( szMessage, 1024, "%s was specified as the %s building for the %s civilization, but the building is not a member of that building class.\n\nCurrent XML file is: %s", szTypeVal, szClassVal, m_szType.GetCString(), GC.getCurrentXMLFile().GetCString());
							gDLL->MessageBox(szMessage, "XML Load Error");
						}
					}
					else
			...
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "Building"));
	...
	// MOD - START - Robust XML Loader
	// Setup default civilization units
	pXML->InitUnitDefaults(&m_piCivilizationUnits);

	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Units"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Unit"))
		{
			do
			{
				if (pXML->GetChildXmlValByName(szClassVal, "UnitClassType") &&
					pXML->GetChildXmlValByName(szTypeVal, "UnitType"))
				{
					UnitClassTypes eUnitClass = (UnitClassTypes)pXML->FindInInfoClass(szClassVal);
					if (eUnitClass > -1)
					{
						// Note: Assigns -1 if not found
						UnitTypes eCivUnit = (UnitTypes)pXML->FindInInfoClass(szTypeVal);
						if (eCivUnit == NO_UNIT || GC.getUnitInfo(eCivUnit).getUnitClassType() == eUnitClass)
						{
							m_piCivilizationUnits[eUnitClass] = eCivUnit;
						}
						else
						{
							TCHAR	szMessage[1024];
							snprintf( szMessage, 1024, "%s was specified as the %s unit for the %s civilization, but the unit is not a member of that unit class.\n\nCurrent XML file is: %s", szTypeVal, szClassVal, m_szType.GetCString(), GC.getCurrentXMLFile().GetCString());
							gDLL->MessageBox(szMessage, "XML Load Error");
						}
					}
					else
			...
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "Unit"));
	...
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "CivicType"))
		{
			j = 0;
			do
			{
				FAssertMsg((j < GC.getNumCivicOptionInfos()),"InitialCivics list is greater than array size");
				pXML->GetXmlVal(szTypeVal);
				m_piCivilizationInitialCivics[j++] = pXML->FindInInfoClass(szTypeVal);
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "CivicType"));
	...
	// MOD - START - Multiple Derivative Civs
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"DerivativeCivs"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "DerivativeCiv"))
		{
			do
			{
				EraTypes eMinEraType;

				if (pXML->GetChildXmlValByName(szTypeVal, "MinEraType"))
				{
					eMinEraType = (EraTypes)(pXML->FindInInfoClass(szTypeVal));
					if (eMinEraType == NO_ERA)
					{
						// Invalid era; default to first era
						eMinEraType = (EraTypes)0;
					}
				}
				else
				{
					// No era; default to first era
					eMinEraType = (EraTypes)0;
				}

				if (pXML->GetChildXmlValByName(szTypeVal, "CivilizationType"))
				{
					int iWeight;
					pXML->GetChildXmlValByName(&iWeight, "iWeight");

					m_aExtraXMLforDerivativesPass3.push_back(std::make_pair(szTypeVal, std::make_pair(eMinEraType, iWeight)));
				}
			}
			while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "DerivativeCiv"));
	...
	{
		UnitTypes eCivUnit = (UnitTypes)getCivilizationUnits(iI);
		if (eCivUnit != NO_UNIT)
		{
			CvUnitInfo& kCivUnit = GC.getUnitInfo(eCivUnit);
			bool bBuilder = false;

			for (int iJ = 0; iJ < GC.getNumBuildInfos(); iJ++)
			{
				if (kCivUnit.getBuilds(iJ))
				{
					bBuilder = true;
					m_abBuilds[iJ] = true;
					...
					while (eImprovement != NO_IMPROVEMENT && (--iLoopDetector > 0))
					{
						m_abImprovements[eImprovement] = true;
			...
			if (bBuilder)
			{
				m_aeBuilderUnits.push_back(eCivUnit);
			}
		}
	}
	// MOD - END - Unique Improvements
...
{
	int iNumDerivatives = m_aExtraXMLforDerivativesPass3.size();

	if (0 < iNumDerivatives)
	{
		m_aDerivativeCivs.clear();

		for(int iI = 0; iI < iNumDerivatives; iI++)
		{
			CvString szCivilizationType	= m_aExtraXMLforDerivativesPass3[iI].first;
			EraTypes eMinEraType		= m_aExtraXMLforDerivativesPass3[iI].second.first;
			int iWeight					= m_aExtraXMLforDerivativesPass3[iI].second.second;

			CivilizationTypes eCivilizationType = (CivilizationTypes)GC.getInfoTypeForString(szCivilizationType);
			if (eCivilizationType != NO_CIVILIZATION)
			{
				CvDerivativeCivData kData = CvDerivativeCivData(eCivilizationType, eMinEraType, iWeight);
				m_aDerivativeCivs.push_back(kData);
			}
		}
	}

	m_aExtraXMLforDerivativesPass3.clear();
...
m_iPerCityResearchCostModifier(0),
...
m_iMetricChangePercent(0),
// MOD - END - Population Metrics
// MOD - START - Epidemics
m_iEpidemicBaseRate(0),
m_iEpidemicPopulationRate(0),
m_iEpidemicUnhealthRate(0),
m_iEpidemicTradeRouteRate(0),
// MOD - END - Epidemics
m_iHealthBonus(0),
m_iHappyBonus(0),
m_iAttitudeChange(0),
...
// MOD - START - Per City Research Cost
int CvHandicapInfo::getPerCityResearchCostModifier() const
{
	return m_iPerCityResearchCostModifier;
}
// MOD - END - Per City Research Cost
...
// MOD - START - Population Metrics
int CvHandicapInfo::getMetricChangePercent() const
{
	return m_iMetricChangePercent;
}
// MOD - END - Population Metrics

// MOD - START - Epidemics
int CvHandicapInfo::getEpidemicBaseRate() const
{
	return m_iEpidemicBaseRate;
}

int CvHandicapInfo::getEpidemicPopulationRate() const
{
	return m_iEpidemicPopulationRate;
}

int CvHandicapInfo::getEpidemicUnhealthRate() const
{
	return m_iEpidemicUnhealthRate;
}

int CvHandicapInfo::getEpidemicTradeRouteRate() const
{
	return m_iEpidemicTradeRouteRate;
}
// MOD - END - Epidemics
...
// MOD - START - Population Metrics
int CvHandicapInfo::getAIMetricChangePercent() const
{
	return m_iAIMetricChangePercent;
}
// MOD - END - Population Metrics
...
void CvHandicapInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Per City Research Cost
	stream->Read(&m_iPerCityResearchCostModifier);
	// MOD - END - Per City Research Cost
	...
	// MOD - START - Population Metrics
	stream->Read(&m_iMetricChangePercent);
	// MOD - END - Population Metrics
	// MOD - START - Epidemics
	stream->Read(&m_iEpidemicBaseRate);
	stream->Read(&m_iEpidemicPopulationRate);
	stream->Read(&m_iEpidemicUnhealthRate);
	stream->Read(&m_iEpidemicTradeRouteRate);
	// MOD - END - Epidemics
	...
	// MOD - START - Population Metrics
	stream->Read(&m_iAIMetricChangePercent);
	// MOD - END - Population Metrics
...
void CvHandicapInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Per City Research Cost
	stream->Write(m_iPerCityResearchCostModifier);
	// MOD - END - Per City Research Cost
	...
	// MOD - START - Population Metrics
	stream->Write(m_iMetricChangePercent);
	// MOD - END - Population Metrics
	// MOD - START - Epidemics
	stream->Write(m_iEpidemicBaseRate);
	stream->Write(m_iEpidemicPopulationRate);
	stream->Write(m_iEpidemicUnhealthRate);
	stream->Write(m_iEpidemicTradeRouteRate);
	// MOD - END - Epidemics
	...
	// MOD - START - Population Metrics
	stream->Write(m_iAIMetricChangePercent);
	// MOD - END - Population Metrics
...
bool CvHandicapInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Per City Research Cost
	pXML->GetChildXmlValByName(&m_iPerCityResearchCostModifier, "iPerCityResearchCostModifier");
	// MOD - END - Per City Research Cost
	...
	// MOD - START - Population Metrics
	pXML->GetChildXmlValByName(&m_iMetricChangePercent, "iMetricChangePercent");
	// MOD - END - Population Metrics
	// MOD - START - Epidemics
	pXML->GetChildXmlValByName(&m_iEpidemicBaseRate, "iEpidemicBaseRate");
	pXML->GetChildXmlValByName(&m_iEpidemicPopulationRate, "iEpidemicPopulationRate");
	pXML->GetChildXmlValByName(&m_iEpidemicUnhealthRate, "iEpidemicUnhealthRate");
	pXML->GetChildXmlValByName(&m_iEpidemicTradeRouteRate, "iEpidemicTradeRouteRate");
	// MOD - END - Epidemics
	...
	// MOD - START - Population Metrics
	pXML->GetChildXmlValByName(&m_iAIMetricChangePercent, "iAIMetricChangePercent");
	// MOD - END - Population Metrics
...
// MOD - START - Population Metrics
int CvGameSpeedInfo::getMetricChangePercent() const
{
	return m_iMetricChangePercent;
}
// MOD - END - Population Metrics
...
// MOD - START - Holy City Migration
int CvGameSpeedInfo::getHolyCityInfluenceRate() const
{
	return m_iHolyCityInfluenceRate;
}
// MOD - START - Holy City Migration
...
bool CvGameSpeedInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Holy City Migration
	pXML->GetChildXmlValByName(&m_iHolyCityInfluenceRate, "iHolyCityInfluenceRate");
	// MOD - START - Holy City Migration
...
bool CvRouteInfo::isUniqueRoute()
{
	return m_bUnique;
}
...
// MOD - START - Relation Caching
int CvRouteInfo::getNumBuilds() const
{
	return m_iNumBuilds;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int CvRouteInfo::getBuilds(int i) const
{
	FASSERT_BOUNDS(0, getNumBuilds(), i, "CvRouteInfo::getBuilds");
	return m_piBuilds ? m_piBuilds[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
void CvRouteInfo::cacheRelations()
{
	RouteTypes eRoute = (RouteTypes)GC.getInfoTypeForString(getType());

	// Builds
	std::vector<BuildTypes> aBuilds;
	for (int iBuild = 0; iBuild < GC.getNumBuildInfos(); iBuild++)
	{
		...
		{
			aBuilds.push_back(eBuild);
		}
	}

	m_iNumBuilds = (int)aBuilds.size();
	SAFE_DELETE_ARRAY(m_piBuilds);
	if (m_iNumBuilds > 0)
	{
		m_piBuilds = new int[m_iNumBuilds];
		int iRel = 0;
		for (std::vector<BuildTypes>::const_iterator it=aBuilds.begin(); it!=aBuilds.end(); ++it)
		{
			m_piBuilds[iRel++] = *it;
		}
	}
...
m_iNumImprovementTypes(0),
m_piImprovementTypes(NULL)
{
}
...
CvImprovementClassInfo::~CvImprovementClassInfo()
{
	SAFE_DELETE_ARRAY(m_piImprovementTypes);
}

int CvImprovementClassInfo::getNumImprovementTypes() const
{
	return m_iNumImprovementTypes;
}

int CvImprovementClassInfo::getImprovementTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementTypes(), i, "CvImprovementClassInfo::getImprovementTypes");
	return m_piImprovementTypes ? m_piImprovementTypes[i] : -1;
}
...
{
	ImprovementClassTypes eImprovementClassType = (ImprovementClassTypes)GC.getInfoTypeForString(getType());

	std::vector<ImprovementTypes> aImprovementTypes;
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		ImprovementTypes eImprovement = (ImprovementTypes)iI;

		if (GC.getImprovementInfo(eImprovement).getImprovementClassType() == eImprovementClassType)
		{
			aImprovementTypes.push_back(eImprovement);
		}
	}
	m_iNumImprovementTypes = (int)aImprovementTypes.size();
	SAFE_DELETE_ARRAY(m_piImprovementTypes);
	if (m_iNumImprovementTypes > 0)
	{
		m_piImprovementTypes = new int[m_iNumImprovementTypes];
		for (int iI = 0; iI < m_iNumImprovementTypes; iI++)
		{
			m_piImprovementTypes[iI] = aImprovementTypes[iI];
		}
	}
...
// MOD - START - Improvement Classes
m_iImprovementClassType(NO_IMPROVEMENTCLASS),
// MOD - END - Improvement Classes
m_iAdvancedStartCost(0),
m_iAdvancedStartCostIncrease(0),
m_iTilesPerGoody(0),
...
// MOD - START - Relation Caching
m_iNumBuilds(0),
m_iNumRelatedTechs(0),
m_iNumRelatedTraits(0),
m_iNumRelatedCivics(0),
m_iNumRelatedBuildingClasses(0),
// MOD - END - Relation Caching
// MOD - START - Unique Improvements
m_bUniqueImprovement(false),
// MOD - END - Unique Improvements
// MOD - START - Improvement Civic prerequisite
m_iImpPrereqCivic(NO_CIVIC),
...
// MOD - START - Advanced Yield and Commerce Effects
m_ppiTechIrrigatedYieldChanges(NULL),
// MOD - END - Advanced Yield and Commerce Effects
...
CvImprovementInfo::~CvImprovementInfo()
{
	...
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piBuilds);
	SAFE_DELETE_ARRAY(m_piRelatedTechs);
	SAFE_DELETE_ARRAY(m_piRelatedTraits);
	SAFE_DELETE_ARRAY(m_piRelatedCivics);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	// MOD - END - Relation Caching
	...
	// MOD - START - Advanced Yield and Commerce Effects
	if (m_ppiTechIrrigatedYieldChanges != NULL)
	{
		for (iI=0;iI<GC.getNumTechInfos();iI++)
		{
			SAFE_DELETE_ARRAY(m_ppiTechIrrigatedYieldChanges[iI]);
		}
		SAFE_DELETE_ARRAY(m_ppiTechIrrigatedYieldChanges);
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Improvement Classes
int CvImprovementInfo::getImprovementClassType() const
{
	return m_iImprovementClassType;
}
// MOD - END - Improvement Classes
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvImprovementInfo::getEpidemicModifier() const
{
	return m_iEpidemicModifier;
}
// MOD - END - Epidemics
...
// MOD - START - Aid
int CvImprovementInfo::getNumAidTypes() const
{
	return m_iNumAidTypes;
}
// MOD - END - Aid
...
// MOD - START - Detriments
int CvImprovementInfo::getNumDetrimentTypes() const
{
	return m_iNumDetrimentTypes;
}
// MOD - END - Detriments
...
// MOD - START - Relation Caching
int CvImprovementInfo::getNumBuilds() const
{
	return m_iNumBuilds;
}

int CvImprovementInfo::getNumRelatedTechs() const
{
	return m_iNumRelatedTechs;
}

int CvImprovementInfo::getNumRelatedTraits() const
{
	return m_iNumRelatedTraits;
}

int CvImprovementInfo::getNumRelatedCivics() const
{
	return m_iNumRelatedCivics;
}

int CvImprovementInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
// MOD - START - Unique Improvements
bool CvImprovementInfo::isUniqueImprovement() const
{
	return m_bUniqueImprovement;
}
// MOD - END - Unique Improvements

// MOD - START - Improvement Civic prerequisite
int CvImprovementInfo::getImpPrereqCivic() const
{
  return m_iImpPrereqCivic;
}
// MOD - END - Improvement Civic prerequisite
...
// MOD - START - Aid
int CvImprovementInfo::getAidContribution(AidStyleTypes eAidStyle) const
{
	switch (eAidStyle)
	{
		case AIDSTYLE_FIXED:
			return 1;
		case AIDSTYLE_RELATIVE_STRENGTH:
			FAssertMsg(false, "AIDSTYLE_RELATIVE_STRENGTH is not available to improvements");
			return 0;
		default:
			FAssert(false);
			return 0;
...
// MOD - START - Advanced Yield and Commerce Effects
int CvImprovementInfo::getTechIrrigatedYieldChanges(int i, int j) const
{
	FASSERT_BOUNDS(0, GC.getNumTechInfos(), i, "CvImprovementInfo::getTechIrrigatedYieldChanges");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, j, "CvImprovementInfo::getTechIrrigatedYieldChanges");
	return m_ppiTechIrrigatedYieldChanges[i][j];
}

int* CvImprovementInfo::getTechIrrigatedYieldChangeArray(int i)
{
	return m_ppiTechIrrigatedYieldChanges[i];
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Aid
int CvImprovementInfo::getAidTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumAidTypes(), i, "CvImprovementInfo::getAidTypes");
	return m_piAidTypes ? m_piAidTypes[i] : -1;
}
// MOD - END - Aid

// MOD - START - Detriments
int CvImprovementInfo::getDetrimentTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumDetrimentTypes(), i, "CvImprovementInfo::getDetrimentTypes");
	return m_piDetrimentTypes ? m_piDetrimentTypes[i] : -1;
}
// MOD - END - Detriments

// MOD - START - Relation Caching
int CvImprovementInfo::getBuilds(int i) const
{
	FASSERT_BOUNDS(0, getNumBuilds(), i, "CvImprovementInfo::getBuilds");
	return m_piBuilds ? m_piBuilds[i] : -1;
}

int CvImprovementInfo::getRelatedTechs(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedTechs(), i, "CvImprovementInfo::getRelatedTechs");
	return m_piRelatedTechs ? m_piRelatedTechs[i] : -1;
}

int CvImprovementInfo::getRelatedTraits(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedTraits(), i, "CvImprovementInfo::getRelatedTraits");
	return m_piRelatedTraits ? m_piRelatedTraits[i] : -1;
}

int CvImprovementInfo::getRelatedCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedCivics(), i, "CvImprovementInfo::getRelatedCivics");
	return m_piRelatedCivics ? m_piRelatedCivics[i] : -1;
}

int CvImprovementInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvImprovementInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
void CvImprovementInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Improvement Classes
	stream->Read(&m_iImprovementClassType);
	// MOD - END - Improvement Classes
	stream->Read(&m_iAdvancedStartCost);
	stream->Read(&m_iAdvancedStartCostIncrease);
	...
	// MOD - START - Unique Improvements
	stream->Read(&m_bUniqueImprovement);
	// MOD - END - Unique Improvements
	// MOD - START - Improvement Civic prerequisite
	stream->Read(&m_iImpPrereqCivic);
	// MOD - END - Improvement Civic prerequisite
	...
	// MOD - START - Aid
	stream->Read(&m_iNumAidTypes);
	SAFE_DELETE_ARRAY(m_piAidTypes);
	if (m_iNumAidTypes > 0)
	{
		m_piAidTypes = new int[m_iNumAidTypes];
		stream->Read(m_iNumAidTypes, m_piAidTypes);
	}
	// MOD - END - Aid

	// MOD - START - Detriments
	stream->Read(&m_iNumDetrimentTypes);
	SAFE_DELETE_ARRAY(m_piDetrimentTypes);
	if (m_iNumDetrimentTypes > 0)
	{
		m_piDetrimentTypes = new int[m_iNumDetrimentTypes];
		stream->Read(m_iNumDetrimentTypes, m_piDetrimentTypes);
	}
	// MOD - END - Detriments

	// MOD - START - Relation Caching
	stream->Read(&m_iNumBuilds);
	SAFE_DELETE_ARRAY(m_piBuilds);
	if (m_iNumBuilds > 0)
	{
		m_piBuilds = new int[m_iNumBuilds];
		stream->Read(m_iNumBuilds, m_piBuilds);
	}

	stream->Read(&m_iNumRelatedTechs);
	SAFE_DELETE_ARRAY(m_piRelatedTechs);
	if (m_iNumRelatedTechs > 0)
	{
		m_piRelatedTechs = new int[m_iNumRelatedTechs];
		stream->Read(m_iNumRelatedTechs, m_piRelatedTechs);
	}

	stream->Read(&m_iNumRelatedTraits);
	SAFE_DELETE_ARRAY(m_piRelatedTraits);
	if (m_iNumRelatedTraits > 0)
	{
		m_piRelatedTraits = new int[m_iNumRelatedTraits];
		stream->Read(m_iNumRelatedTraits, m_piRelatedTraits);
	}
	...
	if (m_ppiTechIrrigatedYieldChanges != NULL)
	{
		for(i=0;i<GC.getNumTechInfos();i++)
		{
			SAFE_DELETE_ARRAY(m_ppiTechIrrigatedYieldChanges[i]);
		}
		SAFE_DELETE_ARRAY(m_ppiTechIrrigatedYieldChanges);
	}

	m_ppiTechIrrigatedYieldChanges = new int*[GC.getNumTechInfos()];
	for(i=0;i<GC.getNumTechInfos();i++)
	{
		m_ppiTechIrrigatedYieldChanges[i]  = new int[NUM_YIELD_TYPES];
		stream->Read(NUM_YIELD_TYPES, m_ppiTechIrrigatedYieldChanges[i]);
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
void CvImprovementInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Improvement Classes
	stream->Write(m_iImprovementClassType);
	// MOD - END - Improvement Classes
	stream->Write(m_iAdvancedStartCost);
	stream->Write(m_iAdvancedStartCostIncrease);
	...
	// MOD - START - Unique Improvements
	stream->Write(m_bUniqueImprovement);
	// MOD - END - Unique Improvements
	// MOD - START - Improvement Civic prerequisite
	stream->Write(m_iImpPrereqCivic);
	// MOD - END - Improvement Civic prerequisite
	...
	// MOD - START - Aid
	stream->Write(m_iNumAidTypes);
	if (m_iNumAidTypes > 0)
	{
		stream->Write(m_iNumAidTypes, m_piAidTypes);
	}
	// MOD - END - Aid
	// MOD - START - Detriments
	stream->Write(m_iNumDetrimentTypes);
	if (m_iNumDetrimentTypes > 0)
	{
		stream->Write(m_iNumDetrimentTypes, m_piDetrimentTypes);
	}
	// MOD - END - Detriments
	// MOD - START - Relation Caching
	stream->Write(m_iNumBuilds);
	if (m_iNumBuilds > 0)
	{
		stream->Write(m_iNumBuilds, m_piBuilds);
	}
	stream->Write(m_iNumRelatedTechs);
	if (m_iNumRelatedTechs > 0)
	{
		stream->Write(m_iNumRelatedTechs, m_piRelatedTechs);
	}
	stream->Write(m_iNumRelatedTraits);
	if (m_iNumRelatedTraits > 0)
	{
		stream->Write(m_iNumRelatedTraits, m_piRelatedTraits);
	}
	stream->Write(m_iNumRelatedCivics);
	if (m_iNumRelatedCivics > 0)
	{
	...
	for(i=0;i<GC.getNumTechInfos();i++)
	{
		stream->Write(NUM_YIELD_TYPES, m_ppiTechIrrigatedYieldChanges[i]);
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
bool CvImprovementInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Improvement Classes
	pXML->GetChildXmlValByName( szTextVal, "ImprovementClassType");
	m_iImprovementClassType = pXML->FindInInfoClass(szTextVal);
	// MOD - END - Improvement Classes

	pXML->GetChildXmlValByName(&m_iAdvancedStartCost, "iAdvancedStartCost");
	pXML->GetChildXmlValByName(&m_iAdvancedStartCostIncrease, "iAdvancedStartCostIncrease");
	// MOD - START - Unique Improvements
	pXML->GetChildXmlValByName(&m_bUniqueImprovement, "bUniqueImprovement");
	// MOD - END - Unique Improvements
	// MOD - START - Improvement Civic prerequisite
	//pXML->GetChildXmlValByName(szTextVal, "ImpPrereqCivic", "NONE");
	//m_iImpPrereqCivic = pXML->FindInInfoClass(szTextVal);
	if ( pXML->GetChildXmlValByName(szTextVal, "ImpPrereqCivic") )
	{
		m_iImpPrereqCivic = pXML->FindInInfoClass(szTextVal);
	}
	else
	...
	// initialize the boolean list to the correct size and all the booleans to false
	FAssertMsg((GC.getNumTechInfos() > 0) && (NUM_YIELD_TYPES) > 0,"either the number of tech infos is zero or less or the number of yield types is zero or less");
	pXML->Init2DIntList(&m_ppiTechIrrigatedYieldChanges, GC.getNumTechInfos(), NUM_YIELD_TYPES);
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"TechIrrigatedYieldChanges"))
	{
		if (pXML->SkipToNextVal())
		{
			iNumSibs = gDLL->getXMLIFace()->GetNumChildren(pXML->GetXML());
			if (gDLL->getXMLIFace()->SetToChild(pXML->GetXML()))
			{
				if (0 < iNumSibs)
				{
					for (j=0;j<iNumSibs;j++)
					{
						...
						{
							// delete the array since it will be reallocated
							SAFE_DELETE_ARRAY(m_ppiTechIrrigatedYieldChanges[iIndex]);
							// if we can set the current xml node to it's next sibling
							if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"TechIrrigatedYields"))
							{
								// call the function that sets the yield change variable
								pXML->SetYields(&m_ppiTechIrrigatedYieldChanges[iIndex]);
								gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
							}
							else
							{
	...
	// MOD - START - Aid
	pXML->SetInfoArray(&m_iNumAidTypes, &m_piAidTypes, "AidTypes");
	// MOD - END - Aid

	// MOD - START - Detriments
	pXML->SetInfoArray(&m_iNumDetrimentTypes, &m_piDetrimentTypes, "DetrimentTypes");
	// MOD - END - Detriments
...
// MOD - START - Relation Caching
void CvImprovementInfo::cacheRelations()
{
	...
	// Techs
	std::set<TechTypes> aRelatedTechs;
	for (int iTech = 0; iTech < GC.getNumTechInfos(); iTech++)
	{
		if (isTechRelatedToImprovement((TechTypes)iTech, eImprovement))
		{
			aRelatedTechs.insert((TechTypes)iTech);
		}
	}

	m_iNumRelatedTechs = (int)aRelatedTechs.size();
	SAFE_DELETE_ARRAY(m_piRelatedTechs);
	if (m_iNumRelatedTechs > 0)
	{
		m_piRelatedTechs = new int[m_iNumRelatedTechs];
		int iRel = 0;
		for (std::set<TechTypes>::const_iterator it=aRelatedTechs.begin(); it!=aRelatedTechs.end(); ++it)
		{
			m_piRelatedTechs[iRel++] = *it;
		}
	}
	aRelatedTechs.clear();
	...
	for (int iTrait = 0; iTrait < GC.getNumTraitInfos(); iTrait++)
	{
		if (isTraitRelatedToImprovement((TraitTypes)iTrait, eImprovement))
		{
			aRelatedTraits.insert((TraitTypes)iTrait);
		}
	}
	...
	for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
	{
		if (isCivicRelatedToImprovement((CivicTypes)iCivic, eImprovement))
		{
			aRelatedCivics.insert((CivicTypes)iCivic);
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isImprovementRelatedToBuilding(eImprovement, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
m_pbNoReligionHealthiness(NULL),
m_pbNoReligionHappiness(NULL)
...
CvBonusInfo::~CvBonusInfo()
{
	...
	// MOD - START - Religion Affected Bonuses
	SAFE_DELETE_ARRAY(m_pbNoReligionHealthiness);
	SAFE_DELETE_ARRAY(m_pbNoReligionHappiness);
	// MOD - END - Religion Affected Bonuses
}
...
// MOD - START - Bonus Category for Globe View
int CvBonusInfo::getBonusCategory() const
{
	return m_iBonusCategory;
}
// MOD - END - Bonus Category for Globe View
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvBonusInfo::getEpidemicModifier() const
{
	return m_iEpidemicModifier;
}
// MOD - END - Epidemics
...
// MOD - START - Relation Caching
int CvBonusInfo::getNumRelatedBuilds() const
{
	return m_iNumRelatedBuilds;
}

int CvBonusInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvBonusInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int  CvBonusInfo::getRelatedBuilds(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuilds(), i, "CvBonusInfo::getRelatedBuilds");
	return m_piRelatedBuilds ? m_piRelatedBuilds[i] : -1;
}

int CvBonusInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvBonusInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvBonusInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvBonusInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Religion Affected Bonuses
bool CvBonusInfo::isNoReligionHealthiness(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumReligionInfos(), i, "CvBonusInfo::isNoReligionHealthiness");
	return m_pbNoReligionHealthiness ? m_pbNoReligionHealthiness[i] : false;
}

bool CvBonusInfo::isNoReligionHappiness(int i) const
{
	FASSERT_BOUNDS(0, GC.getNumReligionInfos(), i, "CvBonusInfo::isNoReligionHappiness");
	return m_pbNoReligionHappiness ? m_pbNoReligionHappiness[i] : false;
}
// MOD - END - Religion Affected Bonuses
...
void CvBonusInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Religion Affected Bonuses
	SAFE_DELETE_ARRAY(m_pbNoReligionHealthiness);
	m_pbNoReligionHealthiness = new bool[GC.getNumReligionInfos()];
	stream->Read(GC.getNumReligionInfos(), m_pbNoReligionHealthiness);

	SAFE_DELETE_ARRAY(m_pbNoReligionHappiness);
	m_pbNoReligionHappiness = new bool[GC.getNumReligionInfos()];
	stream->Read(GC.getNumReligionInfos(), m_pbNoReligionHappiness);
	// MOD - END - Religion Affected Bonuses
}
...
void CvBonusInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Religion Affected Bonuses
	stream->Write(GC.getNumReligionInfos(), m_pbNoReligionHealthiness);
	stream->Write(GC.getNumReligionInfos(), m_pbNoReligionHappiness);
	// MOD - END - Religion Affected Bonuses
}
#endif
bool CvBonusInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Bonus Category for Globe View
	pXML->GetChildXmlValByName(&m_iBonusCategory, "BonusCategory");
	// MOD - END - Bonus Category for Globe View
	...
	// MOD - START - Religion Affected Bonuses
	pXML->SetVariableListTagPair(&m_pbNoReligionHealthiness, "NoReligionHealthinesses", sizeof(GC.getReligionInfo((ReligionTypes)0)), GC.getNumReligionInfos());
	pXML->SetVariableListTagPair(&m_pbNoReligionHappiness, "NoReligionHappinesses", sizeof(GC.getReligionInfo((ReligionTypes)0)), GC.getNumReligionInfos());
	// MOD - END - Religion Affected Bonuses
...
// MOD - START - Relation Caching
void CvBonusInfo::cacheRelations()
{
	BonusTypes eBonus = (BonusTypes) GC.getInfoTypeForString(getType());

	// Note: If any member of a class is dependent, the entire class is considered dependent

	// TODO: Move most of this to a centralized relation caching function on CvXMLLoadUtility?
	//       Make this a setter function and pass the relations into it, perhaps as arrays?

	// Builds
	std::set<BuildTypes> aRelatedBuilds;
	for (int iBuild = 0; iBuild < GC.getNumBuildInfos(); iBuild++)
	{
		if (isBonusRelatedToBuild(eBonus, (BuildTypes)iBuild))
		{
			aRelatedBuilds.insert((BuildTypes)iBuild);
		}
	}
	...
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isBonusRelatedToUnit(eBonus, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isBonusRelatedToBuilding(eBonus, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvFeatureInfo::getEpidemicModifier() const
{
	return m_iEpidemicModifier;
}
// MOD - END - Epidemics
...
m_iBaseOpenBordersFreeCityCommerce(0),
m_iBaseOpenBordersCommerceRateModifier(0),
m_iAdditionalOpenBordersFreeCityCommerce(0),
m_iAdditionalOpenBordersCommerceRateModifier(0),
// MOD - END - Open Borders Commerce
// MOD - START - Tech Transfer Commerce
//m_iBaseTechTransferFreeCityCommerce(0),
m_iBaseTechTransferCommerceRateModifier(0),
//m_iAdditionalTechTransferFreeCityCommerce(0),
m_iAdditionalTechTransferCommerceRateModifier(0),
m_iVassalTechTransferCommerceRateModifier(0),
// MOD - END - Tech Transfer Commerce
m_iAIWeightPercent(0),
m_bFlexiblePercent(false),
// MOD - START - Open Borders Commerce
m_pbOpenBordersCommerceAttitudeTypes(NULL)
// MOD - END - Open Borders Commerce
{
}
...
{
	// MOD - START - Open Borders Commerce
	SAFE_DELETE_ARRAY(m_pbOpenBordersCommerceAttitudeTypes);
	// MOD - END - Open Borders Commerce
}
...
// MOD - START - Open Borders Commerce
bool CvCommerceInfo::getOpenBordersCommerceAttitudeTypes(int i) const
{
	FAssertMsg(i < NUM_ATTITUDE_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_pbOpenBordersCommerceAttitudeTypes[i];
}

bool* CvCommerceInfo::getOpenBordersCommerceAttitudeTypes() const
{
	return m_pbOpenBordersCommerceAttitudeTypes;
}

void CvCommerceInfo::setOpenBordersCommerceAttitudeTypes(int i, bool bVal)
{
	FAssertMsg(i < NUM_ATTITUDE_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	m_pbOpenBordersCommerceAttitudeTypes[i] = bVal;
}

int CvCommerceInfo::getBaseOpenBordersFreeCityCommerce() const
{
	return m_iBaseOpenBordersFreeCityCommerce;
}

int CvCommerceInfo::getBaseOpenBordersCommerceRateModifier() const
{
	return m_iBaseOpenBordersCommerceRateModifier;
}

int CvCommerceInfo::getAdditionalOpenBordersFreeCityCommerce() const
{
	return m_iAdditionalOpenBordersFreeCityCommerce;
}

int CvCommerceInfo::getAdditionalOpenBordersCommerceRateModifier() const
{
	return m_iAdditionalOpenBordersCommerceRateModifier;
}
// MOD - END - Open Borders Commerce

// MOD - START - Tech Transfer Commerce
/*
int CvCommerceInfo::getBaseTechTransferFreeCityCommerce() const
{
	return m_iBaseTechTransferFreeCityCommerce;
}
*/

int CvCommerceInfo::getBaseTechTransferCommerceRateModifier() const
{
	return m_iBaseTechTransferCommerceRateModifier;
}

/*
int CvCommerceInfo::getAdditionalTechTransferFreeCityCommerce() const
{
	return m_iAdditionalTechTransferFreeCityCommerce;
}
*/

int CvCommerceInfo::getAdditionalTechTransferCommerceRateModifier() const
{
	return m_iAdditionalTechTransferCommerceRateModifier;
}
// MOD - END - Tech Transfer Commerce

// MOD - START - Vassal Tech Transfer
int CvCommerceInfo::getVassalTechTransferCommerceRateModifier() const
{
	return m_iVassalTechTransferCommerceRateModifier;
}
...
bool CvCommerceInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Open Borders Commerce
	pXML->GetChildXmlValByName(&m_iBaseOpenBordersFreeCityCommerce, "iBaseOpenBordersFreeCityCommerce");
	pXML->GetChildXmlValByName(&m_iBaseOpenBordersCommerceRateModifier, "iBaseOpenBordersCommerceRateModifier");
	pXML->GetChildXmlValByName(&m_iAdditionalOpenBordersFreeCityCommerce, "iAdditionalOpenBordersFreeCityCommerce");
	pXML->GetChildXmlValByName(&m_iAdditionalOpenBordersCommerceRateModifier, "iAdditionalOpenBordersCommerceRateModifier");
	// MOD - END - Open Borders Commerce

	// MOD - START - Tech Transfer Commerce
	//pXML->GetChildXmlValByName(&m_iBaseTechTransferFreeCityCommerce, "iBaseTechTransferFreeCityCommerce");
	pXML->GetChildXmlValByName(&m_iBaseTechTransferCommerceRateModifier, "iBaseTechTransferCommerceRateModifier");
	//pXML->GetChildXmlValByName(&m_iAdditionalTechTransferFreeCityCommerce, "iAdditionalTechTransferFreeCityCommerce");
	pXML->GetChildXmlValByName(&m_iAdditionalTechTransferCommerceRateModifier, "iAdditionalTechTransferCommerceRateModifier");
	// MOD - END - Tech Transfer Commerce
	// MOD - START - Vassal Tech Transfer
	pXML->GetChildXmlValByName(&m_iVassalTechTransferCommerceRateModifier, "iVassalTechTransferCommerceRateModifier");
	// MOD - END - Vassal Tech Transfer
	...
	// MOD - START - Open Borders Commerce
	pXML->SetVariableListTagPair(&m_pbOpenBordersCommerceAttitudeTypes, "OpenBordersCommerceAttitudes", sizeof(GC.getAttitudeInfo((AttitudeTypes)0)), NUM_ATTITUDE_TYPES);
	// MOD - END - Open Borders Commerce
...
// MOD - START - Epidemics
m_iAcceptableEpidemicPercent(0),
// MOD - END - Epidemics
m_iBaseAttitude(0),
...
m_iDifferentIdeologyAttitudeChange(0),
m_iDifferentIdeologyAttitudeDivisor(0),
m_iDifferentIdeologyAttitudeChangeLimit(0),
...
m_iFavoriteReligionAttitudeChange(0),
m_iFavoriteReligionAttitudeDivisor(0),
m_iFavoriteReligionAttitudeChangeLimit(0),
...
// MOD - START - Epidemics
int CvLeaderHeadInfo::getAcceptableEpidemicPercent() const
{
	return m_iAcceptableEpidemicPercent;
}
// MOD - END - Epidemics
...
int CvLeaderHeadInfo::getDifferentIdeologyAttitudeChange() const
{
	return m_iDifferentIdeologyAttitudeChange;
}

int CvLeaderHeadInfo::getDifferentIdeologyAttitudeDivisor() const
{
	return m_iDifferentIdeologyAttitudeDivisor;
}

int CvLeaderHeadInfo::getDifferentIdeologyAttitudeChangeLimit() const
{
	return m_iDifferentIdeologyAttitudeChangeLimit;
}
...
// MOD - START - Favorite Religion
int CvLeaderHeadInfo::getFavoriteReligionAttitudeChange() const
{
	return m_iFavoriteReligionAttitudeChange;
}

int CvLeaderHeadInfo::getFavoriteReligionAttitudeDivisor() const
{
	return m_iFavoriteReligionAttitudeDivisor;
}

int CvLeaderHeadInfo::getFavoriteReligionAttitudeChangeLimit() const
{
	return m_iFavoriteReligionAttitudeChangeLimit;
}
// MOD - END - Favorite Religion
...
// MOD - START - Improved Civilopedia
bool CvLeaderHeadInfo::isAIPlayable() const
{
	LeaderHeadTypes eLeader = (LeaderHeadTypes)GC.getInfoTypeForString(getType());

	for (int iCiv = 0; iCiv < GC.getNumCivilizationInfos(); iCiv++)
	{
		CvCivilizationInfo& kCivilization = GC.getCivilizationInfo((CivilizationTypes)iCiv);
		if (kCivilization.isLeaders(eLeader))
		{
			if (kCivilization.isAIPlayable())
			{
				return true;
			}
		}
...
bool CvLeaderHeadInfo::isPlayable() const
{
	LeaderHeadTypes eLeader = (LeaderHeadTypes)GC.getInfoTypeForString(getType());
...
void CvLeaderHeadInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Epidemics
	stream->Read(&m_iAcceptableEpidemicPercent);
	// MOD - END - Epidemics
...
/************************************************************************************************/
	...
	stream->Read(&m_iDifferentIdeologyAttitudeChange);
	stream->Read(&m_iDifferentIdeologyAttitudeDivisor);
	stream->Read(&m_iDifferentIdeologyAttitudeChangeLimit);	
	...
	// MOD - START - Favorite Religion
	stream->Read(&m_iFavoriteReligionAttitudeChange);
	stream->Read(&m_iFavoriteReligionAttitudeDivisor);
	stream->Read(&m_iFavoriteReligionAttitudeChangeLimit);
	// MOD - END - Favorite Religion
...
/************************************************************************************************/
	...
	// MOD - START - Epidemics
	stream->Write(m_iAcceptableEpidemicPercent);
	// MOD - END - Epidemics
...
/************************************************************************************************/
	...
	stream->Write(m_iDifferentIdeologyAttitudeChange);
	stream->Write(m_iDifferentIdeologyAttitudeDivisor);
	stream->Write(m_iDifferentIdeologyAttitudeChangeLimit);	
	...
	// MOD - START - Favorite Religion
	stream->Write(m_iFavoriteReligionAttitudeChange);
	stream->Write(m_iFavoriteReligionAttitudeDivisor);
	stream->Write(m_iFavoriteReligionAttitudeChangeLimit);
	// MOD - END - Favorite Religion
...
bool CvLeaderHeadInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Epidemics
	pXML->GetChildXmlValByName(&m_iAcceptableEpidemicPercent, "iAcceptableEpidemicPercent");
	// MOD - END - Epidemics
...
/************************************************************************************************/
	...
	pXML->GetChildXmlValByName(&m_iDifferentIdeologyAttitudeChange, "iDifferentReligionAttitudeChange");
	pXML->GetChildXmlValByName(&m_iDifferentIdeologyAttitudeDivisor, "iDifferentReligionAttitudeDivisor");
	pXML->GetChildXmlValByName(&m_iDifferentIdeologyAttitudeChangeLimit, "iDifferentReligionAttitudeChangeLimit");	
	...
	// MOD - START - Favorite Religion
	pXML->GetChildXmlValByName(&m_iFavoriteReligionAttitudeChange, "iFavoriteReligionAttitudeChange");
	pXML->GetChildXmlValByName(&m_iFavoriteReligionAttitudeDivisor, "iFavoriteReligionAttitudeDivisor");
	pXML->GetChildXmlValByName(&m_iFavoriteReligionAttitudeChangeLimit, "iFavoriteReligionAttitudeChangeLimit");
	// MOD - END - Favorite Religion
...
// MOD - START - Improved Civilopedia
int CvWorldInfo::getGridWidthInPlots() const
{
	if (GC.getNumLandscapeInfos() > 0)
	{
		return getGridWidth() * GC.getLandscapeInfo(std::max(GC.getActiveLandscapeID(), 0)).getPlotsPerCellX();
	}
	else
	{
		return getGridWidth();
...
int CvWorldInfo::getGridHeightInPlots() const
{
	if (GC.getNumLandscapeInfos() > 0)
	{
		return getGridHeight() * GC.getLandscapeInfo(std::max(GC.getActiveLandscapeID(), 0)).getPlotsPerCellY();
	}
	else
	{
...
// MOD - START - Per City Research Cost
int CvWorldInfo::getPerCityResearchCostModifier() const
{
	return m_iPerCityResearchCostModifier;
}
// MOD - END - Per City Research Cost
...
//					CvDomainInfo
//======================================================================================================
CvDomainInfo::CvDomainInfo() :
m_iNumInsideCityCongestionTypes(0),
m_iNumOutsideCityCongestionTypes(0),
m_piInsideCityCongestionTypes(NULL),
m_piOutsideCityCongestionTypes(NULL)
{
}

CvDomainInfo::~CvDomainInfo()
{
	SAFE_DELETE_ARRAY(m_piInsideCityCongestionTypes);
	SAFE_DELETE_ARRAY(m_piOutsideCityCongestionTypes);
}

int CvDomainInfo::getNumCongestionTypes(bool bInsideCity) const
{
	if (bInsideCity)
	{
		return m_iNumInsideCityCongestionTypes;
	}
	else
	{
		return m_iNumOutsideCityCongestionTypes;
	}
}

int CvDomainInfo::getCongestionTypes(bool bInsideCity, int i) const
{
	FASSERT_BOUNDS(0, getNumCongestionTypes(bInsideCity), i, "CvDomainInfo::getCongestionTypes");
	if (bInsideCity)
	{
		return m_piInsideCityCongestionTypes ? m_piInsideCityCongestionTypes[i] : -1;
	}
	else
	{
		return m_piOutsideCityCongestionTypes ? m_piOutsideCityCongestionTypes[i] : -1;
	}
}

bool CvDomainInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
void CvDomainInfo::cacheRelations()
{
	DomainTypes eDomain = (DomainTypes)GC.getInfoTypeForString(getType());

	std::vector<CongestionTypes> aInsideCityCongestionTypes;
	std::vector<CongestionTypes> aOutsideCityCongestionTypes;

	for (int iI = 0; iI < GC.getNumCongestionInfos(); iI++)
	{
		const CvCongestionInfo& kCongestion = GC.getCongestionInfo((CongestionTypes)iI);
		if (!kCongestion.isDomain(eDomain))
		{
			continue;
		}

		if (kCongestion.isAppliedInsideCities())
		{
			aInsideCityCongestionTypes.push_back((CongestionTypes)iI);
		}
			
		if (kCongestion.isAppliedOutsideCities())
		{
			aOutsideCityCongestionTypes.push_back((CongestionTypes)iI);
		}
	}

	m_iNumInsideCityCongestionTypes = aInsideCityCongestionTypes.size();
	SAFE_DELETE_ARRAY(m_piInsideCityCongestionTypes);
	if (m_iNumInsideCityCongestionTypes > 0)
	{
		m_piInsideCityCongestionTypes = new int[m_iNumInsideCityCongestionTypes];
		int iRel = 0;
		for (std::vector<CongestionTypes>::const_iterator it=aInsideCityCongestionTypes.begin(); it!=aInsideCityCongestionTypes.end(); ++it)
		{
			m_piInsideCityCongestionTypes[iRel++] = *it;
		}
	}
	aInsideCityCongestionTypes.clear();

	m_iNumOutsideCityCongestionTypes = aOutsideCityCongestionTypes.size();
	SAFE_DELETE_ARRAY(m_piOutsideCityCongestionTypes);
	if (m_iNumOutsideCityCongestionTypes > 0)
	{
		m_piOutsideCityCongestionTypes = new int[m_iNumOutsideCityCongestionTypes];
		int iRel = 0;
		for (std::vector<CongestionTypes>::const_iterator it=aOutsideCityCongestionTypes.begin(); it!=aOutsideCityCongestionTypes.end(); ++it)
		{
			m_piOutsideCityCongestionTypes[iRel++] = *it;
		}
	}
	aOutsideCityCongestionTypes.clear();
}
// MOD - END - Congestion
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvProjectInfo::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}
// MOD - END - Epidemics
...
// MOD - START - Relation Caching
int CvReligionInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvReligionInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int CvReligionInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvReligionInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvReligionInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvReligionInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
void CvReligionInfo::cacheRelations()
{
	ReligionTypes eReligion = (ReligionTypes)GC.getInfoTypeForString(getType());

	// Units
	std::set<UnitClassTypes> aRelatedUnitClasses;
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isReligionRelatedToUnit(eReligion, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isReligionRelatedToBuilding(eReligion, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
...
m_iTradeRouteIncomeModifier(0),
m_iForeignTradeRouteIncomeModifier(0),
...
m_pabNoPrereqCivicUnitClass(NULL),
...
CvTraitInfo::~CvTraitInfo()
{
	...
	// MOD - START - Trait UnitClass enabled (no prereq Civic)
	SAFE_DELETE_ARRAY(m_pabNoPrereqCivicUnitClass);
	// MOD - END - Trait UnitClass enabled (no prereq Civic)
	// MOD - START - Trait UnitCombat Free Experience
	SAFE_DELETE_ARRAY(m_piUnitCombatFreeExperience);
	// MOD - END - Trait UnitCombat Free Experience
	...
	// MOD - START - Trait UnitClass Promotions
	SAFE_DELETE_ARRAY(m_pabFreePromotionUnitClass);
	// MOD - END - Trait UnitClass Promotions
	// MOD - START - Trait City Yield Bonuses
	SAFE_DELETE_ARRAY(m_paiCityYieldBonuses);
	// MOD - END - Trait City Yield Bonuses
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvTraitInfo::getGlobalEpidemicModifier() const
{
	return m_iGlobalEpidemicModifier;
}
// MOD - END - Epidemics
...
// MOD - START - Trait Upgrade Modifier
int CvTraitInfo::getUpgradeDiscount() const
{
	return m_iUpgradeDiscount;
}
// MOD - END - Trait Upgrade Modifier
...
// MOD - START - Trait Trade Modifier
int CvTraitInfo::getTradeRouteIncomeModifier() const
{
	return m_iTradeRouteIncomeModifier;
}

int CvTraitInfo::getForeignTradeRouteIncomeModifier() const
{
	return m_iForeignTradeRouteIncomeModifier;
}
// MOD - END - Trait Trade Modifier
...
// MOD - START - Congestion
int CvTraitInfo::getInsideCityCongestionThresholdChange() const
{
	return m_iInsideCityCongestionThresholdChange;
}

int CvTraitInfo::getOutsideCityCongestionThresholdChange() const
{
	return m_iOutsideCityCongestionThresholdChange;
}
// MOD - END - Congestion
...
// MOD - START - Advanced Yield and Commerce Effects
int CvTraitInfo::getNumImprovementYieldChanges() const
{
	return m_iNumImprovementYieldChanges;
}

int CvTraitInfo::getNumSpecialistYieldChanges() const
{
	return m_iNumSpecialistYieldChanges;
}

int CvTraitInfo::getNumSpecialistCommerceChanges() const
{
	return m_iNumSpecialistCommerceChanges;
}
// MOD - END - Advanced Yield and Commerce Effects
...
// MOD - START - Advanced Yield and Commerce Effects
int CvTraitInfo::getImprovementYieldChangeImprovement(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvTraitInfo::getImprovementYieldChangeImprovement");
	return m_paImprovementYieldChanges[i].eImprovement;
}

int CvTraitInfo::getImprovementYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvTraitInfo::getImprovementYieldChangeYield");
	return m_paImprovementYieldChanges[i].eYield;
}

int CvTraitInfo::getImprovementYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumImprovementYieldChanges(), i, "CvTraitInfo::getImprovementYieldChangeValue");
	return m_paImprovementYieldChanges[i].iChange;
}

int CvTraitInfo::getSpecialistYieldChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvTraitInfo::getSpecialistYieldChangeSpecialist");
	return m_paSpecialistYieldChanges[i].eSpecialist;
}

int CvTraitInfo::getSpecialistYieldChangeYield(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvTraitInfo::getSpecialistYieldChangeYield");
	return m_paSpecialistYieldChanges[i].eYield;
}

int CvTraitInfo::getSpecialistYieldChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistYieldChanges(), i, "CvTraitInfo::getSpecialistYieldChangeValue");
	return m_paSpecialistYieldChanges[i].iChange;
}

int CvTraitInfo::getSpecialistCommerceChangeSpecialist(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvTraitInfo::getSpecialistCommerceChangeSpecialist");
	return m_paSpecialistCommerceChanges[i].eSpecialist;
}

int CvTraitInfo::getSpecialistCommerceChangeCommerce(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvTraitInfo::getSpecialistCommerceChangeCommerce");
	return m_paSpecialistCommerceChanges[i].eCommerce;
}

int CvTraitInfo::getSpecialistCommerceChangeValue(int i) const
{
	FASSERT_BOUNDS(0, getNumSpecialistCommerceChanges(), i, "CvTraitInfo::getSpecialistCommerceChangeValue");
	return m_paSpecialistCommerceChanges[i].iChange;
}
// MOD - END - Advanced Yield and Commerce Effects

// MOD - START - Trait UnitClass enabled (no prereq Civic)
int CvTraitInfo::isNoPrereqCivicUnitClass(int i) const
{
	return m_pabNoPrereqCivicUnitClass ? m_pabNoPrereqCivicUnitClass[i] : -1;
}
// MOD - END - Trait UnitClass enabled (no prereq Civic)

// MOD - START - Trait UnitCombat Free Experience
int CvTraitInfo::getUnitCombatFreeExperience(int i) const
{
	FAssertMsg(i < GC.getNumUnitCombatInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piUnitCombatFreeExperience ? m_piUnitCombatFreeExperience[i] : -1;
...
{
	return m_pabFreePromotionUnitClass ? m_pabFreePromotionUnitClass[i] : -1;
}
// MOD - END - Trait UnitClass Promotions

// MOD - START - Trait City Yield Bonuses
int CvTraitInfo::getCityYieldBonus(int i) const
{
	FAssertMsg(i < NUM_YIELD_TYPES, "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_paiCityYieldBonuses ? m_paiCityYieldBonuses[i] : -1;
}
// MOD - END - Trait City Yield Bonuses
...
{
	FASSERT_BOUNDS(0, GC.getNumImprovementInfos(), i, "CvTraitInfo::getImprovementYieldChanges");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, j, "CvTraitInfo::getImprovementYieldChanges");
	return m_ppiImprovementYieldChanges ? m_ppiImprovementYieldChanges[i][j] : 0;
}

int CvTraitInfo::getSpecialistYieldChanges(int i, int j) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), i, "CvTraitInfo::getSpecialistYieldChanges");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, j, "CvTraitInfo::getSpecialistYieldChanges");
	return m_ppiSpecialistYieldChanges ? m_ppiSpecialistYieldChanges[i][j] : 0;
}

int CvTraitInfo::getSpecialistCommerceChanges(int i, int j) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), i, "CvTraitInfo::getSpecialistCommerceChanges");
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, j, "CvTraitInfo::getSpecialistCommerceChanges");
	return m_ppiSpecialistCommerceChanges ? m_ppiSpecialistCommerceChanges[i][j] : 0;
}
// MOD - END - Advanced Yield and Commerce Effects
...
bool CvTraitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - Trait UnitClass enabled (no prereq Civic)
	pXML->SetVariableListTagPair(&m_pabNoPrereqCivicUnitClass, "NoPrereqCivicUnitClasses", sizeof(GC.getUnitClassInfo((UnitClassTypes)0)), GC.getNumUnitClassInfos());
	// MOD - END - Trait UnitClass enabled (no prereq Civic)
	// MOD - START - Trait UnitCombat Free Experience
	pXML->SetVariableListTagPair(&m_piUnitCombatFreeExperience, "UnitCombatFreeExperiences", sizeof(GC.getUnitCombatInfo((UnitCombatTypes)0)), GC.getNumUnitCombatInfos());
	// MOD - END - Trait UnitCombat Free Experience
	...
	// MOD - START - Trait UnitClass Promotions
	pXML->SetVariableListTagPair(&m_pabFreePromotionUnitClass, "FreePromotionUnitClasses", sizeof(GC.getUnitClassInfo((UnitClassTypes)0)), GC.getNumUnitClassInfos());
	// MOD - END - Trait UnitClass Promotions

	// MOD - START - Trait Coastal Trade Routes
	pXML->GetChildXmlValByName(&m_iCoastalTradeRoutes, "iCoastalTradeRoutes", 0);
	// MOD - END - Trait Coastal Trade Routes
	// MOD - START - Trait City Yield Bonuses
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"CityYieldBonuses"))
	{
		pXML->SetYields(&m_paiCityYieldBonuses);
		gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
	}
	else
	{
		pXML->InitList(&m_paiCityYieldBonuses, NUM_YIELD_TYPES);
	}
	// MOD - END - Trait City Yield Bonuses
	// MOD - START - Trait Attitude Modifier
	pXML->GetChildXmlValByName(&m_iAttitudeBonus, "iAttitudeModifier", 0);
	// MOD - END - Trait Attitude Modifier
	// MOD - START - Trait Golden Age Modifier
	pXML->GetChildXmlValByName(&m_iGoldenAgeModifier, "iGoldenAgeModifier", 0);
	// MOD - END - Trait Golden Age Modifier
	// MOD - START - Trait Upgrade Modifier
	pXML->GetChildXmlValByName(&m_iUpgradeDiscount, "iUpgradeDiscount", 0);
	// MOD - END - Trait Upgrade Modifier
	// MOD - START - Trait Trade Modifier
	pXML->GetChildXmlValByName(&m_iTradeRouteIncomeModifier, "iTradeRouteIncomeModifier", 0);
	pXML->GetChildXmlValByName(&m_iForeignTradeRouteIncomeModifier, "iForeignTradeRouteIncomeModifier", 0);
	// MOD - END - Trait Trade Modifier
	// MOD - START - Trait Worker Speed Modifier
...
bool CvEmphasizeInfo::isAvoidUnhappy() const
{
	return m_bAvoidUnhappy;
}

bool CvEmphasizeInfo::isAvoidUnhealth() const
{
	return m_bAvoidUnhealth;
}

bool CvEmphasizeInfo::isAvoidEpidemics() const
{
	return m_bAvoidEpidemics;
}
...
bool CvEmphasizeInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// MOD - START - City Growth Limits
	pXML->GetChildXmlValByName(&m_bAvoidUnhappy, "bAvoidUnhappy");
	pXML->GetChildXmlValByName(&m_bAvoidUnhealth, "bAvoidUnhealth");
	pXML->GetChildXmlValByName(&m_bAvoidEpidemics, "bAvoidEpidemics");
	// MOD - END - City Growth Limits
...
// MOD - START - Era Effects
int CvEraInfo::getDiscoverHappiness() const
{
	return m_iDiscoverHappiness;
}

int CvEraInfo::getDiscoverHealth() const
{
	return m_iDiscoverHealth;
}
// MOD - END - Era Effects
...
/************************************************************************************************/
	...
	pXML->GetChildXmlValByName(&m_iAnarchyPercent, "iAnarchyPercent");
	// MOD - START - Era Effects
	pXML->GetChildXmlValByName(&m_iDiscoverHappiness, "iDiscoverHappiness");
	pXML->GetChildXmlValByName(&m_iDiscoverHealth, "iDiscoverHealth");
	// MOD - END - Era Effects
...
//					CvGameEventInfo
//======================================================================================================
CvGameEventInfo::CvGameEventInfo() :
m_iNumMetricFactors(0),
m_piMetricFactors(NULL)
{
}

CvGameEventInfo::~CvGameEventInfo()
{
	SAFE_DELETE_ARRAY(m_piMetricFactors);
}

int CvGameEventInfo::getNumMetricFactors() const
{
	return m_iNumMetricFactors;
}

int CvGameEventInfo::getMetricFactors(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricFactors(), i, "CvGameEventInfo::getMetricFactors");
	return m_piMetricFactors ? m_piMetricFactors[i] : -1;
}

bool CvGameEventInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
void CvGameEventInfo::cacheRelations()
{
	GameEventTypes eGameEvent = (GameEventTypes)GC.getInfoTypeForString(getType());
	
	std::vector<MetricFactorTypes> aMetricFactors;
	for (int iI = 0; iI < GC.getNumMetricFactorInfos(); iI++)
	{
		MetricFactorTypes eLoopMetricFactor = (MetricFactorTypes)iI;
		if (GC.getMetricFactorInfo(eLoopMetricFactor).getGameEvent() == eGameEvent)
		{
			aMetricFactors.push_back(eLoopMetricFactor);
		}
	}

	m_iNumMetricFactors = (int)aMetricFactors.size();
	SAFE_DELETE_ARRAY(m_piMetricFactors);
	if (m_iNumMetricFactors > 0)
	{
		m_piMetricFactors = new int[m_iNumMetricFactors];
		int iRel = 0;
		for (std::vector<MetricFactorTypes>::const_iterator it=aMetricFactors.begin(); it!=aMetricFactors.end(); ++it)
		{
			m_piMetricFactors[iRel++] = *it;
		}
	}
	aMetricFactors.clear();
}
...
//======================================================================================================
CvSocialStabilityInfo::CvSocialStabilityInfo() :
m_iMaxThreshold(0),
m_iColorType(NO_COLOR)
{
}
...
int CvSocialStabilityInfo::getMaxThreshold() const
{
	return m_iMaxThreshold;
}

int CvSocialStabilityInfo::getColorType() const
{
	return m_iColorType;
}

bool CvSocialStabilityInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(&m_iMaxThreshold, "iMaxThreshold");
	pXML->GetChildXmlValByName(szTextVal, "ColorType");
	m_iColorType = pXML->FindInInfoClass(szTextVal);
...
//					CvMetricClassInfo
//======================================================================================================
CvMetricClassInfo::CvMetricClassInfo() :
m_iNumMetrics(0),
m_piMetrics(NULL)
{
}
...
CvMetricClassInfo::~CvMetricClassInfo()
{
	SAFE_DELETE_ARRAY(m_piMetrics);
}

int CvMetricClassInfo::getNumMetrics() const
{
	return m_iNumMetrics;
}

int CvMetricClassInfo::getMetrics(int i) const
{
	FASSERT_BOUNDS(0, getNumMetrics(), i, "CvMetricClassInfo::getMetrics");
	return m_piMetrics ? m_piMetrics[i] : -1;
}

bool CvMetricClassInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
void CvMetricClassInfo::cacheRelations()
{
	MetricClassTypes eMetricClass = (MetricClassTypes)GC.getInfoTypeForString(getType());
	
	std::vector<MetricTypes> aMetrics;
	for (int iI = 0; iI < GC.getNumMetricInfos(); iI++)
	{
		MetricTypes eLoopMetric = (MetricTypes)iI;
		if (GC.getMetricInfo(eLoopMetric).getMetricClass() == eMetricClass)
		{
			aMetrics.push_back(eLoopMetric);
		}
	}

	m_iNumMetrics = (int)aMetrics.size();
	SAFE_DELETE_ARRAY(m_piMetrics);
	if (m_iNumMetrics > 0)
	{
		m_piMetrics = new int[m_iNumMetrics];
		int iRel = 0;
		for (std::vector<MetricTypes>::const_iterator it=aMetrics.begin(); it!=aMetrics.end(); ++it)
		{
			m_piMetrics[iRel++] = *it;
		}
	}
...
int CvMetricInfo::getMetricClass() const
{
	return m_iMetricClass;
}

int CvMetricInfo::getNumMetricFactors() const
{
	return m_iNumMetricFactors;
}

int CvMetricInfo::getMetricFactors(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricFactors(), i, "CvMetricInfo::getMetricFactors");
	return m_piMetricFactors ? m_piMetricFactors[i] : -1;
}

bool CvMetricInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(szTextVal, "MetricClass");
	m_iMetricClass = pXML->FindInInfoClass(szTextVal);

	return true;
}

void CvMetricInfo::cacheRelations()
{
	MetricTypes eMetric = (MetricTypes)GC.getInfoTypeForString(getType());
	
	std::set<MetricFactorTypes> aMetricFactors;
	for (int iI = 0; iI < GC.getNumMetricFactorInfos(); iI++)
	{
		MetricFactorTypes eLoopMetricFactor = (MetricFactorTypes)iI;
		const CvMetricFactorInfo& kMetricFactor = GC.getMetricFactorInfo(eLoopMetricFactor);
		for (int iMem = 0; iMem < kMetricFactor.getNumMetricChanges(); iMem++)
		{
			if (kMetricFactor.getMetricChangeMetrics(iMem) == eMetric)
			{
				aMetricFactors.insert(eLoopMetricFactor);
			}
		}
	}
	...
	if (m_iNumMetricFactors > 0)
	{
		m_piMetricFactors = new int[m_iNumMetricFactors];
		int iRel = 0;
		for (std::set<MetricFactorTypes>::const_iterator it=aMetricFactors.begin(); it!=aMetricFactors.end(); ++it)
		{
			m_piMetricFactors[iRel++] = *it;
		}
	}
...
//					CvMetricFactorInfo
//======================================================================================================
CvMetricFactorInfo::CvMetricFactorInfo() :
m_iGameEvent(0),
m_iGameEventContext(0),
m_iMinEra(0),
m_iMaxEra(0),
m_iMinPopulation(0),
m_iMaxPopulation(0),
m_iNumPrereqAndTechs(0),
m_iNumPrereqAndCivics(0),
m_iNumObsoleteTechs(0),
m_iNumObsoleteCivics(0),
m_piPrereqAndTechs(NULL),
m_piPrereqAndCivics(NULL),
m_piObsoleteTechs(NULL),
m_piObsoleteCivics(NULL),
m_paMetricChanges(NULL)
{
}
...
CvMetricFactorInfo::~CvMetricFactorInfo()
{
	SAFE_DELETE_ARRAY(m_piPrereqAndTechs);
	SAFE_DELETE_ARRAY(m_piPrereqAndCivics);
	SAFE_DELETE_ARRAY(m_piObsoleteTechs);
	SAFE_DELETE_ARRAY(m_piObsoleteCivics);
	SAFE_DELETE_ARRAY(m_paMetricChanges);
}

int CvMetricFactorInfo::getGameEvent() const
{
	return m_iGameEvent;
}

int CvMetricFactorInfo::getGameEventContext() const
{
	return m_iGameEventContext;
}

int CvMetricFactorInfo::getMinEra() const
{
	return m_iMinEra;
}

int CvMetricFactorInfo::getMaxEra() const
{
	return m_iMaxEra;
}

int CvMetricFactorInfo::getMinPopulation() const
{
	return m_iMinPopulation;
}

int CvMetricFactorInfo::getMaxPopulation() const
{
	return m_iMaxPopulation;
}

int CvMetricFactorInfo::getNumPrereqAndTechs() const
{
	return m_iNumPrereqAndTechs;
}

int CvMetricFactorInfo::getNumPrereqAndCivics() const
{
	return m_iNumPrereqAndCivics;
}

int CvMetricFactorInfo::getNumObsoleteTechs() const
{
	return m_iNumObsoleteTechs;
}

int CvMetricFactorInfo::getNumObsoleteCivics() const
{
	return m_iNumObsoleteCivics;
}

int CvMetricFactorInfo::getNumMetricChanges() const
{
	return m_iNumMetricChanges;
}

int CvMetricFactorInfo::getPrereqAndTechs(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndTechs(), i, "CvMetricFactorInfo::getPrereqAndTechs");
	return m_piPrereqAndTechs ? m_piPrereqAndTechs[i] : -1;
}

int CvMetricFactorInfo::getPrereqAndCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumPrereqAndCivics(), i, "CvMetricFactorInfo::getPrereqAndCivics");
	return m_piPrereqAndCivics ? m_piPrereqAndCivics[i] : -1;
}

int CvMetricFactorInfo::getObsoleteTechs(int i) const
{
	FASSERT_BOUNDS(0, getNumObsoleteTechs(), i, "CvMetricFactorInfo::getObsoleteTechs");
	return m_piObsoleteTechs ? m_piObsoleteTechs[i] : -1;
}

int CvMetricFactorInfo::getObsoleteCivics(int i) const
{
	FASSERT_BOUNDS(0, getNumObsoleteCivics(), i, "CvMetricFactorInfo::getObsoleteCivics");
	return m_piObsoleteCivics ? m_piObsoleteCivics[i] : -1;
}

int CvMetricFactorInfo::getMetricChangeMetrics(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricChanges(), i, "CvMetricFactorInfo::getMetricChangeMetrics");
	return m_paMetricChanges ? m_paMetricChanges[i].eMetric : -1;
}

int CvMetricFactorInfo::getMetricChangeAmounts(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricChanges(), i, "CvMetricFactorInfo::getMetricChangeAmounts");
	return m_paMetricChanges ? m_paMetricChanges[i].iAmount : -1;
}

int CvMetricFactorInfo::getMetricChangeContributionTypes(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricChanges(), i, "CvMetricFactorInfo::getMetricChangeContributionTypes");
	return m_paMetricChanges ? m_paMetricChanges[i].eContributionType : -1;
}

int CvMetricFactorInfo::getMetricChangeSpeedModifiers(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricChanges(), i, "CvMetricFactorInfo::getMetricChangeSpeedModifiers");
	return m_paMetricChanges ? m_paMetricChanges[i].iSpeedModifier : -1;
}

int CvMetricFactorInfo::getMetricChangeHandicapModifiers(int i) const
{
	FASSERT_BOUNDS(0, getNumMetricChanges(), i, "CvMetricFactorInfo::getMetricChangeHandicapModifiers");
	return m_paMetricChanges ? m_paMetricChanges[i].iHandicapModifier : -1;
}

bool CvMetricFactorInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
	...
	pXML->GetChildXmlValByName(szTextVal, "GameEvent");
	m_iGameEvent = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(szTextVal, "GameEventContext");
	m_iGameEventContext = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(szTextVal, "MinEra");
	m_iMinEra = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(szTextVal, "MaxEra");
	m_iMaxEra = pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(&m_iMinPopulation, "iMinPopulation");
	pXML->GetChildXmlValByName(&m_iMaxPopulation, "iMaxPopulation");

	pXML->SetInfoArray(&m_iNumPrereqAndTechs, &m_piPrereqAndTechs, "PrereqAndTechs");
	pXML->SetInfoArray(&m_iNumPrereqAndCivics, &m_piPrereqAndCivics, "PrereqAndCivics");
	pXML->SetInfoArray(&m_iNumObsoleteTechs, &m_piObsoleteTechs, "ObsoleteTechs");
	pXML->SetInfoArray(&m_iNumObsoleteCivics, &m_piObsoleteCivics, "ObsoleteCivics");

	m_iNumMetricChanges = 0;
	SAFE_DELETE_ARRAY(m_paMetricChanges);

	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "MetricChanges"))
	{
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "MetricChange"))
		{
			std::vector<MetricChange> aValues;

			do
			{
				CvString szMetricInfo;
				CvString szContributionInfo;
				int iAmount;
				int iSpeedModifier;
				int iHandicapModifier;

				if (pXML->GetChildXmlValByName(szMetricInfo, "MetricType") &&
					pXML->GetChildXmlValByName(szContributionInfo, "ContributionType") &&
					pXML->GetChildXmlValByName(&iAmount, "iAmount"))
				{
					pXML->GetChildXmlValByName(&iSpeedModifier, "iSpeedModifier", 100);
					pXML->GetChildXmlValByName(&iHandicapModifier, "iHandicapModifier", 100);

					MetricTypes eMetric = (MetricTypes)pXML->FindInInfoClass(szMetricInfo);
					ContributionTypes eContribution = (ContributionTypes)pXML->FindInInfoClass(szContributionInfo);
					if (eMetric > -1 && eContribution > -1 && eContribution < NUM_CONTRIBUTION_TYPES && iAmount != 0)
					{
						MetricChange kMetricChange = { eMetric, eContribution, iAmount, iSpeedModifier, iHandicapModifier };
						aValues.push_back(kMetricChange);
					}
				}
			} while(gDLL->getXMLIFace()->LocateNextSiblingNodeByTagName(pXML->GetXML(), "MetricChange"));

			if (aValues.size() > 0)
			{
				m_iNumMetricChanges = (int)aValues.size();
				m_paMetricChanges = new MetricChange[m_iNumMetricChanges];
				for (uint i = 0; i < aValues.size(); i++)
				{
					m_paMetricChanges[i] = aValues[i];
				}
			}
...
CvEventInfo::CvEventInfo() :
	...
	m_iAIValue(0),
	// MOD - START - Relation Caching
	m_iNumDependentEventTriggers(0),
	m_iNumRelatedEvents(0),
	m_iNumRelatedUnitCombats(0),
	m_iNumRelatedUnitClasses(0),
	m_iNumRelatedBuildingClasses(0),
	// MOD - END - Relation Caching
	...
	// MOD - START - Relation Caching
	m_piDependentEventTriggers(NULL),
	m_piRelatedEvents(NULL),
	m_piRelatedUnitCombats(NULL),
	m_piRelatedUnitClasses(NULL),
	m_piRelatedBuildingClasses(NULL)
	// MOD - END - Relation Caching
...
CvEventInfo::~CvEventInfo()
{
	...
	// MOD - START - Relation Caching
	SAFE_DELETE_ARRAY(m_piDependentEventTriggers);
	SAFE_DELETE_ARRAY(m_piRelatedEvents);
	SAFE_DELETE_ARRAY(m_piRelatedUnitCombats);
	SAFE_DELETE_ARRAY(m_piRelatedUnitClasses);
	SAFE_DELETE_ARRAY(m_piRelatedBuildingClasses);
	// MOD - END - Relation Caching
...
// MOD - START - Relation Caching
int CvEventInfo::getNumDependentEventTriggers() const
{
	return m_iNumDependentEventTriggers;
}

int CvEventInfo::getNumRelatedEvents() const
{
	return m_iNumRelatedEvents;
}

int CvEventInfo::getNumRelatedUnitCombats() const
{
	return m_iNumRelatedUnitCombats;
}

int CvEventInfo::getNumRelatedUnitClasses() const
{
	return m_iNumRelatedUnitClasses;
}

int CvEventInfo::getNumRelatedBuildingClasses() const
{
	return m_iNumRelatedBuildingClasses;
}

int CvEventInfo::getDependentEventTriggers(int i) const
{
	FASSERT_BOUNDS(0, getNumDependentEventTriggers(), i, "CvEventInfo::getDependentEventTriggers");
	return m_piDependentEventTriggers ? m_piDependentEventTriggers[i] : 0;
}

int CvEventInfo::getRelatedEvents(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedEvents(), i, "CvEventInfo::getRelatedEvents");
	return m_piRelatedEvents ? m_piRelatedEvents[i] : -1;
}

int CvEventInfo::getRelatedUnitCombats(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitCombats(), i, "CvEventInfo::getNumRelatedUnitCombats");
	return m_piRelatedUnitCombats ? m_piRelatedUnitCombats[i] : -1;
}

int CvEventInfo::getRelatedUnitClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedUnitClasses(), i, "CvEventInfo::getRelatedUnitClasses");
	return m_piRelatedUnitClasses ? m_piRelatedUnitClasses[i] : -1;
}

int CvEventInfo::getRelatedBuildingClasses(int i) const
{
	FASSERT_BOUNDS(0, getNumRelatedBuildingClasses(), i, "CvEventInfo::getRelatedBuildingClasses");
	return m_piRelatedBuildingClasses ? m_piRelatedBuildingClasses[i] : -1;
}
// MOD - END - Relation Caching
...
void CvEventInfo::read(FDataStreamBase* stream)
{
	...
	// MOD - START - Relation Caching
	stream->Read(&m_iNumDependentEventTriggers);
	SAFE_DELETE_ARRAY(m_piDependentEventTriggers);
	if (m_iNumDependentEventTriggers > 0)
	{
		m_piDependentEventTriggers = new int[m_iNumDependentEventTriggers];
		stream->Read(m_iNumDependentEventTriggers, m_piDependentEventTriggers);
	}

	stream->Read(&m_iNumRelatedEvents);
	SAFE_DELETE_ARRAY(m_piRelatedEvents);
	if (m_iNumRelatedEvents > 0)
	{
		m_piRelatedEvents = new int[m_iNumRelatedEvents];
		stream->Read(m_iNumRelatedEvents, m_piRelatedEvents);
	}
...
void CvEventInfo::write(FDataStreamBase* stream)
{
	...
	// MOD - START - Relation Caching
	stream->Write(m_iNumDependentEventTriggers);
	if (m_iNumDependentEventTriggers > 0)
	{
		stream->Write(m_iNumDependentEventTriggers, m_piDependentEventTriggers);
	}

	stream->Write(m_iNumRelatedEvents);
	if (m_iNumRelatedEvents > 0)
	{
		stream->Write(m_iNumRelatedEvents, m_piRelatedEvents);
	}
...
// MOD - START - Relation Caching
void CvEventInfo::cacheRelations()
{
	EventTypes eEvent = (EventTypes) GC.getInfoTypeForString(getType());

	// Dependent Event Triggers
	std::set<EventTriggerTypes> aDependentEventTriggers;
	for (int iTrigger = 0; iTrigger < GC.getNumEventTriggerInfos(); iTrigger++)
	{
		...
		for (int iPre = 0; iPre < kTrigger.getNumPrereqEvents(); iPre++)
		{
			if (kTrigger.getPrereqEvent(iPre) == eEvent)
			{
				aDependentEventTriggers.insert((EventTriggerTypes)iTrigger);
				break;
			}
		}
	}

	m_iNumDependentEventTriggers = (int)aDependentEventTriggers.size();
	SAFE_DELETE_ARRAY(m_piDependentEventTriggers);
	if (m_iNumDependentEventTriggers > 0)
	{
		m_piDependentEventTriggers = new int[m_iNumDependentEventTriggers];
		int iRel = 0;
		for (std::set<EventTriggerTypes>::const_iterator it=aDependentEventTriggers.begin(); it!=aDependentEventTriggers.end(); ++it)
		{
			m_piDependentEventTriggers[iRel++] = *it;
		}
	}
	aDependentEventTriggers.clear();

	// Events
	std::set<EventTypes> aRelatedEvents;
	for (int iEvent = 0; iEvent < GC.getNumEventInfos(); iEvent++)
	{
		if (isEventRelatedToEvent(eEvent, (EventTypes)iEvent))
		{
			aRelatedEvents.insert((EventTypes)iEvent);
		}
	}

	m_iNumRelatedEvents = (int)aRelatedEvents.size();
	SAFE_DELETE_ARRAY(m_piRelatedEvents);
	if (m_iNumRelatedEvents > 0)
	{
		m_piRelatedEvents = new int[m_iNumRelatedEvents];
		int iRel = 0;
		for (std::set<EventTypes>::const_iterator it=aRelatedEvents.begin(); it!=aRelatedEvents.end(); ++it)
		{
			m_piRelatedEvents[iRel++] = *it;
		}
	}
	aRelatedEvents.clear();

	// Unit Combats
	std::set<UnitCombatTypes> aRelatedUnitCombats;
	for (int iUnitCombat = 0; iUnitCombat < GC.getNumUnitCombatInfos(); iUnitCombat++)
	{
		if (isEventRelatedToUnitCombat(eEvent, (UnitCombatTypes)iUnitCombat))
		{
			aRelatedUnitCombats.insert((UnitCombatTypes)iUnitCombat);
		}
	}
	...
	for (int iUnit = 0; iUnit < GC.getNumUnitInfos(); iUnit++)
	{
		if (isEventRelatedToUnit(eEvent, (UnitTypes)iUnit))
		{
			aRelatedUnitClasses.insert((UnitClassTypes)GC.getUnitInfo((UnitTypes)iUnit).getUnitClassType());
		}
	}
	...
	for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
	{
		if (isEventRelatedToBuilding(eEvent, (BuildingTypes)iBuilding))
		{
			aRelatedBuildingClasses.insert((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType());
		}
	}
