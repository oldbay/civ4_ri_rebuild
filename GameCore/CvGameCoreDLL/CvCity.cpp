...
CvCity::CvCity()
{
	...
	m_aiActualBaseYieldRate = new int[NUM_YIELD_TYPES];
	...
	m_aiSpecialistYield = new int[NUM_YIELD_TYPES];
	...
	m_paiBuildingReplacementCount = NULL;
	// MOD - END - Building Replacement
	// MOD - START - Building Disablement
	m_paiBuildingDisablementCount = NULL;
...
CvCity::~CvCity()
{
	...
	// MOD - START - Negative Yield Acceptance
	SAFE_DELETE_ARRAY(m_aiActualBaseYieldRate);
	// MOD - END - Negative Yield Acceptance
	...
	SAFE_DELETE_ARRAY(m_aiSpecialistYield);
	// MOD - END - Advanced Yield and Commerce Effects
...
*/
void CvCity::init(int iID, PlayerTypes eOwner, PlayerTypes eHighestCulturePlayer, int iX, int iY, bool bBumpUnits, bool bUpdatePlotGroups)
// MOD - END - Captured City Highest Culture Art (DPII 2008-07-28)
{
	...
	{
		setArtStyleType(kOwner.getArtStyleType());
	}
	// MOD - END - Captured City Highest Culture Art (DPII 2008-07-28)
	...
	CvWString szCityName = kOwner.getPlotCityName(pPlot);
	if (szCityName.empty())
	{
		szCityName = kOwner.getNewCityName();
	}
	setName(szCityName);
...
void CvCity::uninit()
{
	...
	SAFE_DELETE_ARRAY(m_paiBuildingOriginalTurn);
	// MOD - START - Building Replacement
	SAFE_DELETE_ARRAY(m_paiBuildingReplacementCount);
	// MOD - END - Building Replacement
	// MOD - START - Building Disablement
	SAFE_DELETE_ARRAY(m_paiBuildingDisablementCount);
	// MOD - END - Building Disablement
	...
	// MOD - START - Defense Modifiers
	SAFE_DELETE_ARRAY(m_paiUnitCombatDefenseModifier);
	SAFE_DELETE_ARRAY(m_paiUnitClassDefenseModifier);
	// MOD - END - Defense Modifiers
	// MOD - START - Building Heal Rate
	SAFE_DELETE_ARRAY(m_piDomainHealRate);
	SAFE_DELETE_ARRAY(m_piUnitClassHealRate);
	SAFE_DELETE_ARRAY(m_piUnitCombatHealRate);
	// MOD - END - Building Heal Rate End
	SAFE_DELETE_ARRAY(m_paiNumRealBuilding);
	SAFE_DELETE_ARRAY(m_paiNumFreeBuilding);
	// MOD - START - Building Activation
	SAFE_DELETE_ARRAY(m_paiNumActiveBuilding);
	// MOD - END - Building Activation
	// MOD - START - City Messages
	SAFE_DELETE_ARRAY(m_paiNumActiveBuildingChange);
	// MOD - END - City Messages
	// MOD - START - Population Metrics
	SAFE_DELETE_ARRAY(m_paiMetricValues);
	// MOD - END - Population Metrics
...
void CvCity::reset(int iID, PlayerTypes eOwner, int iX, int iY, bool bConstructorCall)
{
	...
	m_iEpidemicTurns = 0;
	// MOD - END - Epidemics
	m_aDecimalTradeRouteCommerce = 0;
	...
	if (!bConstructorCall)
	{
		...
		m_paiBuildingOriginalTurn = new int[GC.getNumBuildingInfos()];
		// MOD - START - Building Replacement
		m_paiBuildingReplacementCount = new int[GC.getNumBuildingInfos()];
		// MOD - END - Building Replacement
		// MOD - START - Building Disablement
		m_paiBuildingDisablementCount = new int[GC.getNumBuildingInfos()];
		// MOD - END - Building Disablement
		m_paiNumRealBuilding = new int[GC.getNumBuildingInfos()];
		m_paiNumFreeBuilding = new int[GC.getNumBuildingInfos()];
		// MOD - START - Building Activation
		m_paiNumActiveBuilding = new int[GC.getNumBuildingInfos()];
		// MOD - END - Building Activation
		// MOD - START - City Messages
		m_paiNumActiveBuildingChange = new int[GC.getNumBuildingInfos()];
		// MOD - END - City Messages
		for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
		{
			...
			// MOD - START - Building Replacement
			m_paiBuildingReplacementCount[iI] = 0;
			// MOD - END - Building Replacement
			// MOD - START - Building Disablement
			m_paiBuildingDisablementCount[iI] = 0;
			// MOD - END - Building Disablement
			m_paiNumRealBuilding[iI] = 0;
			m_paiNumFreeBuilding[iI] = 0;
			// MOD - START - Building Activation
			m_paiNumActiveBuilding[iI] = 0;
			// MOD - END - Building Activation
			// MOD - START - City Messages
			m_paiNumActiveBuildingChange[iI] = 0;
			// MOD - END - City Messages
		}
		...
		// MOD - START - Defense Modifiers
		FAssertMsg((0 < GC.getNumUnitCombatInfos()),  "GC.getNumUnitCombatInfos() is not greater than zero but an array is being allocated in CvCity::reset");
		m_paiUnitCombatDefenseModifier = new int[GC.getNumUnitCombatInfos()];
		for (iI = 0; iI < GC.getNumUnitCombatInfos(); iI++)
		{
			m_paiUnitCombatDefenseModifier[iI] = 0;
		}

		FAssertMsg((0 < GC.getNumUnitClassInfos()),  "GC.getNumUnitClassInfos() is not greater than zero but an array is being allocated in CvCity::reset");
		m_paiUnitClassDefenseModifier = new int[GC.getNumUnitClassInfos()];
		for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
		{
			m_paiUnitClassDefenseModifier[iI] = 0;
		}
		// MOD - END - Defense Modifiers

		// MOD - START - Building Heal Rate
		FAssertMsg((0 < NUM_DOMAIN_TYPES),  "NUM_DOMAIN_TYPES is not greater than zero but an array is being allocated in CvCity::reset");
		m_piDomainHealRate = new int[NUM_DOMAIN_TYPES];
		for (iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
		{
			m_piDomainHealRate[iI] = 0;
		}

		FAssertMsg((0 < GC.getNumUnitClassInfos()),  "GC.getNumUnitClassInfos() is not greater than zero but an array is being allocated in CvCity::reset");
		m_piUnitClassHealRate = new int[GC.getNumUnitClassInfos()];
		for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
		{
			m_piUnitClassHealRate[iI] = 0;
		}

		FAssertMsg((0 < GC.getNumUnitCombatInfos()),  "GC.getNumUnitCombatInfos() is not greater than zero but an array is being allocated in CvCity::reset");
		m_piUnitCombatHealRate = new int[GC.getNumUnitCombatInfos()];
		for (iI = 0; iI < GC.getNumUnitCombatInfos(); iI++)
		{
			m_piUnitCombatHealRate[iI] = 0;
		}
		// MOD - END - Building Heal Rate

		// MOD - START - Population Metrics
		FAssertMsg((0 < GC.getNumMetricInfos()),  "GC.getNumMetrics() is not greater than zero but an array is being allocated in CvCity::reset");
		m_paiMetricValues = new int[GC.getNumMetricInfos()];
		for (iI = 0; iI < GC.getNumMetricInfos(); iI++)
		{
			m_paiMetricValues[iI] = 0;
		}
		// MOD - END - Population Metrics
		...
		m_bonusConverters.clear();
		// MOD - END - Bonus Converters
		// MOD - START - Population Metrics
		m_kMetricTurnValues.clear();
		// MOD - END - Population Metrics
		m_aEventsOccured.clear();
		// MOD - START - Advanced Yield and Commerce Effects
		m_aSpecialistYieldChange.clear();
		m_aSpecialistCommerceChange.clear();
		m_aImprovementYieldChange.clear();
		// MOD - END - Advanced Yield and Commerce Effects
	...
	m_iCitySeparatism = 0;
	m_iCitySeparatismTemp = 0;
	m_iCitySeparatismUnit = 0;
}
...
void CvCity::kill(bool bUpdatePlotGroups)
{
	...
	for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
		...
		if (GC.getGameINLINE().isOption(GAMEOPTION_HOLY_CITY_MIGRATION))
		{
			if (isHolyCityCandidate((ReligionTypes)iI))
			{
				GET_PLAYER(getOwnerINLINE()).updateHolyCityCandidate((ReligionTypes)iI, this);
			}
		}
...
void CvCity::doTurn()
{
...
#ifdef _DEBUG
	{
		...
		for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			...
			FAssert(getBaseYieldRate(eLoopYield) >= 0);
			...
			for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
			{
				if (isWorkingPlot(iJ))
				{
					...
					if (pPlot != NULL)
					{
						iCount += pPlot->getYield(eLoopYield);
					}
				}
			}
			...
			{
				// MOD - START - Advanced Yield and Commerce Effects
				//iCount += (GET_PLAYER(getOwnerINLINE()).specialistYield(((SpecialistTypes)iJ), eLoopYield) * (getSpecialistCount((SpecialistTypes)iJ) + getFreeSpecialistCount((SpecialistTypes)iJ)));
				SpecialistTypes eLoopSpecialist = (SpecialistTypes)iJ;
				int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
				int iYieldPerSpecialist = getYieldPerSpecialist(eLoopSpecialist, eLoopYield);
				iCount += iNumSpecialists * iYieldPerSpecialist;
				// MOD - END - Advanced Yield and Commerce Effects
			}
			...
			for (int iJ = 0; iJ < GC.getNumBuildingInfos(); iJ++)
			{
				...
				int iNumActiveBuildings = getNumActiveBuilding(eLoopBuilding);
				int iYieldPerBuilding = getYieldPerBuilding(eLoopBuilding, eLoopYield);
				iCount += iNumActiveBuildings * iYieldPerBuilding;
			}

			iCount += getTradeYield(eLoopYield);
			iCount += getCorporationYield(eLoopYield);

			// MOD - START - Negative Yield Acceptance
			//FAssert(iCount == getBaseYieldRate(eLoopYield));
			FAssert(iCount == getActualBaseYieldRate(eLoopYield));
			// MOD - END - Negative Yield Acceptance
		}
		...
		for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
			...
			// MOD - START - Negative Commerce Buildings
			//FAssert(getBuildingCommerce(eLoopCommerce) >= 0);
			// MOD - END - Negative Commerce Buildings
			// MOD - START - Negative Commerce Specialists
			//FAssert(getSpecialistCommerce(eLoopCommerce) >= 0);
			// MOD - END - Negative Commerce Specialists
			FAssert(getReligionCommerce(eLoopCommerce) >= 0);
			FAssert(getCorporationCommerce(eLoopCommerce) >= 0);
			FAssert(GET_PLAYER(getOwnerINLINE()).getFreeCityCommerce(eLoopCommerce) >= 0);
		}

		for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
		{
			BonusTypes eLoopBonus = (BonusTypes)iI;
			FAssert(isNoBonus(eLoopBonus) || getNumBonuses(eLoopBonus) >= ((isConnectedToCapital()) ? (GET_PLAYER(getOwnerINLINE()).getBonusImport(eLoopBonus) - GET_PLAYER(getOwnerINLINE()).getBonusExport(eLoopBonus)) : 0));
		}
	}
#endif
	// XXX
...
UnitTypes CvCity::allUpgradesAvailable(UnitTypes eUnit, int iUpgradeCount) const
{
	...
	*/
	...
	{
		eLoopUnit = ((UnitTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(*it)));
...
// MOD - START - Building Replacement
bool CvCity::isObsoleteBuilding(BuildingTypes eIndex) const
{
	return isBuildingReplaced(eIndex) || GET_TEAM(getTeam()).isObsoleteBuilding(eIndex);
}
// MOD - END - Building Replacement


// MOD - START - Building Disablement
bool CvCity::isDisabledBuilding(BuildingTypes eIndex, bool bIgnorePower, bool bIgnoreReligion, bool bIgnoreCivic) const
{
	...
	if (!bIgnoreReligion)
	{
		if (kBuilding.isInactiveWithoutPrereqStateReligion())
		{
	...
	if (!bIgnoreCivic)
	{
		if (kBuilding.isInactiveWithoutCivicPrereqs())
		{
			for (int iPre = 0; iPre < kBuilding.getNumPrereqAndCivics(); iPre++)
			{
				CivicTypes ePrereqAndCivic = (CivicTypes)kBuilding.getPrereqAndCivics(iPre);
				if (!GET_PLAYER(getOwnerINLINE()).isCivic(ePrereqAndCivic))
				{
					return true;
				}
			}
	...
	return (getBuildingDisablementCount(eIndex) > 0);
}
// MOD - END - Building Disablement
...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades) const
{
	...
	if (!GC.getUnitInfo(eUnit).isIgnoreUpgrades())
	{	
		if (!bIgnoreUpgrades)
		{
...
// MOD - END - Advanced Building Prerequisites (Civic)
{
	...
	// MOD - START - Building Replacement
	if (isBuildingReplaced(eBuilding))
	{
		return false;
	}
	// MOD - END - Building Replacement
	...
	{	
		...
		if (kSpecialBuilding.isExclusive())
		{
			...
			for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
			{
				...
				{
					if (GC.getBuildingInfo((BuildingTypes)iI).getSpecialBuildingType() == kBuilding.getSpecialBuildingType())
					{
						return false;
					}
				}
	...
	if (!bTestVisible)
	{
		...
		{
			if (!hasBonus((BonusTypes)kBuilding.getPrereqAndBonuses(iPre)))
			{
				return false;
			}
		}
		...
		// MOD - START - Advanced Building Prerequisites (Bonus)
		if (kBuilding.getNumPrereqOrBonuses() > 0)
		{
			bool bNeedsBonus = true;

			for (int iPre = 0; iPre < kBuilding.getNumPrereqOrBonuses(); iPre++)
			{
				if (hasBonus((BonusTypes)kBuilding.getPrereqOrBonuses(iPre)))
				{
					bNeedsBonus = false;
					break;
				}
		...
		{
			if (!canConstructBonusConverter(eBuilding))
			{
				return false;
			}
		}
		...
		for (int iPre = 0; iPre < kBuilding.getNumPrereqAndBuildingClasses(); iPre++)
		{
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getPrereqAndBuildingClasses(iPre);
			BuildingTypes eLoopBuilding = (BuildingTypes)(kCivilization.getCivilizationBuildings(eLoopBuildingClass));

			if (eLoopBuilding != NO_BUILDING)
			{
				if (getNumBuilding(eLoopBuilding) == 0 /* && (bContinue || (getFirstBuildingOrder(ePrereqBuilding) == -1))*/)
				{
					return false;
				}
			}
...
int CvCity::getProductionExperience(UnitTypes eUnit) const
{
	...
	if (eUnit != NO_UNIT)
	{
		if (GC.getUnitInfo(eUnit).getUnitCombatType() != NO_UNITCOMBAT)
		{
			...
			iExperience += GET_PLAYER(getOwnerINLINE()).getUnitCombatFreeExperience((UnitCombatTypes)(GC.getUnitInfo(eUnit).getUnitCombatType()));
...
int CvCity::getProductionModifier(UnitTypes eUnit) const
{
	...
	for (int iRel = 0; iRel < kUnit.getNumRelatedBonuses(); iRel++)
	{
		...
		{
			iMultiplier += kUnit.getBonusProductionModifier(eLoopBonus);
		}
	}
...
int CvCity::getProductionModifier(BuildingTypes eBuilding) const
{
	...
	for (int iRel = 0; iRel < kBuilding.getNumRelatedBonuses(); iRel++)
	{
		...
		if (hasBonus(eLoopBonus))
		{
			iMultiplier += kBuilding.getBonusProductionModifier(eLoopBonus);
		}
	}
	// MOD - END - Relation Caching
...
/************************************************************************************************/
	...
	// MOD - START - Core Game Events
	applyGameEvent(GAMEEVENT_HURRY, eHurry);
	// MOD - END - Core Game Events
...
bool CvCity::canConscript(bool bWarConscript) const
// MOD - END - Wartime Conscription
{
	if (isDisorder())
	{
...
/************************************************************************************************/
	...
	// MOD - START - Core Game Events
	applyGameEvent(GAMEEVENT_CONSCRIPTION);
	// MOD - END - Core Game Events
}
...
int CvCity::getBonusHealth(BonusTypes eBonus) const
{
	...
	if (GET_PLAYER(getOwnerINLINE()).isReligionNoBonusHealthiness(eBonus))
	{
		return 0;
	}
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			iHealth += getNumActiveBuilding(eLoopBuilding) * GC.getBuildingInfo(eLoopBuilding).getBonusHealthChanges(eBonus);
		}
	}
	// MOD - END - Relation Caching
...
int CvCity::getBonusHappiness(BonusTypes eBonus) const
{
	...
	// MOD - START - Religion Affected Bonuses
	if (GET_PLAYER(getOwnerINLINE()).isReligionNoBonusHappiness(eBonus))
	{
		return 0;
	}
	// MOD - END - Religion Affected Bonuses
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			iHappiness += getNumActiveBuilding(eLoopBuilding) * GC.getBuildingInfo(eLoopBuilding).getBonusHappinessChanges(eBonus);
		}
	}
	// MOD - END - Relation Caching
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvCity::getBonusEpidemicModifier(BonusTypes eBonus) const
{
	return GC.getBonusInfo(eBonus).getEpidemicModifier();
}
// MOD - END - Epidemics
...
int CvCity::getBonusPower(BonusTypes eBonus, bool bDirty) const
{
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			int iNumBuildings = getNumActiveBuilding(eLoopBuilding);
			if (iNumBuildings > 0)
			{
				const CvBuildingInfo& kLoopBuilding = GC.getBuildingInfo(eLoopBuilding);
				if (kLoopBuilding.getPowerBonus() == eBonus)
				{
					if (kLoopBuilding.isDirtyPower() == bDirty)
					{
						iCount += iNumBuildings;
					}
				}
			}
		}
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			if (iNumBuildings > 0)
			{
				...
				for (int iMem = 0; iMem < kLoopBuilding.getNumBonusYieldModifiers(); iMem++)
				{
					if (kLoopBuilding.getBonusYieldModifierBonus(iMem) == eBonus && kLoopBuilding.getBonusYieldModifierYield(iMem) == eIndex)
					{
						iModifier += iNumBuildings * kLoopBuilding.getBonusYieldModifierValue(iMem);
					}
				}
			}
		}
...
void CvCity::processBonus(BonusTypes eBonus, int iChange)
{
	...
	// MOD - START - Religion Affected Bonuses
	// Note: Original code moved to processBonusHealthiness
	if (!kOwner.isReligionNoBonusHealthiness(eBonus))
	{
		processBonusHealthiness(eBonus, iChange);
	}
	// MOD - END - Religion Affected Bonuses

	// MOD - START - Religion Affected Bonuses
	// Note: Original code moved to processBonusHappiness
	if (!kOwner.isReligionNoBonusHappiness(eBonus))
	{
		processBonusHappiness(eBonus, iChange);
	}
	// MOD - END - Religion Affected Bonuses
	...
	*/
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			if (iNumBuildings > 0)
			{
				...
				for (int iMem = 0; iMem < kLoopBuilding.getNumBonusYieldModifiers(); iMem++)
				{
					if (kLoopBuilding.getBonusYieldModifierBonus(iMem) == eBonus)
					{
						aiYieldChanges[kLoopBuilding.getBonusYieldModifierYield(iMem)] += iNumBuildings * kLoopBuilding.getBonusYieldModifierValue(iMem) * iChange;
					}
				}
			}
		}
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		changeBonusYieldRateModifier((YieldTypes)iI, aiYieldChanges[iI]);
	}
	// MOD - END - Advanced Yield and Commerce Effects
}
...
// MOD - START - Religion Affected Bonuses
void CvCity::processBonusHealthiness(BonusTypes eBonus, int iChange)
{
	const CvBonusInfo& kBonus = GC.getBonusInfo(eBonus);
	const CvCivilizationInfo& kCivilization = GC.getCivilizationInfo(getCivilizationType());
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			iValue = GC.getBuildingInfo(eLoopBuilding).getBonusHealthChanges(eBonus) * getNumActiveBuilding(eLoopBuilding);
...
void CvCity::processBonusHappiness(BonusTypes eBonus, int iChange)
{
	const CvBonusInfo& kBonus = GC.getBonusInfo(eBonus);
	const CvCivilizationInfo& kCivilization = GC.getCivilizationInfo(getCivilizationType());

	int iValue = GC.getBonusInfo(eBonus).getHappiness();
	int iGoodValue = std::max(0, iValue);
	int iBadValue = std::min(0, iValue);
	...
	for (int iRel = 0; iRel < kBonus.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			iValue = getNumActiveBuilding(eLoopBuilding) * GC.getBuildingInfo(eLoopBuilding).getBonusHappinessChanges(eBonus);
...
// MOD - START - Building Activation
void CvCity::processBuildingExistence(BuildingTypes eBuilding, int iChange)
{
	...
	if (kBuilding.isBonusConverter())
	{
		...
		{
			setBonusConverterIsPresent(eBuilding, true);	// Can safely be called repeatedly
		}
		else
		{
			setBonusConverterIsPresent(eBuilding, false);	// Can safely be called repeatedly
		}
	}
	// MOD - END - Bonus Converters
	...
	changeBuildingDefense(kBuilding.getObsoleteSafeDefenseModifier() * iChange);
	changeBuildingBombardDefense(kBuilding.getObsoleteSafeBombardDefenseModifier() * iChange);
	// MOD - END - Obsolete Defense
	changeBuildingNaturalDefense(kBuilding.getNaturalDefenseModifier() * iChange);

	changeBaseGreatPeopleRate(kBuilding.getGreatPeopleRateChange() * iChange);

	if (kBuilding.getGreatPeopleUnitClass() != NO_UNITCLASS)
	{
		eGreatPeopleUnit = ((UnitTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(kBuilding.getGreatPeopleUnitClass())));
	...
	GET_PLAYER(getOwnerINLINE()).changeWondersScore(getWonderScore(eBuildingClass) * iChange);
	
	if (iChange == -1 && eBuilding == ((BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getDefineINT("CAPITAL_BUILDINGCLASS")))))
	{
		
		const CvCivilizationInfo& kCivilization = GC.getCivilizationInfo(getCivilizationType());
		const CvBuildingInfo& kCapitalBuilding = GC.getBuildingInfo(eBuilding);
		for (int iRel = 0; iRel < kCapitalBuilding.getNumRelatedBuildingClasses(); iRel++)
		{
			BuildingClassTypes eRelatedBuildingClass = (BuildingClassTypes)kCapitalBuilding.getRelatedBuildingClasses(iRel);
			BuildingTypes eRelatedBuilding = (BuildingTypes)(kCivilization.getCivilizationBuildings(eRelatedBuildingClass));

			if (eRelatedBuilding != NO_BUILDING)
			{
				if (getNumBuilding(eRelatedBuilding) > 0)
				{
					// Loop through present Building's requirements
					// MOD - START - Advanced Building Prerequisites
					const CvBuildingInfo& kRelatedBuilding = GC.getBuildingInfo(eRelatedBuilding);
					for (int iPre = 0; iPre < kRelatedBuilding.getNumPrereqAndBuildingClasses(); iPre++)
					{
						BuildingClassTypes ePrereqBuildingClass = (BuildingClassTypes)kRelatedBuilding.getPrereqAndBuildingClasses(iPre);
						BuildingTypes ePrereqBuilding = (BuildingTypes)(kCivilization.getCivilizationBuildings(ePrereqBuildingClass));
						if (ePrereqBuilding == eBuilding)
						{
							setNumRealBuilding(eRelatedBuilding, 0);
						}
					}
					// MOD - END - Advanced Building Prerequisites
				}
...
void CvCity::processBuildingActivation(BuildingTypes eBuilding, int iChange)
// MOD - END - Building Activation
{
	// MOD - START - Building Activation
	//UnitTypes eGreatPeopleUnit;
	...
	//{
		changeNumActiveBuilding(eBuilding, iChange);
	// MOD - END - Building Activation
		...
		std::vector<CvBonusAmountChange> aFreeBonusChanges;
		// MOD - END - PlotGroup Bonus Efficiency

		// MOD - START - Bonus Converters
		if (kBuilding.isBonusConverter())
		{
			for (int iI = 0; iI < kBuilding.getNumConsumedBonuses(); iI++)
			{
				plot()->changeNumPlotGroupBonusConverterConsumedBonuses( eBuilding, (BonusTypes)kBuilding.getConsumedBonusType(iI), (kBuilding.getConsumedBonusAmount(iI) * iChange) );
			}

			for (int iI = 0; iI < kBuilding.getNumProducedBonuses(); iI++)
			{
				aFreeBonusChanges.push_back(CvBonusAmountChange( (BonusTypes)kBuilding.getProducedBonusType(iI), (kBuilding.getProducedBonusAmount(iI) * iChange) ));
			}

			kPlayer.setCityMessagesDirty(true);
		}
		// MOD - END - Bonus Converters
		...
		if (kBuilding.getFreeBonus() != NO_BONUS)
		{
			...
			aFreeBonusChanges.push_back(CvBonusAmountChange( ((BonusTypes)(kBuilding.getFreeBonus())), (GC.getGameINLINE().getNumFreeBonuses(eBuilding) * iChange) ));
			// MOD - END - PlotGroup Bonus Efficiency
		}

		// MOD - START - PlotGroup Bonus Efficiency
		changeFreeBonus(aFreeBonusChanges);
		// MOD - END - PlotGroup Bonus Efficiency
		...
		// MOD - START - Wartime Conscription
		changeWarConscript(kBuilding.getWarConscript() * iChange);
		// MOD - END - Wartime Conscription
		...
		// MOD - START - Fresh Water Aquifers
		changeFreshWaterSourceCount(kBuilding.isFreshWaterSource() ? iChange : 0);
		// MOD - END - Fresh Water Aquifers
		...
		for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			...
			changeActualBaseYieldRate(eYield, getYieldPerBuilding(eBuilding, eYield) * iChange);
			// MOD - END - Negative Yield Acceptance
			// MOD - END - Advanced Yield and Commerce Effects
		...
		// MOD - START - Advanced Yield and Commerce Effects
		/*
		const std::vector<BonusYieldChange>& aBonusYieldChanges = kBuilding.getBonusYieldChanges();
		for (std::vector<BonusYieldChange>::const_iterator it = aBonusYieldChanges.begin(); it != aBonusYieldChanges.end(); ++it)
		{
			changeBonusYieldChange((*it).eBonus, (*it).eYield, (*it).iChange * iChange);
		}
		*/

		for (int iMem = 0; iMem < kBuilding.getNumImprovementYieldChanges(); iMem++)
		{
			changeImprovementYieldChange((ImprovementTypes)kBuilding.getImprovementYieldChangeImprovement(iMem), (YieldTypes)kBuilding.getImprovementYieldChangeYield(iMem), kBuilding.getImprovementYieldChangeValue(iMem) * iChange);
		}

		for (int iMem = 0; iMem < kBuilding.getNumSpecialistYieldChanges(); iMem++)
		{
			changeSpecialistYieldChange((SpecialistTypes)kBuilding.getSpecialistYieldChangeSpecialist(iMem), (YieldTypes)kBuilding.getSpecialistYieldChangeYield(iMem), kBuilding.getSpecialistYieldChangeValue(iMem) * iChange);
		}

		for (int iMem = 0; iMem < kBuilding.getNumSpecialistCommerceChanges(); iMem++)
		{
			changeSpecialistCommerceChange((SpecialistTypes)kBuilding.getSpecialistCommerceChangeSpecialist(iMem), (CommerceTypes)kBuilding.getSpecialistCommerceChangeCommerce(iMem), kBuilding.getSpecialistCommerceChangeValue(iMem) * iChange);
		}

		for (int iMem = 0; iMem < kBuilding.getNumBuildingYieldChanges(); iMem++)
		{
			changeBuildingYieldChange((BuildingClassTypes)kBuilding.getBuildingYieldChangeBuildingClass(iMem), (YieldTypes)kBuilding.getBuildingYieldChangeYield(iMem), kBuilding.getBuildingYieldChangeValue(iMem) * iChange);
		}

		for (int iMem = 0; iMem < kBuilding.getNumBuildingCommerceChanges(); iMem++)
		{
			changeBuildingCommerceChange((BuildingClassTypes)kBuilding.getBuildingCommerceChangeBuildingClass(iMem), (CommerceTypes)kBuilding.getBuildingCommerceChangeCommerce(iMem), kBuilding.getBuildingCommerceChangeValue(iMem) * iChange);
		}
		// MOD - END - Advanced Yield and Commerce Effects
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedReligions(); iRel++)
		{
			...
			changeReligionInfluence(eLoopReligion, (kBuilding.getReligionChange(eLoopReligion) * iChange));
		}
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedSpecialists(); iRel++)
		{
			...
			changeMaxSpecialistCount(eLoopSpecialist, kBuilding.getSpecialistCount(eLoopSpecialist) * iChange);
			changeFreeSpecialistCount(eLoopSpecialist, kBuilding.getFreeSpecialistCount(eLoopSpecialist) * iChange);
		}
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedImprovements(); iRel++)
		{
			...
			changeImprovementFreeSpecialists(eLoopImprovement, kBuilding.getImprovementFreeSpecialist(eLoopImprovement) * iChange);
		}
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedBonuses(); iRel++)
		{
			...
			if (hasBonus(eLoopBonus))
			{
				...
				{
					changeBonusGoodHealth(kBuilding.getBonusHealthChanges(eLoopBonus) * iChange);
				}
				else
				{
					changeBonusBadHealth(kBuilding.getBonusHealthChanges(eLoopBonus) * iChange);
				}
				if (kBuilding.getBonusHappinessChanges(eLoopBonus) > 0)
				{
					changeBonusGoodHappiness(kBuilding.getBonusHappinessChanges(eLoopBonus) * iChange);
				}
				else
				{
					changeBonusBadHappiness(kBuilding.getBonusHappinessChanges(eLoopBonus) * iChange);
				}
		...
		// MOD - START - Advanced Yield and Commerce Effects
		for (int iMem = 0; iMem < kBuilding.getNumBonusYieldModifiers(); iMem++)
		{
			if (hasBonus((BonusTypes)kBuilding.getBonusYieldModifierBonus(iMem)))
			{
				changeBonusYieldRateModifier((YieldTypes)kBuilding.getBonusYieldModifierYield(iMem), kBuilding.getBonusYieldModifierValue(iMem) * iChange);
			}
		}
		// MOD - END - Advanced Yield and Commerce Effects
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitCombats(); iRel++)
		{
			...
			changeUnitCombatFreeExperience(eLoopUnitCombat, kBuilding.getUnitCombatFreeExperience(eLoopUnitCombat) * iChange);
			// MOD - START - Defense Modifiers
			changeUnitCombatDefenseModifier(eLoopUnitCombat, kBuilding.getUnitCombatDefenseModifier(eLoopUnitCombat) * iChange);
			// MOD - END - Defense Modifiers
			// MOD - START - Building Heal Rate
			changeUnitCombatHealRate(eLoopUnitCombat, kBuilding.getUnitCombatHealRateChanges(eLoopUnitCombat) * iChange);
			// MOD - END - Building Heal Rate
		}
		// MOD - END - Relation Caching
		...
		for (int iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
		{
			...
			// MOD - START - Building Heal Rate
			changeDomainHealRate(((DomainTypes)iI), kBuilding.getDomainHealRateChanges(iI) * iChange);
			// MOD - END - Building Heal Rate
		}
		...
		for (int iRel = 0; iRel < kBuilding.getNumRelatedUnitClasses(); iRel++)
		{
			...
			// MOD - START - Defense Modifiers
			changeUnitClassDefenseModifier(eLoopUnitClass, kBuilding.getUnitClassDefenseModifier(eLoopUnitClass) * iChange);
			// MOD - END - Defense Modifiers
			// MOD - START - Building Heal Rate
			changeUnitClassHealRate(eLoopUnitClass, kBuilding.getUnitClassHealRateChanges(eLoopUnitClass) * iChange);
			// MOD - END - Building Heal Rate
		}
		// MOD - END - Relation Caching
		...
		kPlayer.changeAssets(kBuilding.getAssetValue() * iChange);

		area()->changePower(getOwnerINLINE(), (kBuilding.getPowerValue() * iChange));
		kPlayer.changePower(kBuilding.getPowerValue() * iChange);
		...
		for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
		{
			BuildingTypes eReplacedBuilding = (BuildingTypes)kBuildingClass.getReplacedBuildings(iRep);
			changeBuildingReplacementCount(eReplacedBuilding, iChange);
		}
		// MOD - END - Building Replacement
	...
	// Code moved to processBuildingExistence
	/*
	if (!bObsolete)
	{
		// MOD - START - Obsolete Defense
		...
		kPlayer.changeBuildingClassCount((BuildingClassTypes)kBuilding.getBuildingClassType(), iChange);
...
void CvCity::processSpecialist(SpecialistTypes eSpecialist, int iChange)
{
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		...
		changeActualBaseYieldRate(((YieldTypes)iI), (GC.getSpecialistInfo(eSpecialist).getYieldChange(iI) * iChange));
		// MOD - END - Negative Yield Acceptance
	}
	...
	*/
	...
	updateSpecialistYield();
	updateSpecialistCommerce();
	// MOD - END - Advanced Yield and Commerce Effects
...
ArtStyleTypes CvCity::getArtStyleType() const
{
	...
	*/
	return m_eArtStyleType;
	// MOD - END - Captured City Highest Culture Art (DPII 2008-07-28)
}

// MOD - START - Captured City Highest Culture Art (DPII 2008-07-28)
void CvCity::setArtStyleType(ArtStyleTypes eArtStyle)
{
    m_eArtStyleType = eArtStyle;
}
/***************************************************************************/
// MOD - END - Captured City Highest Culture Art (DPII 2008-07-28)
...
// MOD - START - Holy City Migration
int CvCity::getHolyCityCandidateValue(ReligionTypes eReligion) const
{
...
bool CvCity::isHolyCityCandidate(ReligionTypes eReligion) const
{
	return (GET_PLAYER(getOwnerINLINE()).getHolyCityCandidate(eReligion) == this);
}

bool CvCity::isHolyCityCandidate() const
{
	int iI;
...
// MOD - END - Building Additional Happiness
{
	...
	// MOD - START - Building Additional Happiness
	//if (!isNoUnhappiness())
	if (!isNoUnhappiness() || bIgnoreNoUnhappiness)
	// MOD - END - Building Additional Happiness
	{
		iAngerPercent = 0;
...
*/
		...
		// TODO: Consider enabling the new global warming system
...
// K-Mod end
		...
		iUnhappiness -= std::min(0, GET_PLAYER(getOwnerINLINE()).getEraUnhappiness());
		// MOD - END - Era Effects
	}
...
int CvCity::happyLevel() const
{
	...
	// MOD - START - Era Effects
	iHappiness += std::max(0, GET_PLAYER(getOwnerINLINE()).getEraHappiness());
	// MOD - END - Era Effects
...
int CvCity::totalBadBuildingHealth(bool bIgnoreBuildingOnlyHealthy) const
{
	//if (!isBuildingOnlyHealthy())
	if (!isBuildingOnlyHealthy() || bIgnoreBuildingOnlyHealthy)
	{
...
int CvCity::goodHealth() const
{
	...
	// MOD - START - Era Effects
	iHealth = GET_PLAYER(getOwnerINLINE()).getEraHealthiness();
	if (iHealth > 0)
	{
		iTotalHealth += iHealth;
	}
...
int CvCity::badHealth(bool bNoAngry, int iExtra) const
{
	...
	// MOD - START - Era Effects
	iHealth = GET_PLAYER(getOwnerINLINE()).getEraUnhealthiness();
	if (iHealth < 0)
	{
		iTotalHealth += iHealth;
	}
...
// MOD - START - Bonus Converters
int CvCity::getNumNonObsoleteBuilding(BuildingTypes eIndex) const
{
	FAssertMsg(eIndex != NO_BUILDING, "BuildingType eIndex is expected to not be NO_BUILDING");
...
int CvCity::getNumActiveBuilding(BuildingTypes eIndex) const
{
	...
	*/
	...
	return m_paiNumActiveBuilding[eIndex];
}


// MOD - START - Building Activation
int CvCity::calculateNumActiveBuilding(BuildingTypes eBuilding, bool bIgnorePower, bool bIgnoreReligion, bool bIgnoreCivic) const
{
	FAssertMsg(eBuilding >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eBuilding < GC.getNumBuildingInfos(), "eIndex expected to be < GC.getNumBuildingInfos()");

	int iNumBuilding = getNumBuilding(eBuilding);
	if (iNumBuilding == 0)
	{
		return 0;
	}
	...
	// MOD - START - Building Disablement
	if (isDisabledBuilding(eBuilding, bIgnorePower, bIgnoreReligion, bIgnoreCivic))
	{
		return 0;
	}
	// MOD - END - Building Disablement
	...
	if (kBuilding.isBonusConverter())
	{
		if (plot()->isPlotGroupBonusDisabled())
		{
			// Converters shut off when the plot's bonuses have been removed
			return 0;
		}

		CvPlotGroup* pPlotGroup = plot()->getOwnerPlotGroup();

		if (pPlotGroup != NULL)
		{
			int iOldNumActiveBuilding = getNumActiveBuilding(eBuilding);
			int iNumAvailableBonus;
			int iNewNumActiveBuilding = 0;
			...
			for (int iI = 0; iI < iNumBuilding; iI++)
			{
				for (int iConsumedBonusIndex = 0; iConsumedBonusIndex < kBuilding.getNumConsumedBonuses(); iConsumedBonusIndex++)
				{
					...
					// Calculate the number of bonuses available by starting with the number in this city
					iNumAvailableBonus = getNumBonuses(eConsumedBonus);	// This will be zero if the city has a NoBonus

					// Subtract the number of bonsues already consumed by any instances of this
					// building anywhere in the entire plot group
					iNumAvailableBonus -= pPlotGroup->getNumBonusConverterConsumedBonusesByIndex(eBuilding, iConsumedBonusIndex);

					// Some of those bonuses may have been consumed by this city;
					// to compensate for this we add them back in so that our count reflects
					// all of the bonuses available for this city.
					iNumAvailableBonus += iConsumedAmount * iOldNumActiveBuilding;

					if ((iConsumedAmount * (iI + 1)) > iNumAvailableBonus)
					{
						return iNewNumActiveBuilding;
					}
				}

				iNewNumActiveBuilding++;
			}
...
void CvCity::changeNumActiveBuilding(BuildingTypes eBuilding, int iChange)
{
	...
	m_paiNumActiveBuilding[eBuilding] += iChange;

	FAssert(getNumActiveBuilding(eBuilding) >= 0);
	FAssert(getNumActiveBuilding(eBuilding) <= getNumBuilding(eBuilding));

	// MOD - START - City Messages
	changeNumActiveBuildingChange(eBuilding, iChange);
	// MOD - END - City Messages
}


void CvCity::updateNumActiveBuilding(BuildingTypes eBuilding)
{
	...
	int iChange = calculateNumActiveBuilding(eBuilding) - getNumActiveBuilding(eBuilding);
	if (iChange != 0)
	{
		processBuildingActivation(eBuilding, iChange);
	}
}


void CvCity::updateNumActiveBuilding(std::vector<BuildingTypes>& aeBuildings)
{
	for (std::vector<BuildingTypes>::const_iterator it = aeBuildings.begin(); it != aeBuildings.end(); ++it)
	{
		updateNumActiveBuilding((*it));
	}
}
// MOD - END - Building Activation
...
// MOD - START - City Messages
int CvCity::getNumActiveBuildingChange(BuildingTypes eBuilding) const
{
	FAssertMsg(eBuilding >= 0, "eBuilding expected to be >= 0");
	FAssertMsg(eBuilding < GC.getNumBuildingInfos(), "eBuilding expected to be < GC.getNumBuildingInfos()");

	return m_paiNumActiveBuildingChange[eBuilding];
}


void CvCity::setNumActiveBuildingChange(BuildingTypes eBuilding, int iNewValue)
{
	FAssertMsg(eBuilding >= 0, "eBuilding expected to be >= 0");
	FAssertMsg(eBuilding < GC.getNumBuildingInfos(), "eBuilding expected to be < GC.getNumBuildingInfos()");

	m_paiNumActiveBuildingChange[eBuilding] = iNewValue;
}


void CvCity::changeNumActiveBuildingChange(BuildingTypes eBuilding, int iChange)
{
	setNumActiveBuildingChange(eBuilding, getNumActiveBuildingChange(eBuilding) + iChange);
}
// MOD - END - City Messages
...
/*
CvArea* CvCity::secondWaterArea(bool bNoImpassable) const
{
	return plot()->secondWaterArea(bNoImpassable);
}
*/

void CvCity::waterAreas(CvArea* (&aWaterAreas)[MAX_CITY_WATER_AREAS], int &iNumWaterAreas, bool bNoImpassable, TeamTypes eRelevantTeam) const
{
	return plot()->waterAreas(aWaterAreas, iNumWaterAreas, bNoImpassable, eRelevantTeam);
}
...
CvArea* CvCity::sharedWaterArea(CvCity* pOtherCity) const
{
	...
	*/
	...
	int iNumWaterAreas;
	CvArea* aWaterAreas[MAX_CITY_WATER_AREAS];
	waterAreas(aWaterAreas, iNumWaterAreas, true);
	if (iNumWaterAreas > 0)
	{
		int iNumOtherWaterAreas;
		CvArea* aOtherWaterAreas[MAX_CITY_WATER_AREAS];
		pOtherCity->waterAreas(aOtherWaterAreas, iNumOtherWaterAreas, true);
		if (iNumOtherWaterAreas > 0)
		{
			for (int iI = 0; iI < iNumWaterAreas; iI++)
			{
				for (int iJ = 0; iJ < iNumOtherWaterAreas; iJ++)
				{
					if (aWaterAreas[iI] == aOtherWaterAreas[iJ])
					{
						return aWaterAreas[iI];
					}
				}
			}
		}
...
long CvCity::getGreatPeopleRate() const
{
	if (isDisorder())
	{
		return 0;
	...
	return ((getBaseGreatPeopleRate() * getTotalGreatPeopleRateModifier()));
}
...
int CvCity::getAdditionalBaseGreatPeopleRateByBuilding(BuildingTypes eBuilding) const
{
	...
	bool bObsolete = isObsoleteBuilding(eBuilding);
	// MOD - END - Building Replacement
	int iExtraRate = 0;
...
{
	return (m_iGreatPeopleProgress / 100);
}
...
void CvCity::updateMaintenance()
{
	...
	long iOldMaintenance;
	long iNewMaintenance;
...
// MOD - START - Wartime Conscription
int CvCity::getWarConscript() const
{
	return m_iWarConscript;
}


void CvCity::changeWarConscript(int iChange)
{
	if (iChange != 0)
	{
		m_iWarConscript = (m_iWarConscript + iChange);
	}
}
// MOD - END - Wartime Conscription
...
// MOD - START - Advanced Building Prerequisites (Power)
void CvCity::updatePowerBuildings()
{
	// MOD - START - Attribute Caching
	for (int iMem = 0; iMem < GC.getNumBuildingClassesInactiveWithoutPower(); iMem++)
	{
		BuildingClassTypes eLoopBuildingClass = GC.getBuildingClassesInactiveWithoutPower(iMem);
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			updateNumActiveBuilding(eLoopBuilding);
		}
	}
...
int CvCity::getAdditionalHappinessByBuilding(BuildingTypes eBuilding, int& iGood, int& iBad) const
{
	...
	int iWarWearinessModifier = kBuilding.getWarWearinessModifier();
	int iIsNoUnhappinessChange = kBuilding.isNoUnhappiness() ? 1 : 0;

	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		BuildingTypes eReplacedBuilding = (BuildingTypes)kBuildingClass.getReplacedBuildings(iRep);

		int iNumBuildings = getNumActiveBuilding(eReplacedBuilding);
		if (iNumBuildings > 0)
		{
			const CvBuildingInfo& kReplacedBuilding = GC.getBuildingInfo(eReplacedBuilding);
			BuildingClassTypes eReplacedBuildingClass = (BuildingClassTypes)kReplacedBuilding.getBuildingClassType();

			// Basic
			subtractGoodOrBad(kReplacedBuilding.getHappiness() * iNumBuildings, iGood, iBad);
	
			// Building Class
			subtractGoodOrBad(getBuildingHappyChange(eReplacedBuildingClass) * iNumBuildings, iGood, iBad);

			// Player Building
			subtractGoodOrBad(kOwner.getExtraBuildingHappiness(eReplacedBuilding) * iNumBuildings, iGood, iBad);

			// Area
			subtractGoodOrBad(kReplacedBuilding.getAreaHappiness() * iNumBuildings, iGood, iBad);

			// Religion
			if (kReplacedBuilding.getReligionType() != NO_RELIGION && kReplacedBuilding.getReligionType() == kOwner.getStateReligion())
			{
				iGood -= kReplacedBuilding.getStateReligionHappiness() * iNumBuildings;
			}

			// MOD - START - Relation Caching
			// MOD - START - Religion Affected Bonuses
			for (int iRelatedBonus = 0; iRelatedBonus < kReplacedBuilding.getNumRelatedBonuses(); iRelatedBonus++)
			{
				BonusTypes eLoopBonus = (BonusTypes)kReplacedBuilding.getRelatedBonuses(iRelatedBonus);

				if ((hasBonus(eLoopBonus) || kReplacedBuilding.getFreeBonus() == eLoopBonus) && kReplacedBuilding.getNoBonus() != eLoopBonus && !kOwner.isReligionNoBonusHappiness(eLoopBonus))
				{
					subtractGoodOrBad(kReplacedBuilding.getBonusHappinessChanges(eLoopBonus) * iNumBuildings, iGood, iBad);
				}
			}
			// MOD - END - Religion Affected Bonuses
			// MOD - END - Relation Caching
			...
			for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
			{
				subtractGoodOrBad(kReplacedBuilding.getCommerceHappiness(iI) * iNumBuildings * kOwner.getCommercePercent((CommerceTypes)iI) / 100, iGood, iBad);
			}

			iWarWearinessModifier -= kReplacedBuilding.getWarWearinessModifier() * iNumBuildings;
			iIsNoUnhappinessChange -= kReplacedBuilding.isNoUnhappiness() ? iNumBuildings : 0;
		}
	}
	...
	// Building Class
	addGoodOrBad(getBuildingHappyChange(eBuildingClass), iGood, iBad);
	...
	// MOD - START - Relation Caching
	// MOD - START - Religion Affected Bonuses
	for (int iRelatedBonus = 0; iRelatedBonus < kBuilding.getNumRelatedBonuses(); iRelatedBonus++)
	{
		BonusTypes eLoopBonus = (BonusTypes)kBuilding.getRelatedBonuses(iRelatedBonus);

		if ((hasBonus(eLoopBonus) || kBuilding.getFreeBonus() == eLoopBonus) && kBuilding.getNoBonus() != eLoopBonus && !kOwner.isReligionNoBonusHappiness(eLoopBonus))
		{
	...
	for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		addGoodOrBad(kBuilding.getCommerceHappiness(iI) * kOwner.getCommercePercent((CommerceTypes)iI) / 100, iGood, iBad);
	}
	...
	if (iWarWearinessModifier != 0)
	{
		...
		iNewWarAngerPercent *= std::max(0, (iWarWearinessModifier + getWarWearinessModifier() + kOwner.getWarWearinessModifier() + 100));
	...
	// No Unhappiness
	bool bIsNoUnhappiness = (getNoUnhappinessCount() + iIsNoUnhappinessChange) > 0;
	if (isNoUnhappiness())
	{
		if (bIsNoUnhappiness)
		{
			// We're maintaining our isNoUnhappiness status, so undo all of the bad from this building
			iBad = iStartingBad;
		}
		else
		{
			// We're losing our isNoUnhappiness status, so add all of the bad back in
			iBad += unhappyLevel(true);
		}
	}
	else
	{
		if (bIsNoUnhappiness)
		{
			// We're gaining isNoUnhappiness status, so undo all of the bad from this building and all existing buildings
...
{
	FASSERT_BOUNDS(0, GC.getNumBuildingInfos(), eBuilding, "CvCity::getAdditionalHealthByBuilding");
	...
	int iBuildingOnlyHealthyChange = kBuilding.isBuildingOnlyHealthy() ? 1 : 0;
	bool bBuildingSuppliesPower = kBuilding.isPower() || (kBuilding.getPowerBonus() != NO_BONUS && hasBonus((BonusTypes)kBuilding.getPowerBonus()));
	int iPowerCountChange = bBuildingSuppliesPower ? 1 : 0;
	int iDirtyPowerCountChange = bBuildingSuppliesPower && kBuilding.isDirtyPower() ? 1 : 0;
	int iAreaCleanPowerChange = kBuilding.isAreaCleanPower() ? 1 : 0;
	int iUnhealthyPopulationModifier = kBuilding.getUnhealthyPopulationModifier();

	int iLostBonusGood = 0;
	int iLostBonusBad = 0;
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			// Basic
			subtractGoodOrBad(kReplacedBuilding.getHealth() * iNumBuildings, iGood, iBad);

			// Building Class
			subtractGoodOrBad(getBuildingHealthChange(eReplacedBuildingClass) * iNumBuildings, iGood, iBad);

			// Player Building
			subtractGoodOrBad(kOwner.getExtraBuildingHealth(eReplacedBuilding) * iNumBuildings, iGood, iBad);

			// Area
			subtractGoodOrBad(kReplacedBuilding.getAreaHealth() * iNumBuildings, iGood, iBad);

			// No Unhealthiness from Buildings
			iBuildingOnlyHealthyChange -= kReplacedBuilding.isBuildingOnlyHealthy() ? iNumBuildings : 0;
			...
			for (int iRelatedBonus = 0; iRelatedBonus < kReplacedBuilding.getNumRelatedBonuses(); iRelatedBonus++)
			{
				...
				if ((hasBonus(eLoopBonus) || kReplacedBuilding.getFreeBonus() == eLoopBonus) && kReplacedBuilding.getNoBonus() != eLoopBonus && !kOwner.isReligionNoBonusHealthiness(eLoopBonus))
				{
					// Note: This is subtracted from separate good and bad variables so that the bonus effects aren't lost when there is no building unhealth
					subtractGoodOrBad(kReplacedBuilding.getBonusHealthChanges(eLoopBonus) * iNumBuildings, iLostBonusGood, iLostBonusBad);
				}
			}
			// MOD - END - Religion Affected Bonuses
			// MOD - END - Relation Caching
			...
			// Power
			bBuildingSuppliesPower = kReplacedBuilding.isPower() || (kReplacedBuilding.getPowerBonus() != NO_BONUS && hasBonus((BonusTypes)kReplacedBuilding.getPowerBonus()));
			iPowerCountChange -= bBuildingSuppliesPower ? iNumBuildings : 0;
			iDirtyPowerCountChange -= bBuildingSuppliesPower && kReplacedBuilding.isDirtyPower() ? iNumBuildings : 0;
			iAreaCleanPowerChange -= kReplacedBuilding.isAreaCleanPower() ? iNumBuildings : 0;

			// Modified unhealthiness from population
			iUnhealthyPopulationModifier -= kReplacedBuilding.getUnhealthyPopulationModifier() * iNumBuildings;
		}
	}
	...
	// Building Class
	addGoodOrBad(getBuildingHealthChange(eBuildingClass), iGood, iBad);
	...
	// No Unhealthiness from Buildings
	bool bIsBuildingOnlyHealthy = kOwner.isBuildingOnlyHealthy() || (getBuildingOnlyHealthyCount() + iBuildingOnlyHealthyChange) > 0;
	if (isBuildingOnlyHealthy())
	{
		if (bIsBuildingOnlyHealthy)
		{
			// We're maintaining our isBuildingOnlyHealth status, so undo all of the bad from this building
			iBad = iStartingBad;
		}
		else
		{
			// We're losing our isBuildingOnlyHealth status, so add all of the bad back in from all existing buildings
			iBad += totalBadBuildingHealth(true);
		}
	}
	else
	{
		if (bIsBuildingOnlyHealthy)
		{
			// We're gaining isBuildingOnlyHealth status, so undo all of the bad from this building and all existing buildings
	...
	for (int iRelatedBonus = 0; iRelatedBonus < kBuilding.getNumRelatedBonuses(); iRelatedBonus++)
	{
		...
		if ((hasBonus(eLoopBonus) || kBuilding.getFreeBonus() == eLoopBonus) && kBuilding.getNoBonus() != eLoopBonus && !kOwner.isReligionNoBonusHealthiness(eLoopBonus))
		{
			addGoodOrBad(kBuilding.getBonusHealthChanges(eLoopBonus), iGood, iBad);
		}
	}

	iGood += iLostBonusGood;
	iBad += iLostBonusBad;
	// MOD - END - Religion Affected Bonuses
	// MOD - END - Relation Caching

	// Power
	int iPowerCount = getPowerCount() + iPowerCountChange;
	int iDirtyPowerCount = getDirtyPowerCount() + iDirtyPowerCountChange;

	bool bIsAreaCleanPower = (area() != NULL) && (area()->getCleanPowerCount(getTeam()) + iAreaCleanPowerChange > 0);
	bool bIsPower = (iPowerCount > 0) || bIsAreaCleanPower;
	bool bIsDirtyPower = bIsPower && (iDirtyPowerCount == iPowerCount) && !bIsAreaCleanPower;
	...
	else if (isPower() && !bIsPower)
	{
		// Losing power
		subtractGoodOrBad(GC.getDefineINT("POWER_HEALTH_CHANGE"), iGood, iBad);
	}

	if (isDirtyPower() && !bIsDirtyPower)
	{
		// Replacing dirty power with clean power
		subtractGoodOrBad(GC.getDefineINT("DIRTY_POWER_HEALTH_CHANGE"), iGood, iBad);
	}
	else if (bIsDirtyPower && !isDirtyPower())
	{
		// Replacing clean power with dirty power
	...
	*/
	...
	if (getUnhealthyPopulationModifier() + iUnhealthyPopulationModifier < -100)
	{
		iEffectiveModifier = std::min(0, -100 - getUnhealthyPopulationModifier());
	}
	else
	{
		iEffectiveModifier = std::max(-100, iUnhealthyPopulationModifier);
	}
...
// MOD - START - Epidemics
int CvCity::getAdditionalEpidemicModifierByBuilding(BuildingTypes eBuilding, int& iGood, int& iBad) const
{
	FASSERT_BOUNDS(0, GC.getNumBuildingInfos(), eBuilding, "CvCity::getAdditionalEpidemicModifierByBuilding");
	...
	for (int iRel = 0; iRel < kBuilding.getNumRelatedBuildingClasses(); iRel++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			if (kLoopBuilding.getObsoleteBuildingClass() == eBuildingClass)
			{
				...
				{
					// Local
					subtractGoodOrBad(kBuilding.getEpidemicModifier() * iNumBuildings, iGood, iBad);

					// Global
					subtractGoodOrBad(kBuilding.getGlobalEpidemicModifier() * iNumBuildings, iGood, iBad);
				}
			}
		}
	}
	...
	// Local
	addGoodOrBad(kBuilding.getEpidemicModifier(), iGood, iBad);

	// Global
	addGoodOrBad(kBuilding.getGlobalEpidemicModifier(), iGood, iBad);
...
int CvCity::getBuildingNaturalDefense() const
{
	return m_iBuildingNaturalDefense;
}


void CvCity::changeBuildingNaturalDefense(int iChange)
{
	if (iChange != 0)
	{
		m_iBuildingNaturalDefense = (m_iBuildingNaturalDefense + iChange);
		FAssert(getBuildingNaturalDefense() >= 0);
...
int CvCity::getAdditionalDefenseByBuilding(BuildingTypes eBuilding) const
{
	...
	int iDefenseModifier = kBuilding.getDefenseModifier() + kBuilding.getObsoleteSafeDefenseModifier();
	int iAllCityDefenseModifier = kBuilding.getAllCityDefenseModifier();
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			// Note: Obsolete-safe defense is intentionally excluded here
			iDefenseModifier -= kReplacedBuilding.getDefenseModifier() * iNumBuildings;
			iAllCityDefenseModifier -= kBuilding.getAllCityDefenseModifier() * iNumBuildings;
		}
	}

	int iDefense = std::max(getBuildingDefense() + iDefenseModifier, getNaturalDefense()) + kOwner.getCityDefenseModifier() + iAllCityDefenseModifier;
...
bool CvCity::isBombardable(const CvUnit* pUnit) const
{
	...
	if (pUnit->isStrategic())
		return (getPopulation() > 5);
...
{
	int iNaturalDefense = 0;
	if (getCultureLevel() != NO_CULTURELEVEL)
	{
		iNaturalDefense += GC.getCultureLevelInfo(getCultureLevel()).getCityDefenseModifier();
	}
	iNaturalDefense += getBuildingNaturalDefense();

	return iNaturalDefense;
}
...
int CvCity::getAdditionalBaseYieldRateByBuilding(YieldTypes eIndex, BuildingTypes eBuilding) const
{
	...
	int iBuildingTradeRouteModifier = kBuilding.getTradeRouteModifier();
	int iBuildingForeignTradeRouteModifier = kBuilding.getForeignTradeRouteModifier();
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			// Sea Plots
			if (kReplacedBuilding.getSeaPlotYieldChange(eIndex) != 0)
			{
				int iChange = kReplacedBuilding.getSeaPlotYieldChange(eIndex) * iNumBuildings;
				for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
				{
			...
			// River Plots
			if (kReplacedBuilding.getRiverPlotYieldChange(eIndex) != 0)
			{
				int iChange = kReplacedBuilding.getRiverPlotYieldChange(eIndex) * iNumBuildings;
				for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
				{
			...
			{
				if (kReplacedBuilding.getGlobalImprovementYieldChangeYield(iMem) == eIndex)
				{
					ImprovementTypes eLoopImprovement = (ImprovementTypes)kReplacedBuilding.getGlobalImprovementYieldChangeImprovement(iMem);
					int iChange = kReplacedBuilding.getGlobalImprovementYieldChangeValue(iMem);
					for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
					{
						if (isWorkingPlot(iI) && getCityIndexPlot(iI)->getImprovementType() == eLoopImprovement)
						{
							iExtraRate -= iChange;
						}
					}
			...
			{
				if (kReplacedBuilding.getImprovementYieldChangeYield(iMem) == eIndex)
				{
					ImprovementTypes eLoopImprovement = (ImprovementTypes)kReplacedBuilding.getImprovementYieldChangeImprovement(iMem);
					int iChange = kReplacedBuilding.getImprovementYieldChangeValue(iMem);
					for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
					{
						if (isWorkingPlot(iI) && getCityIndexPlot(iI)->getImprovementType() == eLoopImprovement)
						{
			...
			// Specialists
			for (int iMem = 0; iMem < kReplacedBuilding.getNumGlobalSpecialistYieldChanges(); iMem++)
			{
				if (kReplacedBuilding.getGlobalSpecialistYieldChangeYield(iMem) == eIndex)
				{
					SpecialistTypes eLoopSpecialist = (SpecialistTypes)kReplacedBuilding.getGlobalSpecialistYieldChangeSpecialist(iMem);
					int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
					iExtraRate -= kReplacedBuilding.getGlobalSpecialistYieldChangeValue(iMem) * iNumSpecialists;
				}
			}

			for (int iMem = 0; iMem < kReplacedBuilding.getNumSpecialistYieldChanges(); iMem++)
			{
				if (kReplacedBuilding.getSpecialistYieldChangeYield(iMem) == eIndex)
				{
					SpecialistTypes eLoopSpecialist = (SpecialistTypes)kReplacedBuilding.getSpecialistYieldChangeSpecialist(iMem);
					int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
					iExtraRate -= kReplacedBuilding.getSpecialistYieldChangeValue(iMem) * iNumSpecialists;
				}
			}

			// Buildings
			for (int iMem = 0; iMem < kReplacedBuilding.getNumGlobalBuildingYieldChanges(); iMem++)
			{
				if (kReplacedBuilding.getGlobalBuildingYieldChangeYield(iMem) == eIndex)
				{
					BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kReplacedBuilding.getGlobalBuildingYieldChangeBuildingClass(iMem);
					BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
					if (eLoopBuilding != NO_BUILDING)
					{
						iExtraRate -= kReplacedBuilding.getGlobalBuildingYieldChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
					}
				}
			}

			for (int iMem = 0; iMem < kReplacedBuilding.getNumBuildingYieldChanges(); iMem++)
			{
				if (kReplacedBuilding.getBuildingYieldChangeYield(iMem) == eIndex)
				{
					BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kReplacedBuilding.getBuildingYieldChangeBuildingClass(iMem);
					BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
					if (eLoopBuilding != NO_BUILDING)
					{
						iExtraRate -= kReplacedBuilding.getBuildingYieldChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
					}
				}
			}
			// MOD - END - Advanced Yield and Commerce Effects
			...
			// Basic
			iExtraRate -= kReplacedBuilding.getYieldChange(eIndex) * iNumBuildings;

			// Local extra
			iExtraRate -= getBuildingYieldChange(eReplacedBuildingClass, eIndex) * iNumBuildings;

			// MOD - START - Advanced Yield and Commerce Effects
			// Player extra
			iExtraRate -= kOwner.getBuildingYieldChange(eReplacedBuildingClass, eIndex) * iNumBuildings;
			// MOD - END - Advanced Yield and Commerce Effects

			// Trade
			iBuildingTradeRouteModifier -= kReplacedBuilding.getTradeRouteModifier() * iNumBuildings;
			iBuildingForeignTradeRouteModifier -= kReplacedBuilding.getForeignTradeRouteModifier() * iNumBuildings;

			// Specialists
			for (int iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
			{
				if (kReplacedBuilding.getFreeSpecialistCount((SpecialistTypes)iI) != 0)
				{
					// Note: This will be inaccurate if the specialist yield rate is affected by any of the buildings that are being obsoleted
					// TODO: Improve this
					iExtraRate -= getAdditionalBaseYieldRateBySpecialist(eIndex, (SpecialistTypes)iI, kReplacedBuilding.getFreeSpecialistCount((SpecialistTypes)iI) * iNumBuildings);
				}
			}
		}
	}
	...
	{
		if (kBuilding.getGlobalImprovementYieldChangeYield(iMem) == eIndex)
		{
			ImprovementTypes eLoopImprovement = (ImprovementTypes)kBuilding.getGlobalImprovementYieldChangeImprovement(iMem);
			int iChange = kBuilding.getGlobalImprovementYieldChangeValue(iMem);
			for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
			{
				if (isWorkingPlot(iI) && getCityIndexPlot(iI)->getImprovementType() == eLoopImprovement)
				{
	...
	for (int iMem = 0; iMem < kBuilding.getNumImprovementYieldChanges(); iMem++)
	{
		if (kBuilding.getImprovementYieldChangeYield(iMem) == eIndex)
		{
			ImprovementTypes eLoopImprovement = (ImprovementTypes)kBuilding.getImprovementYieldChangeImprovement(iMem);
			int iChange = kBuilding.getImprovementYieldChangeValue(iMem);
			for (int iI = 0; iI < NUM_CITY_PLOTS; ++iI)
			{
				if (isWorkingPlot(iI) && getCityIndexPlot(iI)->getImprovementType() == eLoopImprovement)
				{
	...
	// Specialists
	for (int iMem = 0; iMem < kBuilding.getNumGlobalSpecialistYieldChanges(); iMem++)
	{
		if (kBuilding.getGlobalSpecialistYieldChangeYield(iMem) == eIndex)
		{
			SpecialistTypes eLoopSpecialist = (SpecialistTypes)kBuilding.getGlobalSpecialistYieldChangeSpecialist(iMem);
			int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
			iExtraRate += kBuilding.getGlobalSpecialistYieldChangeValue(iMem) * iNumSpecialists;
		}
	}

	for (int iMem = 0; iMem < kBuilding.getNumSpecialistYieldChanges(); iMem++)
	{
		if (kBuilding.getSpecialistYieldChangeYield(iMem) == eIndex)
		{
			SpecialistTypes eLoopSpecialist = (SpecialistTypes)kBuilding.getSpecialistYieldChangeSpecialist(iMem);
			int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
			iExtraRate += kBuilding.getSpecialistYieldChangeValue(iMem) * iNumSpecialists;
		}
	}

	// Buildings
	for (int iMem = 0; iMem < kBuilding.getNumGlobalBuildingYieldChanges(); iMem++)
	{
		if (kBuilding.getGlobalBuildingYieldChangeYield(iMem) == eIndex)
		{
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getGlobalBuildingYieldChangeBuildingClass(iMem);
			BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
			if (eLoopBuilding != NO_BUILDING)
			{
				iExtraRate += kBuilding.getGlobalBuildingYieldChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
			}
		}
	}
	...
	for (int iMem = 0; iMem < kBuilding.getNumBuildingYieldChanges(); iMem++)
	{
		if (kBuilding.getBuildingYieldChangeYield(iMem) == eIndex)
		{
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getBuildingYieldChangeBuildingClass(iMem);
			BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
			if (eLoopBuilding != NO_BUILDING)
			{
				iExtraRate += kBuilding.getBuildingYieldChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
			}
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
	...
	// Local Extra
	iExtraRate += getBuildingYieldChange(eBuildingClass, eIndex);

	// MOD - START - Advanced Yield and Commerce Effects
	// Player Extra
	iExtraRate += kOwner.getBuildingYieldChange(eBuildingClass, eIndex);
	// MOD - END - Advanced Yield and Commerce Effects

	// Trade
	int iPlayerTradeYieldModifier = kOwner.getTradeYieldModifier(eIndex);
	if (iPlayerTradeYieldModifier > 0 && (iBuildingTradeRouteModifier != 0 || iBuildingForeignTradeRouteModifier != 0))
	{
...
// BUG - Fractional Trade Routes - end
		...
		for (int iI = 0; iI < getTradeRoutes(); ++iI)
		{
			...
			if (pCity)
			{
				...
				iTradeModifier += iBuildingTradeRouteModifier;
				if (pCity->getOwnerINLINE() != getOwnerINLINE())
				{
					iTradeModifier += iBuildingForeignTradeRouteModifier;
				}
...
// BUG - Fractional Trade Routes - end
	...
	for (int iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
	{
		if (kBuilding.getFreeSpecialistCount((SpecialistTypes)iI) != 0)
		{
			// Note: This will be inaccurate if the specialist yield rate is affected by the new building or by any of the buildings that are being obsoleted
			// TODO: Improve this
...
int CvCity::getAdditionalYieldRateModifierByBuilding(YieldTypes eIndex, BuildingTypes eBuilding) const
{
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			// Basic
			iExtraModifier -= kReplacedBuilding.getYieldModifier(eIndex) * iNumBuildings;

			// Power
			bBuildingSuppliesPower = kReplacedBuilding.isPower() || (kReplacedBuilding.getPowerBonus() != NO_BONUS && hasBonus((BonusTypes)kReplacedBuilding.getPowerBonus()));
			iPowerCountChange -= bBuildingSuppliesPower ? iNumBuildings : 0;
			iAreaCleanPowerChange -= kReplacedBuilding.isAreaCleanPower() ? iNumBuildings : 0;
			iLostPowerYieldModifier -= kReplacedBuilding.getPowerYieldModifier(eIndex) * iNumBuildings;

			// Production Modifiers
			if (eIndex == YIELD_PRODUCTION)
			{
				iExtraModifier -= kReplacedBuilding.getMilitaryProductionModifier() * iNumBuildings;
				iExtraModifier -= kReplacedBuilding.getSpaceProductionModifier() * iNumBuildings;
				iExtraModifier -= kReplacedBuilding.getGlobalSpaceProductionModifier() * iNumBuildings;

				int iMaxModifier = 0;
				for (int i = 0; i < NUM_DOMAIN_TYPES; i++)
				{
					iMaxModifier = std::max(iMaxModifier, kReplacedBuilding.getDomainProductionModifier((DomainTypes)i));
				}
				iExtraModifier -= iMaxModifier * iNumBuildings;
			}

			// MOD - START - Advanced Yield and Commerce Effects
			for (int iMem = 0; iMem < kReplacedBuilding.getNumBonusYieldModifiers(); iMem++)
			{
				if (kReplacedBuilding.getBonusYieldModifierYield(iMem) == eIndex)
				{
					if (hasBonus((BonusTypes)kReplacedBuilding.getBonusYieldModifierBonus(iMem)))
					{
						iExtraModifier -= kReplacedBuilding.getBonusYieldModifierValue(iMem) * iNumBuildings;
					}
				}
			}
			// MOD - END - Advanced Yield and Commerce Effects
	...
	if (bIsPower)
	{
		...
		if (isPower())
		{
			iExtraModifier += iLostPowerYieldModifier; // Adding negative value
		}
	}
	...
	*/
	for (int iMem = 0; iMem < kBuilding.getNumBonusYieldModifiers(); iMem++)
	{
		if (kBuilding.getBonusYieldModifierYield(iMem) == eIndex)
		{
			if (hasBonus((BonusTypes)kBuilding.getBonusYieldModifierBonus(iMem)))
			{
				iExtraModifier += kBuilding.getBonusYieldModifierValue(iMem);
			}
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
...
int CvCity::getAdditionalBaseYieldRateBySpecialist(YieldTypes eIndex, SpecialistTypes eSpecialist, int iChange) const
{
	...
	return iChange * getYieldPerSpecialist(eSpecialist, eIndex);
	// MOD - END - Advanced Yield and Commerce Effects
}
// BUG - Specialist Additional Yield - end

// MOD - START - Negative Yield Acceptance
int CvCity::getActualBaseYieldRate(YieldTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_YIELD_TYPES, "eIndex expected to be < NUM_YIELD_TYPES");
	return m_aiActualBaseYieldRate[eIndex];
}


void CvCity::setActualBaseYieldRate(YieldTypes eIndex, int iNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_YIELD_TYPES, "eIndex expected to be < NUM_YIELD_TYPES");

	if (getActualBaseYieldRate(eIndex) != iNewValue)
	{
		// We can't validate assertions for the new value because any value is legal
		m_aiActualBaseYieldRate[eIndex] = iNewValue;

		// Set the base yield rate that everything pays attention to,
		// but prevent negative values
		setBaseYieldRate(eIndex, std::max(0, iNewValue));
	}
}


void CvCity::changeActualBaseYieldRate(YieldTypes eIndex, int iChange)
{
	setActualBaseYieldRate(eIndex, (getActualBaseYieldRate(eIndex) + iChange));
}
// MOD - END - Negative Yield Acceptance
...
int CvCity::totalTradeModifier(CvCity* pOtherCity) const
{
	...
	iModifier += GET_PLAYER(getOwnerINLINE()).getTradeRouteIncomeModifier();
	...
	if (NULL != pOtherCity)
	{
		...
		if (getTeam() != pOtherCity->getTeam())
		{
			...
			// MOD - START - Trait Trade Modifier
			iModifier += GET_PLAYER(getOwnerINLINE()).getForeignTradeRouteIncomeModifier();
			// MOD - END - Trait Trade Modifier
...
void CvCity::setTradeYield(YieldTypes eIndex, int iNewValue)
{
	...
	if (iOldValue != iNewValue)
	{
		...
		changeActualBaseYieldRate(eIndex, (iNewValue - iOldValue));
		// MOD - END - Negative Yield Acceptance
	}
}
...
void CvCity::updateExtraSpecialistYield(YieldTypes eYield)
{
	...
	if (iOldYield != iNewYield)
	{
		...
		changeActualBaseYieldRate(eYield, (iNewYield - iOldYield));
		// MOD - END - Negative Yield Acceptance
	}
}
...
int CvCity::getYieldPerSpecialist(SpecialistTypes eSpecialist, YieldTypes eYield) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), eSpecialist, "CvCity::getSpecialistYield");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvCity::getSpecialistYield");
	CvPlayer& kPlayer = GET_PLAYER(getOwnerINLINE());
	CvSpecialistInfo& kSpecialist = GC.getSpecialistInfo(eSpecialist);
	return kSpecialist.getYieldChange(eYield) + getSpecialistYieldChange(eSpecialist, eYield) + kPlayer.getSpecialistYieldChange(eSpecialist, eYield);
}


int CvCity::getSpecialistYield(YieldTypes eYield) const
{
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvCity::getSpecialistYield");
	return m_aiSpecialistYield[eYield];
}


int CvCity::getSpecialistYield(SpecialistTypes eSpecialist, YieldTypes eYield) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), eSpecialist, "CvCity::getSpecialistYield");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvCity::getSpecialistYield");
	int iTotal = getSpecialistCount(eSpecialist) + getFreeSpecialistCount(eSpecialist);
	return iTotal != 0 ? iTotal * getYieldPerSpecialist(eSpecialist, eYield) : 0;
}


void CvCity::updateSpecialistYield(YieldTypes eYield)
{
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvCity::updateSpecialistYield");

	int iOldYield = getSpecialistYield(eYield);
	int iNewYield = 0;

	for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		iNewYield += getSpecialistYield((SpecialistTypes)iI, eYield);
	}

	if (iOldYield != iNewYield)
	{
		m_aiSpecialistYield[eYield] = iNewYield;
		//FAssert(getSpecialistYield(eYield) >= 0);
...
void CvCity::updateSpecialistYield()
{
	int iI;

	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		updateSpecialistYield((YieldTypes)iI);
	}
}


int CvCity::getSpecialistYieldChange(SpecialistTypes eSpecialist, YieldTypes eYield) const
{
	for (std::vector<SpecialistYieldChange>::const_iterator it = m_aSpecialistYieldChange.begin(); it != m_aSpecialistYieldChange.end(); ++it)
	{
		if ((*it).eSpecialist == eSpecialist && (*it).eYield == eYield)
		{
			return (*it).iChange;
		}
	}
...
void CvCity::changeSpecialistYieldChange(SpecialistTypes eSpecialist, YieldTypes eYield, int iChange)
{
	if (iChange != 0)
	{
		setSpecialistYieldChange(eSpecialist, eYield, getSpecialistYieldChange(eSpecialist, eYield) + iChange);
	}
}


void CvCity::setSpecialistYieldChange(SpecialistTypes eSpecialist, YieldTypes eYield, int iChange)
{
	for (std::vector<SpecialistYieldChange>::iterator it = m_aSpecialistYieldChange.begin(); it != m_aSpecialistYieldChange.end(); ++it)
	{
		if ((*it).eSpecialist == eSpecialist && (*it).eYield == eYield)
		{
			if ((*it).iChange != iChange)
			{
				if (iChange == 0)
				{
					m_aSpecialistYieldChange.erase(it);
				}
				else
				{
					(*it).iChange = iChange;
	...
	if (0 != iChange)
	{
		SpecialistYieldChange kChange;
		kChange.eSpecialist = eSpecialist;
		kChange.eYield = eYield;
		kChange.iChange = iChange;
		m_aSpecialistYieldChange.push_back(kChange);
...
{
	for (std::vector<ImprovementYieldChange>::const_iterator it = m_aImprovementYieldChange.begin(); it != m_aImprovementYieldChange.end(); ++it)
	{
		if ((*it).eImprovement == eImprovement && (*it).eYield == eYield)
		{
			return (*it).iChange;
		}
	}
...
void CvCity::changeImprovementYieldChange(ImprovementTypes eImprovement, YieldTypes eYield, int iChange)
{
	if (iChange != 0)
	{
		setImprovementYieldChange(eImprovement, eYield, getImprovementYieldChange(eImprovement, eYield) + iChange);
	}
}


void CvCity::setImprovementYieldChange(ImprovementTypes eImprovement, YieldTypes eYield, int iChange)
{
	for (std::vector<ImprovementYieldChange>::iterator it = m_aImprovementYieldChange.begin(); it != m_aImprovementYieldChange.end(); ++it)
	{
		if ((*it).eImprovement == eImprovement && (*it).eYield == eYield)
		{
			if ((*it).iChange != iChange)
			{
				if (iChange == 0)
				{
					m_aImprovementYieldChange.erase(it);
				}
				else
				{
					(*it).iChange = iChange;
	...
	if (0 != iChange)
	{
		ImprovementYieldChange kChange;
		kChange.eImprovement = eImprovement;
		kChange.eYield = eYield;
		kChange.iChange = iChange;
		m_aImprovementYieldChange.push_back(kChange);
...
int CvCity::getDecimalTrade() const
{
	return m_aDecimalTradeRouteCommerce;
}

int CvCity::getBaseCommerceRateTimes100(CommerceTypes eIndex) const
{
	long iBaseCommerceRate;
	long iTrueYieldRate;
	
	iTrueYieldRate = (getYieldRate(YIELD_COMMERCE) * 100) + getDecimalTrade();

	iBaseCommerceRate = getCommerceFromPercent(eIndex, iTrueYieldRate);
	...
	iBaseCommerceRate += 100 * GET_PLAYER(getOwnerINLINE()).getTotalOpenBordersFreeCityCommerce(eIndex);
	// MOD - END - Open Borders Commerce

	// MOD - START - Tech Transfer Commerce
	//iBaseCommerceRate += 100 * GET_PLAYER(getOwnerINLINE()).getTotalTechTransferFreeCityCommerce(eIndex);
	// MOD - END - Tech Transfer Commerce

	// MOD - START - Negative Commerce Acceptance
	// Note: Some buildings provide -1 gold and can make the base commerce
	// go negative in some circumstances
	//return iBaseCommerceRate;
	return std::max(long(0), iBaseCommerceRate);
	// MOD - END - Negative Commerce Acceptance
}


int CvCity::getTotalCommerceRateModifier(CommerceTypes eIndex) const
{
	// MOD - START - Open Borders/Tech Transfer Commerce
	...
	int iCommerceRateModifier;

	iCommerceRateModifier = 100;

	iCommerceRateModifier += getCommerceRateModifier(eIndex);
	iCommerceRateModifier += GET_PLAYER(getOwnerINLINE()).getCommerceRateModifier(eIndex);

	if (isCapital())
	{
		iCommerceRateModifier += GET_PLAYER(getOwnerINLINE()).getCapitalCommerceRateModifier(eIndex);
	}

	// MOD - START - Open Borders Commerce
	iCommerceRateModifier += GET_PLAYER(getOwnerINLINE()).getTotalOpenBordersCommerceRateModifier(eIndex);
	// MOD - END - Open Borders Commerce

	// MOD - START - Tech Transfer Commerce
	//iCommerceRateModifier += GET_PLAYER(getOwnerINLINE()).getTotalTechTransferCommerceRateModifier(eIndex);
	// MOD - END - Tech Transfer Commerce

	return std::max(0, iCommerceRateModifier);
	// MOD - END - Open Borders/Tech Transfer Commerce
}
...
int CvCity::getBuildingCommerceByBuilding(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	if (getNumBuilding(eBuilding) > 0)
	{
		...
		{
			// MOD - START - Building Disablement
			// No commerce for disabled buildings
			...
			if (getNumActiveBuilding(eBuilding) > 0)
			{
				...
				// MOD - START - Advanced Yield and Commerce Effects
				iCommerce += GET_PLAYER(getOwnerINLINE()).getBuildingCommerceChange((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType(), eIndex) * getNumActiveBuilding(eBuilding);
				// MOD - END - Advanced Yield and Commerce Effects
				...
				{
					iCommerce += (GC.getReligionInfo((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities() * 6 * GC.getGameINLINE().calculateReligionPercent((ReligionTypes)(kBuilding.getGlobalReligionCommerce())) / 100 * getNumActiveBuilding(eBuilding));
				}
...
int CvCity::getAdditionalBaseCommerceRateByBuildingImpl(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			// MOD - START - Advanced Yield and Commerce Effects
			// Specialists
			for (int iMem = 0; iMem < kReplacedBuilding.getNumGlobalSpecialistCommerceChanges(); iMem++)
			{
				if (kReplacedBuilding.getGlobalSpecialistCommerceChangeCommerce(iMem) == eIndex)
				{
					SpecialistTypes eLoopSpecialist = (SpecialistTypes)kReplacedBuilding.getGlobalSpecialistCommerceChangeSpecialist(iMem);
					int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
					iExtraRate -= kReplacedBuilding.getGlobalSpecialistCommerceChangeValue(iMem) * iNumSpecialists;
				}
			}

			for (int iMem = 0; iMem < kReplacedBuilding.getNumSpecialistCommerceChanges(); iMem++)
			{
				if (kReplacedBuilding.getSpecialistCommerceChangeCommerce(iMem) == eIndex)
				{
					SpecialistTypes eLoopSpecialist = (SpecialistTypes)kReplacedBuilding.getSpecialistCommerceChangeSpecialist(iMem);
					int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
					iExtraRate -= kReplacedBuilding.getSpecialistCommerceChangeValue(iMem) * iNumSpecialists;
				}
			}

			// Buildings
			for (int iMem = 0; iMem < kReplacedBuilding.getNumGlobalBuildingCommerceChanges(); iMem++)
			{
				if (kReplacedBuilding.getGlobalBuildingCommerceChangeCommerce(iMem) == eIndex)
				{
					BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kReplacedBuilding.getGlobalBuildingCommerceChangeBuildingClass(iMem);
					BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
					if (eLoopBuilding != NO_BUILDING)
					{
						iExtraRate -= kReplacedBuilding.getGlobalBuildingCommerceChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
					}
				}
			}

			for (int iMem = 0; iMem < kReplacedBuilding.getNumBuildingCommerceChanges(); iMem++)
			{
				if (kReplacedBuilding.getBuildingCommerceChangeCommerce(iMem) == eIndex)
				{
					BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kReplacedBuilding.getBuildingCommerceChangeBuildingClass(iMem);
					BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
					if (eLoopBuilding != NO_BUILDING)
					{
						iExtraRate -= kReplacedBuilding.getBuildingCommerceChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
					}
				}
			}
			// MOD - END - Advanced Yield and Commerce Effects

			iExtraRate -= kReplacedBuilding.getCommerceChange(eIndex) * iNumBuildings;
			iExtraRate -= getBuildingCommerceChange(eReplacedBuildingClass, eIndex) * iNumBuildings;
			// MOD - START - Advanced Yield and Commerce Effects
			iExtraRate -= kOwner.getBuildingCommerceChange(eReplacedBuildingClass, eIndex) * iNumBuildings;
			// MOD - END - Advanced Yield and Commerce Effects
			if (kReplacedBuilding.getReligionType() != NO_RELIGION)
			{
				if (kReplacedBuilding.getReligionType() == kOwner.getStateReligion())
				{
					iExtraRate -= kOwner.getStateReligionBuildingCommerce(eIndex) * iNumBuildings;
				}
			}
			if (kReplacedBuilding.getGlobalReligionCommerce() != NO_RELIGION)
			{
				iExtraRate -= GC.getReligionInfo((ReligionTypes)(kReplacedBuilding.getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities() * 6 * GC.getGameINLINE().calculateReligionPercent((ReligionTypes)(kReplacedBuilding.getGlobalReligionCommerce())) / 100 * iNumBuildings;
			}
			if (kReplacedBuilding.getGlobalCorporationCommerce() != NO_CORPORATION)
			{
				iExtraRate -= GC.getCorporationInfo((CorporationTypes)(kReplacedBuilding.getGlobalCorporationCommerce())).getHeadquarterCommerce(eIndex) * GC.getGameINLINE().countCorporationLevels((CorporationTypes)(kReplacedBuilding.getGlobalCorporationCommerce())) * iNumBuildings;
			}
			...
			for (int iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
			{
				if (kReplacedBuilding.getFreeSpecialistCount((SpecialistTypes)iI) != 0)
				{
					// Note: This will be inaccurate if the specialist commerce rate is affected by any of the buildings that are being obsoleted
					iExtraRate -= getAdditionalBaseCommerceRateBySpecialistImpl(eIndex, (SpecialistTypes)iI, kReplacedBuilding.getFreeSpecialistCount((SpecialistTypes)iI)) * iNumBuildings;
				}
			}
		}
	}
	...
	// MOD - START - Advanced Yield and Commerce Effects
	// Specialists
	for (int iMem = 0; iMem < kBuilding.getNumGlobalSpecialistCommerceChanges(); iMem++)
	{
		if (kBuilding.getGlobalSpecialistCommerceChangeCommerce(iMem) == eIndex)
		{
			SpecialistTypes eLoopSpecialist = (SpecialistTypes)kBuilding.getGlobalSpecialistCommerceChangeSpecialist(iMem);
			int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
			iExtraRate += kBuilding.getGlobalSpecialistCommerceChangeValue(iMem) * iNumSpecialists;
		}
	}

	for (int iMem = 0; iMem < kBuilding.getNumSpecialistCommerceChanges(); iMem++)
	{
		if (kBuilding.getSpecialistCommerceChangeCommerce(iMem) == eIndex)
		{
			SpecialistTypes eLoopSpecialist = (SpecialistTypes)kBuilding.getSpecialistCommerceChangeSpecialist(iMem);
			int iNumSpecialists = getSpecialistCount(eLoopSpecialist) + getFreeSpecialistCount(eLoopSpecialist);
			iExtraRate += kBuilding.getSpecialistCommerceChangeValue(iMem) * iNumSpecialists;
		}
	}

	// Buildings
	for (int iMem = 0; iMem < kBuilding.getNumGlobalBuildingCommerceChanges(); iMem++)
	{
		if (kBuilding.getGlobalBuildingCommerceChangeCommerce(iMem) == eIndex)
		{
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getGlobalBuildingCommerceChangeBuildingClass(iMem);
			BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
			if (eLoopBuilding != NO_BUILDING)
			{
				iExtraRate += kBuilding.getGlobalBuildingCommerceChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
			}
		}
	}
	...
	for (int iMem = 0; iMem < kBuilding.getNumBuildingCommerceChanges(); iMem++)
	{
		if (kBuilding.getBuildingCommerceChangeCommerce(iMem) == eIndex)
		{
			BuildingClassTypes eLoopBuildingClass = (BuildingClassTypes)kBuilding.getBuildingCommerceChangeBuildingClass(iMem);
			BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
			if (eLoopBuilding != NO_BUILDING)
			{
				iExtraRate += kBuilding.getBuildingCommerceChangeValue(iMem) * getNumActiveBuilding(eLoopBuilding);
			}
		}
	}
	// MOD - END - Advanced Yield and Commerce Effects
	...
	iExtraRate += getBuildingCommerceChange(eBuildingClass, eIndex);
	// MOD - START - Advanced Yield and Commerce Effects
	iExtraRate += kOwner.getBuildingCommerceChange(eBuildingClass, eIndex);
	// MOD - END - Advanced Yield and Commerce Effects
	if (kBuilding.getReligionType() != NO_RELIGION)
	{
	...
	{
		iExtraRate += GC.getReligionInfo((ReligionTypes)(kBuilding.getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities() * 6 * GC.getGameINLINE().calculateReligionPercent((ReligionTypes)(kBuilding.getGlobalReligionCommerce())) / 100;
	}
	...
	for (int iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
	{
		if (kBuilding.getFreeSpecialistCount((SpecialistTypes)iI) != 0)
		{
			// Note: This will be inaccurate if the specialist commerce rate is affected by the new building or by any of the buildings that are being obsoleted
...
int CvCity::getAdditionalCommerceRateModifierByBuildingImpl(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	for (int iRep = 0; iRep < kBuildingClass.getNumReplacedBuildings(); iRep++)
	{
		...
		if (iNumBuildings > 0)
		{
			...
			iExtraModifier -= kReplacedBuilding.getCommerceModifier(eIndex) * iNumBuildings;
			iExtraModifier -= kReplacedBuilding.getGlobalCommerceModifier(eIndex) * iNumBuildings;
		}
	}
...
// MOD - START - Advanced Yield and Commerce Effects
int CvCity::getCommercePerSpecialist(SpecialistTypes eSpecialist, CommerceTypes eCommerce) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), eSpecialist, "CvCity::getSpecialistCommerce");
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, eCommerce, "CvCity::getSpecialistCommerce");
	CvPlayer& kPlayer = GET_PLAYER(getOwnerINLINE());
	CvSpecialistInfo& kSpecialist = GC.getSpecialistInfo(eSpecialist);
	return kSpecialist.getCommerceChange(eCommerce) + getSpecialistCommerceChange(eSpecialist, eCommerce) + kPlayer.getSpecialistCommerceChange(eSpecialist, eCommerce) + kPlayer.getAllSpecialistCommerceChange(eCommerce);
}
...
int CvCity::getSpecialistCommerce(SpecialistTypes eSpecialist, CommerceTypes eCommerce) const
{
	FASSERT_BOUNDS(0, GC.getNumSpecialistInfos(), eSpecialist, "CvCity::getSpecialistCommerce");
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, eCommerce, "CvCity::getSpecialistCommerce");
	int iTotal = getSpecialistCount(eSpecialist) + getFreeSpecialistCount(eSpecialist);
	return iTotal != 0 ? iTotal * getCommercePerSpecialist(eSpecialist, eCommerce) : 0;
}
...
{
	FASSERT_BOUNDS(0, NUM_COMMERCE_TYPES, eCommerce, "CvCity::updateSpecialistCommerce");

	int iOldCommerce = getSpecialistCommerce(eCommerce);
	int iNewCommerce = 0;

	for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		iNewCommerce += getSpecialistCommerce((SpecialistTypes)iI, eCommerce);
	}

	if (iOldCommerce != iNewCommerce)
	{
		m_aiSpecialistCommerce[eCommerce] = iNewCommerce;
...
int CvCity::getSpecialistCommerceChange(SpecialistTypes eSpecialist, CommerceTypes eCommerce) const
{
	for (std::vector<SpecialistCommerceChange>::const_iterator it = m_aSpecialistCommerceChange.begin(); it != m_aSpecialistCommerceChange.end(); ++it)
	{
		if ((*it).eSpecialist == eSpecialist && (*it).eCommerce == eCommerce)
		{
			return (*it).iChange;
		}
	}
...
void CvCity::changeSpecialistCommerceChange(SpecialistTypes eSpecialist, CommerceTypes eCommerce, int iChange)
{
	if (iChange != 0)
	{
		setSpecialistCommerceChange(eSpecialist, eCommerce, getSpecialistCommerceChange(eSpecialist, eCommerce) + iChange);
	}
}

void CvCity::setSpecialistCommerceChange(SpecialistTypes eSpecialist, CommerceTypes eCommerce, int iChange)
{
	for (std::vector<SpecialistCommerceChange>::iterator it = m_aSpecialistCommerceChange.begin(); it != m_aSpecialistCommerceChange.end(); ++it)
	{
		if ((*it).eSpecialist == eSpecialist && (*it).eCommerce == eCommerce)
		{
			if ((*it).iChange != iChange)
			{
				if (iChange == 0)
				{
					m_aSpecialistCommerceChange.erase(it);
				}
				else
				{
					(*it).iChange = iChange;
				...
				updateSpecialistCommerce(eCommerce);
			}
	...
	if (0 != iChange)
	{
		SpecialistCommerceChange kChange;
		kChange.eSpecialist = eSpecialist;
		kChange.eCommerce = eCommerce;
		kChange.iChange = iChange;
		m_aSpecialistCommerceChange.push_back(kChange);
...
int CvCity::getAdditionalBaseCommerceRateBySpecialistImpl(CommerceTypes eIndex, SpecialistTypes eSpecialist, int iChange) const
{
	...
	return iChange * getCommercePerSpecialist(eSpecialist, eIndex);
	// MOD - END - Advanced Yield and Commerce Effects
}
...
void CvCity::updateCorporationYield(YieldTypes eIndex)
{
	...
	if (iOldYield != iNewYield)
	{
		...
		changeActualBaseYieldRate(eIndex, (iNewYield - iOldYield));
		// MOD - END - Negative Yield Acceptance
	}
}
...
void CvCity::setName(const wchar* szNewValue, bool bFound)
{
	...
	if (!szName.empty())
	{
		if (kOwner.isCityNameValid(szName, false))
		{
			m_szName = szName;
			...
			// MOD - START - City Name Preservation
			kOwner.setPlotCityName(plot(), szName);
			// MOD - END - City Name Preservation
		}
		if (bFound)
		{
...
// MOD - START - City Messages
void CvCity::updateMessages()
{
	CvWString szBuffer;
	CvWString szTempBuffer;

	// MOD - START - Bonus Converters
	CLLNode<BuildingTypes>* pBonusConverterNode = headBonusConverterNode();

	while (pBonusConverterNode != NULL)
	{
		BuildingTypes eBuilding = pBonusConverterNode->m_data;
		int iChange = getNumActiveBuildingChange(eBuilding);

		if (iChange != 0)
		{
			if (!isObsoleteBuilding(eBuilding) && !isDisabledBuilding(eBuilding))
			{
				CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);
				int iNumBuilding = getNumBuilding(eBuilding);

				FAssert(!plot()->isPlotGroupBonusDisabled());
				CvPlotGroup* pPlotGroup = plot()->getOwnerPlotGroup();
				FAssert(pPlotGroup != NULL);

				CvWString szBonusTempBuffer;
				szBonusTempBuffer.clear();
				...
				if (iChange > 0)
				{
					...
					{
						BonusTypes eConsumedBonus = (BonusTypes)kBuilding.getConsumedBonusType(iConsumedBonusIndex);
						CvBonusInfo& kConsumedBonus = GC.getBonusInfo(eConsumedBonus);

						if (!szBonusTempBuffer.empty())
						{
							szBonusTempBuffer.append(L" and "); // What about localization?
						}

						szTempBuffer.Format(L"%c%s", kConsumedBonus.getChar(), kConsumedBonus.getDescription());
						szBonusTempBuffer.append(szTempBuffer);
					}

					szBuffer = gDLL->getText("TXT_KEY_BONUS_CONVERTER_ACTIVATED", getNameKey(), kBuilding.getTextKeyWide(), szBonusTempBuffer.GetCString());
					gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, NULL, MESSAGE_TYPE_INFO, GC.getBuildingInfo(eBuilding).getArtInfo()->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_POSITIVE_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
				}
				else
				{
					for (int iConsumedBonusIndex = 0; iConsumedBonusIndex < kBuilding.getNumConsumedBonuses(); iConsumedBonusIndex++)
					{
						...
						// Calculate the number of bonuses available by starting with the number in this city
						int iNumAvailableBonus = getNumBonuses(eConsumedBonus);	// This will be zero if the city has a NoBonus

						// Subtract the number of bonsues already consumed by any instances of this
						// building anywhere in the entire plot group
						iNumAvailableBonus -= pPlotGroup->getNumBonusConverterConsumedBonusesByIndex(eBuilding, iConsumedBonusIndex);

						if ((iConsumedAmount * std::abs(iChange)) > iNumAvailableBonus)
						{
							if (!szBonusTempBuffer.empty())
							{
								szBonusTempBuffer.append(L" and ");
							}
					...
					szBuffer = gDLL->getText("TXT_KEY_BONUS_CONVERTER_DEACTIVATED", getNameKey(), kBuilding.getTextKeyWide(), szBonusTempBuffer.GetCString());
					gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, NULL, MESSAGE_TYPE_INFO, kBuilding.getArtInfo()->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
				}
			}

			setNumActiveBuildingChange(eBuilding, 0);
		}

		pBonusConverterNode = nextBonusConverterNode(pBonusConverterNode);
	}
	// MOD - END - Bonus Converters
}
// MOD - END - City Messages
...
void CvCity::changeNoBonusCount(BonusTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		// MOD - START - Bonus Converters
		updateBonusConvertersConsumingBonus(eIndex);
		// MOD - END - Bonus Converters
...
*/
		...
		if (!GET_TEAM(getTeam()).isBonusObsolete(eIndex))
		{
			plot()->changeNumPlotGroupBonuses(eIndex, iChange);
		}
		// MOD - END - PlotGroup Bonus Efficiency
	}
}
...
// MOD - START - PlotGroup Bonus Efficiency
void CvCity::changeFreeBonus(std::vector<CvBonusAmountChange>& aBonusChanges)
{
	std::vector<CvBonusAmountChange> aNonObsoleteBonusChanges;

	if (aBonusChanges.size() > 0)
	{
		for (std::vector<CvBonusAmountChange>::const_iterator it = aBonusChanges.begin(); it != aBonusChanges.end(); ++it)
		{
			FAssertMsg((*it).m_eBonus >= 0, "eBonus is expected to be non-negative (invalid Index)");
			FAssertMsg((*it).m_eBonus < GC.getNumBonusInfos(), "eBonus is expected to be within maximum bounds (invalid Index)");

			if ((*it).m_iChange != 0)
			{
				m_paiFreeBonus[(*it).m_eBonus] += (*it).m_iChange;
				FAssert(getFreeBonus((*it).m_eBonus) >= 0);

				if (!GET_TEAM(getTeam()).isBonusObsolete((*it).m_eBonus))
				{
					aNonObsoleteBonusChanges.push_back((*it));
				}
			}
		}

		if (aNonObsoleteBonusChanges.size() > 0)
		{
			plot()->changeNumPlotGroupBonuses(aNonObsoleteBonusChanges);
		}
	}
}
// MOD - END - PlotGroup Bonus Efficiency
...
void CvCity::updateFreeBonusDistribution(std::vector<BonusTypes>& aBonuses, bool bAdd)
{
	std::vector<CvBonusAmountChange> aBonusChanges;

	for (std::vector<BonusTypes>::const_iterator iter = aBonuses.begin(); iter != aBonuses.end(); ++iter)
	{
		BonusTypes eBonus = *iter;
		int iAmount = getFreeBonus(eBonus);
		if (iAmount != 0)
		{
			aBonusChanges.push_back(CvBonusAmountChange( eBonus, ((bAdd) ? iAmount : -iAmount) ));
		}
	}

	if (aBonusChanges.size() > 0)
	{
		plot()->changeNumPlotGroupBonuses(aBonusChanges);
	}
}
...
void CvCity::changeNumBonuses(BonusTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		//FAssert(m_paiNumBonuses[eIndex] >= 0);
		// MOD - END - PlotGroup Bonus Efficiency
...
// MOD - START - Building Replacement
int CvCity::getBuildingReplacementCount(BuildingTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex expected to be < GC.getNumBuildingInfos()");
	return m_paiBuildingReplacementCount[eIndex];
}

bool CvCity::isBuildingReplaced(BuildingTypes eIndex) const
{
	return (getBuildingReplacementCount(eIndex) > 0);
}

void CvCity::changeBuildingReplacementCount(BuildingTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		m_paiBuildingReplacementCount[eIndex] += iChange;
		FAssert(getBuildingReplacementCount(eIndex) >= 0);
		updateNumActiveBuilding(eIndex);
	}
}
// MOD - END - Building Replacement
...
// MOD - START - Building Disablement
int CvCity::getBuildingDisablementCount(BuildingTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex expected to be < GC.getNumBuildingInfos()");
	return m_paiBuildingDisablementCount[eIndex];
}

void CvCity::changeBuildingDisablementCount(BuildingTypes eIndex, int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	...
	if (iChange != 0)
	{
		m_paiBuildingDisablementCount[eIndex] += iChange;
		FAssert(getBuildingDisablementCount(eIndex) >= 0);
		updateNumActiveBuilding(eIndex);
	}
}
// MOD - END - Building Disablement
...
int CvCity::getAddedFreeSpecialistCount(SpecialistTypes eIndex) const
{
	...
	// TODO: Optimize this by splitting the counters?
	for (int iJ = 0; iJ < GC.getNumBuildingInfos(); ++iJ)
	{
...
// MOD - START - Defense Modifiers
int CvCity::getUnitCombatDefenseModifier(UnitCombatTypes eIndex) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitCombatInfos(), eIndex, "CvCity::getUnitCombatDefenseModifier");
	return m_paiUnitCombatDefenseModifier[eIndex];
}

void CvCity::changeUnitCombatDefenseModifier(UnitCombatTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumUnitCombatInfos(), eIndex, "CvCity::changeUnitCombatDefenseModifier");
	m_paiUnitCombatDefenseModifier[eIndex] = (m_paiUnitCombatDefenseModifier[eIndex] + iChange);
}

int CvCity::getUnitClassDefenseModifier(UnitClassTypes eIndex) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitClassInfos(), eIndex, "CvCity::getUnitClassDefenseModifier");
	return m_paiUnitClassDefenseModifier[eIndex];
}

void CvCity::changeUnitClassDefenseModifier(UnitClassTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumUnitClassInfos(), eIndex, "CvCity::changeUnitClassDefenseModifier");
	m_paiUnitClassDefenseModifier[eIndex] = (m_paiUnitClassDefenseModifier[eIndex] + iChange);
}
// MOD - END - Defense Modifiers

// MOD - START - Building Heal Rate
int CvCity::getDomainHealRate(DomainTypes eIndex) const
{
	FASSERT_BOUNDS(0, NUM_DOMAIN_TYPES, eIndex, "CvCity::getDomainHealRate");
	return m_piDomainHealRate[eIndex];
}

void CvCity::changeDomainHealRate(DomainTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, NUM_DOMAIN_TYPES, eIndex, "CvCity::changeDomainHealRate");
	m_piDomainHealRate[eIndex] = (m_piDomainHealRate[eIndex] + iChange);
	FAssert(m_piDomainHealRate[eIndex] >= 0);
}

int CvCity::getUnitClassHealRate(UnitClassTypes eIndex) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitClassInfos(), eIndex, "CvCity::getUnitClassHealRate");
	return m_piUnitClassHealRate[eIndex];
}

void CvCity::changeUnitClassHealRate(UnitClassTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumUnitClassInfos(), eIndex, "CvCity::changeUnitClassHealRate");
	m_piUnitClassHealRate[eIndex] = (m_piUnitClassHealRate[eIndex] + iChange);
	FAssert(m_piUnitClassHealRate[eIndex] >= 0);
}

int CvCity::getUnitCombatHealRate(UnitCombatTypes eIndex) const
{
	FASSERT_BOUNDS(0, GC.getNumUnitCombatInfos(), eIndex, "CvCity::getUnitCombatHealRate");
	return m_piUnitCombatHealRate[eIndex];
}

void CvCity::changeUnitCombatHealRate(UnitCombatTypes eIndex, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumUnitCombatInfos(), eIndex, "CvCity::changeUnitCombatHealRate");
	m_piUnitCombatHealRate[eIndex] = (m_piUnitCombatHealRate[eIndex] + iChange);
	FAssert(m_piUnitCombatHealRate[eIndex] >= 0);
}
// MOD - END - Building Heal Rate
...
bool CvCity::isFreshWaterSource() const
{
...
{
	bool bOldFreshWaterSource;

	if (iChange != 0)
	{
		bOldFreshWaterSource = isFreshWaterSource();

		m_iFreshWaterSourceCount = (m_iFreshWaterSourceCount + iChange);
		FAssert(getFreshWaterSourceCount() >= 0);

		if (bOldFreshWaterSource != isFreshWaterSource())
		{
			plot()->updateIrrigated();
			updateFreshWaterHealth();
...
// MOD - START - Epidemics
// Epidemic system by Mexico
int CvCity::getEpidemicTurns() const
{
	return m_iEpidemicTurns;
}


bool CvCity::isEpidemic() const
{
	return (getEpidemicTurns() > 0);
}
...
void CvCity::setEpidemicTurns(int iNewValue)
{
	m_iEpidemicTurns = iNewValue;
}


void CvCity::changeEpidemicTurns(int iChange)
{
	if (0 != iChange)
	{
		setEpidemicTurns(getEpidemicTurns() + iChange);
	}
}


int CvCity::getEpidemicProbabilityPercent(int iExtra) const
{
	return ((getEpidemicProbability(iExtra) * 100) / GC.getDefineINT("EPIDEMIC_RAND_BASE"));
}


int CvCity::getEpidemicProbability(int iExtra) const
{
	BuildingTypes eLoopBuilding;
	CvCity* pLoopCity;
	CvPlot* pLoopPlot;
	...
	if (isEpidemic() && (GC.getDefineINT("EPIDEMIC_MIN_TURNS")!= -1) && (getEpidemicTurns() < GC.getDefineINT("EPIDEMIC_MIN_TURNS")))
	{
		return (GC.getDefineINT("EPIDEMIC_RAND_BASE"));
	}

	// Check for maximum turns
	if (isEpidemic() && (GC.getDefineINT("EPIDEMIC_MAX_TURNS")!= -1) && (getEpidemicTurns() >= GC.getDefineINT("EPIDEMIC_MAX_TURNS")))
	{
		return 0;
	}

	CvHandicapInfo& kHandicap = GC.getHandicapInfo(getHandicapType());

	// Base
	//iProbability = GC.getDefineINT("EPIDEMIC_BASE_RATE");
	iProbability = kHandicap.getEpidemicBaseRate();

	// Population
	//iProbability += GC.getDefineINT("EPIDEMIC_POPULATION_RATE") * getPopulation();
	iProbability += kHandicap.getEpidemicPopulationRate() * (getPopulation() + iExtra);

	// Unhealthiness
	// We must negate this, because healthRate returns negative points
	//iProbability += (-(healthRate(false,0))) * GC.getDefineINT("EPIDEMIC_UNHEALTH_RATE");
	iProbability += (-(healthRate(false,0))) * kHandicap.getEpidemicUnhealthRate();
	...
	for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			iProbability += getNumActiveBuilding(eLoopBuilding) * GC.getBuildingInfo(eLoopBuilding).getEpidemicModifier();
		}
	}

	// Player global modifier (trait/projects/tech/civic/wonder)
	iProbability += GET_PLAYER(getOwnerINLINE()).getGlobalEpidemicModifier();

	// Team global modifier (projects)
	iProbability += GET_TEAM(getTeam()).getGlobalEpidemicModifier();
	...
	for (int iI = 0; iI < GC.getDefineINT("MAX_TRADE_ROUTES"); iI++)
	{
		...
		if (pLoopCity != NULL)
		{
			...
			{
				//iProbability += GC.getDefineINT("EPIDEMIC_TRADE_RATE");
				iProbability += kHandicap.getEpidemicTradeRouteRate();
			}
		}
	}

	// Nearby features and improvements
	for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		pLoopPlot = getCityIndexPlot(iI);
		...
		if (pLoopPlot != NULL)
		{
			...
			if (eFeature != NO_FEATURE)
			{
				iProbability += GC.getFeatureInfo(eFeature).getEpidemicModifier();
			}

			eImprovement = pLoopPlot->getImprovementType();
			if (eImprovement != NO_IMPROVEMENT)
			{
				iProbability += GC.getImprovementInfo(eImprovement).getEpidemicModifier();
			}
		}
	}
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		if (hasBonus((BonusTypes)iI))
		{
			iProbability += getBonusEpidemicModifier((BonusTypes)iI);
		}
	}
	...
	if (isEpidemic())
	{
		if ( getEpidemicTurns() <  GC.getDefineINT("EPIDEMIC_TURN_IMMUNE"))
		{
			iProbability += GC.getDefineINT("EPIDEMIC_TURN_RATE_SPREAD");
		}
		else
		{
			iProbability -= GC.getDefineINT("EPIDEMIC_TURN_RATE_IMMUNIZE");
		}
	}

	// Limits
	iProbability = std::min(iProbability, GC.getDefineINT("EPIDEMIC_MAX_CHANCE"));
	iProbability = std::max(iProbability, GC.getDefineINT("EPIDEMIC_MIN_CHANCE"));
...
bool CvCity::isGrowthAllowed() const
{
	...
	EraTypes eCurrentEra = kOwner.getCurrentEra();
	EraTypes eResearchEra = GET_TEAM(getTeam()).getCurrentResearchEra();

	if (AI_isEmphasizeAvoidUnhappy())
	{
		int iGrowthHappinessLevel = happyLevel() - unhappyLevel(1);
		if (iGrowthHappinessLevel < 0)
		{
			return false;
		}

		// MOD - START - Era Effects
		if (eResearchEra != NO_ERA && eResearchEra > eCurrentEra)
		{
			iGrowthHappinessLevel += GC.getEraInfo(eResearchEra).getDiscoverHappiness();
			if (iGrowthHappinessLevel < 0)
			{
				return false;
			}
	...
	if (AI_isEmphasizeAvoidUnhealth())
	{
		int iGrowthHealthLevel = goodHealth() - badHealth(false, 1);
		if (iGrowthHealthLevel < 0)
		{
			return false;
		}
		...
		if (eResearchEra != NO_ERA && eResearchEra > eCurrentEra)
		{
			iGrowthHealthLevel += GC.getEraInfo(eResearchEra).getDiscoverHealth();
			if (iGrowthHealthLevel < 0)
			{
				return false;
			}
	...
	if (AI_isEmphasizeAvoidEpidemics())
	{
		int iGrowthEpidemicPercent = getEpidemicProbabilityPercent(1);
		if (iGrowthEpidemicPercent > kOwner.getAcceptableEpidemicPercent())
		{
			return false;
		}
	}
		...
		if (pPlot != NULL)
		{
			...
			if (isWorkingPlot(iIndex))
			{
				...
				for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
				{
					...
					changeActualBaseYieldRate(((YieldTypes)iI), pPlot->getYield((YieldTypes)iI));
					// MOD - END - Negative Yield Acceptance
				}
...
void CvCity::setNumRealBuildingTimed(BuildingTypes eIndex, int iNewValue, bool bFirst, PlayerTypes eOriginalOwner, int iOriginalTime)
{
	...
	if (iChangeNumRealBuilding != 0)
	{
		...
		if (getNumRealBuilding(eIndex) > 0)
		{
			...
			m_paiBuildingOriginalTurn[eIndex] = GC.getGameINLINE().getGameTurn();
		}
		else
		{
			m_paiBuildingOriginalOwner[eIndex] = NO_PLAYER;
			m_paiBuildingOriginalTime[eIndex] = MIN_INT;
			m_paiBuildingOriginalTurn[eIndex] = 0;
		}
		...
		if (iOldNumBuilding != getNumBuilding(eIndex))
		{
			...
			processBuildingExistence(eIndex, getNumBuilding(eIndex) - iOldNumBuilding);
			updateNumActiveBuilding(eIndex);
			// MOD - END - Building Activation
		}
...
void CvCity::setNumFreeBuilding(BuildingTypes eIndex, int iNewValue)
{
	...
	if (getNumFreeBuilding(eIndex) != iNewValue)
	{
		...
		if (iOldNumBuilding != getNumBuilding(eIndex))
		{
			...
			processBuildingExistence(eIndex, iNewValue - iOldNumBuilding);
			updateNumActiveBuilding(eIndex);
			// MOD - END - Building Activation
		}
	}
...
// MOD - START - Bonus Converters
void CvCity::updateBonusConverters()
{
	CLLNode<BuildingTypes>* pBonusConverterNode = headBonusConverterNode();

	while (pBonusConverterNode != NULL)
	{
		updateNumActiveBuilding(pBonusConverterNode->m_data);

		pBonusConverterNode = nextBonusConverterNode(pBonusConverterNode);
	}
}

void CvCity::updateBonusConvertersConsumingBonus(BonusTypes eBonus)
{
	CLLNode<BuildingTypes>* pBonusConverterNode = headBonusConverterNode();
	...
	while (pBonusConverterNode != NULL)
	{
		...
		if (GC.getBuildingInfo(eBuilding).isConsumerOfBonus(eBonus))
		{
			updateNumActiveBuilding(eBuilding);
		}
...
/*
bool CvCity::bonusConverterCanOperate(BuildingTypes eBuilding) const
{
	...
	else
	{
		...
		for (int iI = 0; iI < kBuilding.getNumConsumedBonuses(); iI++)
		{
			// TODO: Check for obsolete bonuses?  But what should we do if a consumed bonus was obsolete?

			// TODO: Stop someone from building more than one at a time?

			CvBonusAmount kBonusAmount = kBuilding.getConsumedBonusAmount(iI);
			int iNumAvailable = getNumBonuses(kBonusAmount.m_eBonus) - pPlotGroup->getNumBonusConverterConsumedBonuses(eBuilding, iI);	// May be negative if city has a NoBonus

			if (kBonusAmount.m_iAmount > iNumAvailable)
			{
				return false;
			}
		}
...
bool CvCity::canConstructBonusConverter(BuildingTypes eBuilding) const
{
	CvPlotGroup* pPlotGroup = plot()->getPlotGroup(getOwnerINLINE());
	if (pPlotGroup == NULL)
	{
	...
	// First pass checks to see if there are enough resources available to construct a new converter in this plotgroup
	// This check should be extremely fast
	for (int iConsumedBonusIndex = 0; iConsumedBonusIndex < kBuilding.getNumConsumedBonuses(); iConsumedBonusIndex++)
	{
		BonusTypes eConsumedBonus = (BonusTypes)kBuilding.getConsumedBonusType(iConsumedBonusIndex);
		int iConsumedAmount = kBuilding.getConsumedBonusAmount(iConsumedBonusIndex);
		int iNumAvailable = getNumBonuses(eConsumedBonus) - pPlotGroup->getNumBonusConverterConsumedBonusesByIndex(eBuilding, iConsumedBonusIndex); // May be negative if city has a NoBonus

		if (iConsumedAmount > iNumAvailable)
		{
			return false;
		}
	}

	// Second pass considers other converters currently in construction within this plotgroup
	// This check is a bit slower because it loops over each city's construction queue, but it shouldn't be too awful thanks to our city plot cache
	CLLNode<XYCoords>* pPlotNode = pPlotGroup->headCityPlotsNode();
	int iConstructionCount = 0;

	// Count the number of converters of this type which are queued in other cities in this plot group
	// Note that converters of other types do not compete for resources so we ignore everything except for the building in question
	while (pPlotNode != NULL)
	{
		...
		if (pCity->getID() != getID())
		{
			...
			while (pOrderNode != NULL)
			{
				if (pOrderNode->m_data.eOrderType == ORDER_CONSTRUCT)
				{
					if (((BuildingTypes)pOrderNode->m_data.iData1) == eBuilding)
					{
						iConstructionCount++;
						break;
					}
				...
				pOrderNode = pCity->nextOrderQueueNode(pOrderNode);
			}
		}

		pPlotNode = pPlotGroup->nextCityPlotsNode(pPlotNode);
	}

	if (iConstructionCount > 0)
	{
		for (int iConsumedBonusIndex = 0; iConsumedBonusIndex < kBuilding.getNumConsumedBonuses(); iConsumedBonusIndex++)
		{
			BonusTypes eConsumedBonus = (BonusTypes)kBuilding.getConsumedBonusType(iConsumedBonusIndex);
			int iConsumedAmount = kBuilding.getConsumedBonusAmount(iConsumedBonusIndex);
			int iNumAvailable = getNumBonuses(eConsumedBonus) - pPlotGroup->getNumBonusConverterConsumedBonusesByIndex(eBuilding, iConsumedBonusIndex);	// May be negative if city has a NoBonus
			iNumAvailable -= iConstructionCount * iConsumedAmount; // Subtract the number of bonuses already spoken for by other converters in city queues
...
/*
bool CvCity::bonusConverterCanOperate(BuildingTypes eBuilding, int iPlotGroupBuildingNumber) const
{
	CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);

	for (int i=0; i < kBuilding.getNumConsumedBonuses(); i++)
	{
		CvBonusAmount kBonusAmount = kBuilding.getConsumedBonusAmount(i);
		// TODO: Check for obsolete bonuses?  But what should we do if a consumed bonus was obsolete?
		if (getNumBonuses(kBonusAmount.m_eBonus) < (kBonusAmount.m_iAmount * iPlotGroupBuildingNumber))
		{
			return false;
		}
	}
...
void CvCity::changeNumBonusConverters(BuildingTypes eBuilding, int iChange)
{
	FAssertMsg(eBuilding >= 0, "eBuilding is expected to be >= 0");
	FAssertMsg(eBuilding < GC.getNumBuildingInfos(), "eBuilding expected to be >= 0");
	...
	if (iChange != 0)
	{
		...
		{
			if (pBuildingCountNode->m_data.m_eBuilding == eBuilding)
			{
				pBuildingCountNode->m_data.m_iCount += iChange;

				FAssert(pBuildingCountNode->m_data.m_iCount >= 0);

				if (pBuildingCountNode->m_data.m_iCount == 0)
				{
					deleteBonusConverterCountsNode(pBuildingCountNode);
				}
		...
		{
			FAssert(iChange > 0);
			insertAtEndBonusConverterCounts(eBuilding, iChange);
		}

		CvPlotGroup* pPlotGroup = plotGroup(getOwnerINLINE());
		if (pPlotGroup != NULL)
		{
			pPlotGroup->changeNumBonusConverters(eBuilding, iChange);
		}
	}
}
*/
...
void CvCity::setBonusConverterIsPresent(BuildingTypes eBuilding, bool bNewValue)
{
	CLLNode<BuildingTypes>* pBonusConverterNode = headBonusConverterNode();
	...
	while (pBonusConverterNode != NULL)
	{
		if (pBonusConverterNode->m_data == eBuilding)
		{
			bOldValue = true;
			break;
		}
	...
	if (bOldValue != bNewValue)
	{
		if (bNewValue)
		{
			insertAtEndBonusConverters(eBuilding);
		}
		else
		{
			FAssert(pBonusConverterNode != NULL);
			deleteBonusConverterNode(pBonusConverterNode);
		}
	}
}


void CvCity::insertAtEndBonusConverters(BuildingTypes eBuilding)
{
	m_bonusConverters.insertAtEnd(eBuilding);
}


void CvCity::deleteBonusConverterNode(CLLNode<BuildingTypes>* pNode)
{
	m_bonusConverters.deleteNode(pNode);
}


CLLNode<BuildingTypes>* CvCity::nextBonusConverterNode(CLLNode<BuildingTypes>* pNode) const
{
	return m_bonusConverters.next(pNode);
}


CLLNode<BuildingTypes>* CvCity::headBonusConverterNode() const
{
	return m_bonusConverters.head();
}
// MOD - END - Bonus Converters
...
// K-Mod end
	...
	if (isHasReligion(eIndex) != bNewValue)
	{
		...
		// MOD - START - Advanced Building Prerequisites (Religion)
		if (kOwner.getStateReligion() == eIndex)
		{
			for (int iMem = 0; iMem < GC.getNumBuildingClassesInactiveWithoutPrereqStateReligion(); iMem++)
			{
				BuildingClassTypes eLoopBuildingClass = GC.getBuildingClassesInactiveWithoutStateRelgion(iMem);
				BuildingTypes eLoopBuilding = (BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eLoopBuildingClass));
...
void CvCity::updateTradeRoutes()
{
	...
	for (iI = 0; iI < GC.getDefineINT("MAX_TRADE_ROUTES"); iI++)
	{
		...
		if (pLoopCity != NULL)
		{
			...
			iTradeProfitTrunk += calculateTradeProfit(pLoopCity) * 100;
		}
	}
	
	m_aDecimalTradeRouteCommerce = iTradeProfit - iTradeProfitTrunk;
...
// MOD - END - K-Mod
{
	...
	case ORDER_TRAIN:
		if (canTrain((UnitTypes)iData1) || bForce)
		{
			...
			{
				UnitClassTypes eUnitClass = (UnitClassTypes)GC.getUnitInfo((UnitTypes)iData1).getUnitClassType();
				CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(eUnitClass);
				for (int iMem = 0; iMem < kUnitClass.getNumRoles(); iMem++)
				{
					UnitRoleTypes eUnitRole = (UnitRoleTypes)kUnitClass.getRoles(iMem);
					GET_PLAYER(getOwnerINLINE()).changeUnitRoleMaking(eUnitRole, 1);
				}
			}
			// MOD - END - Unit Roles
...
void CvCity::popOrder(int iNum, bool bFinish, bool bChoose)
{
	...
	case ORDER_TRAIN:
		...
		// MOD - START - Unit Roles
		{
			UnitClassTypes eUnitClass = (UnitClassTypes)GC.getUnitInfo(eTrainUnit).getUnitClassType();
			CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(eUnitClass);
			for (int iMem = 0; iMem < kUnitClass.getNumRoles(); iMem++)
			{
				UnitRoleTypes eUnitRole = (UnitRoleTypes)kUnitClass.getRoles(iMem);
		...
		if (bFinish)
		{
			...
			if (iLostProduction > 0)
			{
				...
				if (iProductionGold > 0)
				{
					// MOD - START - Excess Gold Bugfix
					// Note from Joshua Sjoding (2013-02-02): I don't know under what circumstances this was ever
					//   intended to work, but it creates a real problem for our limited unit classes. There is
					//   way too much gold getting generated from this. I wish I understand the original purpose
					//   of this.
					//GET_PLAYER(getOwnerINLINE()).changeGold(iProductionGold);
					// MOD - END - Excess Gold Bugfix
				}
			}
...
/************************************************************************************************/
	...
	if (bFinish && !bMessage)
	{
		...
		{
            #if defined(__GNUC__)
            wprintf(szBuffer, gDLL->getText(((isLimitedUnitClass((UnitClassTypes)(GC.getUnitInfo(eTrainUnit).getUnitClassType()))) ? "TXT_KEY_MISC_TRAINED_UNIT_IN_LIMITED" : "TXT_KEY_MISC_TRAINED_UNIT_IN"), GC.getUnitInfo(eTrainUnit).getTextKeyWide(), getNameKey()).GetCString());
            #else
		...
		{
            #if defined(__GNUC__)
            wprintf(szBuffer, gDLL->getText(((isLimitedWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eConstructBuilding).getBuildingClassType()))) ? "TXT_KEY_MISC_CONSTRUCTED_BUILD_IN_LIMITED" : "TXT_KEY_MISC_CONSTRUCTED_BUILD_IN"), GC.getBuildingInfo(eConstructBuilding).getTextKeyWide(), getNameKey()).GetCString());
            #else
		...
		{
            #if defined(__GNUC__)
            wprintf(szBuffer, gDLL->getText(((isLimitedProject(eCreateProject)) ? "TXT_KEY_MISC_CREATED_PROJECT_IN_LIMITED" : "TXT_KEY_MISC_CREATED_PROJECT_IN"), GC.getProjectInfo(eCreateProject).getTextKeyWide(), getNameKey()).GetCString());
            #else
		...
		{
            #if defined(__GNUC__)
            wprintf(szTempBuffer, gDLL->getText(((isProductionLimited()) ? "TXT_KEY_MISC_WORK_HAS_BEGUN_LIMITED" : "TXT_KEY_MISC_WORK_HAS_BEGUN"), getProductionNameKey()).GetCString());
            #else
...
void CvCity::doGrowth()
{
	...
	else if (getFood() < 0)
	{
		...
		if (getPopulation() > 1)
		{
			changePopulation(-1);
			// MOD - START - Core Game Events
			applyGameEvent(GAMEEVENT_STARVATION);
			// MOD - END - Core Game Events
		}
	}
}
...
bool CvCity::doCheckProduction()
{
	...
	for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
	{
		if (getFirstUnitOrder((UnitTypes)iI) != -1)
		{
			...
			if ((eUpgradeUnit != NO_UNIT) && (!GC.getUnitInfo((UnitTypes)iI).isIgnoreUpgrades()))
			{
				...
				while (pOrderNode != NULL)
				{
					if (pOrderNode->m_data.eOrderType == ORDER_TRAIN)
					{
						...
						{
							// MOD - START - Unit Roles
							UnitClassTypes eUnitClass = (UnitClassTypes)GC.getUnitInfo((UnitTypes)(pOrderNode->m_data.iData1)).getUnitClassType();
							CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(eUnitClass);
							for (int iMem = 0; iMem < kUnitClass.getNumRoles(); iMem++)
							{
								UnitRoleTypes eUnitRole = (UnitRoleTypes)kUnitClass.getRoles(iMem);
							...
							// MOD - START - Unit Roles
							eUnitClass = (UnitClassTypes)GC.getUnitInfo((UnitTypes)(pOrderNode->m_data.iData1)).getUnitClassType();
							kUnitClass = GC.getUnitClassInfo(eUnitClass);
							for (int iMem = 0; iMem < kUnitClass.getNumRoles(); iMem++)
							{
								UnitRoleTypes eUnitRole = (UnitRoleTypes)kUnitClass.getRoles(iMem);
								GET_PLAYER(getOwnerINLINE()).changeUnitRoleMaking(eUnitRole, 1);
...
// TODO: Consider using the new religion code
/*
void CvCity::doReligion()
{
	if (GC.getUSE_DO_RELIGION_CALLBACK()) // K-Mod. block unused python callbacks
...
void CvCity::doGreatPeople()
{
	...
	if (getGreatPeopleProgress() >= GET_PLAYER(getOwnerINLINE()).greatPeopleThreshold(false))
	{
		...
		if (eGreatPeopleUnit != NO_UNIT)
		{
			changeGreatPeopleProgress(-(GET_PLAYER(getOwnerINLINE()).greatPeopleThreshold(false)) * 100);
...
void CvCity::doEpidemic()
{
	...
	if (isImmune())
	{
		...
		{
			changeEpidemicTurns(1);
		}
		return;
	}

	int finalSpreadChance = getEpidemicProbability();
	...
	int randRate = 0;
	int rounds = std::max(1,GC.getDefineINT("EPIDEMIC_RANDOM_ROUND"));

	for (int i=0; i<rounds; i++)
	{
		randRate += GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("EPIDEMIC_RAND_BASE"),"Epidemic spread chance generator");
	}
	randRate /= rounds;
	...
	if (randRate <= finalSpreadChance)
	{
		...
		// Display message if epidemic is started
		if (getEpidemicTurns() == 0)
		{
			szMessage = gDLL->getText("TXT_KEY_EPIDEMIC_STRIKE", getName().GetCString());
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szMessage, "AS2D_DESTROY", MESSAGE_TYPE_MAJOR_EVENT, ARTFILEMGR.getInterfaceArtInfo("EPIDEMIC_START")->getPath(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), getX_INLINE(), getY_INLINE(), true, true);
		}

		// do somethig wrong : kill a citizens! - but don't go under min size
		int kpt = GC.getDefineINT("EPIDEMIC_KILL_PER_TURN");
		int oldPop = getPopulation();
		int minPop = GC.getDefineINT("EPIDEMIC_MIN_CITY_SIZE");
		if (oldPop > minPop)
		{
			setPopulation(std::max(minPop,oldPop-kpt));
			szMessage = gDLL->getText("TXT_KEY_EPIDEMIC_KILL", getName().GetCString(), getPopulation());
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szMessage, NULL, MESSAGE_TYPE_MAJOR_EVENT, ARTFILEMGR.getInterfaceArtInfo("EPIDEMIC_KILL")->getPath(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), getX_INLINE(), getY_INLINE(), true, true);
		}

		changeEpidemicTurns(1);
		// health damage for stationed units
		// ......
		//szMessage = gDLL->getText("TXT_KEY_EPIDEMIC_CITY_UNIT_DAMAGED", getName().GetCString());
		...
		// .......
		//szMessage = gDLL->getText("TXT_KEY_EPIDEMIC_AROUND_UNIT_DAMAGED", getName().GetCString());
		//gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szMessage, "", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
		// "build" building as visual representation
		BuildingTypes eReaper = (BuildingTypes)GC.getInfoTypeForString("BUILDING_REAPER");
		if (eReaper != NO_BUILDING)
		{
			setNumRealBuilding(eReaper, getNumRealBuilding(eReaper) + 1);
		}

		// MOD - START - Core Game Events
		applyGameEvent(GAMEEVENT_EPIDEMIC);
		// MOD - END - Core Game Events
	}
	else
	{
...
void CvCity::doEndEpidemic()
{
	CvWString szMessage;
	BuildingTypes eReaper = (BuildingTypes)GC.getInfoTypeForString("BUILDING_REAPER");

	// Display message if epidemic is ended
	szMessage = gDLL->getText("TXT_KEY_EPIDEMIC_STRIKE_END", getName().GetCString());
	gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szMessage, NULL, MESSAGE_TYPE_MINOR_EVENT, ARTFILEMGR.getInterfaceArtInfo("EPIDEMIC_END")->getPath(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
	// set counters to immune turn
	setEpidemicTurns( -GC.getDefineINT("EPIDEMIC_TURNS_CITY_IMMUNE_AFTER_OUTBREAK"));

	// remove visual building representation
	if (eReaper != NO_BUILDING)
	{
		setNumRealBuilding(eReaper, 0);
	}
}
// MOD - END - Epidemics
...
void CvCity::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iBuildingNaturalDefense);	
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	pStream->Read(&m_iEpidemicTurns);
	// MOD - END - Epidemics
	pStream->Read(&m_aDecimalTradeRouteCommerce);	
	...
	pStream->Read(NUM_YIELD_TYPES, m_aiSpecialistYield);
	// MOD - END - Advanced Yield and Commerce Effects
	...
	// MOD - START - Building Replacement
	pStream->Read(GC.getNumBuildingInfos(), m_paiBuildingReplacementCount);
	// MOD - END - Building Replacement
	// MOD - START - Building Disablement
	pStream->Read(GC.getNumBuildingInfos(), m_paiBuildingDisablementCount);
	// MOD - END - Building Disablement
	...
	// MOD - START - Building Activation
	pStream->Read(GC.getNumBuildingInfos(), m_paiNumActiveBuilding);
	// MOD - END - Building Activation
	// MOD - START - City Messages
	pStream->Read(GC.getNumBuildingInfos(), m_paiNumActiveBuildingChange);
	// MOD - END - City Messages

	// MOD - START - Bonus Converters
	m_bonusConverters.Read(pStream);
	// MOD - END - Bonus Converters
	...
	// MOD - START - Defense Modifiers
	pStream->Read(GC.getNumUnitCombatInfos(), m_paiUnitCombatDefenseModifier);
	pStream->Read(GC.getNumUnitClassInfos(), m_paiUnitClassDefenseModifier);
	// MOD - END - Defense Modifiers

	// MOD - START - Building Heal Rate
	pStream->Read(NUM_DOMAIN_TYPES, m_piDomainHealRate);
	pStream->Read(GC.getNumUnitClassInfos(), m_piUnitClassHealRate);
	pStream->Read(GC.getNumUnitCombatInfos(), m_piUnitCombatHealRate);
	// MOD - END - Building Heal Rate
	...
	// MOD - START - Population Metrics
	pStream->Read(GC.getNumMetricInfos(), m_paiMetricValues);
	m_kMetricTurnValues.read(pStream);
	// MOD - END - Population Metrics
	...
	pStream->Read(&iNumElts);
	m_aSpecialistYieldChange.clear();
	m_aSpecialistYieldChange.reserve(iNumElts);
	for (int i = 0; i < iNumElts; ++i)
	{
		SpecialistYieldChange kChange;
	...
	pStream->Read(&iNumElts);
	m_aSpecialistCommerceChange.clear();
	m_aSpecialistCommerceChange.reserve(iNumElts);
	for (int i = 0; i < iNumElts; ++i)
	{
		SpecialistCommerceChange kChange;
		kChange.read(pStream);
	...
	pStream->Read(&iNumElts);
	m_aImprovementYieldChange.clear();
	m_aImprovementYieldChange.reserve(iNumElts);
	for (int i = 0; i < iNumElts; ++i)
	{
		ImprovementYieldChange kChange;
		kChange.read(pStream);
	...
	pStream->Read(&m_iCitySeparatism);
	pStream->Read(&m_iCitySeparatismTemp);
	pStream->Read(&m_iCitySeparatismUnit);
}
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iBuildingNaturalDefense);	
	...
	// MOD - START - Epidemics
	// Epidemic system by Mexico
	pStream->Write(m_iEpidemicTurns);
	// MOD - END - Epidemics
	pStream->Write(m_aDecimalTradeRouteCommerce);	
	...
	// MOD - START - Advanced Yield and Commerce Effects
	pStream->Write(NUM_YIELD_TYPES, m_aiSpecialistYield);
	// MOD - END - Advanced Yield and Commerce Effects
	...
	// MOD - START - Building Replacement
	pStream->Write(GC.getNumBuildingInfos(), m_paiBuildingReplacementCount);
	// MOD - END - Building Replacement
	// MOD - START - Building Disablement
	pStream->Write(GC.getNumBuildingInfos(), m_paiBuildingDisablementCount);
	// MOD - END - Building Disablement
	...
	// MOD - START - Building Activation
	pStream->Write(GC.getNumBuildingInfos(), m_paiNumActiveBuilding);
	// MOD - END - Building Activation
	// MOD - START - City Messages
	pStream->Write(GC.getNumBuildingInfos(), m_paiNumActiveBuildingChange);
	// MOD - END - City Messages

	// MOD - START - Bonus Converters
	m_bonusConverters.Write(pStream);
	// MOD - END - Bonus Converters
	...
	// MOD - START - Defense Modifiers
	pStream->Write(GC.getNumUnitCombatInfos(), m_paiUnitCombatDefenseModifier);
	pStream->Write(GC.getNumUnitClassInfos(), m_paiUnitClassDefenseModifier);
	// MOD - END - Defense Modifiers

	// MOD - START - Building Heal Rate
	pStream->Write(NUM_DOMAIN_TYPES, m_piDomainHealRate);
	pStream->Write(GC.getNumUnitClassInfos(), m_piUnitClassHealRate);
	pStream->Write(GC.getNumUnitCombatInfos(), m_piUnitCombatHealRate);
	// MOD - END - Building Heal Rate
	...
	// MOD - START - Population Metrics
	pStream->Write(GC.getNumMetricInfos(), m_paiMetricValues);
	m_kMetricTurnValues.write(pStream);
	// MOD - END - Population Metrics
	...
	// MOD - START - Advanced Yield and Commerce Effects
	pStream->Write(m_aSpecialistYieldChange.size());
	for (std::vector<SpecialistYieldChange>::iterator it = m_aSpecialistYieldChange.begin(); it != m_aSpecialistYieldChange.end(); ++it)
	{
		(*it).write(pStream);
	}

	pStream->Write(m_aSpecialistCommerceChange.size());
	for (std::vector<SpecialistCommerceChange>::iterator it = m_aSpecialistCommerceChange.begin(); it != m_aSpecialistCommerceChange.end(); ++it)
	{
		(*it).write(pStream);
	}

	pStream->Write(m_aImprovementYieldChange.size());
	for (std::vector<ImprovementYieldChange>::iterator it = m_aImprovementYieldChange.begin(); it != m_aImprovementYieldChange.end(); ++it)
	{
		(*it).write(pStream);
	}
	...
	pStream->Write(m_iCitySeparatism);
	pStream->Write(m_iCitySeparatismTemp);
	pStream->Write(m_iCitySeparatismUnit);
}
...
void CvCity::getVisibleBuildings(std::list<BuildingTypes>& kChosenVisible, int& iChosenNumGenerics)
{
	...
	for(int i = 0; i < iNumBuildings; i++)
	{
		...
		if(getNumBuilding(eCurType) > 0)
		{
			...
			// MOD - START - Non-visible Obsolete Buildings
			// Note: Obsolete wonders are still visible
			// kVisible.push_back(eCurType);
			if (isLimitedWonderClass((BuildingClassTypes) kBuilding.getBuildingClassType()) || !isObsoleteBuilding(eCurType))
			{
				// MOD - START - River Building Visibility
				if (kBuilding.isRiver() && !plot()->isRiver())
				{
					// Don't show river buildings in cities that aren't on rivers
					continue;
				}
				// MOD - END - River Building Visibility
				kVisible.push_back(eCurType);
			}
			// MOD - END - Non-visible Obsolete Buildings
		}
	}
	...
	if(_stricmp(GC.getDefineSTRING("GAME_CITY_SIZE_METHOD"), "METHOD_EXPONENTIAL") == 0) 
	{
...
// MOD - START - Core Game Events
void CvCity::applyGameEvent(GameEventTypes eGameEvent, int iGameEventContext)
{
	const CvGameEventInfo& kGameEvent = GC.getGameEventInfo(eGameEvent);
	// MOD - START - Population Metrics
	if (iGameEventContext == -1)
	{
		for (int iMem = 0; iMem < kGameEvent.getNumMetricFactors(); iMem++)
		{
			applyMetricFactor((MetricFactorTypes)kGameEvent.getMetricFactors(iMem));
		}
	}
	else
	{
		for (int iMem = 0; iMem < kGameEvent.getNumMetricFactors(); iMem++)
		{
			MetricFactorTypes eLoopFactor = (MetricFactorTypes)kGameEvent.getMetricFactors(iMem);
			const CvMetricFactorInfo& kFactor = GC.getMetricFactorInfo(eLoopFactor);
			if (kFactor.getGameEventContext() == iGameEventContext)
			{
				applyMetricFactor(eLoopFactor);
			}
		}
	}
	// MOD - END - Population Metrics
...
// MOD - START - Population Metrics
bool CvCity::canApplyMetricFactor(MetricFactorTypes eFactor) const
{
	const CvMetricFactorInfo& kFactor = GC.getMetricFactorInfo(eFactor);
	const CvTeam& kTeam = GET_TEAM(getTeam());
	const CvPlayer& kPlayer = GET_PLAYER(getOwnerINLINE());

	for (int iPre = 0; iPre < kFactor.getNumPrereqAndTechs(); iPre++)
	{
		if (!kTeam.isHasTech((TechTypes)kFactor.getPrereqAndTechs(iPre)))
		{
			return false;
		}
	}

	for (int iPre = 0; iPre < kFactor.getNumObsoleteTechs(); iPre++)
	{
		if (kTeam.isHasTech((TechTypes)kFactor.getObsoleteTechs(iPre)))
		{
			return false;
		}
	}

	for (int iPre = 0; iPre < kFactor.getNumPrereqAndCivics(); iPre++)
	{
		if (!kPlayer.isCivic((CivicTypes)kFactor.getPrereqAndCivics(iPre)))
		{
			return false;
		}
	}

	for (int iPre = 0; iPre < kFactor.getNumObsoleteCivics(); iPre++)
	{
		if (kPlayer.isCivic((CivicTypes)kFactor.getObsoleteCivics(iPre)))
		{
			return false;
		}
	}

	if (kFactor.getMinEra() != NO_ERA)
	{
		if (kPlayer.getCurrentEra() < kFactor.getMinEra())
		{
			return false;
		}
	}

	if (kFactor.getMaxEra() != NO_ERA)
	{
		if (kPlayer.getCurrentEra() > kFactor.getMaxEra())
		{
			return false;
		}
	}

	if (kFactor.getMinPopulation() > 0)
	{
		if (getPopulation() < kFactor.getMinPopulation())
		{
			return false;
		}
	}

	if (kFactor.getMaxPopulation() > -1)
	{
		if (getPopulation() > kFactor.getMaxPopulation())
		{
			return false;
		}
	}
...
void CvCity::applyMetricFactor(MetricFactorTypes eFactor)
{
	if (!canApplyMetricFactor(eFactor))
	{
		return;
	}
	...
	int iSpeedPercent = GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getMetricChangePercent();
	int iHandicapPercent = isHuman() ? GC.getHandicapInfo(getHandicapType()).getMetricChangePercent() : GC.getHandicapInfo(getHandicapType()).getAIMetricChangePercent();

	for (int iI = 0; iI < kFactor.getNumMetricChanges(); iI++)
	{
		MetricTypes eMetric = (MetricTypes)kFactor.getMetricChangeMetrics(iI);
		ContributionTypes eType = (ContributionTypes)kFactor.getMetricChangeContributionTypes(iI);
		int iAmount = kFactor.getMetricChangeAmounts(iI);
		int iSpeedModifier = kFactor.getMetricChangeSpeedModifiers(iI);
		int iHandicapModifier = kFactor.getMetricChangeHandicapModifiers(iI);

		switch (eType)
		{
		case CONTRIBUTION_FIXED:
			break;
		case CONTRIBUTION_PERCENT_CHANGE:
			iAmount = getMetricValue(eMetric) * iAmount / 100;
			break;
		case CONTRIBUTION_PERCENT_CHANGE_TIMES_100:
			iAmount = getMetricValue(eMetric) * iAmount / 10000;
			break;
		case CONTRIBUTION_POPULATION:
			iAmount *= getPopulation();
			break;
		case CONTRIBUTION_ANGRY_POPULATION:
			iAmount *= angryPopulation();
			break;
		case CONTRIBUTION_UNHEALTHY_POPULATION:
			iAmount *= unhealthyPopulation();
			break;
		case CONTRIBUTION_RELIGION:
			iAmount *= getReligionCount();
			break;
		case CONTRIBUTION_STATE_RELIGION:
		case CONTRIBUTION_NONSTATE_RELIGION:
		case CONTRIBUTION_MILITARY_PRESENCE:
			iAmount *= getMilitaryHappinessUnits();
			break;
		case CONTRIBUTION_SUPERIOR_MILITARY_POWER_RATIO:
		case CONTRIBUTION_INFERIOR_MILITARY_POWER_RATIO:
		case CONTRIBUTION_FOREIGN_CULTURE:
		case CONTRIBUTION_EMERGENT_CULTURE:
			break;
		}

		// Perform a linear interpolation of the speed and handicap effects, using their respective modifiers as the weights
		iAmount = (iAmount > 100 ? 0 : iAmount * (100 - iSpeedModifier) / 100) + iAmount * iSpeedPercent * iSpeedModifier / 10000;
		iAmount = (iAmount > 100 ? 0 : iAmount * (100 - iHandicapModifier) / 100) + iAmount * iHandicapPercent * iHandicapModifier / 10000;

		changeMetricValue(eMetric, iAmount);
	}
}

int CvCity::getMetricValue(MetricTypes eMetric) const
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::setMetricValue");
	return m_paiMetricValues[eMetric];
}

void CvCity::setMetricValue(MetricTypes eMetric, int iValue)
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::setMetricValue");
	m_paiMetricValues[eMetric] = iValue;
	setMetricTurnValue(eMetric, GC.getGame().getGameTurn(), iValue);
}

void CvCity::changeMetricValue(MetricTypes eMetric, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::setMetricValue");
	if (iChange != 0)
	{
		m_paiMetricValues[eMetric] = iChange;
		setMetricTurnValue(eMetric, GC.getGame().getGameTurn(), m_paiMetricValues[eMetric]);
	}
}

void CvCity::updateMetricTurnValues()
{
	// Ensure all non-zero metrics are recorded for the current turn
	int iTurn = GC.getGameINLINE().getGameTurn();
	for (int iI = 0; iI < GC.getNumMetricInfos(); iI++)
	{
		MetricTypes eLoopMetric = (MetricTypes)iI;
		int iValue = getMetricValue(eLoopMetric);
		if (iValue != 0)
		{
			setMetricTurnValue(eLoopMetric, iTurn, iValue);
		}
	}
}

int CvCity::getMetricTurnValue(MetricTypes eMetric, int iTurn) const
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::getMetricTurnValue");
	return m_kMetricTurnValues.getTurnValue(eMetric, iTurn);
}

void CvCity::setMetricTurnValue(MetricTypes eMetric, int iTurn, int iValue)
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::setMetricTurnValue");
	m_kMetricTurnValues.setTurnValue(eMetric, iTurn, iValue);
}

void CvCity::changeMetricTurnValue(MetricTypes eMetric, int iTurn, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::changeMetricTurnValue");
	m_kMetricTurnValues.changeTurnValue(eMetric, iTurn, iChange);
}

int CvCity::getMetricMinValue(MetricTypes eMetric) const
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::getMetricMinValue");
	return m_kMetricTurnValues.getMinValue(eMetric);
}

int CvCity::getMetricMaxValue(MetricTypes eMetric) const
{
	FASSERT_BOUNDS(0, GC.getNumMetricInfos(), eMetric, "CvCity::getMetricMaxValue");
	return m_kMetricTurnValues.getMaxValue(eMetric);
}
// MOD - END - Population Metrics
...
bool CvCity::canApplyEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	const CvPlayer& kOwner = GET_PLAYER(getOwnerINLINE());
	int iFood = kOwner.getEventFood(eEvent);
	int iFoodPercent = kOwner.getEventFoodPercent(eEvent);

	if (iFood + ((100 + iFoodPercent) * getFood()) / 100 < 0)
	{
		return false;
	}
...
void CvCity::applyEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData, bool bClear)
{
	...
	if (kEvent.isCityEffect() || kEvent.isOtherPlayerCityEffect())
	{
		...
		if (kEvent.getFood() != 0 || kEvent.getFoodPercent() != 0)
		{
			...
			changeFood(iFood + (iFoodPercent * getFood()) / 100);
			// MOD - END - Game Speed Event Scaling
		}
...
// MOD - START - Advanced Yield and Commerce Effects
int CvCity::getYieldPerBuilding(BuildingTypes eBuilding, YieldTypes eYield) const
{
	FASSERT_BOUNDS(0, GC.getNumBuildingInfos(), eBuilding, "CvCity::getYieldPerBuilding");
	FASSERT_BOUNDS(0, NUM_YIELD_TYPES, eYield, "CvCity::getYieldPerBuilding");
	const CvPlayer& kPlayer = GET_PLAYER(getOwnerINLINE());
	const CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);
	BuildingClassTypes eBuildingClass = (BuildingClassTypes)kBuilding.getBuildingClassType();
	return kBuilding.getYieldChange(eYield) + getBuildingYieldChange(eBuildingClass, eYield) + kPlayer.getBuildingYieldChange(eBuildingClass, eYield);
}

void CvCity::applyBuildingYieldChange(BuildingClassTypes eBuildingClass, YieldTypes eYield, int iChange)
{
	...
	if (NO_BUILDING != eBuilding)
	{
		if (getNumActiveBuilding(eBuilding) > 0)
		{
			changeActualBaseYieldRate(eYield, iChange * getNumActiveBuilding(eBuilding));
		}
	}
}
// MOD - END - Advanced Yield and Commerce Effects
			...
			if (iOldChange != iChange)
			{
				...
				if (NO_BUILDING != eBuilding)
				{
					if (getNumActiveBuilding(eBuilding) > 0)
					{
						...
						changeActualBaseYieldRate(eYield, (iChange - iOldChange) * getNumActiveBuilding(eBuilding));
						// MOD - END - Negative Yield Acceptance
					}
				}
			}
			...
			if (getNumRealBuilding(eLoopBuilding) > 0)
			{
				...
				if ( iTimer > 0)
				{
					if((GC.getGameINLINE().getGameTurn() - getBuildingOriginalTurn(eLoopBuilding)) >= iTimer)
					{
						...
						gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, GC.getBuildingInfo(eLoopBuilding).getConstructSound(), MESSAGE_TYPE_MINOR_EVENT, GC.getBuildingInfo(eLoopBuilding).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), true, true);
					}
				}
			}
		}
...
int CvCity::getCitySeparatism() const
{
	return m_iCitySeparatism;
}

void CvCity::changeCitySeparatism(int iChange)
{
	m_iCitySeparatism = iChange;
}

int CvCity::getCitySeparatismTemp() const
{
	return m_iCitySeparatismTemp;
}

void CvCity::changeCitySeparatismTemp(int iChange)
{
	m_iCitySeparatismTemp = iChange;
}

int CvCity::getCitySeparatismUnit() const
{
	return m_iCitySeparatismUnit;
}

void CvCity::changeCitySeparatismUnit(int iChange)
{
	m_iCitySeparatismUnit = iChange;
}
