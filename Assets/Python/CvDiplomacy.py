...
class CvDiplomacy:
    ...
    def determineResponses(self, eComment):
        ...
        # Or if the AI is no longer a vassal
            ...
            if (
                gc.getTeam(gc.getGame().getActiveTeam()).
                canDeclareWar(gc.getPlayer(self.diploScreen.getWhoTradingWith()).getTeam())
            ):
        ...
        # If they reject our demand
            ...
            if (self.diploScreen.ourOfferEmpty() == 0 or self.diploScreen.theirOfferEmpty() == 0):
                ...
                # Otherwise this is a player offer to the AI
                    ...
                    # Otherwise they have something on the table and we dont
                        ...
                        # Otherwise (during peacetime) ask what they want for our item or
                        # demand they give it to us
                        else:
                            if (
                                gc.getGame().getActiveTeam() == gc.getPlayer(
                                    self.diploScreen.getWhoTradingWith()
                                ...
                                if (
                                    ...
                                    ).isVassal(gc.getGame().getActiveTeam()) and
                                    self.diploScreen.theirVassalTribute()
                                ):
                    ...
                    # Otherwise we have something on the table and they dont
                        ...
                        # If we are at war, use this item to fish for peace or
                        # propose peace with the items
                        if (self.diploScreen.atWar()):
                            self.addUserComment("USER_DIPLOCOMMENT_PROPOSE", -1, -1)

                        # During peace, see what we can get for these items or
                        # simply gift them to the AI
                        else:
                            if (
                                gc.getGame().getActiveTeam() != gc.getPlayer(
                                    self.diploScreen.getWhoTradingWith()
            ...
            for i in range(gc.getNumTechInfos()):
                if (gc.getPlayer(self.diploScreen.getWhoTradingWith()).canResearch(i, False)):
                    self.addUserComment(
                        "USER_DIPLOCOMMENT_RESEARCH_TECH", i, -1, gc.getTechInfo(i).getTextKey()
                    )
    ...
    def handleUserResponse(self, eComment, iData1, iData2):
        ...
        # If we reject their demand
            ...
            if (
                gc.getPlayer(self.diploScreen.getWhoTradingWith()).
                AI_demandRebukedWar(gc.getGame().getActivePlayer())
            ):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_DECLARE_WAR"))
                diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_DEMAND_WAR, -1, -1)
            else:
        ...
        # If we want to ask them to what their attitude is on a specific player
            ...
            if (eAttitude == AttitudeTypes.ATTITUDE_FURIOUS):
                self.setAIComment(
                    self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_FURIOUS"),
                    gc.getPlayer(iData1).getNameKey()
                )
            elif (eAttitude == AttitudeTypes.ATTITUDE_ANNOYED):
                self.setAIComment(
                    self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_ANNOYED"),
                    gc.getPlayer(iData1).getNameKey()
                )
            elif (eAttitude == AttitudeTypes.ATTITUDE_CAUTIOUS):
                self.setAIComment(
                    self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_CAUTIOUS"),
                    gc.getPlayer(iData1).getNameKey()
                )
            elif (eAttitude == AttitudeTypes.ATTITUDE_PLEASED):
                self.setAIComment(
                    self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_PLEASED"),
                    gc.getPlayer(iData1).getNameKey()
                )
            else:
                self.setAIComment(
                    self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_FRIENDLY"),
                    gc.getPlayer(iData1).getNameKey()
                )
        ...
        # K-Mod
            ...
            diploScreen.diploEvent(
                DiploEventTypes.DIPLOEVENT_SET_WARPLAN,
                TeamTypes.NO_TEAM, WarPlanTypes.NO_WARPLAN
            )
