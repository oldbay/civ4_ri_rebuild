...
from Components.BUG import BugConfig
from Components.BUG import BugDll
from Components.BUG import BugUtil
import CvUtil
import re
...
def init():
    ...
    for name, symbol in FontSymbols.__dict__.items():
        if name.endswith("_CHAR") and isinstance(symbol, FontSymbols):
            symbolNames[symbol] = name
    for key, symbol in CvUtil.OtherFontIcons.items():
