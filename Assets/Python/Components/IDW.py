...
class IDW:
    ...
    def checkCulture(self, pWinner, pLoser):
        if pLoser.getUnitCombatType() == -1:
            return  # Non Combat Units
        if pLoser.getDomainType() != (2 or 3):
            return  # Land combat only
    ...
    def checkCity(self, pPlot, pWinner, iLosePlayer):
        if pPlot.getNumVisibleEnemyDefenders(pWinner) > 1:
            return
        if pPlot.isCity():
            ...
            if CyGame().getSorenRandNum(iMaxRandom, "Resistance Chance") >= pCity.getCultureLevel() * pPlot.calculateCulturePercent(iLosePlayer):
                return
            ...
            if pCity.getPopulation() - iPopulationCost < iMinPopulation:
                return
    ...
    def changeCulture(self, pPlot, iCulture, iWinPlayer, iLosePlayer):
        for x in range(pPlot.getX() - 2, pPlot.getX() + 3):
            for y in range(pPlot.getY() - 2, pPlot.getY() + 3):
                ...
                if iDistanceX == 2 and iDistanceY == 2:
                    continue
