...
bool CyGame::isRequireStateReligionForReligionRemoval() const
{
	return m_pGame ? m_pGame->isRequireStateReligionForReligionRemoval() : false;
}
...
{
	return m_pGame ? m_pGame->isRequireCityOwnerStateReligionMatchForReligionRemoval() : false;
}
...
{
	return m_pGame ? m_pGame->isRequireOpenBordersForReligionRemoval() : false;
}
...
int CyGame::getResolutionX() const
{
	return (NULL != m_pGame ? m_pGame->getResolutionX() : -1);
}

int CyGame::getResolutionY() const
{
	return (NULL != m_pGame ? m_pGame->getResolutionY() : -1);
}

void CyGame::setResolution(int iX, int iY)
{
	if (m_pGame)
	{
		m_pGame->setResolution(iX, iY);
	}
}
// MOD - END - Graphical Paging

int CyGame::getInitialNumPlayers() const
{
	return (NULL != m_pGame ? m_pGame->getInitialNumPlayers() : 0);
}

void CyGame::setInitialNumPlayers(int iNewValue)
{
	if (NULL != m_pGame)
	{
		m_pGame->setInitialNumPlayers(iNewValue);
	}
}

int CyGame::getAheadOfTimeEra() const
{
	return (NULL != m_pGame ? m_pGame->getAheadOfTimeEra() : 0);
}

void CyGame::setAheadOfTimeEra( const int newAheadOfTimeEra )			
{
	if (NULL != m_pGame)
	{
		m_pGame->setAheadOfTimeEra(newAheadOfTimeEra);
	}
}

int CyGame::getForcedStyle() const
{
	return (NULL != m_pGame ? m_pGame->getForcedStyle() : -1);
}

void CyGame::setForcedStyle(int iNewValue)
{
	if (NULL != m_pGame)
	{	
		m_pGame->setForcedStyle(iNewValue);
	}
}

// MOD - START - Better Nukes
int CyGame::getNuclearWinterLevel() const
{
	return (NULL != m_pGame ? m_pGame->getNuclearWinterLevel() : -1);
}
