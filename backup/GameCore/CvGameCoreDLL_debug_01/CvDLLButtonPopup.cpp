...
/*                                                                                              */
/************************************************************************************************/
	case BUTTONPOPUP_SELECT_SHADOW_UNIT:
		{
			IDInfo* pShadowUnits = (IDInfo*)info.getData2();

			int iUnit = pPopupReturn->getButtonClicked();
			if (iUnit >= 0)
			{
				IDInfo kUnit = pShadowUnits[iUnit];
				GC.getGameINLINE().selectionListGameNetMessage(GAMEMESSAGE_PUSH_MISSION, MISSION_SHADOW, kUnit.eOwner, kUnit.iID, MISSIONTARGET_UNIT);
			}

			SAFE_DELETE(pShadowUnits);
		}
		break;
...
bool CvDLLButtonPopup::launchMainMenuPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	if ((GC.getGameINLINE().canDoControl(CONTROL_RETIRE)) && !(GC.getGameINLINE().getElapsedGameTurns() == 0))
	{
...
bool CvDLLButtonPopup::launchSelectShadowUnitPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	CvSelectionGroup* pGroup = GET_PLAYER(ePlayer).getSelectionGroup(iGroupID);
	if (pGroup == NULL)
	{
		return false;
	}
	...
	std::vector<IDInfo> aShadowUnits;

	CvWStringBuffer szBuffer;
	int iUnit = 0;
	for (CLLNode<IDInfo>* pShadowUnitNode = pPlot->headUnitNode(); pShadowUnitNode != NULL; pShadowUnitNode = pPlot->nextUnitNode(pShadowUnitNode))
	{
		CvUnit* pShadowUnit = ::getUnit(pShadowUnitNode->m_data);

		if (pGroup->canShadowUnit(pShadowUnit))
		{
			aShadowUnits.push_back(pShadowUnitNode->m_data);
			szBuffer.clear();
			GAMETEXT.setUnitHelp(szBuffer, pShadowUnit, true);
			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, CvWString(szBuffer.getCString()), GC.getUnitInfo(pShadowUnit->getUnitType()).getButton(), iUnit++, WIDGET_GENERAL);
		}
	}

	// Here we save a pointer to the unit list in the popup info so that we can map the buttons back to units later
	// NOTE: The list MUST be deallocated when the popup is closed
	IDInfo* pShadowUnits = new IDInfo[aShadowUnits.size()];
	for (iUnit = 0; iUnit < (int)aShadowUnits.size(); iUnit++)
	{
