...
CvUnit::CvUnit()
{
	...
	m_paiFeatureDamageImmunityCount = NULL;
...
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection)
{
	...
	CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(getUnitClassType());
	for (int iMem = 0; iMem < kUnitClass.getNumRoles(); iMem++)
	{
		UnitRoleTypes eUnitRole = (UnitRoleTypes)kUnitClass.getRoles(iMem);
		GET_PLAYER(getOwnerINLINE()).changeUnitRoleCostModifier(eUnitRole, kUnitClass.getRoleCostModifier());
		GET_PLAYER(getOwnerINLINE()).changeUnitRoleCount(eUnitRole, 1);
		GET_TEAM(getTeam()).changeUnitRoleCount(eUnitRole, 1);
	}
	// MOD - END - Unit Roles
	...
	for (int iRel = 0; iRel < m_pUnitInfo->getNumRelatedBuilds(); iRel++)
	{
		BuildTypes eLoopBuild = (BuildTypes)m_pUnitInfo->getRelatedBuilds(iRel);
		if (m_pUnitInfo->getBuilds(eLoopBuild))
		{
			GET_PLAYER(getOwnerINLINE()).changeBuilderCount(eLoopBuild, 1);
		}
	}
		...
		if (GET_PLAYER(getOwnerINLINE()).hasTrait((TraitTypes)iI))
		{
			for (iJ = 0; iJ < GC.getNumPromotionInfos(); iJ++)
			{
				if (GC.getTraitInfo((TraitTypes) iI).isFreePromotion(iJ))
				{
					...
					if (((getUnitCombatType() != NO_UNITCOMBAT) && GC.getTraitInfo((TraitTypes) iI).isFreePromotionUnitCombat(getUnitCombatType())) || GC.getTraitInfo((TraitTypes) iI).isFreePromotionUnitClass(getUnitClassType()))
	...
	{
		if (GC.getUnitClassInfo((UnitClassTypes)(m_pUnitInfo->getUnitClassType())).getMaxGlobalInstances() < 2)
		{
...
void CvUnit::uninit()
{
	...
	// MOD - START - Feature Damage Immunity
	SAFE_DELETE_ARRAY(m_paiFeatureDamageImmunityCount);
	// MOD - END - Feature Damage Immunity
...
/************************************************************************************************/
	...
	m_bAirCombat = false;
	// MOD - START - Slavery
	m_bIgnoreObstacle = false; // slave rebels ignore Great Wall
	// MOD - END - Slavery
	...
	if (!bConstructorCall)
	{
		...
		// MOD - START - Feature Damage Immunity
		m_paiFeatureDamageImmunityCount = new int[GC.getNumFeatureInfos()];
		// MOD - END - Feature Damage Immunity
		...
		for (iI = 0; iI < GC.getNumFeatureInfos(); iI++)
		{
			...
			// MOD - START - Feature Damage Immunity
			m_paiFeatureDamageImmunityCount[iI] = 0;
			// MOD - END - Feature Damage Immunity
...
void CvUnit::convert(CvUnit* pUnit)
{
	...
	for (int iI = 0; iI < GC.getNumPromotionInfos(); iI++)
	{
		...
		if (!GC.getPromotionInfo((PromotionTypes) iI).isTemporary())
		{
			setHasPromotion(((PromotionTypes)iI), (pUnit->isHasPromotion((PromotionTypes)iI) || m_pUnitInfo->getFreePromotions(iI)));
		}
	...
	// MOD - START - Aid
	updateAidEffects();					// Recalculate this unit's aid promotions
	// MOD - END - Aid

	// MOD - START - Detriments
	updateDetrimentEffects();			// Recalculate this unit's detriment promotions
	// MOD - END - Detriments
...
void CvUnit::doTurn()
{
	...
	if (isSpy() && isIntruding() && !isCargo())
	{
		...
		if (NO_TEAM != eTeam)
		{
			...
			{
				testSpyIntercepted(plot()->getOwnerINLINE(), false, GC.getDefineINT("ESPIONAGE_SPY_NO_INTRUDE_INTERCEPT_MOD")); // K-Mod
			}
			else
			{
				testSpyIntercepted(plot()->getOwnerINLINE(), false, GC.getDefineINT("ESPIONAGE_SPY_INTERCEPT_MOD")); // K-Mod
			}
		}
	}
	...
	if (baseCombatStr() > 0)
	{
		...
		if (NO_FEATURE != eFeature)
		{
			...
			{
				// MOD - START - Feature Damage Immunity
				if (!isImmuneToDamageFromFeature(eFeature) && !isCargo())
				{
...
/********************************************************************************/
	...
	*/
	...
	if( GC.getBBAI_AIR_COMBAT() && (DOMAIN_AIR == pInterceptor->getDomainType() && DOMAIN_AIR == getDomainType()) )
	{
		...
		int iTheirInterception = std::max(((pInterceptor->maxInterceptionProbability() - evasionProbability())/5),2*GC.getDefineINT("MIN_INTERCEPTION_DAMAGE"));
		int iOurInterception = std::max(((maxInterceptionProbability() - pInterceptor->evasionProbability())/5),2*GC.getDefineINT("MIN_INTERCEPTION_DAMAGE"));
	...
	{
		iOurRoundDamage = ((pInterceptor->currInterceptionProbability() - evasionProbability()) * GC.getDefineINT("MAX_INTERCEPTION_DAMAGE")) / 100;
		iTheirRoundDamage = (currInterceptionProbability() * GC.getDefineINT("MAX_INTERCEPTION_DAMAGE")) / 500;
		if (getDomainType() == DOMAIN_AIR)
		{
			iTheirRoundDamage = std::max(GC.getDefineINT("MIN_INTERCEPTION_DAMAGE"), iTheirRoundDamage);
		}
...
/********************************************************************************/
	...
	if (isDead())
	{
		if (iTheirRoundDamage > 0)
		{
			...
			pInterceptor->changeExperience(iExperience, maxXPValue(pInterceptor), true, pPlot->getOwnerINLINE() == pInterceptor->getOwnerINLINE(), !isBarbarian());
			// MOD - START - Advanced Experience Limits
		}
	}
	else if (pInterceptor->isDead())
	{
		...
		changeExperience(iExperience, pInterceptor->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pInterceptor->isBarbarian());
		// MOD - END - Advanced Experience Limits
	}
	else if (iOurDamage > 0)
	{
		if (iTheirRoundDamage > 0)
		{
			...
			pInterceptor->changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), maxXPValue(pInterceptor), true, pPlot->getOwnerINLINE() == pInterceptor->getOwnerINLINE(), !isBarbarian());
			// MOD - END - Advanced Experience Limits
		}
	}
	else if (iTheirDamage > 0)
	{
		...
		changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pInterceptor->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pInterceptor->isBarbarian());
		// MOD - END - Advanced Experience Limits
	}
...
void CvUnit::resolveCombat(CvUnit* pDefender, CvPlot* pPlot, bool bVisible)
{
	...
	if ((GC.getDefineINT("SHOW_ODDS_IN_COMBAT_MESSAGES") == 1) && (isHuman() || pDefender->isHuman()))
	{
		m_iAttackOdds = iLoggedOdds;
	}
	...
	if (iDefenderOdds > 950)
	{
		int iProbNudge = (25 - std::abs(975 - iDefenderOdds))/2;
		iDefenderOdds += iProbNudge;
	}
	
	if (iDefenderOdds < 50)
	{
		int iProbNudge = std::abs(25 - iDefenderOdds)/2;
		iDefenderOdds -= iProbNudge;
	}	
	...
	while (true)
	{
		if (GC.getGameINLINE().getSorenRandNum(GC.getCOMBAT_DIE_SIDES(), "Combat") < iDefenderOdds)
		{
			if (getCombatFirstStrikes() == 0)
			{
				if (getDamage() + iAttackerDamage >= maxHitPoints() && GC.getGameINLINE().getSorenRandNum(100, "Withdrawal") < withdrawalProbability())
				{
					...
					// MOD - START - Great Generals From Barbarian Combat
					// MOD - START - Advanced Experience Limits
					...
					changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pDefender->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isAnimal());
					// MOD - END - Advanced Experience Limits
					// MOD - START - Great Generals From Barbarian Combat
			...
			if (getDamage() + iAttackerDamage >= maxHitPoints() && pDefender->isBarbarian() && (GET_PLAYER(getOwnerINLINE()).getWinsVsBarbs() < GC.getHandicapInfo(GET_PLAYER(getOwnerINLINE()).getHandicapType()).getFreeWinsVsBarbs()))
				{
					pDefender->changeDamage(GC.getMAX_HIT_POINTS());
					changeExperience(1, pDefender->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isAnimal());
					combat_log.push_back(0); 
					cdAttackerDetails.iCurrHitPoints = currHitPoints();
					GET_PLAYER(getOwnerINLINE()).changeWinsVsBarbs(1);
					CvWString szBuffer;
					szBuffer = gDLL->getText("TXT_KEY_MISC_FREE_BARBARIAN_WIN", getNameKey(), pDefender->getNameKey(), getVisualCivAdjective(pDefender->getTeam()));
		...
		{
			if (pDefender->getDamage() + iDefenderDamage >= pDefender->maxHitPoints() && isBarbarian() && (GET_PLAYER(pDefender->getOwnerINLINE()).getWinsVsBarbs() < GC.getHandicapInfo(GET_PLAYER(pDefender->getOwnerINLINE()).getHandicapType()).getFreeWinsVsBarbs()))
				{
					changeDamage(GC.getMAX_HIT_POINTS());
					pDefender->changeExperience(1, maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), isAnimal());
					combat_log.push_back(0); 
					cdDefenderDetails.iCurrHitPoints = currHitPoints();
					GET_PLAYER(pDefender->getOwnerINLINE()).changeWinsVsBarbs(1);
					CvWString szBuffer;
					szBuffer = gDLL->getText("TXT_KEY_MISC_FREE_BARBARIAN_WIN", pDefender->getNameKey(), getNameKey(), getVisualCivAdjective(getTeam()));
					gDLL->getInterfaceIFace()->addHumanMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_UNITGIFTED", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());					
					break;
				}
			...
			if (pDefender->getCombatFirstStrikes() == 0)
			{
				if (std::min(GC.getMAX_HIT_POINTS(), pDefender->getDamage() + iDefenderDamage) > combatLimit())
				{
					// MOD - START - Advanced Experience Limits
					//changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
					changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pDefender->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
					// MOD - END - Advanced Experience Limits
		...
		if (isDead() || pDefender->isDead())
		{
			if (isDead())
			{
				...
				pDefender->changeExperience(iExperience, maxXPValue(pDefender), true, pPlot->getOwnerINLINE() == pDefender->getOwnerINLINE(), !isAnimal());
				// MOD - END - Advanced Experience Limits
				// MOD - END - Great Generals From Barbarian Combat
			}
			else
			{
				flankingStrikeCombat(pPlot, iAttackerStrength, iAttackerFirepower, iAttackerKillOdds, iDefenderDamage, pDefender);
				...
				changeExperience(iExperience, pDefender->maxXPValue(this), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isAnimal());
				// MOD - END - Advanced Experience Limits
				// MOD - END - Great Generals From Barbarian Combat
			}
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (!bFinish)
	{
		if (!isFighting())
		{
			...
			if (bFocused)
			{
				...
				// MOD - START - Battle Camera Zoom (RogerBacon)
				...
				gDLL->getInterfaceIFace()->lookAt(lookAtPoint, (gDLL->getGraphicOption(GRAPHICOPTION_NO_COMBAT_ZOOM) ? CAMERALOOKAT_BATTLE : CAMERALOOKAT_BATTLE_ZOOM_IN), attackDirection);
				// MOD - END - Battle Camera Zoom (RogerBacon)
			}
			else
			{
	...
	if (bFinish)
	{
		...
		bool const bOdds = ((GC.getDefineINT("SHOW_ODDS_IN_COMBAT_MESSAGES") == 1) && m_iAttackOdds >= 0 && pDefender->canDefend());
		CvWString szOdds;
		if (bOdds)
		{
			szOdds.Format(L"%.1f", (std::min(GC.getCOMBAT_DIE_SIDES() - 1, m_iAttackOdds) * 100.f) / GC.getCOMBAT_DIE_SIDES());
		}
...
/* 			if (isBarbarian())
			...
			if (bOdds)
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_KILLED_ENEMY_UNIT_ODDS", pDefender->getNameKey(), getNameKey(), getVisualCivAdjective(pDefender->getTeam()), szOdds.c_str());
...
/* 			if (pDefender->isBarbarian())
			...
			if (bOdds)
			{
				...
				{
					szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WAS_DESTROYED_UNKNOWN_ODDS", pDefender->getNameKey(), getNameKey(), szOdds.c_str());
				}
				else
				{
					szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WAS_DESTROYED_ODDS", pDefender->getNameKey(), getNameKey(), getVisualCivAdjective(pDefender->getTeam()), szOdds.c_str());				
				}					
			}
			else
			{
		...
		{
			bool const bSea = (getDomainType() == DOMAIN_SEA);
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WITHDRAW", getNameKey(), pDefender->getNameKey());
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, bSea ? "AS2D_OUR_SEA_WITHDRAWL" : "AS2D_OUR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			if (bOdds)
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_UNIT_WITHDRAW_ODDS", getNameKey(), pDefender->getNameKey(), szOdds.c_str());
				gDLL->getInterfaceIFace()->addHumanMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, bSea ? "AS2D_THEIR_SEA_WITHDRAWL" : "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			}
			else
			{
...
bool CvUnit::isBetterDefenderThan(const CvUnit* pDefender, const CvUnit* pAttacker, int* pBestDefenderRank, bool bRanged) const
// MOD - END - Ranged Strike AI
{
	...
	// at this point and call our own version instead
	// MOD - START - Protect Valuable Units / Lead From Behind
	// MOD - START - Ranged Strike AI
	//if (GC.getLFBEnable())
	if (GC.getGameINLINE().isOption(GAMEOPTION_PROTECT_VALUABLE_UNITS))
	// MOD - END - Ranged Strike AI
...
bool CvUnit::canEnterArea(TeamTypes eTeam, const CvArea* pArea, bool bIgnoreRightOfPassage) const
{
	...
	if (isBarbarian() && DOMAIN_LAND == getDomainType())
	{
		if (eTeam != NO_TEAM && eTeam != getTeam())
		{
			...
			{
				// MOD - START - Slavery (Mexico)
				// Mexico update for slave rebels (they can ignore great walls)
				if (!isIgnoreObstacle())
				{
					return false;
				}
				// MOD - END - Slavery (Mexico)
			}
		}
	}
	...
	if (bCanEnterArea)
	{
		if (pPlot->getFeatureType() != NO_FEATURE)
		{
			if (m_pUnitInfo->getFeatureImpassable(pPlot->getFeatureType()))
			{
				...
				{
					if (pPlot->getTeam() != getTeam())  // ALL units can enter impassable in own cultural borders
					{
						return false;
					}
				}
...
/************************************************************************************************/
	case AUTOMATE_PILLAGE:
		if (!GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_ADVANCED_AUTOMATIONS))
		{
			return false;
		}
	...
	*/
	...
	while (pUnitNode != NULL)
	{
		...
		if (canLoadUnit(pLoopUnit, pPlot))
		{
			...
			else if (pTeamTransport == NULL)
			{
				if (pLoopUnit->getTeam() == getTeam())
				{
					pTeamTransport = pLoopUnit;
				}
			}
		}
	}

	if (pTeamTransport != NULL)
	{
		setTransportUnit(pTeamTransport);
		return;
	}
	// MOD - END - Efficiency
}
...
int CvUnit::healRate(const CvPlot* pPlot, bool bLocation, bool bUnits) const
{
	...
	if (bLocation) // K-Mod
	{
		if (pPlot->isCity(true, getTeam()))
		{
			...
			if (pCity && !pCity->isOccupation())
			{
				...
				iTotalHeal += pCity->getDomainHealRate(getDomainType());
				iTotalHeal += pCity->getUnitClassHealRate(getUnitClassType());
				UnitCombatTypes eUnitCombat = getUnitCombatType();
				if (eUnitCombat != NO_UNITCOMBAT)
				{
					iTotalHeal += pCity->getUnitCombatHealRate(getUnitCombatType());
				}
	...
	// MOD - START - Negative Heal Rates
	return std::max(iTotalHeal, 0);
	// MOD - END - Negative Heal Rates
}
...
bool CvUnit::canAirBombAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	else
	{
		...
		if(GC.getGameINLINE().isRouteDestructionThroughAirBombs())
		{
			...
			{
				bRoute = true;
				if (GC.getRouteInfo(pTargetPlot->getRouteType()).getAirBombDefense() == -1)
					bRoute = false;
			}
		}
		
		if (bImprovement || bRoute)
			return true;
		else
			return false;
	}
...
bool CvUnit::airBomb(int iX, int iY)
{
	...
	if (pCity != NULL)
	{
		...
		if (isStrategic())
		{
			int iCityPop = pCity->getPopulation();
			if (iCityPop + GC.getGameINLINE().getSorenRandNum(iCityPop,"Random chance for citizens to die") > 10)
			{
				pCity->setPopulation(std::max(5,iCityPop-1));
				
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_POP_REDUCED_TO", getNameKey(), pCity->getNameKey(), pCity->getPopulation());
				gDLL->getInterfaceIFace()->addHumanMessage(pCity->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BOMBARDED", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);

				szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_POP_REDUCED_TO", getNameKey(), pCity->getNameKey(), pCity->getPopulation());
				gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BOMBARD", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE());				
			}
		}
...
bool CvUnit::bombard()
{
	...
	if (pBombardCity != NULL)
	{
		...
		int iBombardDamage = (-(bombardRate() * std::max(0, 100 + iBombardModifier)) / 100);
		pBombardCity->changeDefenseModifier(iBombardDamage);

		FAssertMsg(iBombardDamage > 0, "Bombard damage shouldn't ever be 0");
		FAssertMsg(iBombardModifier < 100, "Bombard modifier isn't supposed to be 100% or more");
...
bool CvUnit::spread(ReligionTypes eReligion)
{
	...
	if (pCity != NULL)
	{
		...
		{
			// MOD - START - Stock Religion Failure
			/* original bts code */
			...
			// K-Mod
			*/
			// MOD - END - Stock Religion Failure
		}
...
bool CvUnit::canRemoveReligion(const CvPlot* pPlot, ReligionTypes eReligion, bool bTestVisible) const
{
	FAssert(pPlot != NULL);
	FAssert(eReligion != NO_RELIGION);
	CvCity* pCity;

	if (!(GC.getUnitInfo(getUnitType()).isRemoveNonStateReligion()))
	{
		return false;
	}
	...
	{
		return false; // Players cannot remove their own state religion
	}
	...
	if (eReligion == GC.getUnitInfo(getUnitType()).getPrereqReligion())
	{
		return false;
	}

	if (GC.getGameINLINE().isRequireStateReligionForReligionRemoval())
	{
	...
	if (GC.getGameINLINE().isRequireCityOwnerStateReligionMatchForReligionRemoval())
	{
		if (eStateReligion == NO_RELIGION)
		{
			return false;
		...
		if (GET_PLAYER(pCity->getOwnerINLINE()).getStateReligion() != eStateReligion)
		{
			return false;
		}
	}
	...
	{
		if(GC.getGameINLINE().isRequireOpenBordersForReligionRemoval() && !GET_TEAM(kOwner.getTeam()).isOpenBorders(GET_PLAYER(pCity->getOwnerINLINE()).getTeam()))
		{
			return false;
		}
	}
...
bool CvUnit::canRemoveReligion(const CvPlot* pPlot, bool bTestVisible) const
{
	CvCity* pCity;
	...
	for (int iI = 0; iI < GC.getNumReligionInfos() ; iI++)
	{
		...
		if (pCity->isHasReligion(eReligion) && eReligion != eStateReligion)
		{
			bCanRemove = true;
		}
...
// TODO: Consider using K-Mod's formula for religion removal probability
bool CvUnit::removeReligion()
{
	CvCity* pCity;

	int iI,iJ;

	int iReligionCount = GC.getNumReligionInfos();

	int iHighestRemovalChance = -99999;

	int iRemovalChance  = 0;

	if (!canRemoveReligion(plot()))
	{
		return false;
	}
	...
	for( iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
		...
		if((ReligionTypes)iI == GC.getUnitInfo(getUnitType()).getPrereqReligion())
		{
			continue;
		}

		if((ReligionTypes)iI != NO_RELIGION && !pCity->isHasReligion((ReligionTypes)iI))
		{
			continue;
		}

		iRemovalChance = ((pCity->getTeam() == getTeam()) ? 40 : 20); // XXX mod

		iRemovalChance += (((GC.getNumReligionInfos() - pCity->getReligionCount()) * (100 - iRemovalChance)) / GC.getNumReligionInfos());
		...
		else if(pCity->isHolyCity())
		{
			iRemovalChance -= 25;
		}

		if(GET_PLAYER(pCity->getOwnerINLINE()).getStateReligion() == (ReligionTypes)iI)
		{
			iRemovalChance -= 15;
		}
		...
		{
			if(pCity->getNumActiveBuilding((BuildingTypes)iJ) > 0 )
			{
				if(GC.getBuildingInfo((BuildingTypes)iJ).getReligionType() == (ReligionTypes)iI)
				{
					iRemovalChance -= (GC.getBuildingInfo((BuildingTypes)iJ).getAssetValue() * 3);
				}
			}
		}

		if(iRemovalChance > iHighestRemovalChance)
		{
			iHighestRemovalChance = iRemovalChance;
	...
	return removeReligion(eReligion);
}

bool CvUnit::removeReligion(ReligionTypes eReligion)
{
	CvCity* pCity;
	CvWString szBuffer;
	int iRemoveProb;
	...
	if (!canRemoveReligion(plot(), eReligion))
	{
		return false;
	}
	...
	if (pCity != NULL)
	{
		...
		iRemoveProb = ((pCity->getTeam() == getTeam()) ? 40 : 20); // XXX mod

		iRemoveProb += (((GC.getNumReligionInfos() - pCity->getReligionCount()) * (100 - iRemoveProb)) / GC.getNumReligionInfos());

		iRand = GC.getGameINLINE().getSorenRandNum(100, "Unit Remove Religion");
		...
		if(GET_PLAYER(pCity->getOwnerINLINE()).getStateReligion() == eReligion)
		{
			iRemoveProb -= 15;
		}
		...
		for(iI = 0; iI < GC.getNumBuildingInfos(); iI++)
		{
			...
			{
				if(GC.getBuildingInfo((BuildingTypes)iI).getReligionType() == eReligion)
				{
					iRemoveProb -= (GC.getBuildingInfo((BuildingTypes)iI).getAssetValue() * 3);
				}
			}
		}

		/*
		szBuffer = gDLL->getText("iRand(%d1) iRemoveProb(%d2) GC.getNumReligionInfos()(%d3) pCity->getReligionCount()(%d4)", iRand, iRemoveProb, GC.getNumReligionInfos(), pCity->getReligionCount());
		gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_NUKE_EXPLODES", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);
		*/

		if (iRand < iRemoveProb)
		{

			if(getOwnerINLINE() != pCity->getOwnerINLINE())
			{
				GET_PLAYER(pCity->getOwnerINLINE()).AI_changeMemoryCount(getOwnerINLINE(), MEMORY_REMOVED_RELIGION_FROM_CITY, 1);
			}

			for (iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if((PlayerTypes)iI == getOwnerINLINE())
				{
					continue;
				}
				...
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					...
					{
						GET_PLAYER((PlayerTypes)iI).AI_changeMemoryCount(getOwnerINLINE(), MEMORY_REMOVED_STATE_RELIGION, 1);
					}
				}
			}

			gDLL->getEngineIFace()->TriggerEffect((EffectTypes)GC.getInfoTypeForString("EFFECT_CITY_BIG_BURNING_SMOKE"), pCity->plot()->getPoint(), (float)(GC.getASyncRand().get(360)));
			gDLL->getEngineIFace()->TriggerEffect((EffectTypes)GC.getInfoTypeForString("EFFECT_CITY_FIRE"), pCity->plot()->getPoint(), (float)(GC.getASyncRand().get(360)));
			gDLL->getEngineIFace()->TriggerEffect((EffectTypes)GC.getInfoTypeForString("EFFECT_CITY_BURNING_SMOKE"), pCity->plot()->getPoint(), (float)(GC.getASyncRand().get(360)));
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_REMOVED_RELIGION", getNameKey(), GC.getReligionInfo(eReligion).getChar(), pCity->getNameKey());
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_NOSPREAD", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX_INLINE(), pCity->getY_INLINE());
			...
			for(iI = 0; iI < GC.getNumBuildingInfos(); iI++)
			{
				if(pCity->getNumActiveBuilding((BuildingTypes)iI) > 0)
				{
					if(GC.getBuildingInfo((BuildingTypes)iI).getReligionType() == eReligion)
					{
						pCity->setNumRealBuilding((BuildingTypes)iI, pCity->getNumRealBuilding((BuildingTypes)iI) - 1);
					}
				}
			}
		}
		...
		{
			szBuffer = gDLL->getText("TXT_KEY_MISC_FAILED_TO_REMOVE_RELIGION", getNameKey(), GC.getReligionInfo(eReligion).getChar(), pCity->getNameKey());
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_NOSPREAD", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX_INLINE(), pCity->getY_INLINE());
	...
	if (plot()->isActiveVisible(false))
	{
		NotifyEntity(MISSION_REMOVE_RELIGION);
	}
...
{
	int iCost = std::max(0, GC.getCorporationInfo(eCorporation).getSpreadCost() * (100 + GET_PLAYER(getOwnerINLINE()).getInflationRate())); // K-Mod
	iCost /= 100;
...
bool CvUnit::espionage(EspionageMissionTypes eMission, int iData)
{
	...
	{
		if (testSpyIntercepted(eTargetPlayer, true, GC.getEspionageMissionInfo(eMission).getDifficultyMod())) // K-Mod
		{
			return false;
		}
		...
		if (GET_PLAYER(getOwnerINLINE()).doEspionageMission(eMission, eTargetPlayer, plot(), iData, this))
		{
			...
			if (!testSpyIntercepted(eTargetPlayer, true, GC.getDefineINT("ESPIONAGE_SPY_MISSION_ESCAPE_MOD"))) // K-Mod
			{
				setFortifyTurns(0);
				setMadeAttack(true);
				finishMoves();
...
bool CvUnit::testSpyIntercepted(PlayerTypes eTargetPlayer, bool bMission, int iModifier) // K-Mod
{
	...
	if (GC.getGameINLINE().getSorenRandNum(10000, "Spy Interception") >= getSpyInterceptPercent(kTargetPlayer.getTeam(), bMission) * (100 + iModifier)) // K-Mod
	{
		return false;
	}
...
int CvUnit::getSpyInterceptPercent(TeamTypes eTargetTeam, bool bMission) const // K-Mod
{
	FAssert(isSpy());
...
bool CvUnit::build(BuildTypes eBuild)
{
	...
	if (bFinished)
	{
		...
		// MOD - START - Slavery
		// chance to consume unit after improvement build by Mexico
		else if (getBuildKillChance() > 0)
			{
				int iRandom = GC.getGameINLINE().getSorenRandNum(100,"Kill worker after built");
				if (iRandom < getBuildKillChance())
				{
					kill(true);
					gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), gDLL->getText("TXT_KEY_MISC_UNIT_CONSUMED", GC.getUnitInfo(getUnitType()).getDescription()), NULL, MESSAGE_TYPE_DISPLAY_ONLY, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), plot()->getX_INLINE(), plot()->getY_INLINE(), true);
				}
			}
		// MOD - END - Slavery
	}
...
int CvUnit::upgradePrice(UnitTypes eUnit) const
{
	...
	// New code
	iPrice -= (iPrice * std::min(100, (getUpgradeDiscount() + GET_PLAYER(getOwnerINLINE()).getUpgradeDiscount()))) / 100;
...
bool CvUnit::upgradeAvailable(UnitTypes eFromUnit, UnitClassTypes eToUnitClass, int iCount) const
{
	...
	for (iI = 0; iI < numUnitClassInfos; iI++)
	{
		...
		{
	*/
	const std::vector<UnitClassTypes>& upgradeClasses = fromUnitInfo.getUpgradeUnitClassTypes();
	...
	// MOD - END - Unit Upgrade Cache (poyuzhe)
...
CvCity* CvUnit::getUpgradeCity(bool bSearch) const
{
	...
	*/
	...
	GC.getCivilizationInfo(kPlayer.getCivilizationType()).getUpgradesForUnit(getUnitType(), aUpgradeUnits);
	for (std::vector<UnitTypes>::const_iterator it = aUpgradeUnits.begin(); it != aUpgradeUnits.end(); ++it)
	{
		int iNewValue = kPlayer.AI_unitValue(*it, eUnitAI, pArea);
		if (iNewValue > iCurrentValue)
		{
			int iSearchValue;
			CvCity* pUpgradeCity = getUpgradeCity(*it, bSearch, &iSearchValue);
			if (pUpgradeCity != NULL)
			{
				// if not searching or close enough, then this match will do
				if (!bSearch || iSearchValue < 16)
...
UnitCombatTypes CvUnit::getUnitPromoType() const
{
	return ((UnitCombatTypes)(m_pUnitInfo->getUnitPromoType()));
}
...
{
	int iChange = iCombat - m_iBaseCombat;
	if (iChange != 0)
	{
		...
		if (pPlot != NULL)
		{
			const CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(getUnitClassType());
			TeamTypes eTeam = getTeam();

			// MOD - START - Aid
			if (kUnitClass.getNumAidTypes() > 0)
			{
				for (int iI = 0; iI < kUnitClass.getNumAidTypes(); iI++)
				{
					AidTypes eLoopAid = (AidTypes)kUnitClass.getAidTypes(iI);
					const CvAidInfo& kAidInfo = GC.getAidInfo(eLoopAid);
					if (kAidInfo.getAidStyleType() == AIDSTYLE_RELATIVE_STRENGTH)
					{
						pPlot->changeAidSuppliedFromPlot(eTeam, eLoopAid, iChange);
					}
				}
			}
			// MOD - END - Aid
			...
			// MOD - START - Detriments
			if (kUnitClass.getNumDetrimentTypes() > 0)
			{
				for (int iI = 0; iI < kUnitClass.getNumDetrimentTypes(); iI++)
				{
					DetrimentTypes eLoopDetriment = (DetrimentTypes)kUnitClass.getDetrimentTypes(iI);
					const CvDetrimentInfo& kDetrimentInfo = GC.getDetrimentInfo(eLoopDetriment);
					if (kDetrimentInfo.getDetrimentStyleType() == DETRIMENTSTYLE_RELATIVE_STRENGTH)
					{
						pPlot->changeDetrimentSuppliedFromPlot(getTeam(), eLoopDetriment, iChange);
					}
				}
			}
			// MOD - END - Detriments
	...
	if (pAttacker != NULL)
	{
		...
		if (isBarbarian())
		{
			if (pAttacker->isHuman())
			{
				...
				// MOD - START - Extra Barbarian Combat Modifier
				iExtraModifier -= pAttacker->getExtraBarbarianCombatModifier();
				// MOD - END - Extra Barbarian Combat Modifier
				iModifier += iExtraModifier;
				if (pCombatDetails != NULL)
				{
		...
		if (pAttacker->isBarbarian())
		{
			if (isHuman())
			{
				...
				// MOD - START - Extra Barbarian Combat Modifier
				iExtraModifier += getExtraBarbarianCombatModifier();
				// MOD - END - Extra Barbarian Combat Modifier
				iModifier += iExtraModifier;
				if (pCombatDetails != NULL)
				{
...
/************************************************************************************************/
			...
			// TODO: We could probably collapse both the UnitClassType
			//       and UnitClass modifiers into a single cached list (based on unit class)
			//       which has both baked into it.
			if (pAttacker != NULL)
			{
				CvCity* pCity = pPlot->getPlotCity();
				if (pCity != NULL)
				{
					...
					{
						iTempModifier += pCity->getUnitCombatDefenseModifier(eAttackerUnitCombatType);
					}

					iTempModifier += pCity->getUnitClassDefenseModifier(eAttackerUnitClassType);
					// Defensive buildings are only effective proportional to their health					
					iTempModifier = iTempModifier * iCityAdjustedModifier / 100;

					iModifier += iTempModifier;
					if (pCombatDetails != NULL)
					{
						pCombatDetails->iPlotDefenseModifier += iTempModifier;
					}
				}
			}
								...
								if (isEnemy(pLoopUnit->getTeam()))
								{
									if (DOMAIN_SEA == pLoopUnit->getDomainType())
									{
										if (ACTIVITY_PATROL == pLoopUnit->getGroup()->getActivityType())
										{
											...
											// MOD - START - Ranged Strike AI
											if (NULL == pBestUnit || pLoopUnit->isBetterDefenderThan(pBestUnit, this, &pBestUnitRank, false))
											// MOD - END - Ranged Strike AI
											// /UncutDragon
											{
...
int CvUnit::maxXPValue(const CvUnit* pAttacker) const
// MOD - END - Advanced Experience Limits
{
	int iMaxValue;
	...
	if (isBarbarian())
	{
		...
		int iBarbarianMaxXpValue = GC.getDefineINT("BARBARIAN_MAX_XP_VALUE");
		if (pAttacker != NULL)
		{
			iBarbarianMaxXpValue += pAttacker->getExtraBarbarianMaxXPValue();
		}
		iMaxValue = std::min(iMaxValue, iBarbarianMaxXpValue);
		// MOD - END - Advanced Experience Limits
	}
...
// MOD - START - Untransportable Units
bool CvUnit::noTransport() const
{
	return m_pUnitInfo->isNoTransport();
}
// MOD - END - Untransportable Units
...
int CvUnit::withdrawalProbability() const
{
	...
	// MOD - START - Capped Withdrawal Probability
	//return std::max(0, (m_pUnitInfo->getWithdrawalProbability() + getExtraWithdrawal()));
	// TODO: Store the real and capped versions so that we don't have to look up the define every time this gets called.
	return std::min(std::max(0, (m_pUnitInfo->getWithdrawalProbability() + getExtraWithdrawal())), GC.getDefineINT("MAX_WITHDRAWAL_PROBABILITY"));
	// MOD - END - Capped Withdrawal Probability
}
...
void CvUnit::setXY(int iX, int iY, bool bGroup, bool bUpdate, bool bShow, bool bCheckPlotVisible)
{
	...
	if (pOldPlot != NULL)
	{
		...
		// MOD - START - Aid
		// This causes any aid supplied by this unit to be removed from the old plot.
		// This unit will have its Temporary Promotions completely recalculated later if it gets a new plot.
		removeAidSuppliedByUnit(pOldPlot);
		// MOD - END - Aid

		// MOD - START - Detriments
		// This causes any detriments supplied by this unit to be removed from the old plot.
		// This unit will have its Temporary Promotions completely recalculated later if it gets a new plot.
		removeDetrimentsSuppliedByUnit(pOldPlot);
		// MOD - END - Detriments
...
/************************************************************************************************/
		...
		// MOD - START - Aid
		applyAidSuppliedByUnit(pNewPlot);	// Supply our aid to the new plot
		updateAidEffects();					// Recalculate this unit's aid promotions
		// MOD - END - Aid

		// MOD - START - Detriments
		applyDetrimentsSuppliedByUnit(pNewPlot);	// Supply our detriment to the new plot
		updateDetrimentEffects();					// Recalculate this unit's detriment promotions
		// MOD - END - Detriments
...
void CvUnit::changeExperience(int iChange, int iMax, bool bFromCombat, bool bInBorders, bool bUpdateGlobal)
{
	...
	if (bFromCombat)
	{
		...
		if (bUpdateGlobal)
		{
			if ((iChange * iCombatExperienceMod) < 100 && (iChange * iCombatExperienceMod) > 0 )
			{
				kPlayer.changeCombatExperience(1);
			}
			
			if ((iChange * iCombatExperienceMod) >= 100)
			{
...
void CvUnit::changeExtraMoveDiscount(int iChange)
{
	...
	// MOD - START - Negative Extra Move Discount
	//FAssert(getExtraMoveDiscount() >= 0);
	// MOD - END - Negative Extra Move Discount
}
...
void CvUnit::changeExtraFirstStrikes(int iChange)
{
	...
	// MOD - START - Negative First Strikes
	//FAssert(getExtraFirstStrikes() >= 0);
	// MOD - END - Negative First Strikes
}
...
void CvUnit::changeExtraWithdrawal(int iChange)
{
	...
	// MOD - START - Negative Withdrawal Acceptance
	// Note: It's okay to go negative because all of the calls to
	// getExtraWithdrawal chance check for negative values
	//FAssert(getExtraWithdrawal() >= 0);
	FAssert(withdrawalProbability() >= 0); // K-Mod. (the 'extra' can be negative during sea-patrol battles.)
	// MOD - END - Negative Withdrawal Acceptance
}
...
// MOD - START - Advanced Experience Limits
int CvUnit::getExtraBarbarianMaxXPValue() const
{
	return m_iExtraBarbarianMaxXPValue;
}

void CvUnit::changeExtraBarbarianMaxXPValue(int iChange)
{
	if (iChange != 0)
	{
		m_iExtraBarbarianMaxXPValue += iChange;
...
// MOD - START - Extra Barbarian Combat Modifier
int CvUnit::getExtraBarbarianCombatModifier() const
{
	return m_iExtraBarbarianCombatModifier;
}

void CvUnit::changeExtraBarbarianCombatModifier(int iChange)
{
	if (iChange != 0)
	{
		m_iExtraBarbarianCombatModifier += iChange;
...
bool CvUnit::isCargo() const
{
	...
	//FAssertMsg((m_transportUnit.eOwner >= 0) == (getTransportUnit() != NULL), "CvUnit::isCargo modification is not equal to the original");
	//return (m_transportUnit.eOwner >= 0);
	// MOD - END - Efficiency
}
...
// MOD - START - Feature Damage Immunity
int CvUnit::getFeatureDamageImmunityCount(FeatureTypes eFeature) const
{
	FASSERT_BOUNDS(0, GC.getNumFeatureInfos(), eFeature, "CvUnit::getFeatureDamageImmunityCount");
	return m_paiFeatureDamageImmunityCount[eFeature];
}


bool CvUnit::isImmuneToDamageFromFeature(FeatureTypes eFeature) const
{
	FASSERT_BOUNDS(0, GC.getNumFeatureInfos(), eFeature, "CvUnit::isImmuneToDamageFromFeature");
	return (m_pUnitInfo->getFeatureDamageImmunity(eFeature) || (getFeatureDamageImmunityCount(eFeature) > 0));
}


void CvUnit::changeFeatureDamageImmunityCount(FeatureTypes eFeature, int iChange)
{
	FASSERT_BOUNDS(0, GC.getNumFeatureInfos(), eFeature, "CvUnit::changeFeatureDamageImmunityCount");
	m_paiFeatureDamageImmunityCount[eFeature] = (m_paiFeatureDamageImmunityCount[eFeature] + iChange);
	FAssert(getFeatureDamageImmunityCount(eFeature) >= 0);
}
// MOD - END - Feature Damage Immunity
...
// MOD - START - Special Promotions and Temporary Promotions
//bool CvUnit::canAcquirePromotion(PromotionTypes ePromotion) const
bool CvUnit::canAcquirePromotion(PromotionTypes ePromotion, bool isSpecial, bool isTemporary) const
// MOD - END - Special Promotions and Temporary Promotions
{
	...
	if (!isSpecial && GC.getPromotionInfo(ePromotion).isSpecial())
	{
		return false;
	}
	...
	// MOD - START - Temporary Promotions
	if (!isTemporary && GC.getPromotionInfo(ePromotion).isTemporary())
	{
		return false;
	}
	// MOD - END - Temporary Promotions
...
bool CvUnit::isPromotionValid(PromotionTypes ePromotion) const
{
	...
	// MOD - START - Capped Withdrawal Probability
	// TODO: Return false if the promotion offers no other benefit,
	// but exclude temporary benefits from the calculation
	/*
	...
	*/
	...
	if (promotionInfo.getInterceptChange() > 0 && maxInterceptionProbability() == 0)
	{
		return false;
	}	
...
void CvUnit::setHasPromotion(PromotionTypes eIndex, bool bNewValue)
{
	if (isHasPromotion(eIndex) != bNewValue)
	{
		...
		// MOD - START - Advanced Experience Limits
		changeExtraBarbarianMaxXPValue(kPromotion.getExtraBarbarianMaxXPValue() * iChange);
		// MOD - END - Advanced Experience Limits
		// MOD - START - Extra Barbarian Combat Modifier
		changeExtraBarbarianCombatModifier(kPromotion.getExtraBarbarianCombatModifier() * iChange);
		// MOD - END - Extra Barbarian Combat Modifier
		...
		for (int iI = 0; iI < GC.getNumFeatureInfos(); iI++)
		{
			...
			changeExtraFeatureAttackPercent(eFeature, (kPromotion.getFeatureAttackPercent(iI) * iChange));
			changeExtraFeatureDefensePercent(eFeature, (kPromotion.getFeatureDefensePercent(iI) * iChange));
			changeFeatureDoubleMoveCount(eFeature, (kPromotion.getFeatureDoubleMove(iI) ? iChange : 0));
			// MOD - START - Feature Damage Immunity
			changeFeatureDamageImmunityCount(eFeature, (kPromotion.getFeatureDamageImmunity(iI) ? iChange : 0));
			// MOD - END - Feature Damage Immunity
		}
...
// MOD - START - Aid
int CvUnit::getAidContribution(AidStyleTypes eAidStyle) const
{
	switch (eAidStyle)
	{
		case AIDSTYLE_FIXED:
...
int CvUnit::getAidAmount(AidTypes eAid) const
{
	CvPlot* pPlot = plot();
	int iAidAmount = 0;
	...
	if (pPlot != NULL)
	{
		// Only apply effects when not cargo
		if (pPlot->isValidDomainForLocation(*this))
		{
			iAidAmount = pPlot->getAidAmount(getTeam(), eAid);
		}
	}

	if (iAidAmount > 0)
	{
		// Remove this unit's contribution from the aid amount if this unit supplies this kind of aid
		// TODO: Add boolean array for fast lookups
		const CvUnitClassInfo& kUnitClass = GC.getUnitClassInfo(getUnitClassType());
		...
		for (int iI = 0; iI < kUnitClass.getNumAidTypes(); iI++)
		{
			...
			if (eLoopAid == eAid)
			{
				iAidAmount -= getAidContribution(GC.getAidInfo(eAid).getAidStyleType());
				break;
			}
		}
	}

	FAssert(iAidAmount > -1);

	return iAidAmount;
}

void CvUnit::applyAidSuppliedByUnit(CvPlot* pPlot)
{
	changeAidSuppliedByUnit(pPlot, true);
}

void CvUnit::removeAidSuppliedByUnit(CvPlot* pPlot)
{
	changeAidSuppliedByUnit(pPlot, false);
}

void CvUnit::changeAidSuppliedByUnit(CvPlot* pPlot, bool bApply)
{
	if (pPlot != NULL)
	{
		if (pPlot->isValidDomainForLocation(*this))
		{
			...
			{
				AidTypes eLoopAid = (AidTypes)kUnitClass.getAidTypes(iI);
				CvAidInfo& kAidInfo = GC.getAidInfo(eLoopAid);
				int iChange = getAidContribution(kAidInfo.getAidStyleType());

				// Don't apply aid if there is no change
				// (This also acts as a shortcircuit for units without strength)
				if (iChange <= 0)
				{
					continue;
				}
...
void CvUnit::updateAidEffects()
{
	for (int iI = 0; iI < GC.getNumAidInfos(); iI++)
	{
		updateAidEffects((AidTypes) iI, false);
	}
}

void CvUnit::updateAidEffects(AidTypes eAid, bool bUpdateSuperseded)
{
	const CvAidInfo& kAidInfo = GC.getAidInfo(eAid);

	int iAidAmount = getAidAmount(eAid);

	// Check to see if this aid has been superseded
	if (iAidAmount > 0)
	{
		for (int iI = 0; iI < kAidInfo.getNumSupersededByAidTypes(); iI++)
		{
			if (getAidAmount((AidTypes)kAidInfo.getSupersededByAidTypes(iI)) > 0)
			{
				iAidAmount = 0;
				break;
			}
	...
	// Early out for no aid
	if (iAidAmount <= 0)
	{
		// Remove all promotions
		for (int iI = 0; iI < kAidInfo.getNumPromotions(); iI++)
		{
			setHasPromotion((PromotionTypes)kAidInfo.getPromotions(iI), false);
		}

		// Update any aid effects for any aid that we might have superseded
		if (bUpdateSuperseded)
		{
			updateAidEffectsForSupersedingAid(eAid);
		}

		return;
	}

	// Update the unit's promotions
	AidStyleTypes eAidStyle = kAidInfo.getAidStyleType();

	if (eAidStyle == AIDSTYLE_FIXED)
	{
		// Fixed aid style; apply all promotions
		for (int iI = 0; iI < kAidInfo.getNumPromotions(); iI++)
		{
			PromotionTypes eAidPromotion = (PromotionTypes) kAidInfo.getPromotions(iI);
			if (canAcquirePromotion(eAidPromotion, false, true))
			{
				setHasPromotion(eAidPromotion, true);
			}
		}
	}
	else if (eAidStyle == AIDSTYLE_RELATIVE_STRENGTH)
	{
		// Relative strength aid style; uses one promotion based on the unit's strength.
		// This works backwards through the list and uses the first (highest) promotion
		// that we have enough aid for.  This assumes that the promotion list is in order
		// from lowest to highest.
		bool bPromotionFound = false;

		for (int iI = kAidInfo.getNumPromotions()-1; iI >= 0; iI--)
		{
			PromotionTypes eAidPromotion = (PromotionTypes) kAidInfo.getPromotions(iI);

			if (bPromotionFound)
			{
				setHasPromotion(eAidPromotion, false);
				continue;
			}

			int iAidThreshold = iAidAmount;
			int iUnitThreshold = baseCombatStr();

			// This snippet here produce a sequence of promotions that match
			// when the aid greater than 50%, 100%, 200%, 300%,... of unit's base strength
			if (iI > 1)
			{
				iUnitThreshold *= iI;
			}
			...
			if (iAidThreshold > iUnitThreshold)
			{
				bPromotionFound = true;
...
void CvUnit::updateAidEffectsForSupersedingAid(AidTypes eAid)
{
	const CvAidInfo& kAidInfo = GC.getAidInfo(eAid);

	for (int iI = 0; iI < kAidInfo.getNumSupersededAidTypes(); iI++)
	{
		updateAidEffects((AidTypes)kAidInfo.getSupersededAidTypes(iI));
	}
}
// MOD - END - Aid
...
// MOD - START - Detriments
int CvUnit::getDetrimentContribution(DetrimentStyleTypes eDetrimentStyle) const
{
	switch (eDetrimentStyle)
	{
		case DETRIMENTSTYLE_FIXED:
			return 1;
		case DETRIMENTSTYLE_RELATIVE_STRENGTH:
			return baseCombatStr();
		default:
			FAssert(false);
			return 0;
...
int CvUnit::getDetrimentAmount(DetrimentTypes eDetriment) const
{
	CvPlot* pPlot = plot();
	...
	if (pPlot != NULL)
	{
		...
		if (pPlot->isValidDomainForLocation(*this))
		{
			return pPlot->getDetrimentAmount(getTeam(), eDetriment);
		}
	}
...
void CvUnit::applyDetrimentsSuppliedByUnit(CvPlot* pPlot)
{
	changeDetrimentsSuppliedByUnit(pPlot, true);
}

void CvUnit::applyDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam)
{
	changeDetrimentsSuppliedByUnit(pPlot, eAffectedTeam, true);
}

void CvUnit::removeDetrimentsSuppliedByUnit(CvPlot* pPlot)
{
	changeDetrimentsSuppliedByUnit(pPlot, false);
}

void CvUnit::removeDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam)
{
	changeDetrimentsSuppliedByUnit(pPlot, eAffectedTeam, false);
}

void CvUnit::changeDetrimentsSuppliedByUnit(CvPlot* pPlot, bool bApply)
{
	...
	{
		TeamTypes eAffectedTeam = (TeamTypes)iI;
		if (GET_TEAM(eAffectedTeam).isAlive())
		{
			if (kTeam.isAtWar(eAffectedTeam))
			{
				changeDetrimentsSuppliedByUnit(pPlot, eAffectedTeam, bApply);
			}
		}
	}
}

void CvUnit::changeDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam, bool bApply)
{
	if (pPlot != NULL)
	{
		if (pPlot->isValidDomainForLocation(*this))
		{
			...
			for (int iI = 0; iI < kUnitClass.getNumDetrimentTypes(); iI++)
			{
				...
				CvDetrimentInfo& detrimentInfo = GC.getDetrimentInfo(eLoopDetriment);
				int iChange = getDetrimentContribution(detrimentInfo.getDetrimentStyleType());

				// Don't apply detriment if there is no change
				// (This also acts as a short-circuit for units without strength)
				if (iChange <= 0)
				{
					continue;
				}
				...
				pPlot->changeDetrimentSuppliedFromPlot(eAffectedTeam, eLoopDetriment, iChange);
			}
		}
	}
}
...
void CvUnit::updateDetrimentEffects()
{
	for (int iI = 0; iI < GC.getNumDetrimentInfos(); iI++)
	{
		updateDetrimentEffects((DetrimentTypes) iI, false);
	}
}

void CvUnit::updateDetrimentEffects(DetrimentTypes eDetriment, bool bUpdateSuperseded)
{
	const CvDetrimentInfo& kDetrimentInfo = GC.getDetrimentInfo(eDetriment);

	int iDetrimentAmount = getDetrimentAmount(eDetriment);

	// Check to see if this detriment has been superseded
	if (iDetrimentAmount > 0)
	{
		for (int iI = 0; iI < kDetrimentInfo.getNumSupersededByDetrimentTypes(); iI++)
		{
			DetrimentTypes eLoopDetrimentType = (DetrimentTypes)kDetrimentInfo.getSupersededByDetrimentTypes(iI);

			if (getDetrimentAmount(eLoopDetrimentType) > 0)
			{
				iDetrimentAmount = 0;
				break;
			}
	...
	// Early out for no detriment
	if (iDetrimentAmount <= 0)
	{
		// Remove all promotions
		for (int iI = 0; iI < kDetrimentInfo.getNumPromotions(); iI++)
		{
			setHasPromotion((PromotionTypes)kDetrimentInfo.getPromotions(iI), false);
		}

		// Update any detriment effects for any detriment that we might have superseded
		if (bUpdateSuperseded)
		{
			updateDetrimentEffectsForSupersedingDetriment(eDetriment);
		}

		return;
	}

	DetrimentStyleTypes eDetrimentStyle = kDetrimentInfo.getDetrimentStyleType();

	// Update the unit's promotions
	if (eDetrimentStyle == DETRIMENTSTYLE_FIXED)
	{
		// Fixed detriment style; apply all promotions
		for (int iI = 0; iI < kDetrimentInfo.getNumPromotions(); iI++)
		{
			PromotionTypes eDetrimentPromotion = (PromotionTypes)kDetrimentInfo.getPromotions(iI);

			// TODO: Fix this so that detriments are applied based on the tech level of the supplier, not the receiver of the detriments
			if (canAcquirePromotion(eDetrimentPromotion, false, true))
			{
				setHasPromotion(eDetrimentPromotion, true);
			}
		}
	}
	else if (eDetrimentStyle == DETRIMENTSTYLE_RELATIVE_STRENGTH)
	{
		// Relative strength detriment style; uses one promotion based on the unit's strength.
		// This works backwards through the list and uses the first (highest) promotion
		// that we have enough detriment for.  This assumes that the promotion list is in order
		// from lowest to highest.
		bool bPromotionFound = false;

		for (int iI = kDetrimentInfo.getNumPromotions()-1; iI >= 0; iI--)
		{
			PromotionTypes eDetrimentPromotion = (PromotionTypes)kDetrimentInfo.getPromotions(iI);

			if (bPromotionFound)
			{
				setHasPromotion(eDetrimentPromotion, false);
				continue;
			}

			int iDetrimentThreshold = iDetrimentAmount;
			int iUnitThreshold = baseCombatStr();

			// This snippet here produces a sequence of promotions that match
			// when the detriment greater than 50%, 100%, 200%, 300%,... of unit's base strength
			if (iI > 1)
			{
				iUnitThreshold *= iI;
			}
			...
			{
				iDetrimentThreshold *= 2;
			}

			if (iDetrimentThreshold > iUnitThreshold)
			{
				bPromotionFound = true;
...
void CvUnit::updateDetrimentEffectsForSupersedingDetriment(DetrimentTypes eDetriment)
{
	const CvDetrimentInfo& kDetrimentInfo = GC.getDetrimentInfo(eDetriment);

	for (int iI = 0; iI < kDetrimentInfo.getNumSupersededDetrimentTypes(); iI++)
	{
		updateDetrimentEffects((DetrimentTypes)kDetrimentInfo.getSupersededDetrimentTypes(iI));
	}
}
// MOD - END - Detriments
...
void CvUnit::read(FDataStreamBase* pStream)
{
	...
	// MOD - START - Advanced Experience Limits
	pStream->Read(&m_iExtraBarbarianMaxXPValue);
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	pStream->Read(&m_iExtraBarbarianCombatModifier);
	// MOD - END - Extra Barbarian Combat Modifier
...
/************************************************************************************************/
	...
	// MOD - START - Slavery
	pStream->Read(&m_bIgnoreObstacle); // slave rebels ignore obstacles (great wall) tag
	// MOD - END - Slavery
	...
	// MOD - START - Feature Damage Immunity
	pStream->Read(GC.getNumFeatureInfos(), m_paiFeatureDamageImmunityCount);
	// MOD - END - Feature Damage Immunity
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	// MOD - START - Advanced Experience Limits
	pStream->Write(m_iExtraBarbarianMaxXPValue);
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	pStream->Write(m_iExtraBarbarianCombatModifier);
	// MOD - END - Extra Barbarian Combat Modifier
...
/************************************************************************************************/
	...
	// MOD - START - Slavery
	pStream->Write(m_bIgnoreObstacle); // slave rebels ignore obstacles (great wall) tag
	// MOD - END - Slavery
	...
	// MOD - START - Feature Damage Immunity
	pStream->Write(GC.getNumFeatureInfos(), m_paiFeatureDamageImmunityCount);
	// MOD - END - Feature Damage Immunity
...
bool CvUnit::interceptTest(const CvPlot* pPlot)
{
	...
	if (pInterceptor != NULL)
	{
		// RI - new formula for interception probability 
		if (GC.getGameINLINE().getSorenRandNum(100, "Intercept Rand (Air)") < std::min((pInterceptor->currInterceptionProbability() - evasionProbability()),95)) 
		{
...
CvUnit* CvUnit::airStrikeTarget(const CvPlot* pPlot) const
{
	...
	// MOD - START - Ranged Strike AI
	//pDefender = pPlot->getBestDefender(NO_PLAYER, getOwnerINLINE(), this, true);
	pDefender = pPlot->getBestDefender(NO_PLAYER, getOwnerINLINE(), this, true, false, false, true);
	// MOD - END - Ranged Strike AI
	// MOD - END - Protect Valuable Units / Lead From Behind
	...
	if (pDefender != NULL)
	{
		if (!pDefender->isDead())
		{
			if (pDefender->canDefend())
			{
				...
				// MOD - END - Improved Range Strike AI
			}
		}
	}
...
bool CvUnit::airStrike(CvPlot* pPlot)
{
	...
	// < Air Combat Experience Start >
	if(GC.getGameINLINE().isExperienceGainByAttackingUnits())
	{
		int iExperience = 0;

		int iTheirStrength = (DOMAIN_AIR == pDefender->getDomainType() ? pDefender->airCurrCombatStr(this) : pDefender->currCombatStr(NULL, NULL));
		int iOurStrength = (DOMAIN_AIR == getDomainType() ? airCurrCombatStr(pDefender) : currCombatStr(NULL, NULL));
		iExperience = pDefender->defenseXPValue();
		iExperience = (iExperience * iTheirStrength) / std::max(1, iOurStrength);
		iExperience = range(iExperience, GC.getDefineINT("MIN_EXPERIENCE_PER_COMBAT"), GC.getDefineINT("MAX_EXPERIENCE_PER_COMBAT"));
...
// MOD - START - Ranged Strike AI
bool CvUnit::isRangeStrikeCapable() const
{
	if (getDomainType() == DOMAIN_AIR)
	{
		return false;
...
{
	// MOD - START - Ranged Strike AI
	if (!isRangeStrikeCapable())
	{
		return false;
	}
	// MOD - END - Ranged Strike AI

	// MOD - START - No Ranged Strikes While Cargo
	if (isCargo())
	{
		return false;
	...
	// MOD - END - No Ranged Strikes While Cargo
...
/************************************************************************************************/
	...
	// MOD - START - Ranged Combat Experience
	// MOD - START - Advanced Experience Limits
...
bool CvUnit::canApplyEvent(EventTypes eEvent) const
{
	...
	if (NO_PROMOTION != kEvent.getUnitPromotion())
	{
		...
		if (!canAcquirePromotion((PromotionTypes)kEvent.getUnitPromotion(), true))
		// MOD - END - Special Promotions
		{
			return false;
		}
...
// MOD - START - Unit Render Priority Bugfix
int CvUnit::getRenderPriority(UnitSubEntityTypes eUnitSubEntity, int iMeshGroupType, int UNIT_MAX_SUB_TYPES) const
{
	//int priority = getOwner() * (GC.getNumUnitInfos() + 2) * UNIT_MAX_SUB_TYPES;
	int priority = 0;
	if (eUnitSubEntity == UNIT_SUB_ENTITY_SIEGE_TOWER)
	{
		priority += iMeshGroupType;
	}
	else
	{
		priority += m_eUnitType * UNIT_MAX_SUB_TYPES + iMeshGroupType;
	}
	return std::min(priority, 99999); // Values of 100000 or more result in clipping issues for boats
}
// MOD - END - Unit Render Priority Bugfix
...
bool CvUnit::isStrategic() const
{
	return m_pUnitInfo->isStrategic();
}
...
bool CvUnit::isBetterDefenderThan(const CvUnit* pDefender, const CvUnit* pAttacker) const
{
	...
	return isBetterDefenderThan(pDefender, pAttacker, NULL, false);
	// MOD - END - Ranged Strike AI
}
...
// MOD - START - Slavery
// rebels can ignore borderObstacles tag (GreatWall)
bool CvUnit::isIgnoreObstacle() const
{
	return m_bIgnoreObstacle;
}

void CvUnit::setIgnoreObstacle(bool bNewValue)
{
	m_bIgnoreObstacle = bNewValue;
}

// chance to consume unit after improvement build by Mexico
int CvUnit::getBuildKillChance() const
{
	return GC.getUnitInfo(getUnitType()).getBuildKillChance();
}
// MOD - END - Slavery
...
bool CvUnit::canShadowPlot(CvPlot* pShadowPlot = NULL) const
{
	if (pShadowPlot == NULL)
	{
		return false;
	...
	for (CLLNode<IDInfo>* pShadowUnitNode = pShadowPlot->headUnitNode(); pShadowUnitNode != NULL; pShadowUnitNode = pShadowPlot->nextUnitNode(pShadowUnitNode))
	{
		CvUnit* pShadowUnit = ::getUnit(pShadowUnitNode->m_data);
		if (canShadowUnit(pShadowUnit))
		{
			return true;
		}
	}
...
bool CvUnit::canShadowUnit(CvUnit* pShadowUnit) const
{
	if (!canShadow())
	{
		return false;
	...
	if (pShadowUnit->getOwnerINLINE() == getOwnerINLINE() && pShadowUnit->getGroupID() == getGroupID())
	{
		return false;
	}

	int iPathTurns;
	if (!generatePath(pShadowUnit->plot(), 0, true, &iPathTurns))
	{
		return false;
	}
