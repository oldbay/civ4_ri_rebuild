...
void CvInitCore::setWorldSize(const CvWString & szWorldSize)
{
	...
	{
		if ( _wcsicmp(szWorldSize.GetCString(), CvWString(GC.getWorldInfo((WorldSizeTypes)i).getType()).GetCString()) == 0 )
		{
...
void CvInitCore::setClimate(const CvWString & szClimate)
{
	...
	{
		if ( _wcsicmp(szClimate.GetCString(), CvWString(GC.getClimateInfo((ClimateTypes)i).getType()).GetCString()) == 0 )
		{
...
void CvInitCore::setSeaLevel(const CvWString & szSeaLevel)
{
	...
	{
		if ( _wcsicmp(szSeaLevel.GetCString(), CvWString(GC.getSeaLevelInfo((SeaLevelTypes)i).getType()).GetCString()) == 0 )
		{
...
void CvInitCore::setEra(const CvWString & szEra)
{
	...
	{
		if ( _wcsicmp(szEra.GetCString(), CvWString(GC.getEraInfo((EraTypes)i).getType()).GetCString()) == 0 )
		{
			setEra((EraTypes)i);
		}
	}
...
void CvInitCore::setGameSpeed(const CvWString & szGameSpeed)
{
	...
	{
		if ( _wcsicmp(szGameSpeed.GetCString(), CvWString(GC.getGameSpeedInfo((GameSpeedTypes)i).getType()).GetCString()) == 0 )
		{
...
void CvInitCore::setTurnTimer(const CvWString & szTurnTimer)
{
	...
	{
		if ( _wcsicmp(szTurnTimer.GetCString(), CvWString(GC.getTurnTimerInfo((TurnTimerTypes)i).getType()).GetCString()) == 0 )
		{
...
void CvInitCore::setCalendar(const CvWString & szCalendar)
{
	...
	{
		if ( _wcsicmp(szCalendar.GetCString(), CvWString(GC.getCalendarInfo((CalendarTypes)i).getType()).GetCString()) == 0 )
		{
...
{
	if (_wcsicmp(szType.GetCString(), L"singleplayer") == 0)
	{
		setType(GAME_SP_NEW);
	}
	else if (_wcsicmp(szType.GetCString(), L"spload") == 0)
	{
...
void CvInitCore::setCiv(PlayerTypes eID, CivilizationTypes eCiv)
{
	...
	if ( checkBounds(eID, 0, MAX_PLAYERS) )
	{
		...
		if (GC.getGameINLINE().isFinalInitialized())
		{
			// In-game civilization switch

			// MOD - START - Improved Civilopedia
			if (eID == getActivePlayer())
			{
				gDLL->getPythonIFace()->callFunction(PYScreensModule, "updatePlayerContext");
			}
			// MOD - END - Improved Civilopedia
			...
			GET_PLAYER(eID).updateBuildCache();
...
void CvInitCore::setLeader(PlayerTypes eID, LeaderHeadTypes eLeader)
{
	...
	if ( checkBounds(eID, 0, MAX_PLAYERS) )
	{
		...
		// MOD - START - Improved Civilopedia
		if (GC.getGameINLINE().isFinalInitialized() && eID == getActivePlayer())
		{
			// In-game leader switch
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "updatePlayerContext");
		}
		// MOD - END - Improved Civilopedia
	}
