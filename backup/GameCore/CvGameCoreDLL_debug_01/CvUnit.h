...
public:
	...
	bool canRemoveReligion(const CvPlot* pPlot, ReligionTypes eReligion, bool bTestVisible = false) const;
	bool canRemoveReligion(const CvPlot* pPlot, bool bTestVisible = false) const;					// Exposed to Python
	bool removeReligion(ReligionTypes eReligion);
	bool removeReligion();																		// Exposed to Python
	// MOD - END - Inquisition
	...
	UnitCombatTypes getUnitPromoType() const;	
	...
	int maxXPValue(const CvUnit* pAttacker = NULL) const; // Exposed to Python
	// MOD - START - Advanced Experience Limits
	...
	bool noTransport() const;																										// Exposed to Python
...
//#ifdef _USRDLL
	...
	void changeExtraBarbarianMaxXPValue(int iChange);
	...
	void changeExtraBarbarianCombatModifier(int iChange);
...
//#ifdef _USRDLL
	...
	int getFeatureDamageImmunityCount(FeatureTypes eFeature) const;
	bool isImmuneToDamageFromFeature(FeatureTypes eFeature) const;													// Exposed to Python 
	void changeFeatureDamageImmunityCount(FeatureTypes eFeature, int iChange);
	// MOD - START - Feature Damage Immunity
	...
	bool canAcquirePromotion(PromotionTypes ePromotion, bool isSpecial = false, bool isTemporary = false) const;															// Exposed to Python
	...
	// MOD - START - Aid
	//bool suppliesAid() const; // Exposed to Python

	int getAidContribution(AidStyleTypes eAidStyle) const;
	int getAidAmount(AidTypes eAid) const;

	void applyAidSuppliedByUnit(CvPlot* pPlot);
	void removeAidSuppliedByUnit(CvPlot* pPlot);
	void changeAidSuppliedByUnit(CvPlot* pPlot, bool bApply);

	void updateAidEffects();
	void updateAidEffects(AidTypes eAid, bool bUpdateSuperseded = true);

	void updateAidEffectsForSupersedingAid(AidTypes eAid);
	// MOD - END - Aid

	// MOD - START - Detriments
	//bool suppliesDetriments() const; // Exposed to Python

	int getDetrimentContribution(DetrimentStyleTypes eDetrimentStyle) const;
	int getDetrimentAmount(DetrimentTypes eDetriment) const;

	void applyDetrimentsSuppliedByUnit(CvPlot* pPlot);
	void applyDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam);

	void removeDetrimentsSuppliedByUnit(CvPlot* pPlot);
	void removeDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam);

	void changeDetrimentsSuppliedByUnit(CvPlot* pPlot, bool bApply);
	void changeDetrimentsSuppliedByUnit(CvPlot* pPlot, TeamTypes eAffectedTeam, bool bApply);

	void updateDetrimentEffects();
	void updateDetrimentEffects(DetrimentTypes eDetriment, bool bUpdateSuperseded = true);

	void updateDetrimentEffectsForSupersedingDetriment(DetrimentTypes eDetriment);
	// MOD - END - Detriments
	...
	// MOD - START - Ranged Strike AI
	bool isRangeStrikeCapable() const;
	// MOD - END - Ranged Strike AI
...
/************************************************************************************************/
	...
	// MOD - START - Slavery
	// tag for slavery rebels - can ignore order obstacle tag (great wall) by Mexico 30.11.2006
	DllExport bool isIgnoreObstacle() const;
	DllExport void setIgnoreObstacle(bool bNewValue);
...
/************************************************************************************************/
	...
	// MOD - START - Feature Damage Immunity
	int* m_paiFeatureDamageImmunityCount;
...
public:
	...
	bool isBetterDefenderThan(const CvUnit* pDefender, const CvUnit* pAttacker, int* pBestDefenderRank, bool bRanged) const;
	// MOD - END - Ranged Strike AI
