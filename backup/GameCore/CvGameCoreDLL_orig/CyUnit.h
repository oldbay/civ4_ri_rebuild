...
public:
	...
	bool canRemoveReligion(CyPlot* pPlot, bool bTestVisible);
	bool removeReligion();
	// MOD - END - Inquisition
	...
	int /*UnitCombatTypes*/ getUnitPromoType();	
	...
	//int maxXPValue();
	int maxXPValue(CyUnit* pAttacker = NULL);
	...
	// MOD - START - Advanced Experience Limits
	int getExtraBarbarianMaxXPValue() const;
	// MOD - END - Advanced Experience Limits
	// MOD - START - Extra Barbarian Combat Modifier
	int getExtraBarbarianCombatModifier() const;
	...
	bool isImmuneToDamageFromFeature(int /*FeatureTypes*/ eFeature);
	...
	void setIgnoreObstacle(bool bNewValue);
	// chance to consume unit after improvement build by Mexico
	int getBuildKillChance();
	// MOD - END - Slavery
