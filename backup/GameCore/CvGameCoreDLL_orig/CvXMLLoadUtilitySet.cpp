...
bool CvXMLLoadUtility::LoadBasicInfos()
{
	...
	// MOD - START - Aid
	LoadGlobalClassInfo(GC.getAidStyleInfo(), "CIV4AidStyleInfos", "BasicInfos", "Civ4AidStyleInfos/AidStyleInfos/AidStyleInfo", false);
	// MOD - END - Aid
	// MOD - START - Detriments
	LoadGlobalClassInfo(GC.getDetrimentStyleInfo(), "CIV4DetrimentStyleInfos", "BasicInfos", "Civ4DetrimentStyleInfos/DetrimentStyleInfos/DetrimentStyleInfo", false);
	// MOD - END - Detriments
	...
	LoadGlobalClassInfo(GC.getGameEventInfo(), "CIV4GameEventInfos", "BasicInfos", "CIV4GameEventInfos/GameEventInfos/GameEventInfo", false);
	// MOD - END - Core Game Events
	// MOD - START - Population Metrics
	LoadGlobalClassInfo(GC.getContributionInfo(), "CIV4ContributionInfos", "BasicInfos", "CIV4ContributionInfos/ContributionInfos/ContributionInfo", false);
...
bool CvXMLLoadUtility::LoadPreMenuGlobals()
{
	...
	// MOD - START - Improved Civilopedia
	LoadGlobalClassInfo(GC.getHotKeyClassInfo(), "CIV4HotKeyClassInfos", "GameInfo", "Civ4HotKeyClassInfos/HotKeyClassInfos/HotKeyClassInfo", false);
	// MOD - END - Improved Civilopedia
	...
	// MOD - START - Congestion
	LoadGlobalClassInfo(GC.getDomainInfo(), "CIV4DomainInfos", "GameInfo", "Civ4DomainInfos/DomainInfos/DomainInfo", false);
	// MOD - END - Congestion
	...
	LoadGlobalClassInfo(GC.getUnitRoleInfo(), "CIV4UnitRoleInfos", "Units", "Civ4UnitRoleInfos/UnitRoleInfos/UnitRoleInfo", false);
	// MOD - END - Unit Roles
	...
	// MOD - START - Aid
	LoadGlobalClassInfo(GC.getAidInfo(), "CIV4AidInfos", "Units", "Civ4AidInfos/AidInfos/AidInfo", true);
	for (int i=0; i < GC.getNumAidInfos(); ++i)
	{
		GC.getAidInfo((AidTypes)i).readPass3();
	}
	// MOD - END - Aid
	// MOD - START - Detriments
    LoadGlobalClassInfo(GC.getDetrimentInfo(), "CIV4DetrimentInfos", "Units", "Civ4DetrimentInfos/DetrimentInfos/DetrimentInfo", true);
	for (int i=0; i < GC.getNumDetrimentInfos(); ++i)
	{
		GC.getDetrimentInfo((DetrimentTypes)i).readPass3();
	}
	// MOD - END - Detriments
	// MOD - START - Congestion
    LoadGlobalClassInfo(GC.getCongestionInfo(), "CIV4CongestionInfos", "Units", "Civ4CongestionInfos/CongestionInfos/CongestionInfo", false);
	// MOD - END - Congestion
	...
	LoadGlobalClassInfo(GC.getImprovementClassInfo(), "CIV4ImprovementClassInfos", "Terrain", "Civ4ImprovementClassInfos/ImprovementClassInfos/ImprovementClassInfo", false);
	...
	{
		GC.getImprovementClassInfo((ImprovementClassTypes)i).readPass3();
	}
	// MOD - END - Improvement Classes
	...
	// MOD - START - Population Metrics
	LoadGlobalClassInfo(GC.getMetricClassInfo(), "CIV4MetricClassInfos", "Metrics", "Civ4MetricClassInfos/MetricClassInfos/MetricClassInfo", false);
	LoadGlobalClassInfo(GC.getMetricInfo(), "CIV4MetricInfos", "Metrics", "Civ4MetricInfos/MetricInfos/MetricInfo", false);
	LoadGlobalClassInfo(GC.getMetricFactorInfo(), "CIV4MetricFactorInfos", "Metrics", "Civ4MetricFactorInfos/MetricFactorInfos/MetricFactorInfo", false);
	// MOD - END - Population Metrics
	...
	LoadGlobalClassInfo(GC.getCivilizationInfo(), "CIV4CivilizationInfos", "Civilizations", "Civ4CivilizationInfos/CivilizationInfos/CivilizationInfo", false, &CvDLLUtilityIFaceBase::createCivilizationInfoCacheObject);
	...
	std::multimap<BuildingClassTypes, BuildingTypes> buildingReplacementMap;
	for (int i=0; i < GC.getNumBuildingInfos(); ++i)
	{
		BuildingTypes eBuilding = (BuildingTypes)i;
		BuildingClassTypes eBuildingClass = (BuildingClassTypes)GC.getBuildingInfo(eBuilding).getObsoleteBuildingClass();
		if (eBuildingClass != NO_BUILDINGCLASS)
		{
			buildingReplacementMap.insert(std::pair<BuildingClassTypes, BuildingTypes>(eBuildingClass, eBuilding));
		}
	}

	for (int i=0; i < GC.getNumBuildingClassInfos(); ++i)
	{
		GC.getBuildingClassInfo((BuildingClassTypes)i).cacheReplacements(buildingReplacementMap);
	}

	buildingReplacementMap.clear();
	// MOD - END - Building Replacement

	// MOD - START - Relation Caching
	for (int i=0; i < GC.getNumTechInfos(); ++i)
	{
		GC.getTechInfo((TechTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumReligionInfos(); ++i)
	{
		GC.getReligionInfo((ReligionTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumCivicInfos(); ++i)
	{
		GC.getCivicInfo((CivicTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumBonusInfos(); ++i)
	{
		GC.getBonusInfo((BonusTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumRouteInfos(); ++i)
	{
		GC.getRouteInfo((RouteTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumImprovementInfos(); ++i)
	{
		GC.getImprovementInfo((ImprovementTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumUnitInfos(); ++i)
	{
		GC.getUnitInfo((UnitTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumBuildingInfos(); ++i)
	{
		GC.getBuildingInfo((BuildingTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumSpecialistInfos(); ++i)
	{
		GC.getSpecialistInfo((SpecialistTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumPromotionInfos(); ++i)
	{
		GC.getPromotionInfo((PromotionTypes)i).cacheRelations();
	}

	// MOD - START - Congestion
	for (int i=0; i < NUM_DOMAIN_TYPES; ++i)
	{
		GC.getDomainInfo((DomainTypes)i).cacheRelations();
	}
	// MOD - END - Congestion

	// MOD - START - Core Game Events
	for (int i=0; i < GC.getNumGameEventInfos(); ++i)
	{
		GC.getGameEventInfo((GameEventTypes)i).cacheRelations();
	}
	// MOD - END - Core Game Events

	// MOD - START - Population Metrics
	for (int i=0; i < GC.getNumMetricClassInfos(); ++i)
	{
		GC.getMetricClassInfo((MetricClassTypes)i).cacheRelations();
	}

	for (int i=0; i < GC.getNumMetricInfos(); ++i)
	{
		GC.getMetricInfo((MetricTypes)i).cacheRelations();
	}
	// MOD - END - Population Metrics
	...
	// Note: These have been moved here from LoadPostMenuGlobals because they are needed
	//       to in order to display the tech graph from the main menu
	...
	// MOD - START - Improved Civilopedia
	for (int i=0; i < GC.getNumHotKeyClassInfos(); ++i)
	{
		GC.getHotKeyClassInfo((HotKeyClassTypes)i).readPass3();
	}
	// MOD - END - Improved Civilopedia
...
bool CvXMLLoadUtility::LoadPostMenuGlobals()
{
	...
	*/
	...
	{
		GC.getEventInfo((EventTypes)i).cacheRelations();
	}
	// MOD - END - Relation Caching
...
void CvXMLLoadUtility::LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*))
{
	...
	if (!bLoaded)
	{
		// MOD - START - Flexible XML Loading
		/*
		...
		*/
		...
		// Flexible files
		std::vector<CvString> aszFlexibleFiles;
		gDLL->enumerateFiles(aszFlexibleFiles, CvString::format("xml\\%s\\*_%s.xml", szFileDirectory, szFileRoot));
		aszFiles.insert(aszFiles.end(), aszFlexibleFiles.begin(), aszFlexibleFiles.end());

		// Modular files
		if (gDLL->isModularXMLLoading())
		{
			std::vector<CvString> aszModularFiles;
			gDLL->enumerateFiles(aszModularFiles, CvString::format("modules\\*_%s.xml", szFileRoot));  // search for the modular files
			aszFiles.insert(aszFiles.end(), aszModularFiles.begin(), aszModularFiles.end());
		}
		...
		if (NULL != pArgFunction && bLoaded && bWriteCache)
		{
			// write info to cache
			bool bOk = gDLL->cacheWrite(pCache);
			if (!bOk)
...
void CvXMLLoadUtility::SetVariableListTagPair(int **ppiList, const TCHAR* szRootTagName,
											  int iInfoBaseSize, int iInfoBaseLength, int iDefaultListVal)
{
	// MOD - START - Allow Comments in VariableTagPairs
	//int i;
	int iIndexVal;
	//int iNumSibs;
	// MOD - END - Allow Comments in VariableTagPairs
	TCHAR szTextVal[256];
	int* piList;
					...
					if (SkipToNextVal())
					{
						if (GetChildXmlVal(szTextVal))
						{
							...
							if (iIndexVal != -1)
							{
								...
								{
									char	szMessage[1024];
									sprintf( szMessage, "%s is an invalid InfoType for the %s tag being processed in CvXMLLoadUtility::SetVariableListTagPair\nThe value is out of range\nCurrent XML file is: %s", szTextVal, szRootTagName, GC.getCurrentXMLFile().GetCString());
									gDLL->MessageBox(szMessage, "XML Error");
								}
							}
...
// Allocate and initialize a list from a variable number of tags in the xml
void CvXMLLoadUtility::SetInfoArray(int *piLength, int **ppiList, const TCHAR* szRootTagName)
{
	CvString szTextVal;
	bool bNextSibling;

	*piLength = 0;
	SAFE_DELETE_ARRAY(*ppiList);
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(m_pFXml, szRootTagName))
	{
		if (gDLL->getXMLIFace()->SetToChild(m_pFXml))
		{
			...
			do
			{
				...
				if ( iIndex != -1 )
					aValues.push_back( iIndex );
				bNextSibling = GetNextXmlVal( szTextVal );
			}
			while( bNextSibling );
...
void CvXMLLoadUtility::SetInfoArray(int *piLength, int **ppiList, std::vector<CvString>& aszTagNames)
{
	*piLength = 0;
	SAFE_DELETE_ARRAY(*ppiList);

	if (aszTagNames.size() == 0)
	{
		return;
	}
	...
	for (uint i = 0; i < aszTagNames.size(); i++)
	{
		int iIndex = GC.getInfoTypeForString(aszTagNames[i], false);
		if (iIndex != -1)
		{
			aiValues.push_back(iIndex);
		}
	}

	if (aiValues.size() > 0)
	{
		*piLength = (int)aiValues.size();
		*ppiList = new int[aiValues.size()];
		for (uint i = 0; i < aiValues.size(); i++)
		{
			(*ppiList)[i] = aiValues[i];
		}
	}
}

void CvXMLLoadUtility::CollectInfoArray(std::vector<CvString>& aszInfos, const TCHAR* szRootTagName)
{
	CvString szTextVal;
	bool bNextSibling;
...
DllExport bool CvXMLLoadUtility::LoadGraphicOptions()
{
	...
	FAssert(GC.getNumGraphicOptionInfos() == NUM_GRAPHICOPTION_UNBOUNDED_TYPES);
